...
Kubernetes operator for deploying and managing Antfly database clusters. Built with Kubebuilder/controller-runtime. Supports GKE Autopilot and AWS EKS with Spot instances.
...
**CRDs:** AntflyCluster, AntflyBackup, AntflyRestore (defined in `api/v1/`).
...
```
cmd/antfly-operator/main.go  # Entrypoint
api/v1/                       # CRD types, webhooks, deepcopy
controllers/                  # Reconcilers + autoscaler
bootstrap/                    # CRD self-installation at startup
manifests/                    # Embedded CRD/RBAC YAML (go:embed)
...
**After modifying `api/v1/antflycluster_types.go`:**
1. `make manifests generate`
2. Update reconciler in `controllers/antflycluster_controller.go`
3. Add/update tests
...
Data nodes only (metadata nodes never autoscaled). Uses Kubernetes metrics API. Gradual scaling: +50%/+2 up, -25%/-1 down. Cooldowns: 60s up, 300s down. AutoScaler initialized in `cmd/antfly-operator/main.go` and injected into reconciler.
