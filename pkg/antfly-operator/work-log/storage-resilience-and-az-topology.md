...
**`pkg/antfly-operator/controllers/antflycluster_controller.go`**:
...
**`pkg/antfly-operator/api/v1/antflycluster_types.go`**:
...
**`pkg/antfly-operator/api/v1/antflycluster_webhook.go`**:
...
**`pkg/antfly-operator/docs/troubleshooting.md`** — New sections under "Storage Issues":
...
**`pkg/antfly-operator/docs/operations/storage.md`** (new) — "PVC Retention Policy" reference:
...
**`pkg/antfly-operator/docs/cloud-platforms/aws-eks.md`** — New section "Multi-AZ Storage Best Practices":
...
**`pkg/antfly-operator/docs/cloud-platforms/gcp-gke.md`** — Add StorageClass warning:
...
**`pkg/antfly-operator/docs/cloud-platforms/generic-kubernetes.md`** — Add cross-cloud StorageClass table and AKS guidance:
...
**`pkg/antfly-operator/docs/operations/pod-scheduling.md`** — Update "Zone-Aware Scheduling":
...
**`pkg/antfly-operator/docs/getting-started/installation.md`** — Update "Uninstalling":
- Mention `pvcRetentionPolicy.whenDeleted: Delete` as alternative to manual PVC cleanup

**`pkg/antfly-operator/examples/eks-cluster.yaml`** — Add commented `pvcRetentionPolicy` example

### 8. Tests

**`pkg/antfly-operator/controllers/antflycluster_controller_test.go`**:
...
**`pkg/antfly-operator/api/v1/antflycluster_webhook_test.go`**:
...
```bash
# After all changes:
cd pkg/antfly-operator && make manifests generate
...
cd pkg/antfly-operator && go test -cover ./...
```
