...
**Files:**
- `pkg/operator/cmd/antfly-operator/main.go` — add webhook server and `ENABLE_WEBHOOKS` toggle
...
| `config/default/manager_webhook_patch.yaml` | Create | Deployment patch |
| `pkg/operator/cmd/antfly-operator/main.go` | Modify | Add webhook server + ENABLE_WEBHOOKS |
