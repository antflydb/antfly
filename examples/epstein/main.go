...
func parseSyncLevelFlag(value string) (antfly.SyncLevel, error) {
	...
	default:
		return "", fmt.Errorf("invalid --sync-level %q (expected propose, write, query, enrichments, or full_index)", value)
	}
}
...
func loadCmd(args []string) error {
	...
	syncLevelFlag := fs.String("sync-level", "write", "Merge sync level: propose, write, query, enrichments, or full_index")
