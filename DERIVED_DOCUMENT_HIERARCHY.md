...
- Public/internal routing now includes a bounded operational table-range repair endpoint, `POST /db/v1/tables/{table}/artifacts/{artifact}:reprocess`, with `from_key`, `to_key`, `limit`, and `shard_cursors` controls plus scanned/reprocessed/skipped/failed counts, per-key failure codes, and per-shard continuation cursors. Hosted/provisioned implementations fan this out to shard-local group handlers, aggregate the bounded pass response, and can resume caller-supplied shard cursors directly for distributed repair controllers.
...
- Define the user-facing scheduler/status API for long-running background repair jobs beyond the resumable bounded repair primitive.
...
Implementation note: the DB, bound table-source, local public HTTP, generated OpenAPI/httpx, and hosted/provisioned routing layers now expose per-document manifest listing, per-artifact manifest inspection, forced reprocess for a specific artifact, and bounded table-range artifact reprocess. Per-document public routes enforce source-document row filters before exposing or mutating artifact control-plane state. Manifest inspection now has typed source/fingerprint/range/merge/error summaries, plus explicit summary/raw detail modes; raw manifest/state JSON is an admin/debug detail when public auth is enabled. Failed document extraction writes a failed artifact manifest generation with `route_type: "error"`, `merge_status: "failed"`, typed `last_error_code` / `last_error_message`, and no current child ranges. When a previously successful source is replaced by an unextractable source, stale unit/chunk artifacts and state are deleted; a later successful extraction advances the generation and clears the last error. Table-range repair responses now expose per-shard continuation cursors, and follow-up requests can pass those cursors back to resume shard-local progress instead of collapsing it into one global key. The remaining API design work is the user-facing policy and lifecycle for a hosted background scheduler/status surface.
...
- Reprocessing semantics: background job scheduling, priority, concurrency, and durable hosted status reporting beyond the bounded synchronous table-range repair endpoint and its per-shard continuation/resume cursor contract.
