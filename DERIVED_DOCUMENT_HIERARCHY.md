...
- Resolver replay accepts unit asset artifacts as source artifacts and scopes their resolution artifacts under the source artifact key, so unit-level entity extraction can resolve without colliding at the parent document's resolution artifact key.

Still remaining in Phase 4:

- Extend the same resolver/source-artifact contract to chunk-scoped and dedicated mention artifacts, including public query/result shaping for those mention records.
