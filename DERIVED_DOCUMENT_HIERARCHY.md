...
- Hierarchy responses now include a stable `ancestors` envelope when response-local data can hydrate it: source-level payloads carry `ancestors.source.document`, unit hits carry `ancestors.unit.document`, and chunk hits expose unit ancestry fields and provenance recovered from the chunk payload.
...
- Query results now expose matched chunks through existing chunk return modes and include a stable hierarchy envelope with response-local ancestor hydration. First-class `mention` return levels and DB-backed source/unit hydration for direct chunk hits still need a stricter execution contract.
