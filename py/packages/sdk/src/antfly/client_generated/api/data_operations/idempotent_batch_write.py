...
def sync_detailed(
    ...
    *,
    ...
    idempotency receipt retention period (`transaction_sessions.receipt_ttl_seconds`); callers must not
    reuse a key after that period. Terminal rejections persist and replay their original status, code,
    message, and retryability. Every durable receipt store uses atomic owner fencing so an expired
    process incarnation can be recovered after restart; distributed deployments also fail closed unless
    that fenced store is cluster-shared.
