...
termite_venv="${TERMITE_E2E_VENV:-/tmp/antfly-ci-venv/termite}"
export HF_HOME="${HF_HOME:-/tmp/antfly-ci-hf}"
export TERMITE_HOME="${TERMITE_HOME:-/tmp/antfly-ci-termite}"
...
mkdir -p "$HOME" "$UV_CACHE_DIR" "$(dirname "$antfly_venv")" "$(dirname "$termite_venv")" "$HF_HOME" "$TERMITE_HOME" "$ZIG_LOCAL_CACHE_DIR" "$ZIG_GLOBAL_CACHE_DIR"
...
if [[ ! -x ./zig-out/bin/antfly ]]; then
  echo "missing ./zig-out/bin/antfly; rerun without SKIP_BUILD=1 or build install-antfly first" >&2
  exit 1
fi
if [[ ! -x ./zig-out/bin/termite ]]; then
