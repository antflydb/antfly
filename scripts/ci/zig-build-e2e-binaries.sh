...
if [[ "$build_capi" == "true" ]]; then
  # C API packaging is independent of the installed executable edition. This
  # second build should only relink: it reuses the distributed PIC archive
  # generated by the full production graph above.
  python3 tools/run_bounded_zig_build.py --zig zig -- build \
    -Dtarget="$target" \
    -Dcpu="$cpu" \
    -Doptimize="$optimize" \
