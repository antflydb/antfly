...
parser.add_argument('--root', type=Path, default=worktree / '.benchmark-results' / ('vector-refinement-experiments-' + time.strftime('%Y%m%d-%H%M%S')))
...
for feature in (NEXT if args.suite == 'next' else FIRST):
    ...
    command = [python, str(snapshot / 'run_vector_store_enrichment_ab.py'), str(root),
               '--binary', str(binary), '--refinement', feature, '--pairs', str(args.pairs), '--port', str(args.port)]
    result = {'feature': feature, 'command': command, 'started_at': time.time(),
...
if any(result['exit_code'] for result in results):
    raise SystemExit('one or more comparisons failed qualification; inspect experiments.json')
