...
def main():
    ...
    parser.add_argument('--refinement', choices=list(TREATMENTS),
    ...
    environment.update(ANTFLY_HBC_POSTING_SIDECAR='1', ANTFLY_HBC_POSTING_WAL_STORE='1',
                       ANTFLY_HBC_VECTOR_BLOCK_STORE='1', ANTFLY_HBC_VECTOR_BLOCK_ENCODING=args.encoding)
    result = {'refinement': args.refinement, 'binary': str(binary), 'binary_sha256': binary_hash,
              'workload_sha256': workload_hash, 'encoding': args.encoding,
              'rows': args.rows, 'arms': []}
    ...
    for pair in range(args.pairs):
        ...
        for mode in modes:
            ...
            arm = {'pair': pair + 1, 'mode': mode, 'table_mode': table_mode,
                   'refinement_environment': {k: arm_environment.get(k) for k in ALL_FLAGS}, 'load_before': os.getloadavg(), 'rss_samples_kib': []}
            ...
            command = [str(binary), 'standalone', '--host', '127.0.0.1', '--port', str(args.port),
                       ...
                       '--process-memory-budget-mb', '4096', '--data-dir', str(root / 'data')]
            with (root / 'server.log').open('w') as log:
                server = subprocess.Popen(command, stdout=log, stderr=subprocess.STDOUT, env=arm_environment)
                done = threading.Event()

                def sample():
                    while not done.wait(1):
                        try:
                            raw = subprocess.check_output(['ps', '-o', 'rss=', '-p', str(server.pid)], text=True)
                ...
                try:
                    with httpx.Client(timeout=10) as client:
                        def ready(table=False):
                            ...
                            while time.monotonic() < deadline:
                                ...
                                try:
                                    response = client.get(base + ('/source_enrichment' if table else ''))
                                    if response.status_code == 200:
                                        value = response.json()
                                        if not table:
                                            return value
                                        status = client.get(base + '/source_enrichment/indexes/semantic').json().get('status', {})
                                        if status.get('readiness', {}).get('complete') and status.get('query_visible_doc_count') == args.rows:
                                            return value
                        ...
                        ready()
                        subprocess.run([sys.executable, str(workload), '--port', str(args.port), '--mode', table_mode,
                                        '--rows', str(args.rows), '--queries', str(args.queries), '--output', str(root / 'enrichment.json')], check=True)
                        arm['enrichment'] = json.loads((root / 'enrichment.json').read_text())
                        stop(server)
                    ...
                    arm['sampled_peak_rss_kib'] = max(arm['rss_samples_kib'], default=None)
                    save(args.root / 'comparison.json', result)
            print(f"{pair + 1} {mode}: readiness {[p['ready_s'] for p in arm['enrichment']['phases']]}, disk {arm['total_logical_bytes']}", flush=True)
    summaries = {}
    for mode in (['control', 'candidate'] if args.refinement else ['primary_lsm', 'vector_store']):
        ...
        metrics = {
            ...
            'semantic_failed_queries': lambda arm: arm['enrichment']['semantic']['failed_queries'],
            'full_text_qps': lambda arm: arm['enrichment']['full_text']['qps'],
            'full_text_failed_queries': lambda arm: arm['enrichment']['full_text']['failed_queries'],
        ...
        source_counters = (
            'prepare_requests', 'prepare_batches', 'prepared_payloads', 'prepare_lock_wait_ns',
            'decode_outside_lock_ns', 'durable_append_ns', 'snapshot_read_ns',
            'directory_bytes_written', 'directory_entries', 'directory_hits', 'directory_misses',
            'source_segments', 'ownership_index_collections', 'ownership_index_entries_scanned',
            'location_cache_hits', 'location_cache_misses', 'location_cache_bytes', 'cache_reclaimed_bytes',
            'retired_ann_references_skipped', 'retained_payload_bytes', 'collection_bytes_written',
            'collection_mark_ns', 'checkpoint_bytes_written', 'heap_bytes',
        )
        for counter in source_counters:
            metrics['source_' + counter] = lambda arm, key=counter: (
                arm['enrichment'].get('table', {}).get('storage_status', {}).get('source_vectors') or {}
            ).get(key)
        for name, get in metrics.items():
            values = [get(arm) for arm in arms]
            available = [value for value in values if value is not None]
            summaries[mode][name] = {'median': statistics.median(available) if available else None,
                                    'min': min(available) if available else None,
                                    'max': max(available) if available else None, 'runs': values}
    ...
    result['paired_ratios'] = [
        {name: (source / primary if source is not None and primary else None)
         for name, get in metrics.items()
         for source in [get(next(a for a in result['arms'] if a['pair'] == pair and a['mode'] == ('candidate' if args.refinement else 'vector_store')))]
         for primary in [get(next(a for a in result['arms'] if a['pair'] == pair and a['mode'] == ('control' if args.refinement else 'primary_lsm')))]
        } for pair in range(1, args.pairs + 1)
    ]
    save(args.root / 'comparison.json', result)
    if not result['qualified']:
        raise SystemExit('enrichment query failures disqualify this comparison; inspect comparison.json')
