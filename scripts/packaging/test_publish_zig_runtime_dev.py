...
class PublishZigRuntimeDevTest(unittest.TestCase):
    def test_archive_has_root_binary_and_share_antfly(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            result = subprocess.run([str(SCRIPT), "--tag", "test", "--arch", arch], cwd=ROOT, text=True, capture_output=True, env=os.environ | {"PATH": f"{bindir}:{os.environ['PATH']}", "CAPTURED_ARCHIVE": str(archive)})
