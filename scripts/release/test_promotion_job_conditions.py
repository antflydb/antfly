...
def condition(job: str) -> str:
    ...
    match = re.search(
        r"^    if: (.*?)(?=^    [\w-]+:)", block, re.MULTILINE | re.DOTALL
    )
    return (
        re.search(r"\$\{\{(.*?)\}\}", match.group(1), re.DOTALL).group(1)
        if match
        else "success()"
    )
...
def admitted(
    ...
    *,
    ...
    expression = re.sub(
        r"needs\.prepare-release-promotion\.outputs\.publish_(?:npm|pypi)",
        repr(publish),
        expression,
    )
    expression = (
        expression.replace("cancelled()", str(cancelled))
            ...
            for ancestor in ("skipped", "success"):
                with self.subTest(job=job, ancestor=ancestor):
                    self.assertTrue(
                        admitted(
                            condition(job),
                            dict.fromkeys(dependencies, "success"),
                            ancestor=ancestor,
                        )
                    )
            ...
            for policy in ("false", ""):
                with self.subTest(job=job, policy=policy):
                    self.assertFalse(
                        admitted(
                            condition(job),
                            dict.fromkeys(JOBS[job], "success"),
                            publish=policy,
                        )
                    )
