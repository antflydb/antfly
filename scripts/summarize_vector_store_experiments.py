...
def summarize(root):
    ...
    for receipt in json.loads((root / 'experiments.json').read_text()):
        ...
        for arm in comparison.get('arms', []):
            ...
            source = enrichment.get('table', {}).get('storage_status', {}).get('source_vectors') or {}
            restarted = arm.get('table_after_restart', {}).get('storage_status', {}).get('source_vectors') or {}
            arms.append({
                'pair': arm['pair'], 'mode': arm['mode'],
                'semantic_failed_queries': enrichment.get('semantic', {}).get('failed_queries'),
                'full_text_failed_queries': enrichment.get('full_text', {}).get('failed_queries'),
                'source_before_restart': source, 'source_after_restart': restarted,
            })
        summaries.append({
            'feature': receipt['feature'], 'exit_code': receipt.get('exit_code'),
            'qualified': comparison.get('qualified', False) and receipt.get('exit_code') == 0,
            'binary_sha256': receipt['binary_sha256'],
            'median_paired_ratios': medians, 'arms': arms,
        })
    return summaries
...
def main():
    ...
    columns = ['initial_ready_s', 'update_ready_s', 'semantic_qps',
               'semantic_p99_ms', 'total_logical_bytes', 'sampled_peak_rss_kib']
    print('experiment qualified ' + ' '.join(columns))
    for row in result:
        values = row['median_paired_ratios']
        print(row['feature'], row['qualified'],
