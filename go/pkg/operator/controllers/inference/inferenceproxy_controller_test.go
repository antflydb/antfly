...
var _ = Describe("InferenceProxy Controller", func() {
	const (
		routeNamespace = "default"
	)

	Context("When creating a InferenceProxy with valid pool references", func() {
		It("Should mark the route as active", func() {
			ctx := context.Background()

			// First create a InferencePool that the route will reference
			...
			createdRoute := &antflyaiv1alpha1.InferenceProxy{}
	...
	Context("When creating a InferenceProxy with invalid pool references", func() {
	...
	Context("When creating a InferenceProxy with multiple destinations", func() {
		It("Should validate all pool references", func() {
			ctx := context.Background()

			// Create two pools
			pool1 := &antflyaiv1alpha1.InferencePool{
				ObjectMeta: metav1.ObjectMeta{
				...
				Spec: antflyaiv1alpha1.InferencePoolSpec{
					WorkloadType: antflyaiv1alpha1.WorkloadTypeGeneral,
					Models: antflyaiv1alpha1.ModelConfig{
						Preload:         []antflyaiv1alpha1.ModelSpec{{Name: "model-1", Kind: antflyaiv1alpha1.ModelKindGenerator}},
						LoadingStrategy: antflyaiv1alpha1.LoadingStrategyEager,
					},
			...
			pool2 := &antflyaiv1alpha1.InferencePool{
				ObjectMeta: metav1.ObjectMeta{
	...
	Context("When creating a InferenceProxy with model matching", func() {
		It("Should accept routes with model patterns", func() {
			...
			pool := &antflyaiv1alpha1.InferencePool{
				...
				Spec: antflyaiv1alpha1.InferencePoolSpec{
					WorkloadType: antflyaiv1alpha1.WorkloadTypeGeneral,
					Models: antflyaiv1alpha1.ModelConfig{
						Preload:         []antflyaiv1alpha1.ModelSpec{{Name: "bge-small-en-v1.5", Kind: antflyaiv1alpha1.ModelKindEmbedder}},
						LoadingStrategy: antflyaiv1alpha1.LoadingStrategyEager,
					},
					Replicas: antflyaiv1alpha1.ReplicaConfig{Min: 1, Max: 3},
					Hardware: antflyaiv1alpha1.HardwareConfig{},
	...
	Context("When creating a InferenceProxy with fallback configuration", func() {
		It("Should accept routes with fallback settings", func() {
			ctx := context.Background()

			// Create pools
			primaryPool := &antflyaiv1alpha1.InferencePool{
				ObjectMeta: metav1.ObjectMeta{
			...
			fallbackPool := &antflyaiv1alpha1.InferencePool{
				ObjectMeta: metav1.ObjectMeta{
