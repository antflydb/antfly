...
### Hot-Standby Swarm HA (`ha-hot-standby-swarm.yaml`)
A primary/standby swarm-mode pair using the typed `/admin/v1/ha` operator
workflow.

**Features:**
- primary and standby `AntflyCluster` resources in `mode: Swarm`
...
**Deploy:**
```bash
kubectl apply -f examples/ha-hot-standby-swarm.yaml
```
