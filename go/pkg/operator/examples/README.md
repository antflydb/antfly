...
### Standalone Cluster (`standalone-cluster.yaml`)
A single-node standalone-mode Antfly cluster managed by the operator.

**Features:**
- explicit `mode: Standalone`
- single StatefulSet instead of split metadata/data StatefulSets
- one PVC via `storage.standaloneStorage`
...
**Deploy:**
```bash
kubectl apply -f examples/standalone-cluster.yaml
```

### Hot-Standby Standalone HA (`ha-hot-standby-standalone.yaml`)
A primary/standby standalone-mode pair using the typed hot-standby admin API
(the operator still calls the pre-0.3 `/admin/v1/ha` spelling; servers serve
it as an alias of `/admin/v1/standby`).

**Features:**
- primary and standby `AntflyCluster` resources in `mode: Standalone`
- durable HA WAL/runtime flags generated by the operator
- typed admin execution through the hot-standby admin API
...
**Deploy:**
```bash
kubectl apply -f examples/ha-hot-standby-standalone.yaml
```
...
Standalone mode is the exception. It runs a single operator-managed StatefulSet and uses `spec.standalone` plus `storage.standaloneStorage` instead of `metadataNodes` / `dataNodes`.
