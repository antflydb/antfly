...
func newExaProvider(config WebSearchConfig) (*apiKeyProvider, error) {
	apiKey := getConfigOrEnv(&config.ApiKey, "EXA_API_KEY")
	if apiKey == "" {
...
func newSerperProvider(config WebSearchConfig) (*apiKeyProvider, error) {
	apiKey := getConfigOrEnv(&config.ApiKey, "SERPER_API_KEY")
	if apiKey == "" {
...
func newTavilyProvider(config WebSearchConfig) (*apiKeyProvider, error) {
	apiKey := getConfigOrEnv(&config.ApiKey, "TAVILY_API_KEY")
	if apiKey == "" {
...
func newBraveProvider(config WebSearchConfig) (*apiKeyProvider, error) {
	apiKey := getConfigOrEnv(&config.ApiKey, "BRAVE_API_KEY")
	if apiKey == "" {
...
func newYouProvider(config WebSearchConfig) (*apiKeyProvider, error) {
	apiKey := getConfigOrEnv(&config.ApiKey, "YOU_API_KEY")
	if apiKey == "" {
...
func newLinkupProvider(config WebSearchConfig) (*apiKeyProvider, error) {
	apiKey := getConfigOrEnv(&config.ApiKey, "LINKUP_API_KEY")
	if apiKey == "" {
...
func newVertexProvider(config WebSearchConfig) (*apiKeyProvider, error) {
	return nil, fmt.Errorf("vertex web search requires a configured web_search connection; direct websearch_config is not supported")
}
