...
Update, 17 September 2026: the GLiNER2.5 walkthrough and the Training section (`/training` plus three per-model pages) postdate the 9 September review. The snapshot is pinned to published `origin/main` at `b3f6b49ee340dab61337376e88a94003793027da` and currently indexes 72 op kinds, 445 Metal kernels, 25 routes, 1,492 environment-name references and 209 snippets, exporting 26 HTML files (24 routes plus the 404 pages) across 6 model walkthroughs, 4 training pages and the systems pages. The `/dev/kitchen-sink` component fixture is reachable under `pnpm dev` but stripped from the export. The 9 September review covers neither the GLiNER2.5 content nor the Training section.
