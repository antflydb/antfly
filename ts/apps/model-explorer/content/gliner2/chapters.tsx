    ...
    <div>
      ...
      >
        <Scene id="fused" graphic={<FusedKernelPairFigure />}>
          <p>
            ...
            <code>…_bwd_scores_f32</code>, <code>…_bwd_dv_f32</code>,{" "}
            <code>…_bwd_dq_dk_f32</code> and <code>…_bwd_dqr_dkr_f32</code> kernels implement the gradient calculation in <em>training</em>. Antfly
            connects these through a custom VJP (vector-Jacobian product) for the fused graph node; this is
      ...
      >
        <Scene id="gather" graphic={<SpanHeadFigure step={0} />}>
          <p>
            ...
            every label-marker position — <code>[E]</code>, <code>[C]</code> or <code>[R]</code> — while token states pool into word
      ...
      >
        <Scene id="spine" graphic={<GlinerSpineFigure />}>
          <p>
            GLiNER2 comes in through the same server, session manager and tokenizer as every
            decoder here, and the same planner batches its encoder layers into Metal frames. Where
            they are enabled, its Q4_K linear layers can also run through the generated quantized-
            kernel routes. What it skips, it skips structurally: no
            KV stage, no sampler.
          </p>
          <p>
