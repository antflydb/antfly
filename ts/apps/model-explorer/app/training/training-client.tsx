...
const DOORS = [
  { id: "recipe", label: "recipe.json", sub: "gemma4 · qwen · gliner2 lifecycle", color: "var(--kfam-matvec)", x: 20 },
  { id: "flags", label: "flag CLI (~55 flags)", sub: "gliner2-autodiff", color: "var(--kfam-attention)", x: 160 },
  { id: "job", label: "job.json (supervised)", sub: "gliner25 one-shot worker", color: "var(--kfam-fusion)", x: 300 },
];
...
function TrainingMapFigure() {
  return (
    ...
    >
      {DOORS.map((d) => (
        <g key={d.id}>
          <rect x={d.x} y={26} width={120} height={40} rx={5} fill={`color-mix(in oklch, ${d.color} 16%, transparent)`} stroke={d.color} strokeWidth={1.25} />
          <text x={d.x + 60} y={43} textAnchor="middle" fontSize={8.5} className="fill-foreground font-mono">
            {d.label}
          </text>
          <text x={d.x + 60} y={57} textAnchor="middle" fontSize={7} className="fill-muted-foreground font-mono">
            {d.sub}
          </text>
          <path d={`M ${d.x + 60} 66 L 220 108`} stroke="var(--muted-foreground)" strokeWidth={1} />
        </g>
      ))}
      <rect x={140} y={112} width={160} height={30} rx={5} fill="color-mix(in oklch, var(--kfam-mmsg) 14%, transparent)" stroke="var(--kfam-mmsg)" strokeWidth={1.5} />
      ...
      <rect x={175} y={162} width={90} height={26} rx={5} fill="color-mix(in oklch, var(--kfam-sampling) 16%, transparent)" stroke="var(--kfam-sampling)" strokeWidth={1.25} />
...
function ForwardBackwardFigure() {
  return (
    ...
    >
      {FWD_NODES.map((n, i) => (
        <g key={n.id}>
          <rect x={n.x} y={44} width={70} height={26} rx={4} fill="color-mix(in oklch, var(--kfam-attention) 14%, transparent)" stroke="var(--kfam-attention)" strokeWidth={1} />
          <text x={n.x + 35} y={61} textAnchor="middle" fontSize={8.5} className="fill-foreground font-mono">
            {n.label}
          </text>
          {i < FWD_NODES.length - 1 && <path d={`M ${n.x + 70} 57 h 35`} stroke="var(--muted-foreground)" strokeWidth={1.25} />}
        </g>
      ))}
      <text x={425} y={61} textAnchor="end" fontSize={8} className="fill-muted-foreground font-mono">
        →
      </text>
      {FWD_NODES.map((n, i) => (
        <g key={`b-${n.id}`}>
          <rect x={n.x} y={112} width={70} height={26} rx={4} fill="none" stroke={i === 1 ? "var(--kfam-fusion)" : "var(--muted-foreground)"} strokeWidth={i === 1 ? 1.5 : 0.9} strokeDasharray="5 3" />
          <text x={n.x + 35} y={129} textAnchor="middle" fontSize={8.5} className={i === 1 ? "fill-foreground font-mono" : "fill-muted-foreground font-mono"}>
            {i === 1 ? "custom VJP" : `d(${n.label})`}
          </text>
          {i < FWD_NODES.length - 1 && <path d={`M ${n.x + 105} 125 h -35`} stroke="var(--muted-foreground)" strokeWidth={0.9} strokeDasharray="5 3" />}
        </g>
      ))}
      <path d="M 380 70 v 42" stroke="var(--muted-foreground)" strokeWidth={0.9} strokeDasharray="5 3" />
...
function SharedSpineFigure({ step }: { step: 0 | 1 }) {
  return (
    ...
    >
      <rect x={160} y={20} width={120} height={24} rx={4} fill="color-mix(in oklch, var(--kfam-mmsg) 14%, transparent)" stroke="var(--kfam-mmsg)" strokeWidth={1.25} />
      ...
      <rect x={170} y={62} width={100} height={22} rx={4} fill="color-mix(in oklch, var(--kfam-attention) 14%, transparent)" stroke="var(--kfam-attention)" strokeWidth={1.25} />
      <text x={220} y={77} textAnchor="middle" fontSize={8.5} className="fill-foreground font-mono">
        gradient()
      </text>
      <path d="M 220 84 L 120 106 M 220 84 L 320 106" stroke="var(--muted-foreground)" strokeWidth={1} />
      <g opacity={step === 0 ? 1 : 0.7} className="transition-opacity duration-300">
        <rect x={30} y={110} width={180} height={40} rx={5} fill="color-mix(in oklch, var(--kfam-matvec) 14%, transparent)" stroke="var(--kfam-matvec)" strokeWidth={1.25} />
        <text x={120} y={126} textAnchor="middle" fontSize={8.5} className="fill-foreground font-mono">
          RealAutodiffTrainer
        </text>
        <text x={120} y={140} textAnchor="middle" fontSize={7} className="fill-foreground font-mono">
          decoders · gliner2 — accum, clip @ 1.0, LR schedule
        </text>
        <rect x={230} y={110} width={180} height={40} rx={5} fill="color-mix(in oklch, var(--kfam-fusion) 14%, transparent)" stroke="var(--kfam-fusion)" strokeWidth={1.25} />
        <text x={320} y={126} textAnchor="middle" fontSize={8.5} className="fill-foreground font-mono">
          seeded gradient trainer
        </text>
        <text x={320} y={140} textAnchor="middle" fontSize={7} className="fill-foreground font-mono">
          GLiNER2.5 — staged accumulate → validate → publish
        </text>
      </g>
      <path d="M 120 150 L 200 176 M 320 150 L 240 176" stroke="var(--muted-foreground)" strokeWidth={1} />
      <g opacity={step === 1 ? 1 : 0.7} className="transition-opacity duration-300">
        <rect x={165} y={180} width={110} height={26} rx={5} fill="color-mix(in oklch, var(--kfam-sampling) 18%, transparent)" stroke="var(--kfam-sampling)" strokeWidth={step === 1 ? 1.75 : 1.25} />
        <text x={220} y={197} textAnchor="middle" fontSize={9} className="fill-foreground font-mono">
          AdamW step
        </text>
      </g>
    </Figure>
...
const CLI_DOORS = [
  { id: "recipe", cmd: "antfly inference finetune run <recipe.json>", sub: "recipe runner: gemma4 DPO/GRPO, gliner2 lifecycle", color: "var(--kfam-matvec)" },
  { id: "flags", cmd: "… finetune train run gliner2-autodiff [~55 flags]", sub: "flag CLI, in-process", color: "var(--kfam-attention)" },
  { id: "job", cmd: "… finetune train run gliner25 <job.json>", sub: "supervised one-shot worker (parent ⇄ worker)", color: "var(--kfam-fusion)" },
];
      ...
      <div className="py-8">
        <header className="mx-auto max-w-7xl px-4">
          ...
          <p className="mt-2 max-w-3xl text-muted-foreground">
            ...
            follows how models learn in the Zig runtime: preference optimization for Gemma4, and
            two very different finetuning stacks for GLiNER2 and GLiNER2.5. Everything here anchors
            to real training code; nothing is a benchmark or a quality claim.
          </p>
          <div className="mt-4 flex flex-wrap gap-2 font-mono text-xs">
            <Link href="/training/gemma4" className="rounded-full border px-3 py-1 transition-colors hover:border-primary/60">
              Gemma4 · DPO &amp; GRPO →
            </Link>
            <Link href="/training/gliner2" className="rounded-full border px-3 py-1 transition-colors hover:border-primary/60">
              GLiNER2 finetuning →
            </Link>
            <Link href="/training/gliner25" className="rounded-full border px-3 py-1 transition-colors hover:border-primary/60">
        ...
        >
          <Scene id="map" graphic={<TrainingMapFigure />}>
            <p>
              Gemma4 (and the Qwen decoders) train through a <em>recipe</em>: a JSON file the
              recipe runner turns into a CPU autodiff training loop. GLiNER2 trains{" "}
              <em>graph-first</em>: a flag-driven CLI builds the same DeBERTa forward graph
              inference uses, grafts loss nodes on, and lets autodiff do the rest. GLiNER2.5 trains{" "}
              <em>contract-first</em>: a pinned JSON job under a disposable supervised worker,
              where every random draw and detach boundary is named and hashed. All three converge
              on the same differentiation engine.
            </p>
            <p>
        ...
        >
          <Scene id="gradient" graphic={<SharedSpineFigure step={0} />}>
            <p>
              ...
              LoRA parameter slices — while GLiNER2.5 uses the seeded gradient trainer, which
              stages every accumulation and update, validates it, and only then publishes.
            </p>
            <p>
          ...
          <Scene id="adamw" graphic={<SharedSpineFigure step={1} />}>
            <p>
              ...
              configured once in the graph library. What differs per family is the discipline
              around the step: fixed-divisor accumulation for GLiNER2 and the decoders, and
              partial-window renormalization for GLiNER2.5 (its page explains why that matters).
            </p>
            <p>
              <CodeLink link={L("train-adamw-config")} /> · <CodeLink link={L("train-adamw-step")} />
            </p>
          </Scene>
        </ScrollyChapter>
        ...
        >
          <Scene id="doors" graphic={<CliDoorsFigure />}>
            ...
            <p>
              <CodeLink link={L("train-recipe-cli")} /> · <CodeLink link={L("gliner2-ft-cli-registry")} /> ·{" "}
        ...
        >
          <Scene id="determinism" graphic={<ReceiptsFigure step={0} />}>
            ...
            <p>
              <CodeLink link={L("gemma4-train-dpo-report")} /> · <CodeLink link={L("gliner25-ft-export")} />
            </p>
          </Scene>
          <Scene id="not-here" graphic={<ReceiptsFigure step={1} />}>
            <p>
              ...
              CPU by construction. The Metal training graph executor for GLiNER2 is opt-in{" "}
              behind an environment flag, and the heavyweight quality gates (Python-oracle parity,
              Metal/CUDA hardware matrices) run operator-side rather than in CI. The three model
              pages carry their own boundary chapters with specifics.
            </p>
            <p>
