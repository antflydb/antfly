...
function LoraAdapterStackFigure({ step }: { step: 0 | 1 }) {
  return (
    ...
    >
      <rect x={60} y={40} width={170} height={90} rx={6} fill="color-mix(in oklch, var(--dtype-f16) 14%, transparent)" stroke="var(--dtype-f16)" strokeWidth={1.25} />
      <text x={145} y={88} textAnchor="middle" fontSize={9} className="fill-muted-foreground font-mono">
        frozen W
      </text>
      <rect x={252} y={40} width={26} height={90} rx={4} fill="color-mix(in oklch, var(--kfam-sampling) 20%, transparent)" stroke="var(--kfam-sampling)" strokeWidth={1.25} />
      <text x={265} y={88} textAnchor="middle" fontSize={8} className="fill-foreground font-mono">
        A
      </text>
      <rect x={290} y={72} width={90} height={26} rx={4} fill="color-mix(in oklch, var(--kfam-sampling) 20%, transparent)" stroke="var(--kfam-sampling)" strokeWidth={1.25} />
      ...
      {step === 0 ? (
        <g>
          {LORA_TARGETS.map((t, i) => (
            <g key={t}>
              <rect x={30 + i * 55} y={158} width={50} height={18} rx={3} fill="color-mix(in oklch, var(--kfam-matvec) 14%, var(--background))" stroke="var(--kfam-matvec)" strokeWidth={0.9} />
              <text x={55 + i * 55} y={170} textAnchor="middle" fontSize={6.5} className="fill-foreground font-mono">
                {t}
              </text>
            </g>
          ))}
          <text x={220} y={196} textAnchor="middle" fontSize={7.5} className="fill-muted-foreground font-mono">
            preset all-linear: every one of the seven modules gets an adapter
          </text>
        </g>
      ) : (
        <g>
          <rect x={90} y={152} width={260} height={30} rx={5} fill="color-mix(in oklch, var(--kfam-matvec) 14%, transparent)" stroke="var(--kfam-matvec)" strokeWidth={1.25} />
          <text x={220} y={171} textAnchor="middle" fontSize={8.5} className="fill-foreground font-mono">
            RealAutodiffTrainer · LR 1e-4 · clip 1.0 · seed 42
          </text>
          <text x={220} y={200} textAnchor="middle" fontSize={7.5} className="fill-muted-foreground font-mono">
...
const DPO_LAYERS = [
  { id: "loss", label: "preference_loss.zig", sub: "math only — 6 losses, analytic grads, gradient tests", color: "var(--kfam-attention)" },
  { id: "harness", label: "preference_harness.zig", sub: "gradients, no optimizer — calls the model 4×", color: "var(--kfam-fusion)" },
  { id: "recipe", label: "recipe.zig · runOptimizerBackedGemmaDpo", sub: "optimizer + checkpoints — saves a LoRA bundle", color: "var(--kfam-matvec)" },
];
...
function DpoThreeLayerFigure() {
  return (
    ...
    >
      {DPO_LAYERS.map((l, i) => (
        <g key={l.id}>
          <rect x={50} y={26 + i * 58} width={340} height={40} rx={5} fill={`color-mix(in oklch, ${l.color} 14%, transparent)`} stroke={l.color} strokeWidth={1.25} />
          <text x={220} y={43 + i * 58} textAnchor="middle" fontSize={8.5} className="fill-foreground font-mono">
            {l.label}
          </text>
          <text x={220} y={57 + i * 58} textAnchor="middle" fontSize={7} className="fill-muted-foreground font-mono">
            {l.sub}
          </text>
          {i < 2 && <path d={`M 220 ${66 + i * 58} v 18`} stroke="var(--muted-foreground)" strokeWidth={1.25} />}
        </g>
      ))}
    </Figure>
  );
...
function DpoLossFigure() {
  ...
  return (
    ...
    >
      ...
      <line x1={180} y1={40} x2={180} y2={135} stroke="var(--border)" strokeWidth={0.75} strokeDasharray="3 3" />
      ...
      <text x={190} y={144} textAnchor="middle" fontSize={7.5} className="fill-muted-foreground font-mono">
        reward margin
      </text>
      <text x={135} y={158} textAnchor="middle" fontSize={7} className="fill-muted-foreground font-mono">
        rejected
      </text>
      <text x={245} y={158} textAnchor="middle" fontSize={7} className="fill-muted-foreground font-mono">
        chosen
      </text>
      <rect x={310} y={52} width={110} height={110} rx={5} fill="none" stroke="var(--border)" strokeWidth={1} strokeDasharray="6 3" />
      <text x={365} y={68} textAnchor="middle" fontSize={7} className="fill-muted-foreground font-mono">
        library-only
      </text>
      {SIBLING_LOSSES.map((s, i) => (
        <g key={s} opacity={0.7}>
          <rect x={325} y={76 + i * 17} width={80} height={13} rx={3} fill="color-mix(in oklch, var(--muted-foreground) 12%, transparent)" stroke="var(--muted-foreground)" strokeWidth={0.6} />
          <text x={365} y={86 + i * 17} textAnchor="middle" fontSize={7} className="fill-foreground font-mono">
            {s}
          </text>
        </g>
      ))}
...
function ReferencePolicyFigure() {
  return (
    ...
    >
      <text x={115} y={30} textAnchor="middle" fontSize={9} className="fill-muted-foreground font-mono">
        adapter-disable trick
      </text>
      <rect x={45} y={44} width={140} height={70} rx={6} fill="color-mix(in oklch, var(--kfam-matvec) 14%, transparent)" stroke="var(--kfam-matvec)" strokeWidth={1.25} />
      <text x={115} y={74} textAnchor="middle" fontSize={8.5} className="fill-foreground font-mono">
        policy weights
      </text>
      <rect x={150} y={50} width={44} height={16} rx={3} fill="color-mix(in oklch, var(--kfam-sampling) 22%, transparent)" stroke="var(--kfam-sampling)" strokeWidth={1} transform="rotate(18 172 58)" />
      <text x={186} y={44} fontSize={6.5} className="fill-muted-foreground font-mono">
        LoRA off ↷
      </text>
      <text x={115} y={132} textAnchor="middle" fontSize={7} className="fill-muted-foreground font-mono">
        same weights, called twice
      </text>
      <text x={330} y={30} textAnchor="middle" fontSize={9} className="fill-primary font-mono">
        second LoadedModel
      </text>
      <rect x={250} y={44} width={72} height={70} rx={6} fill="color-mix(in oklch, var(--kfam-matvec) 14%, transparent)" stroke="var(--kfam-matvec)" strokeWidth={1.25} />
      <text x={286} y={82} textAnchor="middle" fontSize={7.5} className="fill-foreground font-mono">
        policy
      </text>
      <rect x={338} y={44} width={72} height={70} rx={6} fill="none" stroke="var(--kfam-kv)" strokeWidth={1.25} strokeDasharray="5 3" />
      <rect x={362} y={58} width={24} height={16} rx={2} fill="none" stroke="var(--kfam-kv)" strokeWidth={1} />
      <path d="M 368 58 v -5 a 6 6 0 0 1 12 0 v 5" fill="none" stroke="var(--kfam-kv)" strokeWidth={1} />
      <text x={374} y={100} textAnchor="middle" fontSize={7.5} className="fill-foreground font-mono">
        reference
      </text>
      <text x={330} y={132} textAnchor="middle" fontSize={7} className="fill-muted-foreground font-mono">
...
const CE_CELLS = [0, 1, 2, 3, 4, 5, 6, 7].map((i) => ({ id: `cell-${i}`, x: 60 + i * 36, hot: i === 4 }));
...
function CeInjectionFigure() {
  return (
    ...
    >
      {CE_CELLS.map((c) => (
        <rect key={c.id} x={c.x} y={50} width={30} height={22} rx={3} fill={c.hot ? "color-mix(in oklch, var(--kfam-fusion) 30%, transparent)" : "color-mix(in oklch, var(--muted-foreground) 10%, transparent)"} stroke={c.hot ? "var(--kfam-fusion)" : "var(--border)"} strokeWidth={c.hot ? 1.5 : 0.75} />
      ))}
      <text x={220} y={40} textAnchor="middle" fontSize={8} className="fill-muted-foreground font-mono">
        one-hot CE target row (the sequence's tokens)
      </text>
      <circle cx={220} cy={110} r={20} fill="none" stroke="var(--kfam-sampling)" strokeWidth={1.5} />
      ...
      <text x={220} y={180} textAnchor="middle" fontSize={8.5} className="fill-foreground font-mono">
        CE backward ⇒ exactly the DPO gradient
      </text>
      <text x={220} y={196} textAnchor="middle" fontSize={7} className="fill-muted-foreground font-mono">
...
function GrpoGroupFigure() {
  return (
    ...
    >
      <text x={110} y={26} textAnchor="middle" fontSize={8.5} className="fill-muted-foreground font-mono">
        raw rewards (one group)
      </text>
      <line x1={60} y1={GROUP_MEAN_Y} x2={160} y2={GROUP_MEAN_Y} stroke="var(--muted-foreground)" strokeWidth={0.9} strokeDasharray="4 3" />
      ...
      {GROUP_REWARDS.map((r, i) => (
        <g key={r.id}>
          ...
          <text x={75 + i * 26} y={r.y - 10} textAnchor="middle" fontSize={6.5} className="fill-muted-foreground font-mono">
            {r.v}
          </text>
        </g>
      ))}
      <path d="M 200 62 h 40" stroke="var(--muted-foreground)" strokeWidth={1.25} />
      <text x={220} y={54} textAnchor="middle" fontSize={7} className="fill-muted-foreground font-mono">
        −μ, ÷σ
      </text>
      ...
      {GROUP_REWARDS.map((r, i) => (
        <circle key={`a-${r.id}`} cx={285 + i * 34} cy={62 + (r.y - GROUP_MEAN_Y) * 1.2} r={5} fill="var(--kfam-fusion)" opacity={0.9} />
      ))}
      <text x={220} y={140} textAnchor="middle" fontSize={8} className="fill-muted-foreground font-mono">
        group_size: 8 in the loss struct and scalar route — but 2 on every
      </text>
      <text x={220} y={154} textAnchor="middle" fontSize={8} className="fill-muted-foreground font-mono">
...
function GrpoLossFigure() {
  ...
  return (
    ...
    >
      ...
      <polyline points={clipPts.join(" ")} fill="none" stroke="var(--kfam-attention)" strokeWidth={2} />
      <rect x={60} y={150 - 1.2 * 60} width={310} height={0.4 * 60} fill="color-mix(in oklch, var(--kfam-attention) 8%, transparent)" />
      <text x={215} y={64} textAnchor="middle" fontSize={7.5} className="fill-muted-foreground font-mono">
        clip band: ratio ∈ [0.8, 1.2]
      </text>
      {/* k3 is a U with its minimum at Δ=0: lowest value at the center. */}
      <path d="M 60 162 q 155 26 310 0" fill="none" stroke="var(--kfam-kv)" strokeWidth={1.5} strokeDasharray="5 3" />
      <text x={215} y={196} textAnchor="middle" fontSize={7.5} className="fill-muted-foreground font-mono">
      ...
      {step === 0 ? (
        <g>
          {ROLLOUT_RANKS.map((r, i) => (
            <g key={r.id}>
              <rect x={60} y={36 + i * 34} width={r.w} height={20} rx={3} fill="color-mix(in oklch, var(--kfam-matvec) 18%, transparent)" stroke="var(--kfam-matvec)" strokeWidth={1} />
              <path d={`M ${60 + r.w} ${46 + i * 34} h ${40}`} stroke="var(--muted-foreground)" strokeWidth={1} />
              <rect x={100 + r.w} y={36 + i * 34} width={150} height={20} rx={3} fill="color-mix(in oklch, var(--kfam-fusion) 12%, transparent)" stroke="var(--kfam-fusion)" strokeWidth={0.9} />
              <text x={175 + r.w} y={49 + i * 34} textAnchor="middle" fontSize={7} className="fill-foreground font-mono">
                {r.label} → rollout lane {i}
              </text>
            </g>
          ))}
          <rect x={352} y={70} width={40} height={40} rx={6} fill="none" stroke="var(--muted-foreground)" strokeWidth={1.25} />
          ...
          <text x={372} y={132} textAnchor="middle" fontSize={7} className="fill-muted-foreground font-mono">
            no sampling
          </text>
          <text x={220} y={196} textAnchor="middle" fontSize={7.5} className="fill-muted-foreground font-mono">
        ...
        <g>
          {[
            { id: "em", rule: "exact_match", detail: "1.0 exact · 0.5 substring · 0.0 else", y: 44 },
            ...
            <g key={r.id}>
              <rect x={70} y={r.y} width={300} height={34} rx={5} fill="color-mix(in oklch, var(--kfam-sampling) 12%, transparent)" stroke="var(--kfam-sampling)" strokeWidth={1} />
              <text x={220} y={r.y + 14} textAnchor="middle" fontSize={8.5} className="fill-foreground font-mono">
                {r.rule}
              </text>
              <text x={220} y={r.y + 27} textAnchor="middle" fontSize={7} className="fill-muted-foreground font-mono">
                {r.detail}
              </text>
            </g>
          ))}
          <text x={220} y={198} textAnchor="middle" fontSize={7.5} className="fill-muted-foreground font-mono">
...
function TwoTrainerFigure() {
  return (
    ...
    >
      <rect x={30} y={60} width={60} height={44} rx={5} fill="color-mix(in oklch, var(--kfam-moe) 16%, transparent)" stroke="var(--kfam-moe)" strokeWidth={1.25} />
      ...
      <rect x={118} y={62} width={80} height={40} rx={5} fill="color-mix(in oklch, var(--kfam-mmsg) 16%, transparent)" stroke="var(--kfam-mmsg)" strokeWidth={1.25} />
      <text x={158} y={86} textAnchor="middle" fontSize={8} className="fill-foreground font-mono">
        projector
      </text>
      <path d="M 198 76 L 250 52 M 198 90 L 250 118" stroke="var(--muted-foreground)" strokeWidth={1} />
      <rect x={254} y={34} width={150} height={38} rx={5} fill="color-mix(in oklch, var(--kfam-matvec) 16%, transparent)" stroke="var(--kfam-matvec)" strokeWidth={1.25} />
      <text x={329} y={57} textAnchor="middle" fontSize={8} className="fill-foreground font-mono">
        policy trainer (LoRA)
      </text>
      <rect x={254} y={102} width={150} height={38} rx={5} fill="none" stroke="var(--kfam-kv)" strokeWidth={1.25} strokeDasharray="5 3" />
      <rect x={318} y={110} width={18} height={12} rx={2} fill="none" stroke="var(--kfam-kv)" strokeWidth={1} />
      <path d="M 322 110 v -4 a 5 5 0 0 1 10 0 v 4" fill="none" stroke="var(--kfam-kv)" strokeWidth={1} />
      <text x={329} y={134} textAnchor="middle" fontSize={7.5} className="fill-foreground font-mono">
        reference trainer (frozen)
      </text>
      <path d="M 329 142 v 14 L 240 172" stroke="var(--muted-foreground)" strokeWidth={0.9} />
      <rect x={140} y={164} width={200} height={24} rx={4} fill="color-mix(in oklch, var(--kfam-kv) 10%, transparent)" stroke="var(--kfam-kv)" strokeWidth={1} />
      <text x={240} y={180} textAnchor="middle" fontSize={7.5} className="fill-foreground font-mono">
...
function DatasetFormatsFigure() {
  return (
    <div className="flex h-full flex-col justify-center gap-3">
      <div>
        <div className="mb-1.5 font-mono text-[10px] uppercase tracking-wider text-muted-foreground">DPO datasets (JSONL)</div>
      ...
      <div>
        <div className="mb-1.5 font-mono text-[10px] uppercase tracking-wider text-muted-foreground">GRPO datasets (JSONL)</div>
        <div className="grid grid-cols-3 gap-1.5">
          {GRPO_FORMATS.map((f) => (
            <div key={f.id} className="rounded-md border p-2">
              <div className="font-mono text-[10px]">{f.label}</div>
      ...
      {step === 0 ? (
        <>
          <div className="overflow-hidden rounded-lg border">
            {DEFAULTS_ROWS.map(([k, v]) => (
              <div key={k} className="grid grid-cols-2 border-b font-mono text-[10.5px] last:border-b-0">
        ...
        <>
          <div className="rounded-lg border p-3">
            <div className="font-mono text-[10px] text-muted-foreground">antfly_inference_finetune_dpo_report/v1</div>
            <div className="mt-1 font-mono text-[11px]">{"{ examples, loss, mean_reward_margin, accuracy, beta }"}</div>
          </div>
          <div className="rounded-lg border p-3">
            <div className="font-mono text-[10px] text-muted-foreground">antfly_inference_finetune_grpo_report/v1</div>
            <div className="mt-1 font-mono text-[11px]">{"{ completions, tokens, groups, loss, pg_loss, kl_loss, clip_fraction }"}</div>
          </div>
          <p className="text-center font-mono text-[10px] text-muted-foreground">
...
function SilentDowngradeFigure({ step }: { step: 0 | 1 }) {
  return (
    <div className="flex h-full flex-col justify-center gap-4">
      {step === 0 ? (
        <>
          <div className="mx-auto rounded-lg border px-4 py-2 font-mono text-xs">recipe.json (kind: dpo)</div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="rounded-lg border border-dashed p-3">
              <div className="font-mono text-[10px] text-muted-foreground">no adapter anywhere ⌀</div>
              <div className="mt-2 font-mono text-[10px] text-muted-foreground">
                → harness-only scoring
                <br />→ dpo_report.json written, <span style={{ color: "var(--kfam-text-kv)" }}>no gradients ever flow</span>
              </div>
            </div>
          </div>
          <p className="text-center font-mono text-[10px] text-muted-foreground">
...
export function Gemma4TrainingClient({
  snippets,
  gitCommit,
  permalinkBase,
}: {
        ...
        >
          <Scene id="layers" graphic={<DpoThreeLayerFigure />}>
            <p>
              ...
              knows how to call <em>any</em> model's log-probability function four times (policy
              and reference, chosen and rejected) and hand back gradients — it owns no optimizer by
              contract. At the top, the optimizer-backed Gemma path calls the same loss math directly, one
              example at a time, and wires its analytic gradients into the trainer — the harness
              itself serves the scoring-only fallthrough. A LoRA bundle is saved when it's done.
            </p>
            <p>
          ...
          <Scene id="loss" graphic={<DpoLossFigure />}>
            <p>
              ...
              <code>−logSigmoid(β · ((π_chosen − π_ref_chosen) − (π_rejected − π_ref_rejected)))</code>{" "}
              with β defaulting to 0.1 — no label smoothing, no conservative-DPO epsilon. The
              enum next to it names five siblings — IPO, KTO, SimPO, ORPO, CPO — all implemented
              and tested (five of the six carry finite-difference gradient checks; IPO's test pins
              its optimum instead), and all <em>library-only</em>: the recipe runner only ever
              constructs DPO.
            </p>
            <p>
        ...
        >
          <Scene id="two-ways" graphic={<ReferencePolicyFigure />}>
            <p>
              ...
              path takes the sturdier road instead and loads a second full model from disk,
              scoring reference log-probabilities through a frozen forward pass. Both shipped DPO routes in fact take the second road — the adapter-disable
              default is exercised only by the harness's own tests. Either way a reference is a
              hard requirement for DPO, IPO and KTO — missing reference logps are a
        ...
        >
          <Scene id="advantages" graphic={<GrpoGroupFigure />}>
            <p>
              ...
              statistic is the <em>entire</em> baseline. There is no value function and no GAE;
              the group does the critic's job. The loss struct defaults to groups of 8, but every
          ...
          <Scene id="surrogate" graphic={<GrpoLossFigure />}>
            <p>
              ...
              adds a KL leash at weight 0.04, and the run reports its <code>clip_fraction</code> —
              a record of how often the clamp fired. A finite-difference gradient test lives
              beside the loss.
            </p>
            <p>
        ...
        >
          <Scene id="ranked" graphic={<RolloutRankFigure step={0} />}>
            <p>
              ...
              the runner-up, and so on, capped at min(group, 8). Completions run to at most 4
              tokens by default. There is no temperature, no top-p, no seeded multinomial — the
              entire rollout is reproducible by construction.
            </p>
            <p>
          ...
          <Scene id="multimodal" graphic={<TwoTrainerFigure />}>
            <p>
              GRPO even runs multimodal: with a vision projector configured, the recipe
              instantiates <em>two</em> trainers over the same backend — the LoRA policy and a
              frozen reference — and hard-refuses any reference path that isn't the base model, so
              the KL leash always measures drift from the true starting point.
            </p>
            <p>
        ...
        >
          <Scene id="formats" graphic={<DatasetFormatsFigure />}>
            <p>
              ...
              without sampling — or <code>text-grpo</code> / <code>rendered-text-grpo</code> rows
              of prompt and target, optionally with image paths for the multimodal variant.
            </p>
            <p>
          ...
          <Scene id="defaults" graphic={<DefaultsTableFigure step={0} />}>
            <p>
              The numbers worth knowing, exactly as pinned in source. Two asymmetries are real and deliberate. The first is sequence length: DPO trains
                at 512, GRPO at 128, and only DPO's scoring-only path stretches to 2048. The
                second is group size: the model-backed GRPO routes run groups of 2, while the
                scalar route keeps the struct default of 8.
            </p>
          </Scene>
          <Scene id="reports" graphic={<DefaultsTableFigure step={1} />}>
            <p>
              Every run ends in a versioned report. DPO writes the loss, the mean reward margin,
              the accuracy — the fraction of pairs where that margin came out positive — and β.
              GRPO writes its policy-gradient loss, its KL loss, the clip fraction and the group
              count. The smoke harness
              exercises both end-to-end against a synthetic one-layer, hidden-32 Gemma checkpoint —
              real code, toy weights.
            </p>
            <p>
        ...
        >
          <Scene id="downgrade" graphic={<SilentDowngradeFigure step={0} />}>
            <p>
              ...
              the run falls through to harness-only scoring: it still exits
              cleanly and still writes <code>dpo_report.json</code> — but no gradient ever flows.
              For a run that was meant to train, the report looking plausible is exactly the trap.
            </p>
            <p>
              <CodeLink link={L("gemma4-train-dpo-gate")} />
            </p>
          ...
          <Scene id="scope" graphic={<SilentDowngradeFigure step={1} />}>
            <p>
              The rest of the fence, plainly: decoder training is CPU-only by construction, with
              the reference model doubling host memory and activation checkpointing off. The five
              sibling losses have no recipe route. Several finetune modules in the tree are
              orphaned and deliberately undocumented here. And nothing in this runtime does PPO,
              RLHF, or learned rewards — GRPO's group mean plus string-match rules is the entire
              story. The GLiNER pages tell a very different one:{" "}
