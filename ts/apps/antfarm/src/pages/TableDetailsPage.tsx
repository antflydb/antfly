...
const TableDetailsPage: React.FC<TableDetailsPageProps> = ({ currentSection = "overview" }) => {
  ...
  const [artifactManifests, setArtifactManifests] = useState<DocumentArtifactManifest[]>([]);
  ...
  const [reprocessArtifactName, setReprocessArtifactName] = useState("document_units_v1");
  const [reprocessFromKey, setReprocessFromKey] = useState("");
  const [reprocessToKey, setReprocessToKey] = useState("");
  const [reprocessLimit, setReprocessLimit] = useState("100");
  const [reprocessJob, setReprocessJob] = useState<DocumentArtifactReprocessJob | null>(null);
  const [reprocessPass, setReprocessPass] = useState<DocumentArtifactTableReprocessResponse | null>(
    null
  );
  ...
  const loadArtifactDetail = useCallback(
    async (artifactName: string) => {
      if (!tableName || !artifactDocumentKey.trim() || !artifactName) return;
      setArtifactLoading(true);
      setArtifactError(null);
      try {
        const detail = await api.tables.artifacts.get(
          tableName,
          artifactDocumentKey.trim(),
          artifactName,
          "raw"
        );
        setSelectedArtifactDetail(detail ?? null);
      } catch (e) {
        setArtifactError(`Failed to load artifact ${artifactName}.`);
        console.error(e);
      } finally {
        setArtifactLoading(false);
      }
    },
    [artifactDocumentKey, tableName]
  );

  const loadArtifactManifests = useCallback(async () => {
    if (!tableName || !artifactDocumentKey.trim()) return;
    setArtifactLoading(true);
    setArtifactError(null);
    setArtifactMessage(null);
    try {
      const response = await api.tables.artifacts.list(tableName, artifactDocumentKey.trim());
      const artifacts = response?.artifacts ?? [];
      setArtifactManifests(artifacts);
      const nextSelected =
      ...
      setArtifactManifests([]);
      setSelectedArtifactDetail(null);
      setArtifactError(`No artifact manifests found for ${artifactDocumentKey.trim()}.`);
      console.error(e);
    } finally {
      setArtifactLoading(false);
    }
  }, [artifactDocumentKey, selectedArtifactName, tableName]);
  ...
  const handleReprocessDocumentArtifact = async () => {
    if (!tableName || !artifactDocumentKey.trim() || !selectedArtifactName) return;
    setArtifactLoading(true);
    setArtifactError(null);
    setArtifactMessage(null);
    try {
      await api.tables.artifacts.reprocessDocument(
        tableName,
        artifactDocumentKey.trim(),
        selectedArtifactName
      );
      setArtifactMessage(`Reprocess accepted for ${selectedArtifactName}.`);
      await loadArtifactDetail(selectedArtifactName);
    } catch (e) {
      setArtifactError(`Failed to reprocess ${selectedArtifactName}.`);
      console.error(e);
    } finally {
      setArtifactLoading(false);
    }
  ...
  const reprocessRequest = () => ({
    ...(reprocessFromKey.trim() ? { from_key: reprocessFromKey.trim() } : {}),
    ...(reprocessToKey.trim() ? { to_key: reprocessToKey.trim() } : {}),
    ...(parseOptionalLimit(reprocessLimit) ? { limit: parseOptionalLimit(reprocessLimit) } : {}),
  });

  const handleRunReprocessPass = async () => {
    if (!tableName || !reprocessArtifactName.trim()) return;
    setReprocessLoading(true);
    setReprocessError(null);
    try {
      const response = await api.tables.artifacts.reprocessRange(
        tableName,
        reprocessArtifactName.trim(),
        reprocessRequest()
      );
      setReprocessPass(response ?? null);
    } catch (e) {
      setReprocessError(`Failed to run reprocess pass for ${reprocessArtifactName.trim()}.`);
      console.error(e);
    } finally {
  ...
  const handleStartReprocessJob = async () => {
    ...
    setReprocessError(null);
    try {
      const response = await api.tables.artifacts.startReprocessJob(
        tableName,
        reprocessArtifactName.trim(),
        { ...reprocessRequest(), advance: true }
      );
      setReprocessJob(response ?? null);
      setReprocessPass(null);
    } catch (e) {
      setReprocessError(`Failed to start reprocess job for ${reprocessArtifactName.trim()}.`);
      console.error(e);
    } finally {
      setReprocessLoading(false);
    }
  ...
  const handleAdvanceReprocessJob = async () => {
    if (!tableName || !reprocessJob) return;
    setReprocessLoading(true);
    setReprocessError(null);
    try {
      const response = await api.tables.artifacts.advanceReprocessJob(
        tableName,
        reprocessJob.artifact_name,
        String(reprocessJob.job_id)
      );
      setReprocessJob(response ?? null);
    } catch (e) {
      setReprocessError(`Failed to advance job ${reprocessJob.job_id}.`);
      console.error(e);
    } finally {
      setReprocessLoading(false);
    }
  ...
  const handleCancelReprocessJob = async () => {
    if (!tableName || !reprocessJob) return;
    setReprocessLoading(true);
    setReprocessError(null);
    try {
      ...
      setReprocessError(`Failed to cancel job ${reprocessJob.job_id}.`);
      console.error(e);
    } finally {
      setReprocessLoading(false);
    }
          ...
          <div className="grid gap-4 xl:grid-cols-[360px_1fr]">
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Input
                      ...
                      value={artifactDocumentKey}
                      onChange={(event) => setArtifactDocumentKey(event.target.value)}
                      onKeyDown={(event) => {
                        if (event.key === "Enter") {
                          event.preventDefault();
                      ...
                      placeholder="doc:a"
                    />
                    <Button onClick={loadArtifactManifests} disabled={artifactLoading}>
                      Load
                    </Button>
                  </div>
                </div>
                ...
                {artifactMessage && (
                  <Alert>
                    <AlertDescription>{artifactMessage}</AlertDescription>
                  </Alert>
                )}
                ...
                <div className="space-y-2">
                  ...
                  {artifactManifests.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {artifactManifests.map((artifact) => (
                        <button
                ...
                <Button
                  variant="outline"
                  onClick={handleReprocessDocumentArtifact}
                  disabled={artifactLoading || !selectedArtifactName}
                  className="w-full"
                >
              ...
              <CardContent className="space-y-4">
                {selectedArtifactDetail ? (
                  ...
                  <div className="rounded border border-dashed p-6 text-sm text-muted-foreground">
                    Select an artifact manifest to inspect its route, generation, chunks, ranges,
                    errors, and raw manifest state.
                  </div>
                )}
              </CardContent>
            </Card>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    value={reprocessArtifactName}
                    onChange={(event) => setReprocessArtifactName(event.target.value)}
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      value={reprocessFromKey}
                      onChange={(event) => setReprocessFromKey(event.target.value)}
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      id="reprocess-to-key"
                      value={reprocessToKey}
                      onChange={(event) => setReprocessToKey(event.target.value)}
                ...
                <div className="flex flex-wrap gap-2">
                  <Button onClick={handleStartReprocessJob} disabled={reprocessLoading}>
                    Start job
                  </Button>
                  <Button
                    variant="outline"
                    onClick={handleRunReprocessPass}
                ...
                {reprocessJob ? (
                  <>
                    <div className="flex flex-wrap items-center gap-2">
                      <Badge className={jobStatusClass(reprocessJob)}>{reprocessJob.phase}</Badge>
                      <Badge>{reprocessJob.reprocess_status}</Badge>
                      <span className="text-sm text-muted-foreground">
                        Job {reprocessJob.job_id} for {reprocessJob.artifact_name}
                      </span>
                    </div>
                    <div className="grid gap-3 md:grid-cols-5">
                      <div>
                        ...
                        <div className="text-lg font-semibold">
                          {formatCount(reprocessJob.scanned)}
                        </div>
                      </div>
                      <div>
                        <div className="text-xs text-muted-foreground">Reprocessed</div>
                        <div className="text-lg font-semibold">
                          {formatCount(reprocessJob.reprocessed)}
                        </div>
                      </div>
                      <div>
                        <div className="text-xs text-muted-foreground">Skipped</div>
                        <div className="text-lg font-semibold">
                          {formatCount(reprocessJob.skipped)}
                        </div>
                      </div>
                      <div>
                        <div className="text-xs text-muted-foreground">Failed</div>
                        <div className="text-lg font-semibold">
                          {formatCount(reprocessJob.failed)}
                        </div>
                      </div>
                      <div>
                        <div className="text-xs text-muted-foreground">Pending Shards</div>
                        <div className="text-lg font-semibold">
                          {formatCount(reprocessJob.pending_shards)}
                        </div>
                      </div>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      <Button
                        variant="outline"
                        onClick={handleAdvanceReprocessJob}
                        disabled={
                          reprocessLoading ||
                          ["succeeded", "failed", "cancelled"].includes(reprocessJob.phase)
                        }
                      >
                        Advance
                      </Button>
                      <Button
                        variant="ghost"
                        onClick={handleCancelReprocessJob}
                        disabled={
                          reprocessLoading ||
                          ["succeeded", "failed", "cancelled"].includes(reprocessJob.phase)
                        }
                    ...
                    {reprocessJob.failures.length > 0 && (
                      <JsonViewer json={{ failures: reprocessJob.failures }} />
                    )}
                  </>
                ) : reprocessPass ? (
                  <>
                    <div className="flex flex-wrap items-center gap-2">
                      <Badge>{reprocessPass.reprocess_status}</Badge>
                      <span className="text-sm text-muted-foreground">
                        Bounded pass for {reprocessPass.artifact_name}
                      </span>
                    </div>
                    <div className="grid gap-3 md:grid-cols-4">
                      <div>
                        <div className="text-xs text-muted-foreground">Scanned</div>
                        <div className="text-lg font-semibold">
                          {formatCount(reprocessPass.scanned)}
                        </div>
                      </div>
                      <div>
                        <div className="text-xs text-muted-foreground">Reprocessed</div>
                        <div className="text-lg font-semibold">
                          {formatCount(reprocessPass.reprocessed)}
                        </div>
                      </div>
                      <div>
                        <div className="text-xs text-muted-foreground">Skipped</div>
                        <div className="text-lg font-semibold">
                          {formatCount(reprocessPass.skipped)}
                        </div>
                      </div>
                      <div>
                        <div className="text-xs text-muted-foreground">Failed</div>
                        <div className="text-lg font-semibold">
                          {formatCount(reprocessPass.failed)}
                        </div>
                      </div>
                    </div>
                    <JsonViewer json={reprocessPass} />
                  </>
                ) : (
                  <div className="rounded border border-dashed p-6 text-sm text-muted-foreground">
                    Start a durable job for table-wide repair, or run one bounded pass when you want
                    to manually inspect cursor progress.
                  </div>
                )}
              </CardContent>
            </Card>
