...
- `degree` now has the first executable planned-build path. The opt-in planned
  ...
  raised. A conservative internal `auto` gate now chooses the planned path for
  one already-active planned build, one queued degree metric, one queued small
  PageRank metric, one queued bounded eigenvector metric, or an internally
  opted-in bounded queued compatible HITS pair. Larger single
  PageRank/eigenvector builds, default queued HITS, incompatible HITS pairs, and
  multi-metric graph indexes remain on local maintenance until latency and
  multi-metric coverage are ready. Coverage now proves queued degree, queued
  small PageRank, queued bounded eigenvector, and opt-in queued bounded
  compatible HITS enter planned maintenance, while the active-build side of that
  gate resumes explicitly started planned degree, eigenvector, and paired HITS
  builds through planned maintenance. The default idle path now uses that
  conservative `auto` gate, so safe queued single-metric work and already-active
  planned work use planned maintenance by default while larger/multi-metric and
  unsupported HITS cases still fall back to local maintenance. Default-mode
  coverage now proves queued degree, queued small PageRank, queued bounded
  eigenvector, and already-active planned PageRank enter planned maintenance,
  while larger PageRank, default queued HITS, and multi-metric indexes fall back
  to local maintenance under the same default `.auto` setting. A first internal graph-metric
...
- Distributed public query fan-in now preserves direct `graph_metric_results`
  ...
  hosted compatible HITS authority/hub direct and traversal
  projection/order/filter merge coverage with prior-pair preservation,
  `fresh` rejection, remote HITS generation/metadata/edge-filter mismatch
  rejection, and missing-status rejection.
...
1. Keep the conservative planned-maintenance `auto` gate and use telemetry to
   pick production-safe idle budgets. Do not make larger PageRank/eigenvector,
   default HITS, incompatible HITS, or multi-metric indexes planned by default
...
| --- | --- | --- | --- |
| 1. Scheduler default readiness | Planned maintenance becomes the scheduler path for safe workloads. | Keep the conservative `auto` gate while collecting latency data. The bounded planned-maintenance primitive should be the single internal entrypoint for queued build startup, worker page sweeps, coordinator sweeps, pending-work stats, and budget exhaustion. | `runUntilIdle` and background ticks drain degree, small PageRank, bounded eigenvector, compatible HITS, and already-active planned work without unbounded latency or partial visibility. Larger/multi-metric fallback remains intact. |
...
2. Promote planned maintenance by measured workload class.

   The default idle path now uses a conservative `auto` gate for safe single
   metric work and already-active planned builds. Keep that gate narrow until
   latency data and coverage prove each broader class. Do not promote all graph
   metric maintenance at once.

   Promotion sequence:

   - already-active planned builds
   - queued degree
   - queued small PageRank
   - queued bounded eigenvector
   - internally opted-in compatible HITS pairs
   - larger PageRank/eigenvector after budget and latency evidence
   - multi-metric indexes after fairness and fan-in coverage
   - default HITS and incompatible HITS only after paired-vector hardening
...
| --- | --- | --- | --- |
| M1. Scheduler default readiness | No public API change. Background maintenance may take longer than local execution for some workloads, but visible reads keep the same published/fresh contract. | Promote the bounded planned-maintenance primitive behind latency-safe scheduler budgets. Keep the conservative `auto` gate until larger PageRank/eigenvector, default HITS, incompatible HITS, and multi-metric indexes have latency data. | `runUntilIdle` and background ticks drain degree, PageRank, eigenvector, and compatible HITS; tiny budgets return resumable `budget_exhausted`; gated cases still use the local runner. |
...
| --- | --- | --- | --- |
| 1 | Default planned maintenance readiness | Keep the public API unchanged and move scheduler-style background graph metric work through one bounded planned-maintenance primitive. `runUntilIdle` has an explicit planned mode and a conservative internal `auto` gate that selects planned maintenance only for one queued degree, one small queued PageRank, one bounded queued eigenvector, one internally opted-in bounded queued compatible HITS pair, or one active planned build; the active-build path now has degree, eigenvector, and paired HITS evidence. Larger single PageRank/eigenvector, default queued HITS, incompatible HITS pairs, and multi-metric indexes remain on local maintenance until their latency and fan-in coverage is complete. | Budget exhaustion is resumable, pending-work stats expose queued/active graph metric work, and PageRank/degree/eigenvector/HITS complete through scheduler ticks without partial visibility or unbounded latency. |
...
The remaining implementation should land as narrow milestones with explicit
promotion gates:
...
1. Scheduler-ready planned maintenance.
   ...
   integration. The planned path can be selected explicitly and by conservative
   `auto` gates, but it should not become the broad default until latency is
   measured on single-metric, multi-metric, small-graph, and larger-graph cases.
...
| --- | --- | --- |
| Scheduler entrypoint | One bounded graph-metric maintenance primitive starts queued builds, ticks workers, ticks coordinators, reports progress, returns budget exhaustion as a resumable result, contributes queued/active graph-metric hints to pending-work stats, and is now the default `runUntilIdle` path through the conservative `auto` gate for safe workloads. Explicit planned mode remains available for tests/operators, and unsupported auto cases still fall back to local maintenance. Worker-only sweeps now also report budget exhaustion when they consume their page-step budget and the durable build remains active, so split runtimes can expose resumable work without relying on the combined maintenance loop. Worker-pool rounds spend page budget on actual worker steps rather than idle worker IDs. The auto gate now has coverage for default queued degree, explicit queued degree, one queued small PageRank build, one queued bounded eigenvector build, one internally opted-in bounded queued compatible HITS pair, default queued HITS fallback, incompatible opt-in HITS fallback, and already-active planned degree, eigenvector, and paired HITS builds. Default and explicit auto tests now inspect the gate's active/eligible/ineligible queued counters before `runUntilIdle` runs, proving queued degree, small PageRank, and bounded eigenvector are eligible, larger PageRank/eigenvector, multi-metric, default HITS, and incompatible HITS fallbacks are intentional gate decisions rather than accidental local-runner success, compatible opt-in HITS reports exactly one eligible queued pair before planned execution, and active planned PageRank/degree/eigenvector/HITS report exactly one active build before resumable planned execution. Controlled-widening tests now raise the internal PageRank and eigenvector iteration thresholds and prove those larger single-vector workloads become eligible planned work, exhaust tiny budgets as resumable active work, and finish after budget expansion. | Broaden default planned-maintenance promotion with latency evidence for larger PageRank/eigenvector, default HITS, incompatible HITS, and multi-metric `runUntilIdle` behavior. |
...
| --- | --- | --- | --- |
| 1 | Default planned maintenance | Route background graph metric rebuilds through the bounded planned-maintenance primitive instead of the local whole-metric runner. The explicit `runUntilIdle` planned gate proves the path can publish a small background PageRank build through planned maintenance, tiny planned-idle budgets fail fast while preserving resumable active work, and the default conservative auto gate proves queued degree, small queued PageRank, bounded queued eigenvector, internally opted-in bounded queued compatible HITS, and already-active planned PageRank, degree, eigenvector, and paired HITS can use planned maintenance without regressing larger single PageRank/eigenvector, default queued HITS, incompatible HITS pairs, or multi-metric indexes. The auto gate now exposes an internal decision summary for active builds plus eligible/ineligible queued work, default and explicit auto tests assert that queued degree, small PageRank, and bounded eigenvector are eligible while larger PageRank/eigenvector, multi-metric indexes, default HITS, and incompatible HITS remain gated before fallback, compatible opt-in HITS reports one eligible queued pair before planned execution, active planned PageRank/degree/eigenvector/HITS report one active build before planned execution, and controlled-widening tests prove raising the PageRank/eigenvector iteration thresholds makes the larger single-vector cases planned and resumable under tiny budgets. The remaining work is broader latency evidence and promotion for larger graph metric workloads. The call must make bounded progress and return `budget_exhausted` instead of treating unfinished work as failure. | `runUntilIdle` and scheduler-style ticks finish PageRank, degree, eigenvector, and paired HITS without unbounded latency or partial visibility. |
...
- `zig build graph-metric-process-test` builds the real `antfly` executable,
  seeds a graph degree DB through the library, runs supervisor mode with
  spawned child role processes, and verifies the degree metric is fresh for the
  target edge generation after child completion.
...
3. **Eigenvector parity gate**: reuse the PageRank single-vector executor
   ...
   drain against the local oracle. The lifecycle gate now covers scan,
...
6. **Latency and default-widening gate**: widen `auto` one family at a time only
   ...
   are routine. The `graph-metric-default-gate-test` gate now makes the current
...
- Promotion-scale use of the harness still needs deployment-sized graph indexes
  ...
  compatible HITS authority/hub direct and traversal projection/order/filter
  merges with active/stale prior-pair preservation, remote HITS
...
- Promotion-scale fan-in should use shard layouts beyond the focused hosted
  ...
  compatible HITS direct and traversal projection/order/filter fan-in with
  active/stale prior-pair preservation, remote HITS
  generation/metadata/edge-filter mismatch rejection, and missing remote HITS
