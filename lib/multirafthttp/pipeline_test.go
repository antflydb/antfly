...
func TestPipelineSend(t *testing.T) {
	...
	p.msgc <- multiMessage{msg: raftpb.Message{Type: raftpb.MsgApp}, shardID: types.ID(1)}
	tr.rec.Wait(1)
	p.stop()
...
func TestPipelineKeepSendingWhenPostErr(t *testing.T) {
...
func TestPipelineExceedMaximumServing(t *testing.T) {
	...
	for range connPerPipeline + pipelineBufSize {
		select {
		case p.msgc <- multiMessage{}:
...
// TestPipelineSendErr tests that when send func meets the post error,
// it increases fail count in stats.
func TestPipelineSendErr(t *testing.T) {
	picker := mustNewURLPicker(t, []string{"http://localhost:2380"})
...
func TestPipelinePost(t *testing.T) {
	...
	tp := &Transport{PipelineRt: tr}
	p := startTestPipeline(t, tp, picker)
	if err := p.post(types.ID(1), []byte("some data")); err != nil {
	...
	if g := req.Header.Get("X-Server-Version"); g != Version {
		t.Errorf("version = %s, want %s", g, Version)
	}
	if g := req.Header.Get("X-Min-Cluster-Version"); g != MinClusterVersion {
		t.Errorf("min version = %s, want %s", g, MinClusterVersion)
	}
	if g := req.Header.Get("X-Raft-Shard-ID"); g != "1" {
		t.Errorf("shard id = %s, want %s", g, "1")
	}
...
func TestPipelinePostBad(t *testing.T) {
	...
	for i, tt := range tests {
		...
		p := startTestPipeline(t, tp, picker)
		err := p.post(types.ID(1), []byte("some data"))
		p.stop()
...
func TestPipelinePostErrChan(t *testing.T) {
	tests := []struct {
		u    string
		code int
		err  error
	...
	for i, tt := range tests {
		...
		tp := &Transport{PipelineRt: newRespRoundTripper(tt.code, tt.err)}
		p := startTestPipeline(t, tp, picker)
		p.post(types.ID(1), []byte("some data"))
		p.stop()
		select {
		case <-p.errorc:
		default:
