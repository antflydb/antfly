...
row-query `OR` plan. Top-level non-negated scalar `IN` branches stay native as
typed `in_predicates` before composition and lower to composed
`access_or_predicates` only when every branch pair is proven disjoint from same
field scalar equality, `IS NOT DISTINCT FROM`, `IS NULL`, or another scalar
`IN` array over safe JSON scalar literals.
...
8. **CTEs and materialized stream bounds.**
   ...
   the composed plan stores each branch as an `access_or_predicates` group, and
   the proof accepts only same-field scalar equality, `IS NOT DISTINCT FROM`,
   `IS NULL`, or another scalar `IN` array when every compared JSON literal is a
   safe scalar or raw JSON number. Numeric range
