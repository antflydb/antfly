...
| `FOR [NO KEY] UPDATE [NOWAIT|SKIP LOCKED]` queues | Lockable base-row streams with durable owner/lease payloads, expired-lease reclaim, OCC integration, and ordered multi-range fill for catalog-routed non-joined mutation sources and joined target streams. | Keep claims illegal over joins/aggregates/CTEs; preserve claim owner/lease payloads and reclaim expired leases; test range movement, retries, aborts, and concurrent writers in chaos. |
...
3. **Mutation plans over lockable row sources.**
   ...
   rows back to their owner groups. Hosted remote mutation-source owners fail closed
   until a typed collect/stage wire contract lands; transaction resolution remains
   coordinator-owned for supported owner-local staging. Production coverage still needs
...
5. **Lateral, window, and advanced aggregate stages.**
   ...
   `FILTER`/`DISTINCT` expressions, and spill/backpressure policy for large aggregate and window stages are not
...
| `FOR [NO KEY] UPDATE [NOWAIT|SKIP LOCKED]` | Lockable base-row query plans with row-claim metadata, plus ordered catalog-routed `SKIP LOCKED` fill across owner ranges for non-joined mutation sources and joined target streams. | Keep claims limited to base-row streams. Add range-movement chaos coverage, retry hardening, and broader OCC/2PC evidence before allowing queue workloads at production scale. |
...
across two catalog tables. Compatible two-branch AS-OF cross-table set
operations require both branches to name the same commit sequence or timestamp
snapshot, read each catalog target through the system-time table-read vtable,
and then apply the native set reducer to those historical branch streams.
Cross-table branches are
...
10. **Order, limit, offset, and row claiming.**
   ...
   is an explicit lockable row-source contract. Catalog-routed mutation sources
   fill ordered `SKIP LOCKED` claims across owner ranges; queue claims still
   need retry and chaos coverage with range movement.
...
3. **Reporting and rollup SQL.**
   ...
   filters, aggregate expression pushdown, routed aggregate/window spill policy, and execution tests for usage
