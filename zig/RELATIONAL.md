...
9. **Aggregates, windows, and ordered streams.**
   ...
   `WITHIN GROUP (ORDER BY numeric_field_or_expression [ASC|DESC])` lower to native
   ...
   `percentile_max_items`, and `percentile_order`; `DISTINCT`, explicit
   ordered-set null placement, non-numeric keys, multiple fractions, and hidden
   SQL text are not accepted.
