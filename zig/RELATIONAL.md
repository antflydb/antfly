...
`CREATE UNIQUE INDEX ... NULLS NOT DISTINCT` is intentionally classified as an
unsupported DDL shape until null-equal uniqueness is represented in native
unique-owner keys, conflict-target inference, validation jobs, repair, and
write-time enforcement. The adapter must not normalize it to ordinary unique
