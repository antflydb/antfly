...
containers and relation-population plan container that wrap Antfly-native typed
query, aggregate, join, lateral, window, and CTE plans while preserving the
existing public entrypoints through facade aliases; `api/sql_adapter/lower_expr.zig`
owns the expression keyword, function-name, and tail-boundary predicates that
the shared expression lowerer uses; `api/relational_sql.zig` keeps the public
