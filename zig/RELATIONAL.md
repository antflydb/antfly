...
joined-source counts, and CTE chain counts, live in checked-in fixture JSON
under `api/fixtures/` and are interpreted by a generic `corpus.zig` runner with
named expected coverage buckets. That keeps regression examples data-driven
without hand-editing the generated parity corpus. Cross-table source-schema
coverage tokens for joined mutation, join, lateral, merge, and wrapped read
classifier plans are also validated in `corpus.zig`. Fixture summary-family policy
and structural summary-to-plan assertions, including DDL operation totals, DDL
