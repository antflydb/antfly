...
through the promotion target. The source set rejects
...
1. **SQL frontend and corpus gate.**
   ...
   corpus entries should be added to the checked-in source fixture. If a case
   seems to need local helper setup, prefer adding an explicit source-fixture
   field and `corpus.zig` validation path before reintroducing Zig-owned case
   data. The source set is promoted with
