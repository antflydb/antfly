...
ordinary `in_predicates` for `INTERSECT` conjunction, and typed
`access_not_predicates` for `EXCEPT` negation when the right branch has no
expression predicates. Scalar OR branches also compose natively:
...
typed `expression_not_predicates`. `UNION ALL` accepts scalar `IN` only when
every branch pair is proven disjoint from same-field scalar equality,
...
8. **CTEs and materialized stream bounds.**
   ...
   bounded branch conjunctions, and `EXCEPT` lowers a right-branch scalar OR to
   one `not_predicates` entry per branch. A right-branch scalar `IN`
   conjunction lowers to one `access_not_predicates` group unless that branch
   also has expression predicates. A right-branch expression OR lowers to one
   `expression_not_predicates` entry per branch; expression OR under
   `INTERSECT` waits for bounded expression-branch distribution. Numeric range
