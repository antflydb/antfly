...
expression predicates. `UNION ALL` accepts scalar `IN` only when every branch
pair is proven disjoint from same-field scalar equality, `IS NOT DISTINCT
FROM`, `IS NULL`, or another non-negated scalar `IN` array over safe JSON
scalar literals.
...
8. **CTEs and materialized stream bounds.**
   ...
   `IN` predicates to the conjunction, and `EXCEPT` lowers a right-branch
   scalar `IN` conjunction to one `access_not_predicates` group unless that
   branch also has expression predicates. Numeric range
