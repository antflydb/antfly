...
8. **CTEs and materialized stream bounds.**
   ...
   catalog-routed cross-table set operations, including `UNION ALL` and exact
   projected-row `EXCEPT` subtraction across two catalog tables. Cross-table
   branches are admitted only when the catalog supplies the right table's
   relational schema, so the adapter remains fail-closed for unknown sources
   and type-incompatible branch outputs. Different CTE sources, branch
   ordering/pagination, set-result tail ordering/pagination, recursive CTEs,
   and other general set-operation streams still fail closed as
   `set_operation_plan`.
