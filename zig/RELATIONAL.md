...
`array_agg`, `string_agg`, `bool_or`, or `bool_and`, `field` and
...
`percentile`/`percentile_max_items` are legal only for `percentile_cont`, and
...
| Query composition | Typed row queries for scalar, array, JSONB, OR, NOT, ordering, pagination, row claims, aggregates including exact bounded continuous percentile aggregates, local/CTE/declared multi-range equality joins, bounded lateral stages, non-recursive CTEs with output-field validation across row-query and derived-stage consumers, fail-closed derived-result output-reference validation, and local or declared multi-range ranking/value windows over ordered base/CTE streams. | Add catalog-owned cross-table routing, richer derived-stream type metadata, spill bounds for materialized streams, broader ordered-set/discrete percentile aggregate stages, ordered-set/percentile window stages, and full scalar-expression ordering/projection/filtering. |
...
| Expression indexes and expression unique keys | Generated columns for simple stable secondary-index expressions; typed unique metadata for case-fold and stable unary hash expression keys | Lower secondary expressions such as `lower(email)`, `upper(email)`, `md5(email)`, and simple `concat(...)` keys to generated columns when possible; persist unique expressions such as `lower(email)`, `upper(email)`, and `md5(email)` as deterministic unique metadata with typed input fields. Native schema JSON also accepts `generated: { "op": "expression", "expression": <row-expression AST> }` for deterministic generated columns, and storage persists that AST with the same row-expression representation used by checks, partial predicates, update transforms, and returning projections. Write planning, row rewrite/backfill, index generation hashing, dependency checks, and column rename/drop handling must operate on the AST rather than raw SQL text. SQL DDL may keep lowering common expression-index syntax to shorthand generated-column forms, but broader generated expressions should enter through the typed schema/API contract until the SQL adapter can produce the same AST fail-closed. Expand the common expression AST with casts/functions, collation, null semantics, rebuild metadata, and write-time maintenance as richer expression-derived indexes land. |
...
9. **Aggregates, windows, and ordered streams.**
   ...
   boundary.
   Exact continuous percentile aggregates have a bounded typed reducer:
   `percentile_cont(fraction) WITHIN GROUP (ORDER BY numeric_field_or_expression)`
   lowers to native `op: percentile_cont` with explicit `percentile` and
   `percentile_max_items` metadata. The reducer skips missing/JSON-null inputs,
   rejects non-numeric values, sorts only the bounded per-group sample, and uses
   PostgreSQL-style continuous interpolation between adjacent ordered values.
   The native API exposes the same contract directly through
   `percentile_cont`, `percentile`, and `percentile_max_items`; `DISTINCT`,
   descending/null-modified ordered-set keys, non-numeric keys, multiple
   fractions, and hidden SQL text are not accepted.
   ...
   them, plus broader ordered-set/discrete percentile aggregate and window
