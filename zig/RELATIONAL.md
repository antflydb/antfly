...
columns, stream endpoint, direction, format, CSV header intent, and delimiter
byte, then fails closed when applied to table schema or runtime storage. The
...
checks, 2PC staging, retryable range routing, and deterministic error reporting
for imports, and through typed snapshot/range-read plans for exports. PostgreSQL `COPY` syntax
...
`PROGRAM`, alternate stream endpoints, and richer COPY options such as quote,
escape, null-marker, freeze, and encoding controls have
