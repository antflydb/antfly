...
`CREATE UNIQUE INDEX ... NULLS DISTINCT` is accepted as explicit syntax for the
ordinary unique-owner semantics. `CREATE UNIQUE INDEX ... NULLS NOT DISTINCT`
is intentionally classified as an unsupported DDL shape until null-equal
uniqueness is represented in native unique-owner keys, conflict-target
inference, validation jobs, repair, and write-time enforcement. The adapter must
not normalize it to ordinary unique metadata, because ordinary unique semantics
do not treat multiple `NULL` values as the same key.
