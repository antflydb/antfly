...
9. **Aggregates, windows, and ordered streams.**
   ...
   boundary.
   Ordered-set aggregate syntax such as
   `percentile_cont(...) WITHIN GROUP (ORDER BY ...)` is reserved for a future
   typed percentile/ordered-set reducer. Until that reducer exists, SQL/API
   parity fixtures require those shapes to fail closed as
   `ordered_set_aggregate_plan`; storage must not receive a generic aggregate
   with hidden SQL text or an unbounded coordinator sort.
   ...
   them, plus native percentile/ordered-set aggregate and window stages with
   deterministic order metadata, bounded memory/spill policy, null handling,
   interpolation rules, and routed merge semantics. Aggregate source queries
