...
9. **Aggregates, windows, and ordered streams.**
   ...
   `WITHIN GROUP (ORDER BY numeric_field_or_expression [ASC|DESC] [NULLS FIRST|LAST])` lower to native
   ...
   `percentile_max_items`, and `percentile_order`. Explicit SQL
   ...
   non-numeric keys, multiple fractions, and hidden SQL text are not accepted.
