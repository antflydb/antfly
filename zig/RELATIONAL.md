...
`replace`, `regexp_replace`, `regexp_like`, `regexp_count`, `translate`, `substring`, and `overlay`, and text-boundary predicates such as `starts_with` and
...
hit row/byte caps. Those output schemas preserve base identity only for streams
that directly project every primary-key component from the source stream;
derived aliases or partial-key streams remain read-only materialized streams
rather than pretending to be base rows. Aggregate
...
temporal DML, temporal unique conflict upserts, and range-column query lowering. The parity coverage asserts
...
by storage. Mutating temporal update actions remain rejected at the SQL/API
boundary because changing a parent interval/key requires update-side period
action semantics that are broader than a scalar child-row rewrite. The parity
corpus explicitly pins supported temporal delete-action DDL and fail-closed
coverage for update-side mutating temporal FK actions. Typed
...
selectors from non-owner API nodes. Remaining production work is expanded chaos
coverage that combines `FOR PORTION OF` DML, temporal FK checks, range movement,
repair, and catalog promotion in one generated workload.
...
predicate branches, plus proven-disjoint scalar and expression `UNION ALL`
branches over canonical string, boolean, null equality, exact complementary
...
The seeded in-code parity corpus also requires computed `LIKE` / `ILIKE`
...
unique-owner row in the same 2PC path as ordinary column unique owners. Supported
unique expressions include case-fold keys, `lower(field)` and `upper(field)`, and
stable unary hash keys, `md5(field)`, over declared string-like relational
columns.
...
`upper`, `initcap`, `trim`, `ltrim`, `rtrim`, `replace`, `translate`, `substring`, `overlay`, `split_part`, `strpos`, `left`, `right`, `lpad`, `rpad`, `repeat`, `reverse`, `concat`, `concat_ws`, `length`, `octet_length`, `ascii`, `chr`, `md5`, `starts_with`, `ends_with`, `like`, `ilike`, boolean `and`/`or`/`not`, `nullif`, `greatest`, `least`, `abs`, `round`, `trunc`, `floor`, `ceil`, `sqrt`, `sign`, `power`, numeric and fixed-duration interval arithmetic, searched `case`, typed `cast` to text, numeric, boolean, or datetime,
...
`UPPER(...)`, `INITCAP(...)`,
`TRIM(...)`, `BTRIM(...)`, `LTRIM(...)`, `RTRIM(...)`, `SUBSTRING(...)`, `SUBSTRING(... FROM ... [FOR ...])`, `SUBSTR(...)`, `SPLIT_PART(...)`, `STRPOS(...)`, `POSITION(... IN ...)`, `LEFT(...)`, `RIGHT(...)`, `LPAD(...)`, `RPAD(...)`, `REPEAT(...)`, `REVERSE(...)`, `STARTS_WITH(...)`, `ENDS_WITH(...)`, `ASCII(...)`, `CHR(...)`, `MD5(...)`, `DATE_TRUNC(...)`, `DATE_BIN(...)`, `DATE_PART(...)`, `EXTRACT(...)`, `CONCAT(...)`, `CONCAT_WS(...)`, text-like `lhs || rhs` concat expressions, `LENGTH(...)`, `OCTET_LENGTH(...)`, `COALESCE(...)`, `NULLIF(...)`, `GREATEST(...)`, `LEAST(...)`, `ABS(...)`, `ROUND(...)`, `TRUNC(...)`, `FLOOR(...)`, `CEIL(...)`, `SQRT(...)`, `SIGN(...)`, `POWER(...)`, searched `CASE`, `CAST(...)` including encoded timestamp casts,
...
`for_update`; the distinct mode is durable metadata, not a weaker write claim.
...
`lower`, `upper`, `initcap`, `trim`, `ltrim`, `rtrim`, `replace`, `regexp_replace`, `translate`,
...
`concat_ws`, `length`, `octet_length`, `nullif`, `greatest`, `least`, `abs`,
...
`upper`, `initcap`, `trim`/`btrim`, `ltrim`, `rtrim`, `replace`, `regexp_replace`, `substring`, `overlay`, `split_part`, `strpos`,
...
matches through the same compiled regex evaluator.
...
`LENGTH(text_expr) AS label`, `OCTET_LENGTH(text_expr) AS label`, `NULLIF(...) AS label`, `GREATEST(...) AS label`, `LEAST(...) AS label`, `ABS(...) AS label`, `ROUND(...) AS label`, `FLOOR(...) AS label`, `CEIL(...) AS label`, searched `CASE WHEN ... THEN ... ELSE ... END AS label`,
...
spans.
Claimed row queries require a transaction-bound `for_update` or
`for_no_key_update` claim and reject
...
`op` is one of `count`, `sum`, `min`, `max`, `avg`, `array_agg`, `string_agg`, `bool_or`, or `bool_and`, `field` and
...
- Implemented: fail-closed SQL boundary parsing for the supported PostgreSQL-shaped
  ...
  `LENGTH(...)`, `OCTET_LENGTH(...)`, `NULLIF(...)`, `GREATEST(...)`, `LEAST(...)`, `ABS(...)`, `ROUND(...)`, `FLOOR(...)`, `CEIL(...)`, numeric `+`/`-`/`*`/`/`/`%` and `MOD(...)` projection expressions, and field/cast
  ...
  including `COALESCE`, `LOWER`, `UPPER`, `TRIM` / `BTRIM` / `LTRIM` / `RTRIM`, `REPLACE`, `REGEXP_REPLACE`, `LENGTH`, searched `CASE`, `NULLIF`, `GREATEST`, `LEAST`, `ABS`, `ROUND`, `FLOOR`, `CEIL`, casts,
  ...
  rewrite primitive before constraint validation and schema promotion.
- Remaining: non-additive migration-equivalence compilation, broader
  ...
  catalog-owned rewrite/backfill job scheduling and progress tracking,
  type-cast row transforms, ordered generated-column dependency rewrites beyond
  the local append-only safe-source case, and unique/FK repair orchestration
  around row rewrites; full typed scalar expression trees for
...
authorization context, and invalidation rules. `DECLARE ... CURSOR`,
`FETCH ... FROM cursor`, and `CLOSE cursor` lower to typed cursor-portal intents
that capture portal name, scroll/hold/binary metadata, fetch direction, and
close scope. They still fail closed when applied to storage until there is a
...
`CREATE SEQUENCE` records the sequence name, `IF NOT EXISTS`, and supported
allocation options (`START`, `INCREMENT`, `MINVALUE`, `MAXVALUE`, `CACHE`, and
`CYCLE`); `ALTER SEQUENCE` records typed restart and option-change operations;
and `DROP SEQUENCE [IF EXISTS] ... [CASCADE]` records typed removal metadata.
...
Sequence type clauses, identity option clauses, and `OWNED BY` still fail
closed until the same durable allocator metadata owns them.
...
method, key count, and range bounds. Schema application remains fail-closed
until native partition metadata owns placement and routing. The production shape
...
language, and drop metadata, then fails closed when applied to table schema or
runtime storage. The known updated-at helper definition uses that same typed
routine-catalog boundary; the native behavior still lives in table-owned
update-policy metadata created by `CREATE TRIGGER`. Routine bodies and richer
routine options remain unsupported until there is a native mutation-hook/catalog
...
| `UNION`, `UNION ALL`, `INTERSECT`, and `EXCEPT` | A conservative same-table subset lowers to ordinary typed row-query predicate composition when both branches read the same table or the same CTE-backed row source, project identical simple fields or identical expression projections, and use only scalar or expression predicates: `UNION` becomes predicate `OR`, promoting scalar branch predicates to equivalent expression conditions when either branch has expression predicates; `UNION ALL` uses the same typed `OR` plan only when same-field or same-expression equality predicates over distinct canonical string, boolean, or null JSON literals, equality versus exact `!=` / `IS DISTINCT FROM` on the same safe literal, `IS NOT DISTINCT FROM` versus `IS DISTINCT FROM` on the same safe literal, exact numeric equality versus complementary inequality/distinctness over the same raw JSON number literal, adjacent numeric field or expression range partitions such as `< bound` versus `>= bound` or `<= bound` versus `> bound`, `IS NULL` versus `IS NOT NULL`, or `IS NULL` versus equality to a definitely non-null safe scalar or numeric literal prove the branches are disjoint, so duplicate semantics are unchanged; `INTERSECT` becomes predicate conjunction; and `EXCEPT` becomes one typed `NOT` group for the full right-branch conjunction, promoting scalar right-branch predicates into expression conditions when needed so mixed scalar/expression branches evaluate as `NOT (scalar AND expression)`. Overlapping `UNION ALL`, mismatched expression outputs, cross-table branches, different CTE sources, branch ordering/pagination, and broader stream-composition shapes still fail closed as `set_operation_plan`. | Keep the predicate-composition subset in row-query plans because it has the same ownership, filtering, and result-shape semantics as the native API. Model general branch output-schema reconciliation, duplicate handling, final ordering/pagination, routed merge behavior, and spill bounds as explicit typed stream-composition plan metadata before accepting broader PostgreSQL set-operation syntax. |
...
1. **SQL frontend and corpus gate.**
   ...
   corpus entries are added to the in-code source corpus, promoted with
...
2. **DDL and migration compiler.**
   ...
   indexes for case-fold, stable unary hash, simple concat keys, and simple
   `concat_ws` keys, and
...
3. **Typed scalar expression tree.**
   ...
   result types. `COALESCE`, text-expression `LOWER`/`UPPER`/`TRIM`/`BTRIM`/`LTRIM`/`RTRIM`/`REPLACE`/`REGEXP_REPLACE`/`LPAD`/`RPAD`/`REPEAT`/`REVERSE`/`STARTS_WITH`/`ENDS_WITH`/`ASCII`/`CHR`/`MD5`/`CONCAT`/`CONCAT_WS`/`LENGTH`, temporal `DATE_TRUNC`, `DATE_BIN` including fixed-duration `INTERVAL` strides, and `DATE_PART` / `EXTRACT`, JSON `jsonb_extract_path[_text]`, `jsonb_typeof`, and `jsonb_array_length`, `GREATEST`, `LEAST`, `ABS`, `ROUND`, `TRUNC`, `FLOOR`, `CEIL`, `SQRT`, `SIGN`, `POWER`, `id::text`, `gen_random_uuid()`/`uuid_generate_v4()`, `NOW()`/`CURRENT_TIMESTAMP[(0..6)]`/`CURRENT_DATE`, `DEFAULT`,
...
7. **Joins, lateral joins, and routed execution.**
   ...
   row-version visibility rules. Remaining planner work is choosing lookup,
   hash, or merge execution from typed cardinality/index hints. Public typed plan
   envelopes expose paired left/right declared range sets for join and lateral
...
8. **CTEs and materialized stream bounds.**
   ...
   CTE-emitted scalar and array outputs. CTE stream schemas preserve primary-key
   ...
   simple fields or identical expression projections and carry only scalar or
   expression predicates. `UNION` becomes `or_predicates` or
   ...
   `UNION ALL` uses the same plan only when same-field or same-expression
   equality predicates over distinct canonical string, boolean, or null JSON
   literals, equality versus exact `!=` / `IS DISTINCT FROM` on the same safe
   literal, `IS NOT DISTINCT FROM` versus `IS DISTINCT FROM` on the same safe
   literal, exact numeric equality versus complementary inequality/distinctness
   over the same raw JSON number literal, adjacent numeric field or expression
   range partitions such as `< bound` versus `>= bound` or `<= bound` versus
   `> bound`, `IS NULL` versus `IS NOT NULL`, or `IS NULL` versus equality to a
   definitely non-null safe scalar or numeric literal prove the branches are
   disjoint, so the duplicate-preserving SQL result matches the row-query
   result. Numeric range proof compares parsed numeric bounds and only proves
...
9. **Aggregates, windows, and ordered streams.**
   ...
   `SUM(amount - discount)` execute through the same row-expression evaluator
...
mutation-source plan with explicit restart intent, while multi-table and cascade
...
| Indexes and planner trust | Ordinary secondary indexes have lifecycle state and rebuild generations; unique constraints have validation state; simple partial indexes, generated expression secondary indexes for case-fold, stable unary hash, simple concat keys, and simple `concat_ws` keys, including harmlessly parenthesized SQL expression-index elements, and case-fold plus stable unary hash unique expression keys are modeled. Partial index and unique metadata accepts deterministic `where_expressions`; write-time secondary-index maintenance and partial unique-owner maintenance apply the supported row-local typed expression subset, including case-fold, stable unary hash, simple `concat`, simple `concat_ws`, rich text transforms and tests such as `initcap`, `trim`, `replace`, `translate`, `substring`, `overlay`, `split_part`, `left`/`right`, `lpad`/`rpad`, `repeat`, `reverse`, `starts_with`/`ends_with`, `length`/`octet_length`, `strpos`, `ascii`, `chr`, and null-preserving `coalesce`. Schema validation checks expression-predicate operand domains per operation before catalog application. Planners and conflict-target inference use expression-partial indexes and partial unique-owner rows after exact typed-expression predicate proof or semantic proof from complete concrete equality predicates through the shared expression evaluator, and missing proof falls back to base-row scans or rejects unsupported conflict targets. | Extend the shared semantic expression implication checker across additional safe equivalence classes, and route expression-derived rebuild/promotion through the same generation/CAS lifecycle as ordinary indexes. | Planner choices depend only on ready/enforced catalog metadata; stale, building, or unvalidated derived artifacts cannot affect query answers or write enforcement. |
| Joins and routed reads | Local equality `INNER`/`LEFT` joins, multi-range equality joins over declared left/right range sets, provisioned and hosted row-query merge, provisioned and hosted aggregate/window source execution, hosted remote owner row-source reads with topology-epoch validation, hosted static source-row coordinator reduction for remote-only plans, scan-backed catalog-routed cross-table join/lateral execution for REST/SDK typed plans and lowered SQL read plans, non-recursive CTE lowering, native ordered CTE plan parsing with per-CTE `max_rows` and `max_bytes` materialization caps, bounded fail-closed routed scan collection using the default materialization row/byte caps, coordinator-side global ordering/offset/limit over range-collected candidates, CTE output-field validation across row-query and derived-stage consumers, final read-plan output-column metadata plus public `result_schema` response structs for row-query, aggregate, window, join, and lateral plans, fail-closed output-reference validation for aggregate/join/lateral/window result streams, scalar aggregate plans, grouping, group-only distinct projection, row-query `distinct_on_expressions`, aggregate-output `HAVING` predicates, `having_expressions`, `having_any`, `having_not`, ordering, and pagination have typed plan homes. | Add row-version visibility rules across remote owners, lookup/hash/merge physical strategy choice, and explicit spill-backed CTE materialization. | Routed reads execute with correct pagination and ordering across range movement and live writes. |
...
row-version visibility under live owner movement, physical strategy selection
for lookup/hash/merge joins, and spill/backpressure policy over those already
typed plans.
