...
byte, and quote byte, then fails closed when applied to table schema or runtime
storage. The production shape should stream rows through typed schema
...
those contracts, but it must not bypass
row-batch, mutation-source, or routed read semantics. Until server-file,
`PROGRAM`, alternate stream endpoints, and richer COPY options such as escape,
null-marker, freeze, and encoding controls have
