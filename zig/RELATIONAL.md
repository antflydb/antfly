...
`not_predicates`. `UNION ALL` accepts scalar `IN` only when every branch pair is
proven disjoint from same-field scalar equality, `IS NOT DISTINCT FROM`,
`IS NULL`, or another non-negated scalar `IN` array over safe JSON scalar
literals.
...
8. **CTEs and materialized stream bounds.**
   ...
   also has expression predicates. Numeric range
