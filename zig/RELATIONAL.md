...
Unique expression constraints store the supported expression key directly on the
unique constraint, evaluate the key from the committed row, and maintain the
unique-owner row in the same 2PC path as ordinary column unique owners.
Supported unique expressions include case-fold keys, `lower(field)` and
`upper(field)`, and stable unary hash keys, `md5(field)`, over declared
string-like relational columns. Richer unique expression keys such as
`replace(status, 'old', 'new')` remain fail-closed at SQL DDL with the
`unique_expression_owner_ast` reason until unique-owner rows can encode and
resolve the shared row-expression AST as a durable owner tuple.
...
| Expression indexes and expression unique keys | Generated columns for stable secondary-index expressions; typed unique metadata for case-fold and stable unary hash expression keys | Lower secondary expressions such as `lower(email)`, `upper(email)`, `md5(email)`, simple `concat(...)`, and richer deterministic row-expression keys to generated columns when possible; persist unique expressions such as `lower(email)`, `upper(email)`, and `md5(email)` as deterministic unique metadata with typed input fields. Native schema JSON also accepts `generated: { "op": "expression", "expression": <row-expression AST> }` for deterministic generated columns, and storage persists that AST with the same row-expression representation used by checks, partial predicates, update transforms, and returning projections. Write planning, row rewrite/backfill, index generation hashing, dependency checks, and column rename/drop handling must operate on the AST rather than raw SQL text. SQL DDL lowers supported non-unique expression-index syntax into that generated-expression AST; SQL DDL for broader unique expression indexes remains fail-closed as `unique_expression_owner_ast` until unique-owner rows can encode, repair, route, and resolve expression AST tuples durably. Expand the common expression AST with casts/functions, collation, null semantics, rebuild metadata, and write-time maintenance as richer expression-derived indexes land. |
