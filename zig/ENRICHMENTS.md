...
Query visibility sync must not trigger enrichment generation.
`sync_level=query` waits for full-text/query derived indexes to apply and
publish the document/artifact changes already present in the committed stream.
It must not call readers, transcribers, OCR, chunkers, embedders, or other
enrichment producers merely because an enrichment is configured as a full-text
source.

The sync boundary is:

- `write` and `propose`: commit primary rows and replay debt only.
- `query`: wait for query-visible full-text index publication only.
...
This means a write with `sync_level=query` may return before a newly
