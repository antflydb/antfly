...
| Full-cluster v11-v23 production owners | **Integrated at the stated production-owner seams at the cited checkpoints.** V11 instantiates the real metadata quorum, three production `DataServer`/data-Raft owners, real HTTP/Raft, two-table public clients, and the serverless public catalog; its ReleaseSafe smoke gate passes 30/30. V12-v22 add the named split, graph, transport, restart, short-write, resource, join, overlapping-fault, and socket-pressure seams at their cited green gates. V23 applies node-wide two-times logical slowdown through the DataServer cost ports, graph, and serverless adapters, heals before the public graph workload, and proves exact pre/post-heal costs, continued DataServer and graph work, zero effects, visibility, cleanup, and exact replay; its 90,000-transition gate passes 15/15. The DataServer port can charge Raft and LSM classes, but this cluster property requires aggregate DataServer work; the focused LSM gate remains the exact LSM witness. All complete gates include properties, cleanup, and leak checks | V9's remaining topology breadth, disk-capacity pressure, broader socket-pressure overlap, broader client/request/response/Raft short-write surfaces, and arbitrary owner/coordinator/metadata restart combinations are not yet on these production owners. Query-cache/backfill/deadline overlap in v23, disjoint donor/receiver placement, retained-history pressure, snapshot/derived-state rehydration, HA co-residency, broader join forms, partition-worker recovery, and broader storage/process/link/resource overlap remain roadmap work |
...
| Deployment-shaped full cluster | Antfly `vopr/full_cluster.zig`, `vopr/serverless_workflow.zig`, `serverless_http_server.zig`, `metadata/sim_harness.zig`, and Raft `transport/httpx_runtime.zig`; one `VoprIo` owns metadata, hosted ranges, public/serverless HTTP, clients, per-node resources, and workers. V9 exact-replays nine hosted/public fault modes. Complementary v11-v22 supply the real metadata quorum, three `DataServer`/data-Raft/resource owners, active split, public graph/join paths, and the named fault/recovery seams. V23 composes reversible DataServer, graph, and serverless logical costs before and after healing on that same clock. Remaining topology breadth, disk pressure, broader socket/short-write/restart overlap, query-cache/backfill service-rate composition, and HA/data-plane co-residency remain future work | `zig build full-cluster-vopr-test` plus the focused production-cluster v11-v23 gates |
| Production-owned full-cluster composition | Antfly `vopr/production_cluster.zig` plus `vopr/full_cluster.zig`; v11 places the real metadata quorum, three production `DataServer`/data-Raft owners, public clients, two isolated tables, and the production serverless catalog on one `VoprIo`. V12-v22 add the named split, graph, transport, restart, short-write, resource, join, overlapping-fault, and socket-pressure seams. V23 installs one shared service-rate model through production-neutral ports on all three DataServers, their graph workers, and the serverless runtime. Node-wide two-times slowdown is charged during bootstrap/serverless work, all effects heal before the public graph, and exact accounting distinguishes pre-heal from baseline post-heal work. Public Provisioned reads adapt to current-owner routing and propagate the derived-state barrier, graph lifecycle hook, and graph work-cost port; remote phases cross typed internal HTTP endpoints, while local phases retain the resident DB/read-admission owner. Every strong derived query waits for matching applied ReadState and local full-index visibility. Borrowed-I/O contention and topology changes retain typed retryable status, and modeled time borrows the shared runtime clock | The v11 smoke target passes 30/30; the cited v12 deep target and current-tree v13-v23 complete targets pass 15/15 at their stated budgets. V23's `production-cluster-service-rate-vopr-test` passes at 90,000 transitions with record/fresh-world exact replay, exact pre/post-heal accounting, properties, cleanup, and leak checks. The 2,000-transition subprocess remains lifecycle-only evidence |
...
| --- | --- | --- |
| Where should Antfly add VOPR testing? | At production orchestration boundaries that combine durable state, ownership, public visibility, and recovery | Deepen the v23 production-owner cluster with disk-capacity pressure, broader socket/topology faults, storage/process/restart/short-write overlaps, and query-cache/backfill/deadline service-rate composition; finish the repository-wide strong-read contract and managed-index publication/readiness; extend the durable-finalizer join seam into partition-worker recovery and global-query workflows; then compose metadata administration, MCP/A2A, cloud authentication, extension invocation, and live credential/provider replacement |
...
are green, and the cited v13-v23 production-owner gates passed their complete
...
| Metadata quorum, production `DataServer` replicas, public clients, and real HTTP/Raft transport in one history | **Integrated at the named v11-v23 seams.** Full-cluster v9 remains the complementary nine-fault hosted/public graph campaign, and `data-server-transition-vopr-test` independently proves replicated merge-to-split behavior across three production owners. V11 joins the real metadata quorum, three real `DataServer`/data-Raft owners, public clients, two tables, and the serverless public catalog on one `VoprIo`; its ReleaseSafe smoke gate passes 30/30. V12-v22 add the named split, graph, transport, restart, short-write, resource, join, overlapping-fault, and socket-pressure seams. V23 composes exact slowed/healed DataServer, graph, and serverless work on the shared clock, then completes the public graph and cleanup. The current-tree v13-v23 complete gates each pass 15/15 with fresh-world exact replay, properties, cleanup, and leak checks; v23 uses 90,000 transitions | Add v9's remaining topology breadth, broader partial-write and socket-pressure surfaces, disk-capacity pressure, coordinator/metadata/storage/process/restart overlap, and query-cache/backfill/deadline service-rate combinations. Add disjoint donor/receiver placement, retained-history paging, snapshot-install DB/derived-state rehydration, derived graph/index equality, partitions, and richer overlapping failures |
