...
| --- | --- | --- |
| Where should Antfly add VOPR testing? | At production orchestration boundaries that combine durable state, ownership, public visibility, and recovery | Deepen the v30 production-owner cluster with cancellation under resource/storage/process/restart and multi-fault combinations, disk-capacity pressure, broader socket/topology faults, storage/process/restart/short-write overlaps, and query-cache/backfill/deadline service-rate composition; finish the repository-wide strong-read contract and managed-index publication/readiness; extend the durable join seam into partition-worker crash recovery, fault overlap, broader join forms, and global-query workflows; then compose metadata administration, MCP/A2A, cloud authentication, extension invocation, and live credential/provider replacement |
...
are green, and the cited v13-v30 production-owner gates passed their complete
...
focused production-owner gate through v30; each large subprocess uses ordinary
...
| Distributed joins and global-query orchestration | **Partially integrated.** V19 executes one public inner `_id` join with two left rows and two independently owned right groups before, during, and after an active split, requiring exact no-partial results and typed ownership retry. V20 forces durable shuffle and proves a second finalizer imports the first owner's persisted result after an acknowledgement failure. Forward-only v30 cancels a public durable shuffle at a real internal partition-worker boundary, requires the canceled worker not to complete, and proves an exact clean retry. Focused composed-query tests cover additional result assembly. No global-query production-owner claim is made | Add partition-worker crash recovery, cancellation under faults, retry exhaustion, auth and stale-generation changes, right/nested/foreign and multi-range-left joins, overlapping owner faults, and fail-closed global-query recovery histories |
...
2. **Deepen public distributed operations.** V19 adds a public inner `_id`
   ...
   outstanding partition worker and that a clean retry drains. Add
   cancellation under resource/storage/process/restart and multi-fault
   combinations, partition-worker crash recovery,
...
2. Deepen the public-HTTP/cross-range graph composition. Its public request now
   ...
   clean retry. Next compose cancellation with
   resource, storage, process, restart, and overlapping faults.
   Extend v20/v30 across partition-worker crash recovery,
