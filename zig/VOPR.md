...
public production-owner hydration only; public cancellation, authorization
mutation, stale-generation rejection, retry exhaustion, and their fault
...
| Full-cluster v11-v24 production owners | **Integrated at the stated production-owner seams at the cited checkpoints.** V11 instantiates the real metadata quorum, three production `DataServer`/data-Raft owners, real HTTP/Raft, two-table public clients, and the serverless public catalog; its ReleaseSafe smoke gate passes 30/30. V12-v22 add the named split, graph, transport, restart, short-write, resource, join, overlapping-fault, and socket-pressure seams at their cited green gates. V23 applies node-wide two-times logical slowdown through the DataServer cost ports, graph, and serverless adapters, heals before the public graph workload, and proves exact pre/post-heal costs, continued DataServer and graph work, zero effects, visibility, cleanup, and exact replay. V24 sends a public graph request with document hydration through the production owners, validates exact hydrated fields, and proves one hydration lifecycle. Both dedicated 90,000-transition gates pass 15/15 with fresh-world replay. The DataServer port can charge Raft and LSM classes, but this cluster property requires aggregate DataServer work; the focused LSM gate remains the exact LSM witness. All complete gates include properties, cleanup, and leak checks | V9's remaining topology breadth, disk-capacity pressure, broader socket-pressure overlap, broader client/request/response/Raft short-write surfaces, and arbitrary owner/coordinator/metadata restart combinations are not yet on these production owners. Query-cache/backfill/deadline overlap in v23, public cancellation/auth/stale-generation/retry-exhaustion composition after v24, disjoint donor/receiver placement, retained-history pressure, snapshot/derived-state rehydration, HA co-residency, broader join forms, partition-worker recovery, and broader storage/process/link/resource overlap remain roadmap work |
...
| --- | --- | --- |
| Where should Antfly add VOPR testing? | At production orchestration boundaries that combine durable state, ownership, public visibility, and recovery | Deepen the v24 production-owner cluster with public cancellation/auth/stale-generation/retry-exhaustion composition, disk-capacity pressure, broader socket/topology faults, storage/process/restart/short-write overlaps, and query-cache/backfill/deadline service-rate composition; finish the repository-wide strong-read contract and managed-index publication/readiness; extend the durable-finalizer join seam into partition-worker recovery and global-query workflows; then compose metadata administration, MCP/A2A, cloud authentication, extension invocation, and live credential/provider replacement |
...
are green, and the cited v13-v24 production-owner gates passed their complete
...
response and proves one production hydration lifecycle. Cancellation,
authorization mutation, stale-generation rejection, and retry exhaustion are
...
| Metadata quorum, production `DataServer` replicas, public clients, and real HTTP/Raft transport in one history | **Integrated at the named v11-v24 seams.** Full-cluster v9 remains the complementary nine-fault hosted/public graph campaign, and `data-server-transition-vopr-test` independently proves replicated merge-to-split behavior across three production owners. V11 joins the real metadata quorum, three real `DataServer`/data-Raft owners, public clients, two tables, and the serverless public catalog on one `VoprIo`; its ReleaseSafe smoke gate passes 30/30. V12-v22 add the named split, graph, transport, restart, short-write, resource, join, overlapping-fault, and socket-pressure seams. V23 composes exact slowed/healed DataServer, graph, and serverless work on the shared clock. V24 completes an exact document-hydrating public graph request through those owners. The current-tree v13-v24 complete gates each pass 15/15 with fresh-world exact replay, properties, cleanup, and leak checks; v23 and v24 use 90,000 transitions | Add v9's remaining topology breadth, broader partial-write and socket-pressure surfaces, disk-capacity pressure, coordinator/metadata/storage/process/restart overlap, query-cache/backfill/deadline service-rate combinations, and public cancellation/auth/stale-generation/retry-exhaustion composition. Add disjoint donor/receiver placement, retained-history paging, snapshot-install DB/derived-state rehydration, derived graph/index equality, partitions, and richer overlapping failures |
...
| Public distributed graph request from HTTP planning through fanout/hydration | **Partially integrated, with static, active-split, transport-fault, owner-restart, recoverable short-write, three-owner memory-pressure, one overlapping link-plus-memory path, selected-listener socket pressure, and exact document hydration promoted.** Full-cluster v9 executes a public depth-two graph across hosted ranges. V13 executes it across real `DataServer`/data-Raft owners with current-owner routing and matching ReadState/derived-index visibility; v14 composes active split; v15 adds a next-owner transport cut; v16 adds owner reconstruction; v17 adds a one-byte request write; v18 adds all-owner memory denial/recovery; v21 overlaps that pressure with the selected graph link cut; v22 denies then heals new connections at one exact public listener; and v24 validates exact hydrated titles plus one start/completion lifecycle through the real public response. The focused distributed suite separately covers topology retry/exhaustion, cancellation, stale generations, and authorization | Add broader restart/topology and request/response/Raft short-write faults, disk-capacity pressure, broader socket-pressure and storage/process/restart overlaps, and public cancellation, authorization mutation, stale-generation rejection, and retry exhaustion to the production-owner history. Add global queries with the same fail-closed publication rule; v19/v20's join seams are audited separately |
...
1. **Deepen the promoted full-cluster reconfiguration seam.** The focused
   ...
   proves exactly one production hydration start/completion lifecycle, and
   passes its 90,000-transition record and fresh-world replay 15/15. Next add the
...
2. **Deepen public distributed operations.** V19 adds a public inner `_id`
   ...
   public production-owner document hydration. Add public cancellation,
   authorization mutation, stale-generation rejection, retry exhaustion,
