...
Debug and ReleaseSafe. Authorization/generation mutation, cancellation under
faults, broader join forms, and global-query composition remain future work.
...
v30-v33 then promote durable-worker cancellation, same-partition group
failover, actual partition-owner process destruction/reconstruction, and
retry exhaustion while a real resource domain overlaps one exact-group link
cut. Every incomplete public graph or join operation must fail closed without
...
| --- | --- | --- |
| Where should Antfly add VOPR testing? | At production orchestration boundaries that combine durable state, ownership, public visibility, and recovery | Deepen the v33 production-owner cluster with cancellation under resource/storage/process/restart and multi-fault combinations, disk-capacity pressure, broader socket/topology faults, storage/process/restart/short-write overlaps, and query-cache/backfill/deadline service-rate composition; finish the repository-wide strong-read contract and managed-index publication/readiness; extend the durable join seam into cancellation under faults, authorization and generation changes, broader join forms, and global-query workflows; then compose metadata administration, MCP/A2A, cloud authentication, extension invocation, and live credential/provider replacement |
...
are green, and the cited v13-v33 production-owner gates passed their complete
...
partition-owner reconstruction, and v33 overlapping-fault retry exhaustion.
`production-cluster-vopr-test` requires every focused production-owner gate
through v33; each large subprocess uses ordinary
...
1. **Deepen the promoted full-cluster reconfiguration seam.** The focused
   ...
   Next add cancellation under resource/storage/process/restart and multi-fault
   combinations, plus the
...
2. **Deepen public distributed operations.** V19 adds a public inner `_id`
   ...
   identical request. Add
   cancellation under resource/storage/process/restart and multi-fault
   combinations, right/nested/foreign and multi-range-left joins, additional
...
2. Deepen the public-HTTP/cross-range graph composition. Its public request now
   ...
   independent healing, and complete fresh recovery.
   Next compose cancellation with
   resource, storage, process, restart, and overlapping faults.
   Extend v20/v30/v31/v32/v33 across right/nested/foreign joins, multi-range
   left inputs, cancellation combined with other faults, and additional
