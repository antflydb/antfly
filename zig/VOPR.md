...
Forward-only v42 adds a clean production replication snapshot-to-stream
...
Backfill crash, cancellation, topology, schema, and overlapping-fault variants,
plus cache deadline/topology/fault overlap, remain future breadth.
...
| Full-cluster v11-v42 production owners | **Integrated at the explicitly named production-owner seams.** V11 joins the real metadata quorum, three production `DataServer`/data-Raft owners, real HTTP/Raft, two-table public clients, and the serverless catalog on one `VoprIo`. V12-v22 promote active split, graph, transport, restart, short-write, resource, join, overlapping-fault, and socket-pressure seams. V23 proves shared reversible DataServer/graph/serverless costs; v24-v35 add hydration, cancellation, authorization, stale topology, durable-worker retry and takeover, resource/link overlap, and owner reconstruction. V36-v40 promote exact ordered-global-query baseline, cancellation, live authorization revocation, directional transport failure, and exact tenant-owner destruction/reconstruction with fail-closed no-partial recovery. Forward-only v41 assigns the real production cache instance to node 1's public `ApiHttpServer`/DataServer process domain. Forward-only v42 borrows the production replication runner into the same cluster, sends two snapshot batches and one streaming upsert through alternating public coordinators into production routing, leader forwarding, DataServer Raft, and index visibility, and proves pre-heal slowed snapshot work through the first accepted public batch plus baseline snapshot/stream work after healing. The cited complete gates use record plus fresh-world replay, named properties, cleanup, and leak checks; v41's 120,000-transition and v42's 160,000-transition Debug and ReleaseSafe gates pass 15/15. The focused LSM gate remains the exact LSM-maintenance witness | Remaining breadth includes v9 topology combinations; disk-capacity and broader socket/short-write surfaces; cache deadline/topology/link/storage/resource/restart overlap; replication crash, cancellation, topology, ownership, schema, and overlapping-fault variants in this deployment; cancellation under storage/process and richer multi-fault combinations; arbitrary coordinator/metadata or multi-owner loss; disjoint placement; retained-history pressure; snapshot/derived-state rehydration; HA co-residency; broader join/global-query forms; and richer storage/process/link/resource overlap |
...
| Reversible node and operation service rates | `service_rate.zig` registers unambiguous stable node/operation identities, composes fully checked node/operation costs, charges logical time through borrowed `std.Io`, and heals individual effects by stable fault ID. Accounting distinguishes calls, work units, and logical nanoseconds. Six production-neutral boundaries are integrated: query cache, DataServer Raft and LSM maintenance, distributed graph, replication snapshot/stream, and serverless workflow work. Focused histories prove exact slowed/healed behavior. Full-cluster v23 installs one shared model into DataServer, graph, and serverless owners; v41 adds cache producer/coalesced/hit work; v42 adds production replication snapshot/stream work targeting the real public/DataServer/Raft path. Together the named modes prove exact pre/post-heal charging, continued cluster work, visibility, cleanup, and replay | `zig build vopr-engine-test query-embedding-cache-vopr-test lib-data-runtime-test data-server-vopr-test distributed-query-vopr-test replication-backfill-vopr-test serverless-workflow-vopr-test production-cluster-service-rate-vopr-test production-cluster-query-cache-service-rate-vopr-test production-cluster-replication-backfill-vopr-test`; deadline/topology/fault overlap and replication fault variants in the deployment remain roadmap breadth |
...
| Deployment-shaped full cluster | Antfly `vopr/full_cluster.zig`, `vopr/production_cluster.zig`, `vopr/serverless_workflow.zig`, `metadata/sim_harness.zig`, and the production HTTP/Raft runtimes share one `VoprIo`. V9 retains the hosted/public nine-fault campaign. V11-v22 add the real metadata quorum, three `DataServer`/data-Raft/resource owners, active split, graph/join, transport, restart, short-write, resource-overlap, and socket seams. V23-v40 add the cited shared costs, hydration, cancellation, authorization, stale topology, durable workers, reconstruction, and global-query recovery histories. V41 adds production query-cache composition. Forward-only v42 adds clean replication snapshot-to-stream work through alternating public coordinators, production routing, leader forwarding, DataServer Raft, and index visibility, with exact slowed/healed accounting and a completion fence that keeps those owners live. Remaining work is targeted fault breadth rather than the clean replication composition itself | `zig build full-cluster-vopr-test` plus the focused production-cluster v11-v42 gates |
| Production-owned full-cluster composition | Antfly `vopr/production_cluster.zig` plus `vopr/full_cluster.zig`; v11-v40 provide the detailed production metadata/DataServer/public/serverless, active-transition, graph, join, global-query, fault, cancellation, authorization, and reconstruction seams cataloged in Distributed Coverage. Public strong reads route to the current owner and require matching applied ReadState plus local derived-index visibility; remote work crosses typed internal HTTP while modeled time borrows the shared runtime. V23 installs one shared service-rate model; v41 adds production cache coalescing and hits; v42 runs the production replication runner against a public BatchRequest adapter and holds production owners behind a generic completion fence until all three batches are Raft/index visible through every public coordinator | The v11 smoke target passes 30/30; the cited v12-v40 gates use their documented budgets and fresh-world replay. V41's 120,000-transition cache gate and v42's 160,000-transition replication gate pass Debug and ReleaseSafe 15/15. Complete gates include named properties, record/fresh-world replay, cleanup, and leak checks; the 2,000-transition subprocess remains lifecycle-only evidence |
...
crash, cancellation, topology, stale-owner, schema, and duplicate-work modes
remain in the focused suite until separately composed with these owners.
V41 and v42 directly add their modes, properties, trace revisions,
...
are green, and the cited v13-v40 production-owner gates plus the v41 and v42
...
through v42; each large subprocess uses ordinary
...
| Metadata quorum, production `DataServer` replicas, public clients, and real HTTP/Raft transport in one history | **Integrated at the named v11-v42 seams.** Full-cluster v9 remains the complementary hosted/public nine-fault campaign, and `data-server-transition-vopr-test` independently proves replicated merge-to-split behavior. V11-v40 provide the cited production-owner split, graph, join, fault, durable-worker, reconstruction, and global-query seams. V41 places the production query cache on node 1's shared service-rate domain. V42 runs the production replication snapshot-to-stream runner against alternating public coordinators and proves all three batches cross routing, DataServer Raft, and index visibility, with exact pre/post-heal costs and every document readable through every coordinator. The complete v41 and v42 modes pass Debug and ReleaseSafe 15/15 at 120,000 and 160,000 transitions respectively | Add remaining topology breadth, disk and broader socket/short-write pressure, cache deadline/topology/fault overlap, replication crash/cancellation/topology/schema and overlapping-fault variants, cancellation under storage/process and richer multi-fault combinations, disjoint placement, retained-history paging, snapshot/derived-state rehydration, partitions, and coordinator/metadata or multi-owner loss |
...
| Query cache, replication backfill, and service rates | **Integrated focused seams plus deployment composition.** Cache, DataServer Raft/LSM, distributed graph, replication snapshot/stream, and serverless workflow histories each prove exact slowed/healed production charging. V23 installs one shared model across DataServer, graph, and serverless owners. V41 adds exact production cache coalescing and a retained hit on node 1. V42 adds the production replication runner: snapshot work and the first accepted public batch execute while node 1 is slowed; the fault then heals and the remaining snapshot plus stream work reaches every public coordinator through production DataServer Raft/index owners. Both retain the complete cluster visibility, cleanup, and fresh-world replay oracle | Overlap operation-specific slowdown with public deadlines, ownership changes, replication crash/cancellation/topology/schema cases, and the existing link/storage/resource/restart algebra |
...
| --- | --- | --- |
| P0 integrated | Replication backfill and rebalancing | `replication-backfill-vopr-test` covers snapshot-to-streaming cutover, resumable checkpoints, duplicate work, cancellation, source and target crashes, topology changes, stale ownership, schema changes, and exact replay through the production runners. The same gate types production work-permit checkpoints as snapshot or stream steps and proves overlapping node/snapshot slowdown, independent healing, exact logical cost, and completed streaming. Forward-only full-cluster v42 promotes the clean snapshot-to-stream composition: the reusable fixture borrows the cluster `VoprIo`, targets the canonical public BatchRequest path, crosses routing/leader forwarding/DataServer Raft/index visibility, and proves exact pre/post-heal work plus visibility from every public node. Focused fault cases are not yet promoted into that deployment. |
...
| P0 integrated combined active-transition/graph/resource/service-rate seam | Full-cluster distributed composition | Full-cluster v9 retains the registered hosted/public nine-fault deployment. V11-v40 add the cited real metadata/DataServer/public/serverless, active-transition, graph/join/global-query, fault, cancellation, authorization, durable-worker, and reconstruction seams. V41 adds production query-cache coalescing and retained-hit behavior to node 1's slowed/healed domain; its 120,000-transition Debug and ReleaseSafe gates pass 15/15. Forward-only v42 adds production replication snapshot-to-stream work through alternating public coordinators, routing, DataServer Raft, and index visibility while retaining every cluster oracle; its 160,000-transition Debug and ReleaseSafe gates pass 15/15. These promote only the named seams. Next add cache deadline/topology/fault overlap; replication crash/cancellation/topology/schema/fault variants; storage/process/multi-owner cancellation and recovery; global-query topology/storage/resource/coordinator/metadata loss; broader socket/short-write/restart targets; disjoint placement; retained-history and snapshot/derived-state recovery; HA/data-plane co-residency; richer public queries; and multi-worker placement. |
...
12. a reversible service-rate model with unambiguous stable node and operation
    ...
    fresh-world replay. Forward-only v42 adds the production replication
    runner and public BatchRequest target to the same deployment: one snapshot
    unit is slowed before healing, remaining snapshot and stream work runs at
    baseline, and every replicated document is Raft/index visible through all
    public coordinators with exact accounting and fresh-world replay.
...
- extend the v41/v42 cluster compositions with search policies that overlap
  node-wide and operation-class slowdowns
  with public deadlines, topology/ownership changes, the existing
  link/storage/resource/restart algebra, replication crash/cancellation/schema
  cases, and quiet recovery. The reusable effect and clean backfill composition
  are complete; arbitrary instruction-level native-thread throttling is still
...
3. Co-locate production HA and data-plane owners, extend the integrated
   ...
   distinct fault from the CPU-work exhaustion budget. Forward-only v41
   composes query-cache request,
   producer, coalesced-wait, and retained-hit costs on node 1's shared public
   `ApiHttpServer`/DataServer domain before and after healing. Forward-only v42
   composes clean replication snapshot/stream work against that production
   cluster domain. Next overlap
