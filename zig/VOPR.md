...
| Deployment-shaped full cluster | Antfly `vopr/full_cluster.zig`, `vopr/production_cluster.zig`, `vopr/serverless_workflow.zig`, `metadata/sim_harness.zig`, and the production HTTP/Raft runtimes share one `VoprIo`. V9 retains the hosted/public campaign. V11-v40 add the cited production metadata/DataServer/public/serverless, transition, graph/join/global-query, fault, durable-worker, and reconstruction seams. V42 adds clean replication snapshot-to-stream work through alternating public coordinators and DataServer Raft/index visibility. Forward-only v43 adds schema-change interruption after the first accepted batch, durable resume with one exact duplicate batch, CDC completion, slowed/healed accounting, and the unchanged cluster cleanup/replay oracle. Forward-only v44 adds exact target-leader process teardown/reconstruction, bounded client reconnect, durable resume, and direct plus all-coordinator visibility. Forward-only v45 adds actual provider-query failure, exact source-session replacement, and resume without extra target work. Forward-only v46 adds post-checkpoint lease cancellation, replacement-session resume, and composed charging without a duplicate target batch. Forward-only v47 adds pre-checkpoint stale-owner rejection and one exact idempotent target replay. Forward-only v48 adds metadata-Raft source-catalog publication, exact-cutover authority replacement, predecessor retirement, and one exact replay. Forward-only v49 adds the actual `ApiHttpServer` cache's slowed deadline, process restart, empty replacement, recomputation, bounded reconnect, and durable rebound read. Forward-only v50 adds a real serverless candidate-publication/progress-CAS conflict, authoritative generation cutover, stale-enrichment rejection, public version-4 visibility, and concurrent ordinary production-cluster work. Forward-only v51 adds two disjoint authenticated table identities, concurrent authorized cross-node work, exact bidirectional 403 read/write denials, and exact forbidden-key absence. Forward-only v52 drives node 2's real persistent-cache reservation through zero-capacity denial, healing, one exact write/release, and public-read continuity. Forward-only v53 drives a public managed-index request through pending readiness, coherent coverage/replay publication, durable owner reconstruction, and all-node semantic-query recovery. Remaining work is targeted timing and cross-domain fault breadth | `zig build full-cluster-vopr-test` plus the focused production-cluster v11-v53 gates |
| Production-owned full-cluster composition | Antfly `vopr/production_cluster.zig` plus `vopr/full_cluster.zig`; v11-v40 provide the detailed production-owner seams cataloged in Distributed Coverage. Public strong reads route to the current owner and require matching applied ReadState plus local derived-index visibility; remote work crosses typed internal HTTP while modeled time borrows the shared runtime. V42-v48 compose production replication and its named interruption/recovery/fencing modes; v49 runs deadline/coalescing/restart/recompute against the cache owned by node 1's real `ApiHttpServer`; v50 runs ordinary production work concurrently with a real serverless generation/progress conflict; v51 enables authentication on all three public servers and uses separate docs/tenant identities for ordinary work plus exact cross-table denial and absence proof; v52 attaches the production persistent-cache worker to node 2's live capacity source and proves denial/healing without breaking public reads; v53 routes managed-index metadata mutation, repair, status, reconstruction, and semantic queries through those same owners | The v11 smoke target passes 30/30; the cited v12-v40 gates use their documented budgets and fresh-world replay. V42-v52's named 120,000–260,000-transition Debug and ReleaseSafe gates pass 15/15; v53's 180,000-transition complete gate is ReleaseSafe. Complete gates include named properties, record/fresh-world replay, cleanup, and leak checks; the 2,000-transition subprocess remains lifecycle-only evidence |
...
Forward-only v53 promotes one managed-index publication and reconstruction
...
The public status endpoint must expose pending readiness and real document
attempts before the provider is released. Publication then requires coherent
coverage and replay watermarks, three indexed documents, and semantic query
visibility. The history destroys and reconstructs node 1 with the same durable
root and endpoints, requires every live owner to converge to ready state, and
executes the same semantic query through all three public nodes. Its dedicated
180,000-transition gate owns record/fresh-world replay, cluster cleanup, and
leak evidence. This promotes pending-to-ready publication and post-publication
owner reconstruction; crash inside each candidate/catalog/alias publication
gap and managed-index disk-pressure overlap remain future breadth.
...
| Which Antithesis ideas remain worth porting locally? | The large engine features, saved cross-run event-set programs, non-blocking bounded live streams, and reversible logical service rates with per-node/per-operation evidence are implemented at the registered in-process boundary; query-cache, DataServer, graph, replication, and serverless work are production-charged seams, and v42-v53 add schema recovery, target-owner restart, source-session replacement, durable cancellation, stale-owner fencing, metadata-backed exact-cutover rotation, actual cache-owner deadline/restart recovery, serverless generation/progress-CAS fencing, authenticated tenant isolation, real per-node disk-capacity denial/recovery, and managed-index publication/reconstruction | Transitive determinism auditing; nightly sharding, retention, quarantine review, notifications, and dashboards; compiler coverage as guidance when Zig instrumentation is stable; broader service-rate fault combinations and production/search adoption |
...
oracles. V53 uses the same complete oracle in its ReleaseSafe deep gate. The
...
| P1 integrated selected seam / ongoing breadth | Managed-index publication and public readiness | Full-cluster v53 sends public create-index through metadata Raft, observes pending readiness while document enrichment returns a retryable provider failure, releases the provider, requires coherent coverage and replay watermarks plus three indexed documents, reconstructs one durable DataServer owner, and requires ready state and the exact semantic query through all three public nodes. The 180,000-transition named gate owns record/fresh-world replay, cleanup, and leak evidence. Atomic versus progressive publication policy, cancellation, disk/rate-limit overlap, and crashes inside each durable-generation/catalog/alias/readiness gap remain future breadth. |
| P0 integrated combined active-transition/graph/resource/service-rate seam | Full-cluster distributed composition | Full-cluster v9 retains the registered hosted/public deployment. V11-v40 add the cited real metadata/DataServer/public/serverless, active-transition, graph/join/global-query, fault, cancellation, authorization, durable-worker, and reconstruction seams. V42-v48 add production replication through public coordinators, DataServer Raft/index visibility, schema interruption, target/source failure, cancellation, stale ownership, and metadata-backed cutover-authority rotation. V49 uses the live node-1 `ApiHttpServer` cache across deadline, healing, owner reconstruction, recomputation, reconnect, and durable read. V50 composes real serverless candidate persistence, authoritative generation cutover, losing progress CAS, stale-derived rejection, public version-4 visibility, and concurrent ordinary cluster work. V51 authenticates two disjoint table identities, performs concurrent authorized cross-node work, requires exact bidirectional 403 read/write denials, and proves exact forbidden-key absence. V52 drives node 2's production persistent-cache capacity reservation through exact denial, healing, write/release, and public-read continuity. V53 adds public managed-index pending/readiness, repair, owner reconstruction, and all-node semantic-query recovery. The v42-v52 Debug and ReleaseSafe record/fresh-world gates pass 15/15 at their documented 120,000–260,000-transition budgets; v53's 180,000-transition complete gate is ReleaseSafe. These promote only the named seams. Next add managed-index and cache topology/link/storage/resource disk-pressure combinations; serverless multi-worker/object-store/process/resource overlap; row-level tenant scoping and identity mutation races; additional replication topology/cancellation/source/target-crash timings; metadata leadership loss during cutover; cross-domain fault variants; storage/process/multi-owner cancellation and recovery; global-query topology/storage/resource/coordinator/metadata loss; broader socket/short-write/restart targets; disjoint placement; retained-history and snapshot/derived-state recovery; HA/data-plane co-residency; and richer public queries. |
...
1. **Deepen the promoted full-cluster reconfiguration seam.** The focused
   ...
   durable write, release, and cross-node recovery. Forward-only v53 drives a
   public managed-index request through observed pending readiness, coherent
   coverage/replay publication, durable owner reconstruction, and all-node
   semantic recovery. Next add cancellation under storage faults, graph/query
...
2. **Deepen public distributed operations.** V19 adds a public inner `_id`
   ...
   V53 now promotes the first managed-index progressive-readiness lifecycle:
   public mutation, pending evidence, retryable provider recovery, coherent
   coverage/replay watermarks, one owner reconstruction, and semantic reads
   from every node. Extend it with atomic-versus-progressive policy,
...
2. Deepen the public-HTTP/cross-range graph composition. Its public request now
   ...
   results. V53 now proves one public managed-index pending-to-ready and
   post-publication reconstruction path. Extend that history across atomic and
   progressive policy, rate-limit and disk pressure, cancellation, alias
   fencing, and crashes between generation durability, catalog publication,
