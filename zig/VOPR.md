...
| --- | --- | --- |
| Where should Antfly add VOPR testing? | At production orchestration boundaries that combine durable state, ownership, public visibility, and recovery | Deepen the v31 production-owner cluster with cancellation under resource/storage/process/restart and multi-fault combinations, disk-capacity pressure, broader socket/topology faults, storage/process/restart/short-write overlaps, and query-cache/backfill/deadline service-rate composition; finish the repository-wide strong-read contract and managed-index publication/readiness; extend the durable join seam into actual partition-owner crash/reconstruction, retry exhaustion, fault overlap, broader join forms, and global-query workflows; then compose metadata administration, MCP/A2A, cloud authentication, extension invocation, and live credential/provider replacement |
...
are green, and the cited v13-v31 production-owner gates passed their complete
...
2. **Deepen public distributed operations.** V19 adds a public inner `_id`
   ...
   a different production group with an exact ledger and complete result. Add
   cancellation under resource/storage/process/restart and multi-fault
   combinations, actual partition-owner process crash/reconstruction, join
   retry exhaustion,
...
2. Deepen the public-HTTP/cross-range graph composition. Its public request now
   ...
   Extend v20/v30/v31 across actual partition-owner process
   crash/reconstruction, join retry exhaustion,
