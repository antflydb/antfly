...
fresh-world replay pass 15/15. This promotes one link-plus-memory overlap, not
disk/socket pressure, storage/process overlap, multiple failed owners, or the
...
- A borrowed DataServer runtime could still construct a native
  ...
  v11/v12/v13/v14/v15/v16/v17/v18/v19/v20/v21 production modes to keep the real Raft HTTP codec and
...
| --- | --- | --- |
| Where should Antfly add VOPR testing? | At production orchestration boundaries that combine durable state, ownership, public visibility, and recovery | Deepen the v21 production-owner cluster with disk/socket pressure, topology faults, and storage/process/restart/short-write overlaps; finish the repository-wide strong-read contract and managed-index publication/readiness; extend the durable-finalizer join seam into partition-worker recovery and global-query workflows; then compose metadata administration, MCP/A2A, cloud authentication, extension invocation, and live credential/provider replacement |
...
| Is distributed VOPR missing? | **Partly.** In-process application-level distributed VOPR exists: logical nodes, directional links, process/storage/resource domains, independent and overlapping link-plus-resource faults, quiet suffixes, and exact replay are integrated | Antithesis-style separate-address-space orchestration is not implemented, and whole-deployment breadth is incomplete. Co-resident HA/data-plane/serverless ownership, disk/socket pressure, storage/process/restart overlap, federated process agents, and live mixed binaries remain future or conditional work |
...
are green, and the cited v13-v21 production-owner gates passed their complete
...
| P1 partially integrated | Distributed graph/public-query boundaries | `distributed-query-vopr-test` exact-replays production cross-range planning, two-shard fanout with and without document hydration, topology retry/exhaustion, stale generations, cancellation, and cross-table authorization. Full-cluster v9 joins public HTTP to hosted fanout under restart, transport, and topology faults. Production-owner v21 joins public HTTP to real `DataServer` routing, active split, next-owner transport and restart histories, an exactly observed short write, all-owner memory denial/recovery, one overlapping link-plus-memory window, and post-cutover graph execution. V19 adds one public two-right-owner join shape at that split seam; v20 adds one durable-finalizer takeover shape. Cancellation, authorization changes, public hydration, broader join forms, partition-worker recovery, global query, broader restart/short-write overlap, disk/socket pressure, and storage/process/restart overlaps are not yet composed on the production owners. |
...
1. **Deepen the promoted full-cluster reconfiguration seam.** The focused
   ...
   600,000-transition record and fresh-world replay pass 15/15. Next add the
...
3. **Compose independently proven fault domains.** V21 promotes the first
   ...
   Co-locate HA and the data plane, add disk/socket pressure, and extend that
...
2. Deepen the public-HTTP/cross-range graph composition. Its public request now
   ...
   resource, graph, split, and cleanup oracles.
   Next add broader restart/short-write targets and overlaps,
   remaining topology modes, disk/socket pressure, and storage/process/restart
   overlaps, and
