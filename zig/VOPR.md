...
| Reversible node and operation service rates | `service_rate.zig` registers stable node/operation identities, composes checked node-wide and operation-specific parts-per-million costs, charges logical time through borrowed `std.Io`, accounts usage, and heals individual effects by stable fault ID. `fault.zig` and `deployment.zig` expose compatible service-rate fault identities and process/resource domains. Six production-neutral boundaries are integrated: query-cache request/hit/wait/producer work, DataServer Raft rounds, DataServer LSM-maintenance steps, distributed graph expand/hydrate/get-edges fanout by target group, replication snapshot/stream steps, and serverless publish/enrichment/compaction/prune rounds. Their VOPR histories prove overlapping node/operation slowdown and deadline expiry in the cache, node-3 slowdown during leader transfer, group-22 slowdown during parallel graph fanout, a three-times slowed then healed real LSM turn, independently healed node/snapshot slowdown followed by completed streaming, and independently healed node/publish slowdown followed by baseline compaction and fenced catalog visibility, with exact usage, resumed production execution, and cleanup | `zig build vopr-engine-test query-embedding-cache-vopr-test lib-data-runtime-test data-server-vopr-test distributed-query-vopr-test replication-backfill-vopr-test serverless-workflow-vopr-test`; full-cluster composition remains roadmap work |
...
| Which Antithesis ideas remain worth porting locally? | The large engine features, saved cross-run event-set programs, bounded live streams, and reversible logical service rates are implemented at the registered in-process boundary; query-cache operations, DataServer Raft and LSM-maintenance turns, distributed graph fanout, replication snapshot/stream steps, and serverless workflow rounds are production-charged seams, but several useful layers remain | Transitive determinism auditing; nightly sharding, retention, quarantine review, notifications, and dashboards; compiler coverage as guidance when Zig instrumentation is stable; full-cluster composition and broader production/search adoption of service rates |
...
| Query-embedding cache and service rates | **Integrated focused seams.** Cache coalescing, cancellation, deadlines, admission, TTL, byte/LRU/pinned eviction, and cleanup exact replay. Cache v2 charges four real production work boundaries and proves reversible overlapping node/operation slowdown, slowdown-induced timeout, resumed success, exact accounting, and pin cleanup. The three-owner DataServer history charges every Raft progress round and composes a reversible node-3 slowdown with leadership transfer and its full recovery suffix. Distributed-query v3 charges expand/hydrate/get-edges by target group and proves exact slowed/healed parallel fanout passes. A focused DataServer history charges a real LSM-maintenance attempt at three-times cost, heals, repeats at baseline, and verifies exact usage. Replication's typed work permit distinguishes snapshot and stream steps; its focused history overlaps node-wide and snapshot-only slowdown, heals them independently, completes snapshot-to-stream cutover, and checks exact per-class cost. Serverless runtime rounds distinguish publish, enrichment, compaction, and prune; its workflow history heals overlapping node/publish slowdown across two publications, runs baseline compaction, and requires the fenced head and documents to become visible with exact cost | Compose these service rates with public full-cluster requests and broader overlapping ownership/fault searches |
...
- extend production `service_rate.Port` adoption beyond the query-embedding
  cache, DataServer Raft/LSM progress, distributed graph fanout, replication
  snapshot/stream steps, and serverless workflow rounds, and add
  search policies that overlap node-wide and operation-class slowdowns with
  public full-cluster deadlines, cross-domain ownership, and quiet
  recovery. The reusable
...
3. Co-locate production HA and data-plane owners, extend the integrated
   ...
   operation-class service-rate ports beyond the integrated query-cache,
   DataServer Raft/LSM-progress, distributed-graph fanout, replication
   snapshot/stream, and serverless workflow seams as
   a distinct fault from the current
   CPU-work exhaustion budget, then overlap them with cross-domain
   ownership, public full-cluster deadlines, and quiet-suffix recovery. The real
