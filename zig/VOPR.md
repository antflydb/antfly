...
are green, and the cited v13-v25 production-owner gates passed their complete
...
| Metadata quorum, production `DataServer` replicas, public clients, and real HTTP/Raft transport in one history | **Integrated at the named v11-v25 seams.** Full-cluster v9 remains the complementary nine-fault hosted/public graph campaign, and `data-server-transition-vopr-test` independently proves replicated merge-to-split behavior across three production owners. V11 joins the real metadata quorum, three real `DataServer`/data-Raft owners, public clients, two tables, and the serverless public catalog on one `VoprIo`; its ReleaseSafe smoke gate passes 30/30. V12-v22 add the named split, graph, transport, restart, short-write, resource, join, overlapping-fault, and socket-pressure seams. V23 composes exact slowed/healed DataServer, graph, and serverless work on the shared clock. V24 completes an exact document-hydrating public graph request; v25 cancels its scheduled fanout through public HTTP and proves exact recovery. The current-tree v13-v25 complete gates each pass 15/15 with fresh-world exact replay, properties, cleanup, and leak checks; v23/v24 use 90,000 transitions and v25 uses 110,000 | Add v9's remaining topology breadth, broader partial-write and socket-pressure surfaces, disk-capacity pressure, coordinator/metadata/storage/process/restart overlap, query-cache/backfill/deadline service-rate combinations, and public authorization/stale-generation/retry-exhaustion plus cancellation-under-fault composition. Add disjoint donor/receiver placement, retained-history paging, snapshot-install DB/derived-state rehydration, derived graph/index equality, partitions, and richer overlapping failures |
...
| Public distributed graph request from HTTP planning through fanout/hydration | **Partially integrated, with static, active-split, transport-fault, owner-restart, recoverable short-write, three-owner memory-pressure, one overlapping link-plus-memory path, selected-listener socket pressure, exact document hydration, and one in-flight cancellation/recovery shape promoted.** Full-cluster v9 executes a public depth-two graph across hosted ranges. V13 executes it across real `DataServer`/data-Raft owners with current-owner routing and matching ReadState/derived-index visibility; v14 composes active split; v15 adds a next-owner transport cut; v16 adds owner reconstruction; v17 adds a one-byte request write; v18 adds all-owner memory denial/recovery; v21 overlaps that pressure with the selected graph link cut; v22 denies then heals new connections at one exact public listener; v24 validates exact hydrated titles plus one start/fanout/completion lifecycle; and v25 cancels after multi-owner hydration tasks are scheduled, requires the listener cancellation token and no completion, then proves one exact clean retry. The focused distributed suite separately covers topology retry/exhaustion, stale generations, and authorization | Add broader restart/topology and request/response/Raft short-write faults, disk-capacity pressure, broader socket-pressure and storage/process/restart overlaps, and public authorization mutation, stale-generation rejection, retry exhaustion, and cancellation under faults to the production-owner history. Add global queries with the same fail-closed publication rule; v19/v20's join seams are audited separately |
...
2. **Deepen public distributed operations.** V19 adds a public inner `_id`
   ...
   cancellation and recovery shape. Add authorization mutation,
...
2. Deepen the public-HTTP/cross-range graph composition. Its public request now
   ...
   cancellation/recovery shape; next compose authorization changes,
