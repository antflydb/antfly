...
composition. Forward-only v41 adds query-cache work to that same deployment;
replication-backfill and cache deadline/topology/fault overlap remain future
breadth.
...
| Full-cluster v11-v41 production owners | **Integrated at the explicitly named production-owner seams.** V11 joins the real metadata quorum, three production `DataServer`/data-Raft owners, real HTTP/Raft, two-table public clients, and the serverless catalog on one `VoprIo`. V12-v22 promote active split, graph, transport, restart, short-write, resource, join, overlapping-fault, and socket-pressure seams. V23 proves shared reversible DataServer/graph/serverless costs; v24-v35 add hydration, cancellation, authorization, stale topology, durable-worker retry and takeover, resource/link overlap, and owner reconstruction. V36-v40 promote exact ordered-global-query baseline, cancellation, live authorization revocation, directional transport failure, and exact tenant-owner destruction/reconstruction with fail-closed no-partial recovery. Forward-only v41 assigns the real production cache instance to node 1's public `ApiHttpServer`/DataServer process domain: exactly one slowed producer and one coalesced waiter precede healing, followed by one exact baseline retained hit, with strict operation/byte accounting and full cluster cleanup/replay. The cited complete gates use record plus fresh-world replay, named properties, cleanup, and leak checks; v41's 120,000-transition Debug and ReleaseSafe gates pass 15/15. The focused LSM gate remains the exact LSM-maintenance witness | Remaining breadth includes v9 topology combinations; disk-capacity and broader socket/short-write surfaces; cache deadline/topology/link/storage/resource/restart overlap; replication-backfill work in this same deployment; cancellation under storage/process and richer multi-fault combinations; arbitrary coordinator/metadata or multi-owner loss; disjoint placement; retained-history pressure; snapshot/derived-state rehydration; HA co-residency; broader join/global-query forms; and richer storage/process/link/resource overlap |
...
| Reversible node and operation service rates | `service_rate.zig` registers unambiguous stable node/operation identities, composes fully checked node-wide and operation-specific parts-per-million costs, charges logical time through borrowed `std.Io`, and heals individual effects by stable fault ID. Base multiplication, wide rounding addition, and narrowed results all fail with `ServiceRateCostOverflow` rather than trapping. Active effects remain in canonical fault-ID order, so activation and healing order cannot change fractional rounding for the same active set. Accounting distinguishes charge calls, caller-defined work units, and logical nanoseconds both per node and per `(node, operation)` pair. `fault.zig` and `deployment.zig` expose compatible service-rate fault identities and process/resource domains. Six production-neutral boundaries are integrated: query-cache request/hit/wait/producer work, DataServer Raft rounds, DataServer LSM-maintenance steps, distributed graph expand/hydrate/get-edges fanout, replication snapshot/stream steps, and serverless publish/enrichment/compaction/prune rounds. Focused histories prove exact slowed/healed behavior; replication and serverless additionally assert exact per-operation usage. Full-cluster v23 installs one shared model into DataServer, graph, and serverless owners; forward-only v41 adds query-cache producer/coalesced/hit work on node 1's shared public/DataServer process domain. Together they prove exact two-times pre-heal and baseline post-heal charging, continued public graph work, one cache computation with exact retained results and bytes, no live effects, visibility, cleanup, and exact replay | `zig build vopr-engine-test query-embedding-cache-vopr-test lib-data-runtime-test data-server-vopr-test distributed-query-vopr-test replication-backfill-vopr-test serverless-workflow-vopr-test production-cluster-service-rate-vopr-test production-cluster-query-cache-service-rate-vopr-test`; backfill and deadline/topology/fault overlap in the same deployment remain roadmap breadth |
...
| Deployment-shaped full cluster | Antfly `vopr/full_cluster.zig`, `vopr/production_cluster.zig`, `vopr/serverless_workflow.zig`, `metadata/sim_harness.zig`, and the production HTTP/Raft runtimes share one `VoprIo`. V9 retains the hosted/public nine-fault campaign. V11-v22 add the real metadata quorum, three `DataServer`/data-Raft/resource owners, active split, graph/join, transport, restart, short-write, resource-overlap, and socket seams. V23 composes reversible DataServer/graph/serverless costs; v24-v35 add the cited hydration, cancellation, authorization, stale-topology, durable-worker, overlap, and reconstruction histories; v36-v40 add global-query baseline and fail-closed cancellation/auth/transport/process-loss recovery. Forward-only v41 places the production query cache on node 1's slowed/healed public `ApiHttpServer`/DataServer domain and proves coalescing, retained hits, exact costs/bytes, cluster visibility, cleanup, and replay. Remaining work is targeted breadth: replication-backfill in this same deployment, cache deadline/fault overlap, disk and richer socket/short-write faults, storage/process/multi-owner combinations, global-query topology/resource/metadata loss, and HA/data-plane co-residency | `zig build full-cluster-vopr-test` plus the focused production-cluster v11-v41 gates |
| Production-owned full-cluster composition | Antfly `vopr/production_cluster.zig` plus `vopr/full_cluster.zig`; v11-v40 provide the detailed production metadata/DataServer/public/serverless, active-transition, graph, join, global-query, fault, cancellation, authorization, and reconstruction seams cataloged in Distributed Coverage. Public strong reads route to the current owner and require matching applied ReadState plus local derived-index visibility; remote work crosses typed internal HTTP while modeled time borrows the shared runtime. V23 installs one shared service-rate model on all DataServers, graph workers, and serverless runtime. Forward-only v41 binds the real production cache instance to node 1's public `ApiHttpServer`/DataServer port, holds healing until one producer and one waiter have entered production coalescing, then requires one baseline retained hit plus exact operation, result, byte, visibility, and cleanup evidence | The v11 smoke target passes 30/30; the cited v12-v40 gates use their documented budgets and fresh-world replay. V41's `production-cluster-query-cache-service-rate-vopr-test` passes Debug and ReleaseSafe 15/15 at 120,000 transitions. Complete gates include named properties, record/fresh-world replay, cleanup, and leak checks; the 2,000-transition subprocess remains lifecycle-only evidence |
...
Forward-only v41 composes the production query-embedding cache with the same
deployment-shaped service-rate history instead of treating the cache as an
isolated focused suite. The real cache borrows the cluster `VoprIo` and node
1's public `ApiHttpServer`/DataServer service-rate port. While the node-wide two-times
slowdown is active, two concurrent same-key requests must create exactly one
producer and one coalesced waiter; the serverless lifecycle cannot heal the
fault until that production flight is observed. After healing, a third request
must be a retained cache hit at baseline cost while DataServer and graph work
continue. The property requires exact pre/post-heal units, exact charge cost at
each cache operation boundary, one computation, three owned results, one hit,
one miss, one coalesced waiter, zero in-flight work, exact retained-byte
accounting, zero active effects, public visibility, quiet cleanup, and fresh-
world replay. Its dedicated 120,000-transition Debug and ReleaseSafe gates
pass 15/15.
V41 directly adds its mode, property, trace revision, observations, and build
target; there are no legacy aliases, readers, or migration paths.
This is the production cache implementation co-scheduled in the correct
public/DataServer node domain; it does not yet claim that a public semantic
query reached the cache instance embedded inside `ApiHttpServer`. Promoting
that HTTP/provider boundary remains distinct follow-up breadth.
...
| --- | --- | --- |
| Where should Antfly add VOPR testing? | At production orchestration boundaries that combine durable state, ownership, public visibility, and recovery | Deepen the v41 production-owner cluster with cache deadlines plus topology/link/storage/resource/restart overlap; replication-backfill work on the shared deployment model; cancellation under storage faults, disk pressure, simultaneous process loss before cancellation drain, and richer fault combinations; and broader socket/topology/short-write targets. Finish the repository-wide strong-read and managed-index publication/readiness contracts; extend durable joins across authorization/generation and broader forms; extend global query across topology, storage, resource, coordinator/metadata and multi-process loss; then compose metadata administration, MCP/A2A, cloud authentication, extension invocation, and live credential/provider replacement |
| Which Antithesis ideas remain worth porting locally? | The large engine features, saved cross-run event-set programs, non-blocking bounded live streams, and reversible logical service rates with per-node/per-operation evidence are implemented at the registered in-process boundary; query-cache, DataServer, graph, replication, and serverless work are production-charged seams, and v41 composes cache, DataServer, graph, and serverless charging in one cluster | Transitive determinism auditing; nightly sharding, retention, quarantine review, notifications, and dashboards; compiler coverage as guidance when Zig instrumentation is stable; broader v41 service-rate ownership/deadline/fault combinations and production/search adoption |
...
are green, and the cited v13-v40 production-owner gates plus the v41
ReleaseSafe gate passed their complete record/fresh-world replay oracles. The
...
V41 composes one production query-cache coalescing flight before shared-node
service-rate healing and one retained hit afterward, with exact accounting and
full cluster recovery.
`production-cluster-vopr-test` requires every focused production-owner gate
through v41; each large subprocess uses ordinary
...
| Metadata quorum, production `DataServer` replicas, public clients, and real HTTP/Raft transport in one history | **Integrated at the named v11-v41 seams.** Full-cluster v9 remains the complementary hosted/public nine-fault campaign, and `data-server-transition-vopr-test` independently proves replicated merge-to-split behavior. V11 joins the real quorum, three production owners, public clients, two tables, and serverless catalog on one `VoprIo`; v12-v22 add the cited split/graph/join/fault seams; v23 adds shared reversible costs; v24-v35 add hydration, cancellation, authorization, stale topology, durable-worker failover/overlap/reconstruction; and v36-v40 add ordered global query plus fail-closed cancellation/auth/transport/process-loss recovery. Forward-only v41 places the production query cache on node 1's shared public `ApiHttpServer`/DataServer service-rate domain and proves slowed coalescing before healing plus one exact baseline hit afterward. The cited complete modes use properties, cleanup, leak checks, and fresh-world replay; v41 passes Debug and ReleaseSafe 15/15 at 120,000 transitions | Add remaining topology breadth, disk and broader socket/short-write pressure, cache deadline/topology/fault overlap, replication-backfill on the same deployment, cancellation under storage/process and richer multi-fault combinations, disjoint placement, retained-history paging, snapshot/derived-state rehydration, partitions, and coordinator/metadata or multi-owner loss |
...
| Query-embedding cache and service rates | **Integrated focused seams plus deployment composition.** Cache, DataServer Raft/LSM, distributed graph, replication snapshot/stream, and serverless workflow histories each prove exact slowed/healed production charging. V23 installs one shared model across all production DataServers, graph workers, and serverless runtime, charges two-times work before healing, verifies baseline DataServer/graph work after healing, and completes visibility and cleanup. Forward-only v41 binds the production query cache to that model's node 1 public `ApiHttpServer`/DataServer domain, holds healing until one real same-key producer and coalesced waiter have paid slowed costs, then requires an exact baseline retained hit, one computation, three owned results, exact byte/operation accounting, cluster visibility, cleanup, and fresh-world replay | Add replication-backfill work to the same deployment and overlap operation-specific slowdown with public deadlines, ownership changes, and the existing link/storage/resource/restart algebra |
...
| P0 integrated combined active-transition/graph/resource/service-rate seam | Full-cluster distributed composition | Full-cluster v9 retains the registered hosted/public nine-fault deployment. V11-v22 add the real metadata quorum, three production `DataServer`/data-Raft runtimes, public clients, two tables, serverless catalog, active transitions, graph/join, and the cited transport/restart/short-write/resource/socket faults. V23 proves shared reversible DataServer/graph/serverless charging; v24-v35 add hydration, cancellation, authorization, stale topology, durable-worker failover/overlap/reconstruction; and v36-v40 add ordered two-table global query plus fail-closed cancellation/auth/transport/process-loss recovery. Forward-only v41 adds query-cache coalescing and retained-hit behavior to node 1's slowed/healed public `ApiHttpServer`/DataServer domain while retaining the full cluster oracle; its 120,000-transition Debug and ReleaseSafe record/fresh-world gates pass 15/15. These promote only the named seams. Next add cache deadline/topology/fault overlap; replication-backfill on the shared deployment; storage/process/multi-owner cancellation and recovery; global-query topology/storage/resource/coordinator/metadata loss; broader socket/short-write/restart targets; disjoint placement; retained-history and snapshot/derived-state recovery; HA/data-plane co-residency; richer public queries; and multi-worker placement. |
...
12. a reversible service-rate model with unambiguous stable node and operation
    ...
    and public visibility plus quiet cleanup exact-replay. Forward-only v41
    adds the production query cache to that same deployment model and node 1's
    shared public `ApiHttpServer`/DataServer fault domain. It proves exact slowed same-key
    producer/coalesced-wait work before healing, an exact baseline retained hit
    afterward, one computation, three owned results, bounded byte accounting,
    zero in-flight work/effects, full cluster visibility and cleanup, and
    fresh-world replay.
...
- extend the v41 cluster composition to replication work, and
...
1. **Deepen the promoted full-cluster reconfiguration seam.** The focused
   ...
   Forward-only v41 binds the production query cache to the same shared-node
   service-rate model as DataServer, graph, and serverless work. It requires
   one exact slowed producer/coalesced-wait flight before healing and one exact
   baseline retained hit afterward, with three owned results, bounded bytes,
   zero in-flight work/effects, cluster visibility, cleanup, and fresh-world
   replay in 120,000-transition Debug and ReleaseSafe gates.
...
3. Co-locate production HA and data-plane owners, extend the integrated
   ...
   replication work. Forward-only v41 now composes query-cache request,
   producer, coalesced-wait, and retained-hit costs on node 1's shared public
   `ApiHttpServer`/DataServer domain before and after healing. Next overlap
