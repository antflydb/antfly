...
Backfill crash, cancellation, topology/stale-owner, and overlapping-fault
variants, plus cache deadline/topology/fault overlap, remain future breadth.
...
and ReleaseSafe gates pass 15/15. Source/target crash, cancellation,
topology/stale-owner, and cross-domain overlap remain separate future modes.
V41-v43 directly add their modes, properties, trace revisions,
...
are green, and the cited v13-v40 production-owner gates plus the v41-v43
...
through v43; each large subprocess uses ordinary
...
| --- | --- | --- |
| P0 integrated | Replication backfill and rebalancing | `replication-backfill-vopr-test` covers snapshot-to-streaming cutover, resumable checkpoints, duplicate work, cancellation, source and target crashes, topology changes, stale ownership, schema changes, and exact replay through the production runners. The same gate proves typed snapshot/stream charging. Full-cluster v42 promotes the clean path through canonical public BatchRequest routing, leader forwarding, DataServer Raft, index visibility, and exact pre/post-heal work. Forward-only v43 changes schema after the first accepted batch, requires interruption and durable resume, accepts exactly one duplicate batch, completes CDC, and verifies all documents from every public node. Source/target crash, cancellation, topology/stale-owner, and overlapping-fault cases remain focused until separately promoted. |
...
- extend the v41-v43 cluster compositions with search policies that overlap
  node-wide and operation-class slowdowns
  with public deadlines, topology/ownership changes, the existing
  link/storage/resource/restart algebra, remaining replication crash,
  cancellation, topology/stale-owner cases, and quiet recovery. The reusable
  effect, clean backfill, and schema-resume compositions are complete;
...
3. Co-locate production HA and data-plane owners, extend the integrated
   ...
   exact duplicate resume against the same owners. Next overlap
