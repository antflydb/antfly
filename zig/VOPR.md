...
15/15. The remaining graph gap is to add v9's transport, restart, topology,
HTTP, and resource-fault breadth to these production owners, then add disjoint
...
comparison repairs. Production graph v13 and graph-during-split v14 pass their
...
| Distributed graph | Focused production coordinator paths, the public hosted-source composition, v13's static production-owner graph, and v14's production-owner graph during active split are implemented | V9's transport/restart/resource fault breadth is not yet on the production owners; public cancellation/auth/hydration composition, joins, and global query are not complete |
...
| One full Antfly deployment history | Runs a supplied Docker Compose or Kubernetes topology | Integrated in-process at named complementary seams: v14 runs the real metadata quorum, three production DataServer/data-Raft owners, public two-table I/O, serverless catalog, a metadata-driven active split, and public graph work before/during/after that split; v9 supplies the hosted public graph and nine-fault breadth. Both exact-replay, but v9's breadth is not yet on the production owners. Co-resident HA and richer cross-domain overlap remain ongoing. Native sidecars, DNS, kernels, and mixed binaries remain differential concerns |
...
400,000-transition graph-before/during/after-split history; and
`production-cluster-vopr-test` requires all four focused gates.
Record and
...
leak checks all passed. The current-tree v13 graph and v14 graph/split gates
each pass 15/15 with the same complete oracle. The bounded result remains
lifecycle-only evidence; v14 is the fresh combined active-split and production-
owner graph completion evidence.
...
| Metadata quorum, production `DataServer` replicas, public clients, and real HTTP/Raft transport in one history | **Integrated at the v14 combined active-transition/graph seam.** Full-cluster v9 remains the complementary nine-fault hosted/public graph campaign, and `data-server-transition-vopr-test` independently proves replicated merge-to-split behavior across three production owners. V11 joins the real metadata quorum, three real `DataServer`/data-Raft owners, public clients, two tables, and the serverless public catalog on one `VoprIo`; its ReleaseSafe smoke gate passes 30/30. V12 adds source prepare, destination bootstrap, finalize/publication, and the post-split read; its dedicated complete gate passed 15/15 at its cited checkpoint. V13 adds a production-owner depth-two public graph with current-owner routing and combined ReadState/full-index visibility. V14 starts that graph during the v12 split, accepts only complete success or typed fail-closed rejection, then requires complete post-split traversals across three ranges. The current-tree v13 and v14 complete gates each pass 15/15 with fresh-world exact replay, properties, cleanup, and leak checks | Add v9 fault breadth to the v14 production owners. Add disjoint donor/receiver placement, retained-history paging, snapshot-install DB/derived-state rehydration, derived graph/index equality, partitions, disk/socket pressure, and richer overlapping failures |
...
| Public distributed graph request from HTTP planning through fanout/hydration | **Partially integrated, with static and active-split production-owner paths promoted.** Full-cluster v9 executes a public depth-two graph traversal across two hosted ranges through real HTTP, planning, internal expansion fanout, and canonical response assembly. Its restart, transport-cut, and production-coordinator merge modes fail closed and recover completely. V13 executes the left-to-right-to-left traversal across real `DataServer`/data-Raft owners and proves current-owner public routing, matching ReadState apply, derived-index visibility, no stale fallback, complete response assembly, exact replay, properties, cleanup, and leaks. V14 composes that path with active split and post-cutover three-range routing. The focused distributed suite separately covers optional hydration, topology retry/exhaustion, cancellation, stale generations, and authorization | Add v9 transport/restart/resource faults, public cancellation, authorization changes, and hydration to the v14 production-owner history. Add distributed joins and global queries with the same fail-closed publication rule |
