...
fresh-world exact replay pass 15/15 in Debug and ReleaseSafe. Cancellation under storage,
process destruction/restart, disk pressure, and other fault combinations,
...
v30-v34 then promote durable-worker cancellation, same-partition group
failover, actual partition-owner process destruction/reconstruction, and
retry exhaustion plus cancellation while a real resource domain overlaps one
exact-group link cut. Every incomplete public graph or join operation must fail closed without
...
| --- | --- | --- |
| Where should Antfly add VOPR testing? | At production orchestration boundaries that combine durable state, ownership, public visibility, and recovery | Deepen the v34 production-owner cluster with cancellation under storage/process destruction/restart, disk pressure, and other fault combinations; broader socket/topology and storage/process/restart/short-write overlaps; and query-cache/backfill/deadline service-rate composition. Finish the repository-wide strong-read contract and managed-index publication/readiness; extend durable joins into authorization and generation changes, broader join forms, and global-query workflows; then compose metadata administration, MCP/A2A, cloud authentication, extension invocation, and live credential/provider replacement |
...
are green, and the cited v13-v34 production-owner gates passed their complete
...
v34 cancellation under overlapping resource-plus-link faults.
`production-cluster-vopr-test` requires every focused production-owner gate
through v34; each large subprocess uses ordinary
...
1. **Deepen the promoted full-cluster reconfiguration seam.** The focused
   ...
   Next add cancellation under storage/process destruction/restart, disk
   pressure, and other fault combinations, plus the
...
2. **Deepen public distributed operations.** V19 adds a public inner `_id`
   ...
   identical request. Add cancellation under storage/process
   destruction/restart, disk pressure, and other fault combinations;
...
2. Deepen the public-HTTP/cross-range graph composition. Its public request now
   ...
   Next compose cancellation with storage/process destruction/restart, disk
   pressure, and other fault combinations.
   Extend v20/v30/v31/v32/v33/v34 across right/nested/foreign joins, multi-range
