...
| --- | --- | --- |
| Reusable VOPR engine, `VoprIo`, replay, reduction, properties, saved cross-run event-set queries, bounded live event streaming, flight recording, local reports, debug recipes, fault/service-rate algebra, and search-quality fixtures | Implemented and exercised by the focused engine/meta gates named below; production query-cache operations and DataServer Raft rounds are reviewed service-rate charge seams | Nightly sharding, retention, review, notifications, and broader production adoption of service-rate charging are operational or ongoing work |
...
| Which Antithesis ideas remain worth porting locally? | The large engine features, saved cross-run event-set programs, bounded live streams, and reversible logical service rates are implemented at the registered in-process boundary; query-cache operations and DataServer Raft rounds are production-charged seams, but several useful layers remain | Transitive determinism auditing; nightly sharding, retention, quarantine review, notifications, and dashboards; compiler coverage as guidance when Zig instrumentation is stable; broader production adoption and search policies for service rates |
...
| Query-embedding cache and service rates | **Integrated focused seams.** Cache coalescing, cancellation, deadlines, admission, TTL, byte/LRU/pinned eviction, and cleanup exact replay. Cache v2 additionally charges four real production work boundaries and proves reversible overlapping node/operation slowdown, slowdown-induced timeout, resumed success, exact accounting, and pin cleanup. The three-owner DataServer history separately charges every Raft progress round and composes a reversible node-3 slowdown with leadership transfer and the full merge/split/restart recovery suffix | Compose cache pressure with public full-cluster requests; add production-neutral charging at distributed fanout and bounded maintenance work boundaries |
...
- extend production `service_rate.Port` adoption beyond the query-embedding
  cache and DataServer Raft progress, and add search policies that overlap
  node-wide and operation-class slowdowns with fanout, maintenance, deadlines,
  and quiet recovery. The reusable
...
3. Co-locate production HA and data-plane owners, extend the integrated
   ...
   operation-class service-rate ports beyond the integrated query-cache and
   DataServer Raft-progress seams as
   a distinct fault from the current
   CPU-work exhaustion budget, then overlap them with fanout, maintenance, and
   quiet-suffix recovery. The real
