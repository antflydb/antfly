...
in-flight permission-revocation/recovery shape; stale-generation rejection,
retry exhaustion, and cancellation under the broader fault algebra remain
future work.
...
are green, and the cited v13-v27 production-owner gates passed their complete
...
| Metadata quorum, production `DataServer` replicas, public clients, and real HTTP/Raft transport in one history | **Integrated at the named v11-v27 seams.** Full-cluster v9 remains the complementary nine-fault hosted/public graph campaign, and `data-server-transition-vopr-test` independently proves replicated merge-to-split behavior across three production owners. V11 joins the real metadata quorum, three real `DataServer`/data-Raft owners, public clients, two tables, and the serverless public catalog on one `VoprIo`; its ReleaseSafe smoke gate passes 30/30. V12-v22 add the named split, graph, transport, restart, short-write, resource, join, overlapping-fault, and socket-pressure seams. V23 composes exact slowed/healed DataServer, graph, and serverless work on the shared clock. V24 completes an exact document-hydrating public graph request; v25 cancels its scheduled fanout through public HTTP and proves exact recovery; forward-only v27 proves authenticated cross-table permission revocation at the live target-authorization boundary, no-leak concealment, restoration, and exact retry hydration. The current-tree v13-v27 complete gates each pass 15/15 with fresh-world exact replay, properties, cleanup, and leak checks; v23/v24 use 90,000 transitions, v25 uses 110,000, and v27 uses 120,000 | Add v9's remaining topology breadth, broader partial-write and socket-pressure surfaces, disk-capacity pressure, coordinator/metadata/storage/process/restart overlap, query-cache/backfill/deadline service-rate combinations, and public stale-generation/retry-exhaustion plus cancellation-under-fault composition. Add disjoint donor/receiver placement, retained-history paging, snapshot-install DB/derived-state rehydration, derived graph/index equality, partitions, and richer overlapping failures |
...
2. **Deepen public distributed operations.** V19 adds a public inner `_id`
   ...
   restoration, and exact recovery. Add stale-generation rejection, retry
   exhaustion, cancellation under faults,
...
2. Deepen the public-HTTP/cross-range graph composition. Its public request now
   ...
   at the live foreign-table authorization boundary and exact recovery. Next
   compose stale-generation rejection, retry exhaustion, and cancellation with
   faults.
