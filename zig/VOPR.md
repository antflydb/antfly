...
composition; query-cache, replication-backfill, and deadline/fault overlap in
that same deployment remain future breadth.
...
| Full-cluster v11-v35 production owners | **Integrated at the stated production-owner seams at the cited checkpoints.** V11 instantiates the real metadata quorum, three production `DataServer`/data-Raft owners, real HTTP/Raft, two-table public clients, and the serverless public catalog; its ReleaseSafe smoke gate passes 30/30. V12-v22 add the named split, graph, transport, restart, short-write, resource, join, overlapping-fault, and socket-pressure seams at their cited green gates. V23 applies node-wide two-times logical slowdown through the DataServer cost ports, graph, and serverless adapters, heals before the public graph workload, and proves exact pre/post-heal costs, continued DataServer and graph work, zero effects, visibility, cleanup, and exact replay. V24 sends a public graph request with document hydration through the production owners and validates exact hydrated fields. V25 cancels a public request after hydration fanout is scheduled, requires no canceled completion, and proves an exact successful retry. Forward-only v27 replaces the prior between-request authorization mode: it revokes target-table permission at the live foreign-table authorization boundary, proves exact concealment without leakage, restores permission, and proves exact hydration on a fresh request. Forward-only v28 publishes a real split after acquisition of a stamped source snapshot, requires both bounded graph attempts to fail with `TopologyChanged` and one typed no-partial public 503, then proves exact post-split read and hydration recovery. Forward-only v29 proves real hydration traffic reaches a registered coordinator-to-owner transport outage before cancellation, then requires listener cancellation, zero canceled completion, healing, and exact recovery. Forward-only v30 proves request cancellation reaches an outstanding durable-shuffle partition worker, prevents its completion, and permits an exact clean retry. Forward-only v31 proves one failed durable partition worker is not published and the same partition completes on a different group with exact retry evidence. Forward-only v32 destroys the exact serving process, requires typed fail-closed exhaustion without partial rows, reconstructs its stable identity and endpoints, and proves an identical fresh join plus direct rebuilt-endpoint read. Forward-only v33 overlaps full production resource envelopes with one exact durable-worker link cut, proves typed no-partial retry exhaustion, heals both domains, and completes an identical fresh join. Forward-only v34 matches that exact remote link before canceling the alternate worker under all-owner memory saturation, requires zero canceled-worker completion, heals both domains, and completes an identical fresh join. Forward-only v35 cancels the selected worker without completion, destroys and reconstructs its exact DataServer and listeners, and proves an identical join plus direct rebuilt-endpoint read. V23/v24's 90,000-transition gates, v25's 110,000-transition gate, v27's 120,000-transition gate, v28's 340,000-transition gate, v29's 140,000-transition gate, v30/v31's 360,000-transition gates, and v32-v35's 420,000-transition gates use fresh-world replay. The DataServer port can charge Raft and LSM classes, but this cluster property requires aggregate DataServer work; the focused LSM gate remains the exact LSM witness. All complete gates include properties, cleanup, and leak checks | V9's remaining topology breadth, disk-capacity pressure, broader socket-pressure overlap, broader client/request/response/Raft short-write surfaces, and arbitrary owner/coordinator/metadata restart combinations are not yet on these production owners. Query-cache/backfill/deadline overlap in v23, cancellation under storage faults, disk pressure, simultaneous process loss before cancellation drain, and other multi-fault compositions, disjoint donor/receiver placement, retained-history pressure, snapshot/derived-state rehydration, HA co-residency, broader join forms, additional overlapping-owner fault shapes, and broader storage/process/link/resource overlap remain roadmap work |
...
| Deployment-shaped full cluster | Antfly `vopr/full_cluster.zig`, `vopr/serverless_workflow.zig`, `serverless_http_server.zig`, `metadata/sim_harness.zig`, and Raft `transport/httpx_runtime.zig`; one `VoprIo` owns metadata, hosted ranges, public/serverless HTTP, clients, per-node resources, and workers. V9 exact-replays nine hosted/public fault modes. Complementary v11-v22 supply the real metadata quorum, three `DataServer`/data-Raft/resource owners, active split, public graph/join paths, and the named fault/recovery seams. V23 composes reversible DataServer, graph, and serverless logical costs before and after healing on that same clock. V24 promotes exact public document hydration through the production graph owners; v25 cancels scheduled hydration fanout through the real public HTTP disconnect path and recovers; forward-only v27 revokes cross-table permission inside the live target-authorization boundary and proves concealment/restoration; forward-only v28 rejects a retained source snapshot after real split publication, exhausts both bounded retries without partial output, and recovers on a fresh request; forward-only v29 proves cancellation while real outstanding hydration is blocked by a scoped inter-owner transport outage, then heals and recovers; forward-only v30 propagates public durable-join cancellation into an outstanding partition worker and proves a clean retry; forward-only v31 promotes durable partition-worker failover across production groups; forward-only v32 destroys and reconstructs the exact serving DataServer, rejects the original operation without partial rows, and proves identical fresh recovery; forward-only v33 composes matched join-link failure with all-owner memory saturation, typed no-partial exhaustion, independent healing, and exact recovery; forward-only v34 cancels an alternate durable worker while the same matched link and memory domains overlap, then heals and recovers; forward-only v35 cancels a selected worker, destroys and reconstructs that exact production owner, and recovers through both the public join and rebuilt endpoint. Remaining topology breadth, disk pressure, broader socket/short-write/restart overlap, query-cache/backfill service-rate composition, cancellation under storage faults, simultaneous process loss before cancellation drain, and other fault combinations, and HA/data-plane co-residency remain future work | `zig build full-cluster-vopr-test` plus the focused production-cluster v11-v35 gates |
| Production-owned full-cluster composition | Antfly `vopr/production_cluster.zig` plus `vopr/full_cluster.zig`; v11 places the real metadata quorum, three production `DataServer`/data-Raft owners, public clients, two isolated tables, and the production serverless catalog on one `VoprIo`. V12-v22 add the named split, graph, transport, restart, short-write, resource, join, overlapping-fault, and socket-pressure seams. V23 installs one shared service-rate model through production-neutral ports on all three DataServers, their graph workers, and the serverless runtime. Node-wide two-times slowdown is charged during bootstrap/serverless work, all effects heal before the public graph, and exact accounting distinguishes pre-heal from baseline post-heal work. V24 sends an `include_documents` graph traversal through the public client, validates exact node IDs and selected title fields, and requires exactly one production hydration lifecycle. V25 reaches a lease-free `hydration_fanout_started` boundary after shard tasks are scheduled, cancels the public client's `std.Io.Future`, requires the listener-owned cancellation token and zero canceled hydration completions, then validates one exact clean retry. Forward-only v27 gives all public setup and query requests one real Basic-authenticated identity, revokes the independently owned target table's read policy at `target_authorization_started` inside the live graph request, requires an exact empty response without key/document leakage, restores the policy, and validates the exact target table, key, and title on a fresh request. Forward-only v28 reaches `source_snapshot_acquired` with the public request's real stamped group set, publishes and retires a real split, refreshes every live production catalog, requires exactly two `TopologyChanged` attempts and one typed no-partial 503, then validates post-split read and exact hydration recovery. Forward-only v29 registers the actual coordinator-to-target link, blocks the first real `/graph-hydrate` stream, requires a monotonic matched-outage witness before cancellation, heals, and validates the exact clean retry. Forward-only v30 reaches the first durable-shuffle `partition_worker_started` boundary with a nonzero job/owner and listener-owned token, cancels the public future, requires no completion from that worker, and validates exact clean retry plus worker-accounting evidence. Forward-only v31 fails the first partition worker before row collection/publication and validates same-partition completion by a different group, exact worker/finalizer ledgers, and all 64 rows. Forward-only v32 selects the process at the actual worker boundary, tears down its DataServer and both listeners, re-registers its stable identity with a fresh incarnation, restores leadership and routing, and validates typed no-partial rejection plus exact fresh recovery. Forward-only v33 saturates every DataServer memory owner at the first partition-worker boundary while cutting the registered coordinator-to-alternate-owner semantic stream, requires typed no-partial retry exhaustion, heals both domains independently, and validates an identical fresh durable join. Forward-only v34 routes the first worker over a matched remote outage, saturates every DataServer memory envelope when the alternate local worker starts, cancels that worker without completion, heals both domains independently, and validates an identical fresh durable join. Forward-only v35 cancels the selected worker, then tears down and reconstructs its exact DataServer and stable listeners before validating an identical join and direct rebuilt-endpoint read. Public Provisioned reads adapt to current-owner routing and propagate the derived-state barrier, graph lifecycle hook, join lifecycle hook, and graph work-cost port; remote phases cross typed internal HTTP endpoints, while local phases retain the resident DB/read-admission owner. Every strong derived query waits for matching applied ReadState and local full-index visibility. Borrowed-I/O contention and topology changes retain typed retryable status, and modeled time borrows the shared runtime clock | The v11 smoke target passes 30/30; the cited v12 deep target and current-tree v13-v35 complete targets use fresh-world exact replay at their stated budgets. V23's `production-cluster-service-rate-vopr-test` and v24's `production-cluster-graph-hydration-vopr-test` pass at 90,000 transitions; v25's `production-cluster-graph-cancellation-vopr-test` passes at 110,000; v27's `production-cluster-graph-inflight-authorization-vopr-test` passes at 120,000 in Debug and ReleaseSafe; v28's `production-cluster-graph-stale-snapshot-vopr-test` uses 340,000 transitions; v29's `production-cluster-graph-cancellation-transport-fault-vopr-test` passes in ReleaseSafe at 140,000 transitions; v30's `production-cluster-durable-join-cancellation-vopr-test` and v31's `production-cluster-durable-join-worker-retry-vopr-test` use 360,000 transitions; v32's `production-cluster-durable-join-owner-restart-vopr-test`, v33's `production-cluster-durable-join-retry-exhaustion-vopr-test`, and v34's `production-cluster-durable-join-cancellation-overlap-vopr-test` and v35's `production-cluster-durable-join-cancellation-owner-restart-vopr-test` use 420,000 transitions. All include record/fresh-world exact replay, properties, cleanup, and leak checks. The 2,000-transition subprocess remains lifecycle-only evidence |
...
reconstruction. Every incomplete public graph or join operation must fail closed without
partial rows; each faulted history heals and proves a complete fresh request.
Each named complete gate exact-replays and passes 15/15 at its cited Debug and
ReleaseSafe checkpoint.
...
| --- | --- | --- |
| Where should Antfly add VOPR testing? | At production orchestration boundaries that combine durable state, ownership, public visibility, and recovery | Deepen the v35 production-owner cluster with cancellation under storage faults, disk pressure, simultaneous process loss before cancellation drain, and other fault combinations; broader socket/topology and storage/process/restart/short-write overlaps; and query-cache/backfill/deadline service-rate composition. Finish the repository-wide strong-read contract and managed-index publication/readiness; extend durable joins into authorization and generation changes, broader join forms, and global-query workflows; then compose metadata administration, MCP/A2A, cloud authentication, extension invocation, and live credential/provider replacement |
...
are green, and the cited v13-v35 production-owner gates passed their complete
...
cancellation followed by exact worker-owner reconstruction.
`production-cluster-vopr-test` requires every focused production-owner gate
through v35; each large subprocess uses ordinary
...
| Metadata quorum, production `DataServer` replicas, public clients, and real HTTP/Raft transport in one history | **Integrated at the named v11-v35 seams.** Full-cluster v9 remains the complementary nine-fault hosted/public graph campaign, and `data-server-transition-vopr-test` independently proves replicated merge-to-split behavior across three production owners. V11 joins the real metadata quorum, three real `DataServer`/data-Raft owners, public clients, two tables, and the serverless public catalog on one `VoprIo`; its ReleaseSafe smoke gate passes 30/30. V12-v22 add the named split, graph, transport, restart, short-write, resource, join, overlapping-fault, and socket-pressure seams. V23 composes exact slowed/healed DataServer, graph, and serverless work on the shared clock. V24 completes an exact document-hydrating public graph request; v25 cancels its scheduled fanout through public HTTP and proves exact recovery; forward-only v27 proves authenticated cross-table permission revocation at the live target-authorization boundary, no-leak concealment, restoration, and exact retry hydration; forward-only v28 publishes a real split behind a retained source snapshot, requires both topology attempts to fail without partial output, and proves exact post-split recovery; forward-only v29 proves real outstanding hydration reaches a scoped transport outage before cancellation, healing, and exact recovery; forward-only v30 proves public durable-shuffle cancellation reaches a real outstanding partition worker, prevents its completion, and recovers exactly; forward-only v31 proves durable same-partition worker failover to a different production group; forward-only v32 destroys and reconstructs the exact partition-owner process and proves fail-closed then complete recovery; forward-only v33 overlaps real memory saturation with a matched durable-worker link cut, exhausts fail closed, heals independently, and completes an identical request; forward-only v34 matches the same remote fault before canceling an alternate worker under that memory pressure, heals independently, and completes an identical request; forward-only v35 cancels a selected worker, destroys and reconstructs its exact production owner, and completes an identical request plus rebuilt-endpoint read. The current-tree v13-v35 complete gates use fresh-world exact replay, properties, cleanup, and leak checks; v23/v24 use 90,000 transitions, v25 uses 110,000, v27 uses 120,000, v28 uses 340,000, v29 uses 140,000, v30/v31 use 360,000, and v32-v35 use 420,000 | Add v9's remaining topology breadth, broader partial-write and socket-pressure surfaces, disk-capacity pressure, coordinator/metadata/storage/process/restart overlap, query-cache/backfill/deadline service-rate combinations, and cancellation under storage faults, disk pressure, simultaneous process loss before cancellation drain, and other fault combinations. Add disjoint donor/receiver placement, retained-history paging, snapshot-install DB/derived-state rehydration, derived graph/index equality, partitions, and richer overlapping failures |
...
| Distributed joins and global-query orchestration | **Partially integrated.** V19 executes one public inner `_id` join with two left rows and two independently owned right groups before, during, and after an active split, requiring exact no-partial results and typed ownership retry. V20 forces durable shuffle and proves a second finalizer imports the first owner's persisted result after an acknowledgement failure. Forward-only v30 cancels a public durable shuffle at a real internal partition-worker boundary, requires the canceled worker not to complete, and proves an exact clean retry. Forward-only v31 fails one partition worker before publication and proves exact same-partition failover to a different group. Forward-only v32 destroys the exact serving process, exhausts the original request fail closed, rebuilds the owner, and proves an identical fresh 64-row join plus direct rebuilt-endpoint read. Forward-only v33 proves typed no-partial retry exhaustion while all production memory envelopes and one exact-group link fault overlap, then heals both and completes an identical request. Forward-only v34 proves cancellation reaches an alternate worker while the matched link outage and all-owner memory saturation overlap, prevents canceled-worker completion, heals both domains, and completes an identical request. Forward-only v35 proves cancellation without worker completion followed by exact owner destruction, reconstruction, identical join recovery, and direct rebuilt-endpoint visibility. Focused composed-query tests cover additional result assembly. No global-query production-owner claim is made | Add cancellation under storage faults, disk pressure, simultaneous process loss before cancellation drain, and other fault combinations; auth and stale-generation changes; right/nested/foreign and multi-range-left joins; overlapping owner faults; and fail-closed global-query recovery histories |
| Query-embedding cache and service rates | **Integrated focused seams plus one cluster composition.** Cache, DataServer Raft/LSM, distributed graph, replication snapshot/stream, and serverless workflow histories each prove exact slowed/healed production charging. V23 installs one shared model across all production DataServers, graph workers, and serverless runtime, charges two-times work before healing, verifies baseline DataServer/graph work after healing, and completes visibility and cleanup | Extend v23 to query-cache/backfill work and overlap operation-specific slowdown with public deadlines, ownership changes, and the existing link/storage/resource/restart algebra |
...
| P0 integrated combined active-transition/graph/resource/service-rate seam | Full-cluster distributed composition | Full-cluster v9 exact-replays the registered hosted/public deployment under nine fault modes. V11 instantiates the real metadata quorum, three production `DataServer`/data-Raft runtimes, public clients, two tables, and the production serverless catalog; v12-v22 add the named split, graph, transport, restart, short-write, resource, join, overlapping-fault, and socket-pressure seams. V23 installs one shared reversible service-rate model into all DataServers, graph workers, and the serverless runtime and proves exact slowed/healed charging. V24 promotes exact document hydration; v25 cancels scheduled hydration fanout through the public HTTP disconnect path and proves exact recovery; forward-only v27 revokes target-table permission at the live authorization boundary and proves no-leak concealment plus restored exact hydration; forward-only v28 publishes a real split behind a retained source snapshot, proves exact bounded retry exhaustion with a no-partial 503, and recovers on a fresh request; forward-only v29 proves cancellation only after real hydration traffic matches a scoped coordinator-to-owner outage, then heals and exactly recovers; forward-only v30 propagates public durable-shuffle cancellation to a real outstanding partition worker, requires no canceled completion, and exactly recovers; forward-only v31 promotes exact durable same-partition failover across worker groups; forward-only v32 destroys and reconstructs the actual partition-owner process with typed no-partial rejection and exact fresh recovery; forward-only v33 exhausts a durable join under overlapping real resource and exact-link faults, heals both, and exactly recovers; forward-only v34 cancels an alternate worker after the exact link is matched while real all-owner memory pressure remains active, heals both, and exactly recovers; forward-only v35 cancels a selected worker, reconstructs its exact production owner, and exactly recovers through both public and direct rebuilt-endpoint reads. The 90,000-transition v23/v24 gates, 110,000-transition v25 gate, 120,000-transition v27 gate, 340,000-transition v28 gate, 140,000-transition v29 gate, 360,000-transition v30/v31 gates, and the 420,000-transition v32-v35 gates use exact record/replay. These promote the named seams, not the whole roadmap. Next add cancellation under storage faults, disk pressure, simultaneous process loss before cancellation drain, and other fault combinations; query-cache/backfill/deadline service-rate composition; remaining topology breadth; broader short-write/restart/socket targets; and storage/process/restart overlap, then disjoint placement, retained-history paging, snapshot/derived-state rehydration, HA/data-plane co-residency, richer public queries, and multi-worker placement. |
...
| P0 integrated | Query-embedding cache | `query-embedding-cache-vopr-test` exact-replays concurrent-miss coalescing, waiter cancellation, deadlines, in-flight admission, TTL, byte-budget/LRU eviction, pinned hits, and cleanup through the production cache on one `VoprIo`. V2 adds a production-neutral cost port at request, hit-copy, coalesced-wait, and producer-compute boundaries and proves baseline cost, overlapping node/hit slowdown, real deadline expiry, independent healing, resumed success, exact usage, and cleanup. Full-cluster cache/service-rate interference remains future composition. |
| P1 partially integrated | Distributed graph/public-query boundaries | `distributed-query-vopr-test` exact-replays production cross-range planning, two-shard fanout with and without document hydration, topology retry/exhaustion, stale generations, cancellation, cross-table authorization, and per-group operation charging; v3 slows group 22 four times across expand/hydrate, verifies exact usage, heals, and repeats at baseline cost. Full-cluster v9 joins public HTTP to hosted fanout under restart, transport, and topology faults. Production-owner v22 joins public HTTP to real `DataServer` routing, active split, next-owner transport and restart histories, an exactly observed short write, all-owner memory denial/recovery, one overlapping link-plus-memory window, selected-listener socket denial/recovery, and post-cutover graph execution. V24 adds exact selected-field document hydration through the public production-owner response. V25 cancels after multi-owner hydration fanout is scheduled, requires no canceled completion, and proves one exact clean retry. Forward-only v27 authenticates the public workload and proves target-table permission revocation inside the live foreign-table authorization boundary, exact concealment, restoration, and exact fresh-request hydration through the real `UserManager`. Forward-only v28 retains the public request's stamped source group set across real split publication, requires both topology attempts to reject it and return one typed no-partial 503, then proves exact post-split recovery. Forward-only v29 installs a scoped transport outage at the first hydration fanout, proves a real `/graph-hydrate` write matched before cancellation, then heals and exactly recovers. V19 adds one public two-right-owner join shape at the split seam; v20 adds one durable-finalizer takeover shape; forward-only v30 adds one outstanding partition-worker cancellation and clean-retry shape; forward-only v31 adds one pre-publication partition-worker failure and same-partition failover shape; forward-only v32 adds exact serving-process destruction/reconstruction with fail-closed then complete fresh recovery; forward-only v33 adds overlapping resource-plus-link retry exhaustion with no-partial rejection and exact recovery; forward-only v34 adds cancellation of an alternate worker under that matched link and real all-owner memory pressure, independent healing, and exact recovery; forward-only v35 adds cancellation followed by exact worker-owner destruction, reconstruction, and public plus direct-endpoint recovery. Cancellation under storage faults, disk pressure, simultaneous process loss before cancellation drain, and other fault combinations; broader join forms; additional overlapping-owner fault shapes; global query; broader restart/short-write/socket overlap; and storage/process/restart overlaps are not yet composed on the production owners. |
...
12. a reversible service-rate model with unambiguous stable node and operation
    ...
    and public visibility plus quiet cleanup exact-replay.
...
- extend the v23 cluster composition to query-cache and replication work, and
  add search policies that overlap node-wide and operation-class slowdowns
  with public deadlines, topology/ownership changes, the existing
  link/storage/resource/restart algebra, and quiet recovery. The reusable
...
2. **Deepen public distributed operations.** V19 adds a public inner `_id`
   ...
   overlapping-owner fault shapes, and global queries;
...
3. Co-locate production HA and data-plane owners, extend the integrated
   ...
   distinct fault from the CPU-work exhaustion budget. Extend that history to
   query-cache and replication work, then overlap operation-specific slowdown
