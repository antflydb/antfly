...
| --- | --- | --- |
| Reusable VOPR engine, `VoprIo`, replay, reduction, properties, saved cross-run event-set queries, bounded live event streaming, flight recording, local reports, debug recipes, fault/service-rate algebra, and search-quality fixtures | Implemented and exercised by the focused engine/meta gates named below; the production query-embedding cache is the first reviewed service-rate charge seam | Nightly sharding, retention, review, notifications, and broader production adoption of service-rate charging are operational or ongoing work |
...
| Typed strong-read capabilities | `raft/read_gate.zig` separates enqueue-only `ReadIndexRequester` from synchronous `ReadSafetyBarrier`; managed Raft services expose only initiation, while public table-read sources accept only the barrier type. DataServer implements the barrier with canonical matching-group ReadState plus applied-index completion, starts replicated sources with a fail-closed unavailable barrier, and installs `alreadyReadSafeBarrier` only after explicitly selecting direct non-Raft ownership. The former readable-lease API, no-op name, service adapters, and metrics were renamed or deleted directly; there are no compatibility aliases | `zig build raft-test root-test` |
...
| Reversible node and operation service rates | `service_rate.zig` registers stable node/operation identities, composes checked node-wide and operation-specific parts-per-million costs, charges logical time through borrowed `std.Io`, accounts usage, and heals individual effects by stable fault ID. `fault.zig` and `deployment.zig` expose compatible service-rate fault identities and process/resource domains. The production query-embedding cache exposes a VOPR-independent `WorkCostPort` at request, hit-copy, coalesced-wait, and producer-compute boundaries; implementations are required to be thread-safe. Its VOPR adapter proves overlapping node/hit slowdown, deadline expiry, independent healing, resumed success, exact usage, and pin cleanup | `zig build vopr-engine-test query-embedding-cache-vopr-test`; election, distributed fanout, and maintenance adoption remains roadmap work |
...
| Which Antithesis ideas remain worth porting locally? | The large engine features, saved cross-run event-set programs, bounded live streams, and reversible logical service rates are implemented at the registered in-process boundary; query-embedding cache work is the first production-charged seam, but several useful layers remain | Transitive determinism auditing; nightly sharding, retention, quarantine review, notifications, and dashboards; compiler coverage as guidance when Zig instrumentation is stable; broader production adoption and search policies for service rates |
...
| Query-embedding cache | **Integrated focused seam.** Coalescing, cancellation, deadlines, admission, TTL, byte/LRU/pinned eviction, and cleanup exact replay. V2 additionally charges four real production work boundaries and proves reversible overlapping node/operation slowdown, slowdown-induced timeout, resumed success, exact accounting, and pin cleanup | Compose this node-local service-rate and cache pressure with public full-cluster requests |
...
12. a reversible service-rate model with stable node and operation identities,
    ...
    and exact-replay overlap/deadline/healing proof.
...
- extend the first production `service_rate.Port` adoption beyond the
  query-embedding cache and add search policies that overlap node-wide and
  operation-class slowdowns with
...
2. **Deepen public distributed operations.** V19 adds a public inner `_id`
   ...
   and direct non-Raft state is explicitly marked already safe. Add the
   follower, leader-change, timeout, cancellation, group-retirement, and
   derived-state histories that prove each remaining callback barrier really
   blocks through its owner-specific visibility boundary.
...
3. Co-locate production HA and data-plane owners, extend the integrated
   ...
   operation-class service-rate ports beyond the integrated query-cache seam as
