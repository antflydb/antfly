...
fresh-world exact replay pass 15/15 in Debug and ReleaseSafe. Retry exhaustion
under overlapping faults, authorization/generation mutation, and broader join
...
| --- | --- | --- |
| Where should Antfly add VOPR testing? | At production orchestration boundaries that combine durable state, ownership, public visibility, and recovery | Deepen the v32 production-owner cluster with cancellation under resource/storage/process/restart and multi-fault combinations, disk-capacity pressure, broader socket/topology faults, storage/process/restart/short-write overlaps, and query-cache/backfill/deadline service-rate composition; finish the repository-wide strong-read contract and managed-index publication/readiness; extend the durable join seam into retry exhaustion under overlapping faults, authorization and generation changes, broader join forms, and global-query workflows; then compose metadata administration, MCP/A2A, cloud authentication, extension invocation, and live credential/provider replacement |
...
are green, and the cited v13-v32 production-owner gates passed their complete
...
hydration-transport outage. `production-cluster-vopr-test` requires every
focused production-owner gate through v31; each large subprocess uses ordinary
...
2. **Deepen public distributed operations.** V19 adds a public inner `_id`
   ...
   identical fresh join through restored routing. Add
   cancellation under resource/storage/process/restart and multi-fault
   combinations, join retry exhaustion under overlapping faults,
...
2. Deepen the public-HTTP/cross-range graph composition. Its public request now
   ...
   Extend v20/v30/v31/v32 across join retry exhaustion under overlapping faults,
   right/nested/foreign joins, multi-range left inputs, cancellation combined
   with other faults, and overlapping owner faults; add global queries
