...
| --- | --- | --- |
| Where should Antfly add VOPR testing? | At production orchestration boundaries that combine durable state, ownership, public visibility, and recovery | Deepen the v40 production-owner cluster with cancellation under storage faults, disk pressure, simultaneous process loss before cancellation drain, and other fault combinations; broader socket/topology and storage/process/restart/short-write overlaps; and query-cache/backfill/deadline service-rate composition. Finish the repository-wide strong-read contract and managed-index publication/readiness; extend durable joins into authorization and generation changes and broader join forms, and extend global query into topology, storage, resource, coordinator/metadata or multi-process loss, and broader transport/overlap faults; then compose metadata administration, MCP/A2A, cloud authentication, extension invocation, and live credential/provider replacement |
...
are green, and the cited v13-v40 production-owner gates passed their complete
...
through v40; each large subprocess uses ordinary
...
| Metadata quorum, production `DataServer` replicas, public clients, and real HTTP/Raft transport in one history | **Integrated at the named v11-v40 seams.** Full-cluster v9 remains the complementary nine-fault hosted/public graph campaign, and `data-server-transition-vopr-test` independently proves replicated merge-to-split behavior across three production owners. V11 joins the real metadata quorum, three real `DataServer`/data-Raft owners, public clients, two tables, and the serverless public catalog on one `VoprIo`; its ReleaseSafe smoke gate passes 30/30. V12-v22 add the named split, graph, transport, restart, short-write, resource, join, overlapping-fault, and socket-pressure seams. V23 composes exact slowed/healed DataServer, graph, and serverless work on the shared clock. V24-v35 add the named hydration, cancellation, authorization, stale-snapshot, transport, and durable-join lifecycle seams. V36 sends an ordered two-table global NDJSON query through the real public listener and production table owners, requiring exactly two responses and exact disjoint ID sets. Forward-only v37 cancels after the first production result, requires typed cancellation and exactly one canceled-request result, and proves exact recovery. Forward-only v38 revokes live authority after that result, requires an exact 403 with no protected payload, restores policy, and proves exact recovery. Forward-only v39 cuts the registered remote tenant-query stream after that result, requires the exact retryable 503 with one fault match and no partial output, heals, and proves exact recovery. Forward-only v40 destroys the exact tenant-owner process at the same first-result boundary, requires the same no-partial 503, reconstructs its stable DataServer and listeners, requires a direct durable read from the rebound endpoint, and proves exact recovery. The current-tree v13-v40 complete gates use fresh-world exact replay, properties, cleanup, and leak checks; v36 passes ReleaseSafe at 90,000 transitions, v37 at 110,000, v38 at 120,000, v39 at 140,000, and v40 at 240,000 | Add v9's remaining topology breadth, broader partial-write and socket-pressure surfaces, disk-capacity pressure, coordinator/metadata/storage/process/restart overlap, query-cache/backfill/deadline service-rate combinations, and cancellation under storage faults, disk pressure, simultaneous process loss before cancellation drain, and other fault combinations. Add disjoint donor/receiver placement, retained-history paging, snapshot-install DB/derived-state rehydration, derived graph/index equality, partitions, and richer overlapping failures |
...
| P0 integrated combined active-transition/graph/resource/service-rate seam | Full-cluster distributed composition | Full-cluster v9 exact-replays the registered hosted/public deployment under nine fault modes. V11-v22 add the real metadata quorum, three production `DataServer`/data-Raft runtimes, public clients, two tables, serverless catalog, split, graph, transport, restart, short-write, resource, join, overlapping-fault, and socket-pressure seams. V23 proves shared reversible DataServer/graph/serverless charging. V24-v35 promote the cited hydration, cancellation, authorization, topology, transport, durable-worker, resource, and owner-reconstruction histories. V36 proves ordered and isolated two-table global NDJSON dispatch; forward-only v37 cancels after the first production result, requires typed cancellation and exactly one canceled-request result, drains the handler, and proves exact ordered recovery. Forward-only v38 revokes second-table authority at the same boundary, requires an exact 403 and no protected result, restores policy, and proves exact recovery. Forward-only v39 cuts the registered tenant-owner query stream there, requires the exact retryable 503 and no partial result, heals, and proves exact recovery. Forward-only v40 destroys the exact tenant-owner process at the same first-result boundary, requires the same no-partial 503, reconstructs its stable DataServer and listeners, requires a direct durable read from the rebound endpoint, and proves exact recovery. V36's 90,000-transition, v37's 110,000-transition, v38's 120,000-transition, v39's 140,000-transition, and v40's 240,000-transition ReleaseSafe gates pass record and fresh-world replay. These promote the named seams, not the whole roadmap. Next add cancellation under storage faults, disk pressure, simultaneous process loss before cancellation drain, and other fault combinations; global-query topology/storage/resource faults, coordinator/metadata or multi-process loss, and broader transport/overlap shapes; query-cache/backfill/deadline service-rate composition; remaining topology breadth; broader short-write/restart/socket targets; and storage/process/restart overlap, then disjoint placement, retained-history paging, snapshot/derived-state rehydration, HA/data-plane co-residency, richer public queries, and multi-worker placement. |
