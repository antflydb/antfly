...
- The first dedicated posting segment container now exists in
  ...
  allocating the decoded delta array. A bounded segment directory batch writer
  can accumulate posting-local records under entry/byte caps and flush them
  through the existing atomic segment+manifest commit path, including
  coalescing individual and multi-record posting delta appends into one encoded
