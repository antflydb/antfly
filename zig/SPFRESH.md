...
- HBC maintenance paths that only need base generation/member count now use
  ...
  through a linear selected-id scan, and segment-id validation returns that
  sortedness result while skipping the quadratic duplicate scan for ascending
  plan output. Segment garbage and temporary-file cleanup now track live
