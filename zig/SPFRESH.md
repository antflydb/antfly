...
- HBC maintenance paths that only need base generation/member count now use
  ...
  collection paths. Allocation-returning segment delta reads also exact-size
  ...
  replay. Segment compaction also pre-counts each sorted posting
  ...
  a parallel owner array; small selected compactions also stack-back that
