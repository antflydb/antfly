...
def validate_root(root):
    ...
    if kind != "tmpfs" or not MIN_FREE <= total <= BUDGET or available < MIN_FREE:
        raise ValueError(f"fixture root needs bounded tmpfs with 384 MiB free and at most 512 MiB total; {root}: type={kind}, total={total}, free={available}")
    return root
...
def prepare(environ):
    ...
    workspace = Path(tempfile.mkdtemp(prefix="antfly-fixtures.", dir=environ["RUNNER_TEMP"]))
    ...
    workspace.rmdir()
    print("::warning::No bounded fixture tmpfs. Mount denied; using disk. Provision /mnt/antfly-fixtures as a 512Mi memory emptyDir (see zig/tools/CI_UNIT_TAIL_FOLLOWUP.md).")
    return None


def cleanup(workspace, mounted):
    path = Path(workspace)
    if not path.is_absolute() or not path.name.startswith("antfly-fixtures.") or path.is_symlink():
...
def main():
    ...
    if args.action == "cleanup":
        cleanup(os.environ["ANTFLY_TEST_WORKSPACE"], os.environ.get("ANTFLY_TEST_WORKSPACE_MOUNTED") == "1")
        return
    result = prepare(os.environ)
    if result is not None:
        workspace, mounted = result
        with open(os.environ["GITHUB_ENV"], "a") as env:
            env.write(f"ANTFLY_TEST_WORKSPACE={workspace}\nANTFLY_TEST_WORKSPACE_MOUNTED={int(mounted)}\n")
