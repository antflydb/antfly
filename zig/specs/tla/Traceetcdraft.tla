...
StepToNextTrace == 
    /\ l' = l+1
    /\ pl' = l
    /\ (Len(TraceLog) >= 100 /\ l % (Len(TraceLog) \div 100) = 0) => PrintT(<< "Progress %:", (l * 100) \div Len(TraceLog)>>)
...
\* Handle ReceiveSnapshot via a dedicated action. Snapshots don't fit
\* cleanly into HandleAppendEntriesRequest because:
\* (a) The send may be in a prior trace segment (missing from messages bag)
\* (b) Commit advancement for snapshots differs from regular AppendEntries
HandleSnapshotFromTrace ==
    /\ LoglineIsEvent("ReceiveSnapshot")
    /\ LET i == logline.event.msg.to
           j == logline.event.msg.from
           snapshotIndex == logline.event.msg.entries
           msgTerm == logline.event.msg.term
       IN
       /\ ValidatePreStates(i)
       \* 1. Consume the message from the bag if present, otherwise no-op
       /\ IF \E m \in DOMAIN messages : LoglineIsReceivedMessage(m)
          THEN \E m \in DOMAIN messages :
               /\ LoglineIsReceivedMessage(m)
               /\ messages' = WithoutMessage(m, messages)
          ELSE UNCHANGED messages
       \* 2. UpdateTerm + BecomeFollower effect
       /\ IF msgTerm > currentTerm[i]
          THEN /\ currentTerm' = [currentTerm EXCEPT ![i] = msgTerm]
               /\ state' = [state EXCEPT ![i] = Follower]
               /\ votedFor' = [votedFor EXCEPT ![i] = Nil]
          ELSE IF state[i] = Candidate /\ msgTerm = currentTerm[i]
               THEN /\ state' = [state EXCEPT ![i] = Follower]
                    /\ UNCHANGED <<currentTerm, votedFor>>
               ELSE UNCHANGED <<currentTerm, state, votedFor>>
       \* 3. Advance commit to at least the snapshot index
       /\ commitIndex' = [commitIndex EXCEPT ![i] = Max({@, snapshotIndex})]
       \* 4. Extend log to cover snapshot entries if needed
       /\ IF Len(log[i]) < snapshotIndex
          THEN log' = [log EXCEPT ![i] =
                 @ \o [k \in 1..(snapshotIndex - Len(@)) |->
                      [term |-> 1, type |-> ValueEntry,
                       value |-> [val |-> 0]]]]
          ELSE UNCHANGED log
       \* 5. Update config from trace (snapshot may carry new config)
       /\ config' = [config EXCEPT ![i] =
            [jointConfig |-> ConfFromLog(logline),
             learners |-> {}]]
       \* 6. Reply with snapshot response (goes to pendingMessages)
       /\ SendDirect([mtype        |-> AppendEntriesResponse,
                      msubtype     |-> "snapshot",
                      mterm        |-> Max({msgTerm, currentTerm[i]}),
                      msuccess     |-> TRUE,
                      mmatchIndex  |-> snapshotIndex,
                      msource      |-> i,
                      mdest        |-> j])
       /\ UNCHANGED <<candidateVars, leaderVars,
                      reconfigCount, durableState>>
    ...
    /\ LET 
       ...
       IN 
            ...
            /\ \/ /\ TraceLog[k].event.name \in ReceiveMessageTraceNames
               ...
               \* Candidate returns to follower on AppendEntries with same term,
               \* or follower reaffirms follower state (zig raft emits this).
               \/ /\ TraceLog[k].event.name = "ReceiveAppendEntriesRequest"
                  /\ TraceLog[k].event.state.term = TraceLog[k].event.msg.term
                  /\ TraceLog[k].event.msg.term = logline.event.state.term
                  /\ TraceLog[k].event.role \in {Candidate, Follower}
...
\* Ignored higher-term MsgVote: the node receives a ReceiveRequestVoteRequest
\* at a higher term but does NOT process it (shouldIgnoreCampaignMessage returns
\* true when check_quorum is enabled and the node recently heard from a leader).
\* Detected by checking that the next trace event for this node is NOT a
\* BecomeFollower at the message's term — which UpdateTerm would have produced
\* if the message had been processed normally.
LoglineIsIgnoredHigherTermVoteRequest ==
    /\ LoglineIsEvent("ReceiveRequestVoteRequest")
    /\ logline.event.state.term < logline.event.msg.term
    /\ LET k == SelectInSeq(SubSeq(TraceLog, l+1, Len(TraceLog)),
                    LAMBDA x: x.event.nid = logline.event.nid)
       IN \/ k = 0  \* no subsequent events for this node
          \/ TraceLog[l + k].event.name # "BecomeFollower"
          \/ TraceLog[l + k].event.state.term # logline.event.msg.term
...
TraceNextNonReceiveActions ==
    /\ \/ /\ LoglineIsEvents({"SendRequestVoteRequest", "SendRequestVoteResponse"}) 
       ...
       \/ SkipUnusedLogline
       \* Drop ignored MsgVote: the node ignores the incoming MsgVote
       \* (check_quorum with a recent leader heartbeat). Covers both
       \* concurrent PreVote elections (followed by BecomeCandidate) and
       \* leaders silently dropping the vote request.
       \/ /\ LoglineIsIgnoredHigherTermVoteRequest
...
\* After the trace is fully consumed, drain remaining pending and in-flight
\* messages so TLC's queue empties (TraceMatched needs queue=1).
\* Uses Ready (flushes pending->messages) and DropMessage (removes from
\* messages). etcdSpec is relaxed during drain (l > Len(TraceLog)) because
\* DropMessage in NextUnreliable requires messages[m]=1, but accumulated
\* heartbeats can have count>1.
TracePostDrain ==
    /\ l > Len(TraceLog)
...
\* Verify NextDynamic compliance for trace events only. After the trace is
\* consumed (l > Len(TraceLog)), drain actions (DropMessage with count>1) are
\* not in NextDynamic, so we relax the check.
etcdSpec == etcd!Init /\ [][etcd!NextDynamic \/ l > Len(TraceLog)]_etcd!vars
