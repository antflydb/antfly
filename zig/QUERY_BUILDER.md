...
- `graph`: Build `graph_queries` requests.
...
- Generated graph planning now gets one validation-repair pass before deterministic fallback. If the first generated `graph_queries` response fails validation, the coordinator sends the same prompt plus concrete correction rules back to the generator and validates the repaired response. The plan validator can also provide executor/dry-run feedback for otherwise well-formed graph plans, and that feedback is included in the repair prompt.
...
- Generated-graph repair and final graph-search validation use `$query_results` for the final ranked retrieval output. `$graph_results.<search_name>` must point to a compatible graph output in the same plan; MATCH dependencies additionally require an explicit returned binding alias.
