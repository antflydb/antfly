...
- Table-scoped query-builder requests now prefetch index metadata in one table context. The context carries structured full-text, embedding, and graph metadata plus flat index-name slices for existing validators. Structured metadata is preferred when present, while flat semantic and graph index slices remain the permissive path for validators that do not need structured metadata yet. Graph generation prompts and metadata-backed validation now share graph edge-type metadata from that same context.
