...
// Tiled matrix multiplication with B transposed and Q5_1 dequantization.
// C[M,N] = A[M,K] @ dequant(B_q5_1)^T
...
fn read_u32_le(byte_offset: u32) -> u32 {
...
fn dequant_q5_1(b_row: u32, k_abs: u32) -> f32 {
    let blocks_per_row = params.K / 32u;
    let block_idx = k_abs / 32u;
    let in_block = k_abs % 32u;
    let block_byte = (b_row * blocks_per_row + block_idx) * 24u;

    let d = read_f16(block_byte);
    let m = read_f16(block_byte + 2u);
    let qh = read_u32_le(block_byte + 4u);
    let packed = read_byte(block_byte + 8u + (in_block % 16u));
    var low4: u32;
    if (in_block < 16u) {
        low4 = packed & 0x0Fu;
    } else {
    ...
    let high = (qh >> in_block) & 1u;
    let q = low4 + high * 16u;
    return d * f32(q) + m;
}

@compute @workgroup_size(TILE_M, TILE_N)
fn matmul_transb_q5_1(
    ...
    for (var t = 0u; t < num_tiles; t++) {
        ...
        for (var pass = 0u; pass < 2u; pass++) {
            ...
            if (g_col < params.N && g_k < params.K) {
                tile_b[idx] = dequant_q5_1(g_col, g_k);
            } else {
                tile_b[idx] = 0.0;
            }
        }
