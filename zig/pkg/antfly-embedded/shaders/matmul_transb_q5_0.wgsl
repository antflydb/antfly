...
// Tiled matrix multiplication with B transposed and Q5_0 dequantization.
// C[M,N] = A[M,K] @ dequant(B_q5_0)^T
...
fn dequant_q5_0(b_row: u32, k_abs: u32) -> f32 {
    let blocks_per_row = params.K / 32u;
    let block_idx = k_abs / 32u;
    let in_block = k_abs % 32u;
    let block_byte = (b_row * blocks_per_row + block_idx) * 22u;

    let d = read_f16(block_byte);
    let qh = read_u32_le(block_byte + 2u);
    let packed = read_byte(block_byte + 6u + (in_block % 16u));
    var low4: u32;
    if (in_block < 16u) {
        low4 = packed & 0x0Fu;
    } else {
    ...
    for (var t = 0u; t < num_tiles; t++) {
        ...
        for (var pass = 0u; pass < 2u; pass++) {
            ...
            if (g_col < params.N && g_k < params.K) {
                tile_b[idx] = dequant_q5_0(g_col, g_k);
            } else {
                tile_b[idx] = 0.0;
            }
        }
