...
// Fused causal (decoder) self-attention.
// out = softmax(Q @ K^T / sqrt(head_dim) + causal_mask) @ V
//
// Causal mask: positions where ki > qi are set to -inf (no future peeking).
// No external mask buffer needed — the mask is implicit from position indices.
//
