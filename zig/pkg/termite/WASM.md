...
Native link work in `build.zig` / `pkg/termite/build.zig`:
...
For browser worker-mode WebGPU, serve `pkg/termite/web` with COOP/COEP headers. A minimal local helper lives at `web/dev-server.mjs`; run `node web/dev-server.mjs` from `pkg/termite` and open `http://localhost:8000/index.html`.
...
The browser demo now has a minimal Electron shell under `pkg/termite/electron/` that reuses the existing `web/index.html` UI rather than introducing a second desktop-specific frontend.
