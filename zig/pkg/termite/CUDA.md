...
- `pkg/termite/src/native_backend_choice.zig` already has `xla`, mapped to
  `BackendKind.pjrt` for compiled partitions.
- `pkg/termite/src/graph/compiled_pjrt.zig`,
  `pkg/termite/src/graph/pjrt_compiler.zig`, and
  `pkg/termite/src/graph/pjrt_executor.zig` already define the PJRT lane.
- `pkg/termite/src/graph/quant_matmul.zig` already provides the shared
  quantized matmul vocabulary: dispatch buckets, row buckets, packed format
  descriptors, and operator support.
- `pkg/termite/src/gguf/tensor_types.zig` and
  `pkg/termite/src/gguf/quant_codec.zig` are the canonical GGUF type and
  dequantization references.
- `pkg/termite/src/ops/native_compute.zig`,
  `pkg/termite/src/ops/metal_compute.zig`, and
  `pkg/termite/src/ops/wasm_compute.zig` already contain quantized matmul
  behavior and fallback patterns.
- `pkg/termite/src/graph/backend_contracts.zig` does not yet include
  `cuda` as a graph `BackendKind` or `cuda_buffer` storage class.
- `pkg/termite/src/backends/backends.zig` does not yet expose CUDA as a direct
  session backend.
...
| Path | Purpose |
|---|---|
| `pkg/termite/src/ops/cuda/driver.zig` | Driver API dynamic loader |
| `pkg/termite/src/ops/cuda/context.zig` | Device/context/stream lifecycle |
| `pkg/termite/src/ops/cuda/buffer.zig` | Device memory and host copies |
| `pkg/termite/src/ops/cuda/kernels.zig` | Embedded PTX module loading and JIT diagnostics |
| `pkg/termite/src/ops/cuda/quant.zig` | GGUF format descriptors for CUDA |
| `pkg/termite/src/ops/cuda/cuda_compute.zig` | `ComputeBackend` implementation |
| `pkg/termite/src/ops/cuda/kernels/*.cu` | Developer kernel sources |
| `pkg/termite/src/ops/cuda/artifacts/*.ptx` | Checked-in portable PTX |
| `pkg/termite/src/ops/cuda/artifacts/*.fatbin` | Optional checked-in fatbins |
