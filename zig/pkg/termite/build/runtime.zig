...
pub const Paths = struct {
    /// Path from the active build.zig directory to pkg/termite.
...
pub const SharedModules = struct {
    ...
    termite_api: ?*std.Build.Module = null,
...
pub const Graph = struct {
    ...
    termite_api_mod: *std.Build.Module,
...
pub fn create(config: Config) Graph {
    ...
    const termite_api_mod = shared.termite_api orelse addTermiteApiModule(b, target, optimize, httpx_mod, backend.skip_openapi, paths, config.register_public_modules);
    const termite_client_mod = shared.termite_client orelse if (!backend.skip_openapi) blk: {
        ...
        mod.addImport("httpx", httpx_mod);
        mod.addImport("termite_api", termite_api_mod);
        break :blk mod;
    } else null;
    ...
    addTermiteRootImports(termite_mod, .{
        ...
        .httpx_mod = httpx_mod,
        .termite_api_mod = termite_api_mod,
...
pub fn addTermiteRootImports(module: *std.Build.Module, imports: TermiteRootImports) void {
    ...
    module.addImport("termite_api", imports.termite_api_mod);
...
fn addTermiteApiModule(
    ...
    if (skip_openapi) {
        ...
        const empty_root = empty_wf.add("termite_api_stub.zig",
            \\comptime {
            \\    @compileError("termite_api is unavailable because -Dskip-openapi=true was set; build a target that does not import the generated HTTP server/client API.");
            \\}
            \\
        );
    ...
    const spec_path_override = b.option(
        ...
        "Path to the Termite OpenAPI YAML spec used to generate termite_api",
    );

    if (spec_path_override == null) {
        const mod = addOrCreateModule(b, register_public_modules, "termite_api", .{
            .root_source_file = b.path(pathJoin(b, paths.termite_root, "src/api/generated/termite_api/root.zig")),
            .target = target,
            .optimize = optimize,
        });
