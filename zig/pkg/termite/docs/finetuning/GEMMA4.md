...
This plan gates Gemma4 LoRA fine-tuning before distributed Metal work. The target is repeatable evidence from one-device text and multimodal pilots, not maximum throughput.
...
```sh
zig build termite-run-gemma4-lora-pilot-workflow -- text /Users/tim/models/gemma-4-e2b-it /tmp/gemma4-text-pilot --count 1000 --backend metal --epochs 1
```

Recommended larger run:

```sh
zig build termite-run-gemma4-lora-pilot-workflow -- text /Users/tim/models/gemma-4-e2b-it /tmp/gemma4-text-pilot-10k --count 10000 --max-examples 10000 --eval-max-examples 128 --backend metal --epochs 2
```

Acceptance gates:
- `training_config.json` records `trainer=autodiff` and `selected_backend=metal`.
...
```sh
zig build termite-run-gemma4-lora-pilot-workflow -- multimodal /Users/tim/models/gemma-4-e2b-it /tmp/gemma4-mm-pilot --projector /Users/tim/models/gemma-4-e2b-it/mmproj-gemma-4-E2B-it-bf16.gguf --image-path /tmp/gemma4-mm-smoke/images/red.png --count 100 --backend metal --epochs 1
```
...
- Metal train/eval completes with before/after eval `optimizer_steps=0`.

## Task List Before Distributed Metal
...
7. Only then start distributed Metal, using the single-device reports as baseline loss/throughput references.
