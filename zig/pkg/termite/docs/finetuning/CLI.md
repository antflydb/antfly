...
Termite fine-tuning should have one durable user-facing CLI and one typed
programmatic workflow API. The build graph should build and run those entry
points, not define the product surface.

The target shape is:

- `termite finetune ...` is the public interface.
...
```sh
termite finetune dataset inspect <family> ...
termite finetune dataset generate <generator> ...
termite finetune dataset prepare <family> ...

termite finetune adapter bootstrap <family> ...
termite finetune adapter inspect <family> ...
termite finetune adapter materialize <family> ...
termite finetune adapter compose ...

termite finetune train <family-or-task> ...
termite finetune eval <family-or-task> ...

termite finetune workflow <workflow-name> ...
```

The unified dispatcher is now the only `termite finetune` entry from the
top-level CLI. It routes recipe execution through `termite finetune run` and
`termite finetune smoke-fast`, routes hierarchical commands through
...
termite finetune prepare-colqwen2-inputs /models/colqwen2 /data /tmp/examples.jsonl /tmp/prepared.json
```
...
termite finetune train gemma4-lora /models/gemma4 /tmp/adapter /tmp/prepared.json /tmp/out --trainer autodiff --backend metal
termite finetune adapter materialize gemma4 /models/gemma4 /tmp/out /tmp/merged

termite finetune workflow gemma4-pilot text /models/gemma4 /tmp/pilot-run --count 1000 --backend metal
termite finetune workflow recursive-lora-smoke /models/gemma4 /tmp/recursive-smoke --count 16
termite finetune workflow gliner2-entity-cleanup-smoke /models/gliner2 /tmp/adapter train.jsonl eval.jsonl /tmp/out
```
...
| --- | --- | --- | --- | --- | --- |
| Gemma4 text LoRA | prepare + teacher top-k | bootstrap/inspect | supervised, autodiff, recursive preference paths | LoRA merge + recursive base | native/BLAS CPU smoke, Metal optional |
| Gemma4 multimodal LoRA | multimodal prepare + pilot generation | bootstrap/inspect | autodiff with image/audio embeddings | LoRA merge | native/BLAS CPU smoke, Metal optional |
...
Optional lanes should prove Metal, Metal, PJRT, ONNX, and quantized export where
...
- `termite finetune` reaches the unified dispatcher from `src/main.zig`.
- `termite finetune run` and `termite finetune smoke-fast` keep the recipe
  engine stable.
...
- `zig build finetune -- <args>` runs the same binary and dispatcher as the
  installed CLI.
- `zig build test-finetune` and the root termite test cover the dispatcher,
...
```sh
zig build finetune -- <args passed to termite finetune>
...
or workflow. Those belong under `termite finetune`.
...
The exact fields can follow existing Termite conventions, but the important
properties are:
...
```sh
termite finetune dataset generate ...
```
...
2. Add `src/finetune/cli/root.zig` and wire `termite finetune`.
