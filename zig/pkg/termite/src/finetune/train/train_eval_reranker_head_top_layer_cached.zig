...
fn usage() error{InvalidArguments}!void {
    print(
        \\usage: train-eval-reranker-head-top-layer-cached <model-dir> <train-top-layer-cache-json> <eval-top-layer-cache-json> <out-dir> [--backend auto|cuda|blas|mlx] [--max-examples N] [--epochs N] [--learning-rate LR]
