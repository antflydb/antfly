...
fn usage() error{InvalidArguments}!void {
    print(
        \\usage: train-eval-reranker-lora-surrogate-cached <model-dir> <adapter-dir> <head-dir-or-file> <train-cache-json> <eval-cache-json> <out-dir> [--backend auto|cuda|blas|mlx] [--max-examples N] [--epochs N] [--learning-rate LR] [--layer-name NAME]
