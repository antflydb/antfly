...
pub const APITableTestRuns = struct {
    ...
    api_table_writes_production_regression: *std.Build.Step.Run,
    api_table_writes_production_regression_unit: *std.Build.Step.Run,
    provisioned_query_visibility: *std.Build.Step.Run,
    provisioned_query_visibility_unit: *std.Build.Step.Run,
...
pub const APIFocusedTestModules = struct {
    root: *std.Build.Module,
    backup_restore: *std.Build.Module,
};
...
pub const APIFocusedTestRuns = struct {
    ...
    standalone_backup_restore: APIFocusedTestRun,
};
...
pub const MainCaptureTestRuns = struct {
    hf_tokenizer: *std.Build.Step.Run,
    api_restore_jobs: *std.Build.Step.Run,
    portable_backup: *std.Build.Step.Run,
    common_secrets: *std.Build.Step.Run,
    introducer: *std.Build.Step.Run,
    cmd: *std.Build.Step.Run,
    lite_cmd: *std.Build.Step.Run,
    raft_library: *std.Build.Step.Run,
    raft_runtime: *std.Build.Step.Run,
    raft_restore: *std.Build.Step.Run,
    raft_ready_continuation: *std.Build.Step.Run,
    serverless_manifest: *std.Build.Step.Run,
    api_derived_coverage: *std.Build.Step.Run,
};
...
pub const DBTestFilters = struct {
    ...
    pub const write_path = [_][]const u8{
        ...
        "storage.db.relational_store.test.relational ordered tuple column maintenance rejects missing durable identity",
        "storage.db.relational_store.test.relational ordered tuple index maintenance tracks partial predicate membership",
        "storage.db.relational_store.test.relational ordered tuple index scan orders duplicate tuples by document key",
        "storage.db.relational_store.test.relational ordered tuple index maintenance respects lifecycle states",
        "storage.db.relational_store.test.relational ordered tuple index mutation batches are retry idempotent",
        "storage.db.relational_store.test.relational scalar column index maintenance skips unchanged and tracks predicate membership",
        "storage.db.relational_store.test.relational ordered tuple entries track unique constraint metadata",
        "storage.db.relational_store.test.relational ordered tuple unique maintenance uses catalog-only index metadata",
        "storage.db.relational_store.test.relational ordered tuple unique enforcement uses ready partial metadata",
        "storage.db.relational_store.test.relational ordered tuple unique maintenance respects lifecycle states",
    };
...
pub const APITestFilters = struct {
    pub const public_api_parity = [_][]const u8{
        ...
        "api public SQL endpoint admits document truncate table emptying barrier",
        "api public SQL endpoint rejects unsupported document truncate variants",
    ...
    pub const standalone_backup_restore = [_][]const u8{
        ...
        "db split restore doc identity runtime repair repairs managed chunked dense embeddings once for restored shard",
        "db split restore doc identity incomplete deferred restore import recovers before runtime repair",
        "db split restore restore state uses strict structured content identity markers",
    };
    ...
    pub const logic = [_][]const u8{
        ...
        "public schema catalog roundtrip preserves first-class relational indexes",
        "public schema catalog roundtrip preserves admitted relational access methods",
        ...
        "document SQL lowers ASCII case-fold text predicates as residual-only filters",
        ...
        "sql adapter generated advisory lock AST fails closed at public boundary",
        "sql adapter generated transaction AST lowers to transaction mode plans",
        ...
        "api public SQL endpoint executes SQL reads through typed row plan ingress",
        "api public SQL endpoint executes document SQL reads through typed document plan ingress",
        "api public SQL endpoint executes document joined write parity cases",
        "api public SQL endpoint rejects document joined generated proof gaps",
    ...
    pub const rows = [_][]const u8{
        ...
        "relational rows batch returning projects committed mutation images",
        "relational rows materializes server defaults",
        "relational rows materializes sequence defaults through explicit resolver",
        "relational rows enforce expression checks through shared expression AST",
        "sql adapter lower dml lowers on conflict date_bin expression update",
        "sql adapter lower dml lowers on conflict boolean expression update",
        "sql adapter lower dml lowers on conflict arithmetic update",
        "sql adapter lower dml lowers excluded explicit default values",
        "sql adapter lower dml lowers conflict update explicit default values",
        "sql adapter lower dml lowers insert default values into defaulted row batch",
        "sql adapter lower dml materializes sequence defaults through write options",
        "sql adapter lower dml lowers insert values returning into row batch",
        "sql adapter lower dml lowers explicit default insert values",
        "sql adapter lower dml lowers explicit default update values",
        "sql adapter lower dml evaluates concat returning expression from typed plan",
        ...
        "relational rows join strategy selection is explicit and fail closed for unproven merge",
        "relational rows join sorted input proof requires leading ascending join key order",
        ...
        "routed rows materialization tracker uses shared spill and hard cap policy",
        ...
        "lowered sql set operation RHS multi-join CTEs route incompatible side schemas",
        "catalog apply creates clones and replaces public schema json",
        "catalog apply applies incremental ddl plans to public schema json",
        "catalog apply emits typed row rewrite plan for column rename",
        "catalog apply emits typed row rewrite plan for column drop",
        "catalog apply marks generic row rewrite work explicitly full",
        "catalog jobs schedules typed schema rewrite jobs from applied SQL DDL work",
        "catalog jobs schedules row-plan schema rewrite jobs from applied SQL DDL work",
        "catalog jobs schedules full row schema rewrite jobs from applied SQL DDL work",
        "catalog jobs rejects schema rewrite jobs without typed row operation",
        "catalog jobs detects schema rewrite wakeable applied DDL work",
        "catalog jobs promotes completed schema rewrite through table schema CAS",
        "catalog jobs rejects schema rewrite promotion while durable jobs are incomplete",
        "table manager owns schema rewrite jobs",
        "table manager atomically applies table catalog updates with schema rewrite jobs",
        "table manager atomically applies multi-table catalog batch updates with schema rewrite jobs",
        "table manager applies schema rewrite job lifecycle operations",
        "metadata schema progress transition command round-trips",
        "metadata raft apply store projects schema progress records from committed entries",
        "metadata raft apply store persists secondary index rebuild work ranges across reopen",
        "metadata raft apply store persists table emptying jobs across reopen",
        "table provisioner reports local schema progress once all local shards have the target full-text index",
        "table provisioner schema progress probes do not take a writer lease",
        "table provisioner withholds schema progress when any local shard is missing the target full-text index",
        "metadata reconciler finalizes schema migration once every hosting node reports target schema progress",
        "metadata reconciler keeps schema migration open until every hosting node reports target schema progress",
        ...
        "metadata catalog applies untargeted graph metric SQL DDL through shared derived index path",
        "metadata catalog applies SQL ALTER COLUMN USING through typed schema rewrite jobs",
        "metadata admin source advances foreign key validation state through schema update",
        "sql adapter lower dml lowers truncate into claimed table-emptying mutation source",
        "relational rows mutation source validates restart identity only for table-emptying deletes",
        "table manager owns table emptying jobs",
        "table manager applies table emptying job lifecycle operations",
        "table manager promotes table-emptying barriers atomically",
        "table manager promotes restart identity barrier by resetting sequence allocation markers",
        "table manager validates table-emptying identity allocator reset barriers",
        "table manager rejects mixed table-emptying barrier promotion",
        "table manager resets table-owned sequence defaults for restart identity barriers",
        "catalog jobs builds deterministic table emptying jobs for ranges",
        "catalog jobs schedules table emptying jobs from snapshot ranges",
        "catalog jobs rejects malformed table emptying affected tables before scheduling",
        "catalog jobs admits session qualified table emptying barrier with cascade",
        "catalog jobs table emptying barrier waits for every affected table range",
        "catalog jobs table emptying barrier rejects invalid affected range job",
        "catalog jobs repairs table emptying barrier jobs after range topology changes",
        ...
        "catalog jobs promotes one completed table emptying barrier per metadata mutation",
        "catalog jobs promotes completed table emptying barrier by table id",
        "table emptying worker helper claims deletes rows and finishes job",
        "table emptying worker live lease blocks competing worker without deleting rows",
        "table emptying worker stale lease lets next worker delete rows and finish job",
        "table emptying worker leaves job running when concurrent row claim skips rows",
        "table emptying worker invalidates malformed schema after claiming without deleting rows",
        ...
        "relational rows query contract projects cast expressions",
        ...
        "sql adapter lower expr lowers row claim query plans",
        "relational rows aggregate contract accepts typed expression inputs and filters",
        "relational rows aggregate contract accepts boolean folds",
        "sql adapter lower expr lowers cast projections",
        "sql adapter lower dml lowers partial unique conflict target predicates",
        "sql adapter lower dml lowers partial unique point selectors",
        ...
        "relational rows insert source conflict guards validate grouped expressions at storage boundary",
        "relational rows api query contract parses typed row claim request",
        "relational rows mutation source contract parses claimed update plans",
        "relational rows mutation source encoder round trips supported claimed requests",
        "hosted mutation-source remote autocommit dispatches supported stage requests",
        "sql adapter lower dml lowers claimed update mutation source",
        "sql adapter lower dml lowers claimed delete mutation source",
        "relational rows query row claim skip locked returns claimed subset",
        "relational rows query row claim skip locked limit fills from later candidates",
        "relational rows query across ranges merges ordered windows globally",
        "db transactions row claims block transactional and direct mutations until resolution",
        "db transactions row claims prevent double claim until resolution",
        "db transactions row claim search skip locked returns claimed subset",
        ...
        "secondary index rebuild worker invalidates stale range before rebuilding index",
        ...
        "foreign key action page fails closed for transitional ref owner topology",
        "foreign key action page routes update cascade child mutations with replacement parent key",
        "foreign key action page transaction id is stable for durable page retry",
        "distributed txn relational identity workload mixes owner topology churn and actions",
        "foreign key integrity worker plan includes active owner ranges",
        "foreign key local schema controller repairs unvalidated constraint",
        "foreign key local action job controller resumes durable action job",
        "foreign key local action schedule controller seeds durable action schedule",
        "db relational foreign keys enforce parent existence and restrict deletes",
        "db relational foreign keys set nullable children null on parent delete",
        "db relational foreign keys cascade deletes through local children",
        "db relational integrity transaction foreign key parent checks validate final prepared state",
        "db relational integrity transaction foreign key cascade child actions delete exact children",
        ...
        "provisioned table read source composes local system-time as-of with cte plans",
        "api.table_reads.hosted remote system-time as-of routes through rows source group local",
        ...
        "relational rows mutation source skip locked across ranges fills from later candidates",
        ...
        "hosted mutation source local owner calls reject stale topology epoch",
        "hosted mutation source retries remote collect after topology change",
        "hosted mutation source stages global planned candidates across remote owners",
        "hosted joined mutation source retries remote collect before stage only",
        "hosted joined mutation source stages global planned candidates across remote owners",
        "local mutation source staged claims recover after reopen before commit",
        "db transactions row claim lease expiry aborts stale owner and lets next claimer proceed",
        "db transactions row claim survives reopen and expires without mixed state",
        "db transactions recovery resolves committed mutation source row claims after reopen",
        "db transactions row claim lease expiry lets direct mutation reclaim stale owner",
...
pub const RaftTestFilters = struct {
    ...
    pub const ready_continuation = [_][]const u8{
...
pub const StandaloneRuntimeTestFilters = struct {
...
pub fn addStorageBackendTestSteps(
    ...
    const persistent_delete_regression = addFilteredTestRunArtifact(b, b.addTest(.{
        .root_module = modules.persistent,
        .filters = &.{
    ...
    const persistent_delete_regression_step = b.step(
        "persistent-delete-regression-test",
        "Run atomic multi-segment deletion regressions",
    );
    persistent_delete_regression_step.dependOn(&persistent_delete_regression.step);
...
pub fn addAPIFocusedTestSteps(
    b: *std.Build,
    modules: APIFocusedTestModules,
    openapi_root_check_step: *std.Build.Step,
) APIFocusedTestRuns {
    return .{
        .public_api_parity = addAPIFocusedTestRun(b, modules.root, "public-api-parity-test", "Run focused stateful public API parity tests", &APITestFilters.public_api_parity, true, true, openapi_root_check_step),
        .public_api_graph_metric_e2e = addAPIFocusedTestRun(b, modules.root, null, "Run focused public API graph metric e2e tests", &APITestFilters.public_api_graph_metric_e2e, false, false, openapi_root_check_step),
        .resolution_source = addAPIFocusedTestRun(b, modules.root, "lib-resolution-source-test", "Run focused cross-shard resolution candidate-source and entity-sink tests", &APITestFilters.resolution_source, false, false, null),
        .auth = addAPIFocusedTestRun(b, modules.root, "lib-api-auth-test", "Run focused API auth/usermgr HTTP tests", &APITestFilters.auth, true, false, openapi_root_check_step),
        .logic = addAPIFocusedTestRun(b, modules.root, "lib-api-logic-test", "Run focused API table/index encoder, parser, and schema-update logic tests", &APITestFilters.logic, true, false, openapi_root_check_step),
        .docid_lifecycle = addAPIFocusedTestRun(b, modules.root, null, "Run focused DOCID lifecycle and distributed snapshot hardening tests", &APITestFilters.docid_lifecycle, true, false, null),
        .standalone_backup_restore = addAPIFocusedTestRun(b, modules.backup_restore, "lib-api-standalone-backup-restore-test", "Run the focused standalone-like backup/restore e2e test", &APITestFilters.standalone_backup_restore, false, false, null),
    };
}

pub fn makeAPIFocusedTestModules(
    b: *std.Build,
    root: *std.Build.Module,
    target: std.Build.ResolvedTarget,
    optimize: std.builtin.OptimizeMode,
    imports: anytype,
) APIFocusedTestModules {
    const backup_restore = b.createModule(.{
        .root_source_file = b.path("pkg/antfly/src/api_backup_restore_test_root.zig"),
        .target = target,
        .optimize = optimize,
    });
    imports.configure(b, backup_restore, true, true);
    return .{ .root = root, .backup_restore = backup_restore };
}
...
pub fn addAPITableTestSteps(
    ...
    const api_table_writes_production_regression = addSimpleAPITestRun(b, modules.table_writes, &APITestFilters.table_writes, true);
    const api_table_writes_production_regression_unit = addSimpleAPITestRun(b, modules.table_writes, &APITestFilters.table_writes, true);
    ...
    const provisioned_query_visibility_unit = addSimpleAPITestRun(b, modules.table_writes, &APITestFilters.provisioned_query_visibility, true);
    ...
    addFocusedAPITestStep(b, "api-table-writes-production-regression-test", "Run focused restore and writer-cache lifecycle regressions", api_table_writes_production_regression);
    addFocusedAPITestStep(b, "api-table-writes-restore-repeat-test", "Run the focused shared-owner native restore regression", api_table_writes_production_regression);
    addFocusedAPITestStep(b, "api-table-writes-cache-lifecycle-test", "Run focused writer-cache dirty ownership regressions", api_table_writes_production_regression);
    ...
    return .{
        ...
        .api_table_writes_production_regression = api_table_writes_production_regression,
        .api_table_writes_production_regression_unit = api_table_writes_production_regression_unit,
        .provisioned_query_visibility = provisioned_query_visibility,
        .provisioned_query_visibility_unit = provisioned_query_visibility_unit,
...
pub fn dependOnAPITableUnitTestRuns(
    ...
    step.dependOn(&runs.provisioned_query_visibility_unit.step);
    step.dependOn(&runs.docid.step);
    step.dependOn(&runs.rows.step);
...
pub fn addDBStorageTestSteps(
    ...
    _ = addDBFilteredTestStep(b, root_module, .{
        .name = "db-restore-identity-test",
        .description = "Run the focused run-backed identity restore regression",
...
/// Configure the exact-filter runner while keeping compile-only reachability
/// anchors separate from the runtime filters selected by the caller.
...
pub fn addFilteredTestRunArtifact(
    b: *std.Build,
    tests: *std.Build.Step.Compile,
) *std.Build.Step.Run {
...
const main_derived_coverage_filters = [_][]const u8{
...
fn makeConfiguredTestModule(ctx: anytype, path: []const u8) *std.Build.Module {
    const module = ctx.b.createModule(.{
        .root_source_file = ctx.b.path(path),
        .target = ctx.target,
        .optimize = ctx.optimize,
    });
    ctx.antfly_imports.configure(ctx.b, module, true, true);
    return module;
}

fn addNamedFilteredTest(
    b: *std.Build,
    ...
    const run = addFilteredTestRunArtifact(b, tests);
    b.step(name, description).dependOn(&run.step);
    return run;
}

fn makeCommandTestModule(ctx: anytype, path: []const u8) *std.Build.Module {
    const module = ctx.b.createModule(.{
        .root_source_file = ctx.b.path(path),
        .target = ctx.target,
        .optimize = ctx.optimize,
    });
    module.addImport("antfly-zig", ctx.lib_mod);
    module.addImport("antfly-client", ctx.antfly_client_pkg_mod);
    module.addImport("httpx", ctx.httpx_mod);
    module.addImport("antfly_vellum", ctx.vellum_mod);
    module.addImport("raft_engine", ctx.raft_engine_mod);
    module.addImport("structlog", ctx.structlog_mod);
    module.addImport("antfly_platform", ctx.platform_mod);
    module.addImport("handlebars", ctx.handlebars_mod);
    module.addOptions("build_options", ctx.build_options);
    return module;
}

fn addDenseIndexLifecycleRegressionStep(b: *std.Build, modules: anytype) void {
    const run_dense_index_lifecycle_regression_tests = addFilteredTestRunArtifact(b, b.addTest(.{
    ...
    const run_dense_index_repair_job_tests = addFilteredTestRunArtifact(b, b.addTest(.{
        .root_module = modules.artifact_jobs,
        .filters = &.{
    ...
    const run_dense_index_repair_status_tests = addFilteredTestRunArtifact(b, b.addTest(.{
        .root_module = modules.derived_coverage,
        .filters = &.{"index status exposes compact repair state without internal diagnostics"},
        .test_runner = .{ .path = b.path("pkg/antfly/src/test_runner.zig"), .mode = .simple },
    }));
    const run_dense_index_repair_runtime_tests = addFilteredTestRunArtifact(b, b.addTest(.{
        .root_module = modules.data_runtime,
        .filters = &.{
    ...
    step.dependOn(&run_dense_index_lifecycle_regression_tests.step);
    step.dependOn(&run_dense_index_repair_job_tests.step);
    step.dependOn(&run_dense_index_repair_status_tests.step);
    step.dependOn(&run_dense_index_repair_runtime_tests.step);
}

pub fn addMainCaptureTestSteps(ctx: anytype) MainCaptureTestRuns {
    const b = ctx.b;
    ...
    hf_tokenizer_tests.root_module.addImport("sentencepiece_proto", ctx.sentencepiece_proto_mod);
    const run_hf_tokenizer_tests = b.addRunArtifact(hf_tokenizer_tests);
    b.step("hf-tokenizer-test", "Run Hugging Face tokenizer tests").dependOn(&run_hf_tokenizer_tests.step);

    const run_api_restore_jobs_tests = addNamedFilteredTest(
        b,
        makeConfiguredTestModule(ctx, "pkg/antfly/src/api_restore_jobs_test_root.zig"),
        &.{
        ...
        "lib-api-restore-jobs-test",
        "Run durable restore job store tests",
    );

    const run_portable_backup_tests = addNamedFilteredTest(
        b,
        makeConfiguredTestModule(ctx, "pkg/antfly/src/portable_backup_test_root.zig"),
        &.{
        ...
        "lib-portable-backup-test",
        "Run bounded portable backup tests",
    );

    const run_common_secrets_tests = addNamedFilteredTest(
        b,
        ctx.lib_test_mod,
        &.{ "file secret store", "remote content runtime" },
        "lib-common-secrets-test",
        "Run common secret and remote-content reload tests",
    );
    const api_cluster_secret_status_tests = b.addTest(.{
        .root_module = makeConfiguredTestModule(ctx, "pkg/antfly/src/api_cluster_test_root.zig"),
    ...
    run_common_secrets_tests.step.dependOn(&run_api_cluster_secret_status_tests.step);

    const run_introducer_tests = addNamedFilteredTest(
        b,
        makeConfiguredTestModule(ctx, "pkg/antfly/src/introducer.zig"),
        selectTestFilters(b, &.{}),
        "introducer-test",
        "Run segment introducer unit tests",
    );

    _ = addNamedFilteredTest(
        b,
        ctx.pdf_mod,
        &.{
        ...
        "lib-pdf-safety-test",
        "Run focused PDF OCR rendering and parser safety tests",
    );

    const run_cmd_tests = addNamedFilteredTest(
        b,
        makeCommandTestModule(ctx, "pkg/antfly/src/cmd_test.zig"),
        &.{ "cmd.lite", "cmd.serverless", "cmd.cli.backup", "cmd.cli.index", "cmd.cli.mod" },
        "cmd-test",
        "Run Antfly command and client CLI tests",
    );
    const run_lite_cmd_tests = addNamedFilteredTest(
        b,
        makeCommandTestModule(ctx, "pkg/antfly/src/lite_cmd_test.zig"),
        &.{ "cmd.lite", "cmd.cli.backup", "cmd.cli.index", "cmd.cli.mod" },
        "lite-cmd-test",
    ...
    b.step("lib-raft-test", "Run standalone raft library tests").dependOn(&run_raft_library_tests.step);
    const raft_runtime_tests = b.addTest(.{
        .root_module = ctx.raft_runtime_test_mod,
        .filters = selectTestFilters(b, &RaftTestFilters.runtime),
        .test_runner = .{
            .path = b.path("pkg/antfly/src/test_runner.zig"),
            .mode = .simple,
        },
    ...
    const run_raft_restore_tests = addNamedFilteredTest(
        b,
        ctx.raft_restore_test_mod,
        &RaftTestFilters.restore,
        "raft-restore-test",
        "Run focused Raft restore authority and restart tests",
    );
    const raft_ready_continuation_tests = b.addTest(.{
        .root_module = ctx.raft_engine_mod,
        .filters = &RaftTestFilters.ready_continuation,
    });
    ...
    const run_serverless_manifest_tests = addNamedFilteredTest(
        b,
        makeConfiguredTestModule(ctx, "pkg/antfly/src/serverless_manifest_test_root.zig"),
        &.{
        ...
        "lib-serverless-manifest-test",
        "Run focused serverless manifest object-store tests",
    );
    ...
    _ = addNamedFilteredTest(
        ...
        "lib-storage-maintenance-test",
        "Run storage maintenance coordinator ownership and concurrency tests",
    );
    ...
    _ = addNamedFilteredTest(
        ...
        "Run focused full-text scorer regression tests",
    );

    _ = addNamedFilteredTest(
        b,
        makeConfiguredTestModule(ctx, "pkg/antfly/src/api_connections_test_root.zig"),
        &.{
        ...
        "lib-api-connections-test",
        "Run connection inventory and probe-admission tests",
    );

    _ = addNamedFilteredTest(
        b,
        makeConfiguredTestModule(ctx, "pkg/antfly/src/api_storage_authority_test_root.zig"),
        &.{
        ...
        "lib-api-storage-authority-test",
        "Run remote backup credential-boundary tests",
    );

    _ = addNamedFilteredTest(
        b,
        makeConfiguredTestModule(ctx, "pkg/antfly/src/api_session_maintenance_test_root.zig"),
        &.{
            "durable session mutations publish only after persistence succeeds",
            ...
            "api http server enforces session adoption timeout when configured",
        ...
        "lib-api-session-maintenance-test",
        "Run durable session concurrency and background-maintenance tests",
    );

    _ = addNamedFilteredTest(
        b,
        makeConfiguredTestModule(ctx, "pkg/antfly/src/root.zig"),
        &.{
            "api http server serves table lookup with version header",
            "api http server enforces session adoption timeout when configured",
            "api http server returns retryable not leader for local public metadata mutation",
        ...
        "lib-api-http-regression-test",
        "Run focused HTTP allocator, session, and runtime-status regressions",
    );

    const api_derived_coverage_test_mod = makeConfiguredTestModule(ctx, "pkg/antfly/src/api_derived_coverage_test_root.zig");
    const run_lib_api_derived_coverage_tests = addNamedFilteredTest(
        b,
        api_derived_coverage_test_mod,
        &main_derived_coverage_filters,
        "lib-api-derived-coverage-test",
        "Run focused public derived coverage tests",
    );
    run_lib_api_derived_coverage_tests.step.dependOn(ctx.openapi_root_check);

    const run_filesystem_capacity_tests = addNamedFilteredTest(
        b,
        ctx.filesystem_capacity_test_mod,
        &.{"filesystem capacity probe reports the test volume"},
        "filesystem-capacity-test",
        "Run the filesystem capacity probe test",
    );
    const run_resource_budget_tests = ctx.resource_budget_run;
    run_resource_budget_tests.step.dependOn(&run_filesystem_capacity_tests.step);

    addDenseIndexLifecycleRegressionStep(b, .{
        .root = ctx.lib_test_mod,
        .artifact_jobs = ctx.api_artifact_reprocess_jobs_test_mod,
        .derived_coverage = api_derived_coverage_test_mod,
        .data_runtime = ctx.data_runtime_test_mod,
    });

    return .{
        .hf_tokenizer = run_hf_tokenizer_tests,
        .api_restore_jobs = run_api_restore_jobs_tests,
        .portable_backup = run_portable_backup_tests,
        .common_secrets = run_common_secrets_tests,
        .introducer = run_introducer_tests,
        .cmd = run_cmd_tests,
        .lite_cmd = run_lite_cmd_tests,
        .raft_library = run_raft_library_tests,
        .raft_runtime = run_raft_runtime_tests,
        .raft_restore = run_raft_restore_tests,
        .raft_ready_continuation = run_raft_ready_continuation_tests,
        .serverless_manifest = run_serverless_manifest_tests,
        .api_derived_coverage = run_lib_api_derived_coverage_tests,
    };
}
