...
pub fn addTests(b: *std.Build, options: AddTestsOptions) AddTestsResult {
    ...
    const lib_unit_default_filters = [_][]const u8{
        ...
        "term iterator enumerates all terms",
        "suffix filter emits every suffix on UTF-8 boundaries",
        "shingle filter can join tokens without a separator",
        "substring analyzer indexes suffixes across token separators",
        "camel_case filter",
