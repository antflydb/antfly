...
fn decodeBoundedAlloc(
    ...
    for (0..adjacency_count) |idx| {
        ...
        const out_edges = try decodeEdgesAlloc(alloc, data, &pos, out_count, neighbor_tables.len, budget, cancellation);
        errdefer {
            for (out_edges) |*edge| edge.deinit(alloc);
            alloc.free(out_edges);
        }
        const in_edges = try decodeEdgesAlloc(alloc, data, &pos, in_count, neighbor_tables.len, budget, cancellation);
        errdefer {
