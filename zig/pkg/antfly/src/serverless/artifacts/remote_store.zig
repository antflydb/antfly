...
pub const RemoteStore = struct {
    ...
    pub fn artifactStore(self: *RemoteStore) artifact_store.ArtifactStore {
        return self.object.artifactStore();
    }
};

test "remote artifact store opens shared fs backend from file uri" {
    var path_buf: [256]u8 = undefined;
    const path = tmpPath(&path_buf, "artifact-remote");
    ...
    var store = remote.artifactStore();
    defer store.deinit();
