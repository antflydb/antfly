...
const QueryBodyContractFields = struct {
    ...
    has_row_claim: bool,
    has_json_filter: bool,
    has_graph_metric: bool,
    has_graph_metric_rerank: bool,
};
...
fn queryBodyContractFields(alloc: std.mem.Allocator, body: []const u8) !QueryBodyContractFields {
    ...
    return .{
        ...
        .has_row_claim = parsed.value.object.get("claim") != null,
        .has_json_filter = parsed.value.object.get("json_filter") != null,
        .has_graph_metric = parsed.value.object.get("graph_metric") != null,
        .has_graph_metric_rerank = parsed.value.object.get("graph_metric_rerank") != null,
    };
}
...
pub fn parseQueryRequestWithDeadline(
    ...
    const contract_body = try queryBodyForGeneratedContractAlloc(alloc, effective_body, .{
        ...
        .strip_row_claim = contract_fields.has_row_claim,
        .strip_json_filter = contract_fields.has_json_filter,
        .strip_graph_metric = contract_fields.has_graph_metric,
        .strip_graph_metric_rerank = contract_fields.has_graph_metric_rerank,
    ...
    req.row_claim = try parseRowClaimRequestAlloc(alloc, effective_body);
    ...
    try mergeJsonFilterQueryJsonAlloc(alloc, effective_body, &normalized_query.filter_query_json);
    try ensureQueryDeadline(execution_deadline_ns);
...
pub const normalizePublicFilterQueryAlloc = normalizePublicStoredFilterQueryAlloc;
...
fn parseGeneratedBleveQueryValue(alloc: std.mem.Allocator, query: query_openapi.Query) anyerror!db_mod.types.TextQuery {
    ...
    return switch (query) {
        ...
        .geo_shape_query => |shape| blk: {
            const relation = try parseGeoShapeRelation(@tagName(shape.geometry.relation));
...
fn queryBodyForGeneratedContractAlloc(
    ...
    if (!options.strip_internal_shard_fields and
        ...
        !options.strip_public_hierarchy_controls and
        !options.strip_query_timeout and
        !options.strip_row_claim and
        !options.strip_json_filter and
        !options.strip_graph_metric and
        !options.strip_graph_metric_rerank)
    {
        return null;
    }
