...
pub fn translateEmbeddingsIndexConfigJsonWithOptions(
    ...
    if (artifact_sources.len > 0) {
        // Explicit enrichment definitions own source generation.
    } else if (artifact_embedding_name != null) {
...
test "managed embedder registers every multi-source embedding artifact name" {
    ...
    var parsed = try std.json.parseFromSlice(std.json.Value, std.testing.allocator,
        \\{"dimension":3,"sources":[{"artifact":"document_dense_v1"},{"artifact":"document_chunk_dense_v1"}],"embedder":{"provider":"antfly","model":"antflydb/clipclap"}}
    , .{});
    defer parsed.deinit();
