        ...
        if (entry.antfly_provider) |local| {
            if (local.embed_dense_parts) |embed_parts| {
                ...
                const vectors = if (local.embed_dense_parts_with_context) |embed_parts_with_context|
                    embed_parts_with_context(local.ptr, alloc, entry.model, parts, context) catch |err| return normalizeLocalEmbeddingError(err)
                else
                    embed_parts(local.ptr, alloc, entry.model, parts) catch |err| return normalizeLocalEmbeddingError(err);
