...
/// Synchronization level for batch operations: - "propose": Wait for Raft proposal acceptance (fastest, default) - "write": Wait for Pebble KV write - "full_text": Wait for full-text index WAL write - "enrichments": Wait for synchronous enrichment generation after the document commit. A terminal producer failure retains the committed document and returns `committed_repair_required`. - "full_index": Wait for all index writes to complete (full-text + enrichments + vector indexes)
