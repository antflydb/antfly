...
/// Options for Voice Activity Detection (VAD) based audio segmentation. Antfly-specific.
...
pub const Config = struct {
    /// URL of the Antfly embedding/chunking service
    api_url: []const u8,
    /// Base directory containing model subdirectories. Antfly auto-discovers models from: - `{models_dir}/embedders/` - Embedding models (ONNX) - `{models_dir}/chunkers/` - Chunking models (ONNX) - `{models_dir}/rerankers/` - Reranking models (ONNX) - `{models_dir}/recognizers/` - Recognition models (ONNX) - `{models_dir}/rewriters/` - Seq2Seq rewriter models (ONNX) Defaults to ~/.antfly/models (set via viper). If not set, only built-in fixed chunking is available.
    ...
    pool_size: ?i64 = null,
    /// Backend priority order for model loading with optional device specifiers. Format: `backend` or `backend:device` where device defaults to `auto`. Antfly tries entries in order and uses the first available backend+device combination that supports the model. **Backends** (depend on build tags): - `go` - Pure Go inference (always available, CPU only, slowest) - `onnx` - ONNX Runtime (requires -tags="onnx,ORT", fastest) - `xla` - GoMLX XLA (requires -tags="xla,XLA", TPU/CUDA/CPU) **Devices**: - `auto` - Auto-detect best available (default) - `cuda` - NVIDIA CUDA GPU - `coreml` - Apple CoreML (macOS only, used by ONNX) - `tpu` - Google TPU (used by XLA) - `cpu` - Force CPU only **Examples**: - `["onnx", "xla", "go"]` - Try backends with auto device detection - `["onnx:cuda", "xla:tpu", "onnx:cpu", "go"]` - Prefer GPU, fall back to CPU - `["onnx:coreml", "go"]` - macOS with CoreML acceleration
    ...
    /// List of model names to preload at startup (Ollama-compatible). These models are loaded immediately when Antfly starts, avoiding first-request latency. Model names should match those in models_dir/embedders/ (e.g., "BAAI/bge-small-en-v1.5"). Only effective when keep_alive is non-zero (lazy loading mode).
...
/// Audio chunking configuration for Antfly, including VAD options.
...
/// Configuration for chunking requests to Antfly API. Combines shared text options with Antfly-specific audio/VAD options.
...
pub const GenerateRequest = struct {
    ...
    top_p: ?f32 = null,
    /// Top-k sampling (Antfly extension, not in OpenAI API)
    top_k: ?i64 = null,
