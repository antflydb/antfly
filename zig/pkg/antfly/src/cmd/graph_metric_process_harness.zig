...
pub fn main(init: std.process.Init) !void {
    ...
    if (argv.len != 2) {
        std.debug.print("usage: graph_metric_process_harness <antfly-executable>\n", .{});
...
fn verifyPageRankServiceTargetedPublishVerifierFailureProcess(
...

    {
        ...
        var write_source = antfly.public_api.BoundTableWriteSource.init("docs", &db);
        var status_source = ProcessHarnessStatusSource{};
        var server = antfly.public_api.ApiHttpServer.init(
            alloc,
            .{},
            status_source.iface(),
            null,
        ...
        defer server.deinit();
        var listener = antfly.raft.transport.StdHttpListener.init(alloc, .{}, server.executor());
