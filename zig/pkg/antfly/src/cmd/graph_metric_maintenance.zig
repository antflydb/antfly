...
const InProcessGraphMetricService = struct {
    ...
    fn request(
        ...
        const path = try std.fmt.allocPrint(
            alloc,
            "/internal/v1/groups/{d}/tables/{s}/graph-metric-maintenance",
            .{ target.group_id, target.table_name },
        );
        defer alloc.free(path);
        var resp = (try antfly.public_api.http_internal_group_write_routes.handle(.{
            .alloc = alloc,
            .shard_ops = null,
            .writes = self.bound.source(),
            .batch_validator = .{ .ptr = self, .validate = validateBatch },
            .txn_validator = .{ .ptr = self, .validate = validateTxn },
        }, .{
            .method = .POST,
            .uri = path,
            .body = body,
        ...
        if (resp.status != 200) return error.InvalidArguments;
        self.calls += 1;
        return .{
            .content_type = if (resp.content_type) |content_type| try alloc.dupe(u8, content_type) else null,
            .body = try alloc.dupe(u8, resp.body),
        };
    }
