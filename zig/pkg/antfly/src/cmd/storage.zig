...
pub fn runFromIterator(init: std.process.Init, iterator: *std.process.Args.Iterator) !void {
    ...
    antfly.migration_files.requireJsonCatalogAuthority(alloc, init.io, path) catch |err| {
        if (err == error.VectorMigrationAuthoritativeCatalogUnsupported)
            std.log.err("offline vector migration cannot update the authoritative standalone metadata store; use the standalone online storage-migration API for eligible tables", .{});
        return err;
    };
