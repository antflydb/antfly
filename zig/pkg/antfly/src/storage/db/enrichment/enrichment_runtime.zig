...
fn processDocumentExtractionAsset(
    ...
    // An image-only or otherwise empty successful extraction has no downstream
    // chunk request that can close coverage, so finalize it explicitly.
    if (desired_chunk_keys.items.len == 0) {
...
fn renderDocumentPdfPagePngAdaptiveAlloc(
    ...
    return session.renderPagePngAdaptiveAlloc(alloc, page_number, config.ocr_render_dpi, config.ocr_max_rendered_pixels, config.ocr_max_rendered_dimension);
}
...
fn completeRuntimeDocumentExtractionGeneratedTextBatchWithAllocator(
    ...
    for (units, 0..) |unit, idx| {
        ...
        if (kind == .ocr and std.mem.eql(u8, route_type, "pdf")) {
            ...
            const rendered_page = renderDocumentPdfPagePngAdaptiveAlloc(
                working_alloc,
                decoder_alloc,
                source_bytes,
                ...
                logRuntimeOcrRenderProfile(runtime, source_fingerprint, unit.page_number, config.ocr_render_dpi, null, null, null, null, render_started_ns, boundaryFailureErrorName(&render_failure, err));
                if (shouldYieldRequestError(runtime, err)) return err;
                try setRuntimeGeneratedUnitFailureStage(alloc, &units[idx], kind, "render");
                try markRuntimeGeneratedUnitTextFailureNamed(alloc, &units[idx], method, kind, boundaryFailureErrorName(&render_failure, err), isRetryableEnrichmentError(err));
                continue;
            };
