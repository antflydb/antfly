...
pub const StagedGeneration = struct {
    ...
    fn publishPreparedWithIo(self: *StagedGeneration, io: std.Io) !PublicationOutcome {
        ...
        if (!pathExists(io, self.staging_path)) return error.GenerationStagingMissing;
        // The exclusive namespace lease excludes a concurrent local prepare.
        // Reject both retiring an obligated root and activating a copied one,
        // before writing publication markers or exchanging either namespace.
        try requireNoPreparedCompletion(self.alloc, io, self.live_path);
        try requireNoPreparedCompletion(self.alloc, io, self.staging_path);
...
test "workload admission completion guard treats dangling marker symlinks as obligations" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    var io_impl = std.Io.Threaded.init(std.heap.page_allocator, .{});
    defer io_impl.deinit();
    const io = io_impl.io();
    const root = try std.fmt.allocPrint(alloc, ".zig-cache/tmp/{s}/guard-symlink", .{tmp.sub_path});
    defer alloc.free(root);
    // An absent generation cannot own a native completion obligation.
