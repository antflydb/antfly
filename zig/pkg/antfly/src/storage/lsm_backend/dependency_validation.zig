...
pub const Validation = struct {
    ...
    /// The absolute cutoff uses the backend's coordination clock.
    pub fn advanceBudgetedLocked(self: *Validation, backend: anytype, credits: usize, deadline: u64) !Result {
        ...
        const advanced = self.step(backend.allocator, credits, .{
            .io = backend.manifestCoordinationIo(),
            .at_ns = @min(deadline, runtime.workNowNs(backend) +| 2 * std.time.ns_per_ms),
        });
    ...
    fn step(self: *Validation, allocator: std.mem.Allocator, credits_arg: usize, deadline: runtime.WorkDeadline) !void {
        ...
        switch (self.phase) {
            ...
            .cleanup => {
                ...
                while (credits != 0 and !deadline.expired()) {
            ...
            .certificate => if (self.changes) |*changes| {
                while (credits != 0 and !deadline.expired() and !changes.done()) {
