...
test "dependency validation budgets identities overlaps and cleanup" {
    const Fixture = struct {
        ...
        fn check(allocator: std.mem.Allocator, directory: *Directory, handles: []const Directory.Handle) !void {
            ...
            try std.testing.expect(!try job.step(allocator, 1, .{ .io = null, .at_ns = 0 }));
            try std.testing.expect(job.indices == null);
            var slices: usize = 0;
            while (!try job.step(allocator, 1, .{ .io = null, .at_ns = std.math.maxInt(u64) })) slices += 1;
            try std.testing.expect(slices > 2);
            try std.testing.expect(job.valid);
            try std.testing.expect(!job.covered);
            try std.testing.expect(try job.step(allocator, 0, .{ .io = null, .at_ns = 0 }));
            while (true) {
                var credits: usize = 1;
    ...
    while (!try stale.step(allocator, 1, .{ .io = null, .at_ns = std.math.maxInt(u64) })) {}
...
test "dependency certificate rebases newer writes and rejects changed inputs or older dependencies" {
    ...
    for (0..5) |variant| {
        ...
        defer job.deinit(allocator);
        while (!try job.step(allocator, 1, .{ .io = null, .at_ns = std.math.maxInt(u64) })) {}
...
test "dependency certificate delta scaling benchmark" {
    ...
    for ([_]usize{ 1000, 10000, 100000 }) |count| {
        ...
        while (true) {
            ...
            const done = try job.step(allocator, 2048, .{ .io = null, .at_ns = before +| 2 * std.time.ns_per_ms });
...
pub const Job = struct {
    ...
    pub fn step(self: *Job, allocator: std.mem.Allocator, credits_arg: usize, deadline: @import("runtime.zig").WorkDeadline) !bool {
