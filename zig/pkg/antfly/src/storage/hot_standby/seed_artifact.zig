...
test "storage.hot_standby portable production seed schema requires exact capture receipt authority" {
...
test "storage.hot_standby seed artifact rejects incomplete stale and cross-cluster generations" {
    const alloc = std.testing.allocator;
    var memory = object_storage.MemoryObjectStorage.init(alloc);
    defer memory.deinit();
    var client = memory.client();
