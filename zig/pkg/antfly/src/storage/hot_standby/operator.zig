...
test "storage.hot_standby operator plans slots and standby bootstrap" {
...
test "storage.hot_standby operator plans seed finish and bootstrap with manifest paths" {
    const alloc = std.testing.allocator;
    const slots = [_]status.SlotSnapshot{};
    const primary = status.PrimarySnapshot{
        .identity = .{ .cluster_id = 100, .shard_id = 10, .table_id = 20, .timeline_id = 1, .epoch = 1 },
...
test "storage.hot_standby operator plans explicit slot drop for removed standby" {
    const alloc = std.testing.allocator;
    const slots = [_]status.SlotSnapshot{
        .{
            .name = "standby-a",
...
test "storage.hot_standby operator percent-encodes slot admin paths" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby operator reports retention pressure degraded sync and reseed" {
    const alloc = std.testing.allocator;
    const slots = [_]status.SlotSnapshot{
        .{
            .name = "standby-a",
...
test "storage.hot_standby operator plans reseed finish and bootstrap with manifest paths" {
    const alloc = std.testing.allocator;
    const slots = [_]status.SlotSnapshot{
        .{
            .name = "standby-a",
...
test "storage.hot_standby operator gates automatic promotion on fencing and caught up standby" {
    const alloc = std.testing.allocator;
    const slots = [_]status.SlotSnapshot{
        .{
            .name = "standby-a",
...
test "storage.hot_standby operator renders versioned json plan for controllers" {
    const alloc = std.testing.allocator;
    const slots = [_]status.SlotSnapshot{
        .{
            .name = "standby-a",
...
test "storage.hot_standby operator renders executable admin commands for slot and seed actions" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby operator renders executable admin command for fenced promotion" {
    const alloc = std.testing.allocator;
    const identity = primary_mod.Identity{ .cluster_id = 100, .shard_id = 10, .table_id = 20, .timeline_id = 1, .epoch = 1 };
...
test "storage.hot_standby operator omits fence generation flag when fencing authority is not kubernetes lease" {
    const alloc = std.testing.allocator;
    const identity = primary_mod.Identity{ .cluster_id = 100, .shard_id = 10, .table_id = 20, .timeline_id = 1, .epoch = 1 };
...
test "storage.hot_standby operator requires explicit fence generation for kubernetes lease authority" {
    const alloc = std.testing.allocator;
    const identity = primary_mod.Identity{ .cluster_id = 100, .shard_id = 10, .table_id = 20, .timeline_id = 1, .epoch = 1 };
...
test "storage.hot_standby operator plans former primary demotion without fence" {
    const alloc = std.testing.allocator;
    const slots = [_]status.SlotSnapshot{};
    const primary = status.PrimarySnapshot{
...
test "storage.hot_standby operator plans former primary rewind or reseed from fence receipt" {
    const alloc = std.testing.allocator;
    const slots = [_]status.SlotSnapshot{};
...
test "storage.hot_standby operator targets former primary rejoin admin APIs deliberately" {
    const standbys = [_]StandbySpec{
