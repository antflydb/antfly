...
test "storage.hot_standby http admin executes typed former primary log rewind when configured" {
...
test "storage.hot_standby http admin rejects typed former primary rewind on node without local log" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin does not persist rejoin assess receipts" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin assesses the local former primary durable tail" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin rejects unbound rejoin receipts before persisting" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin marks former primary slot for typed reseed" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby reseed serialization survives a concurrent admitted status update and replay" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin rejects typed former primary reseed on node without primary context" {
    const alloc = std.testing.allocator;
    var server = Server.init(alloc, .{});
    defer server.deinit();
...
test "storage.hot_standby http admin serves health and command endpoint" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin accepts legacy /ha/v1 health, ready, and command paths as aliases" {
    const alloc = std.testing.allocator;
    var server = Server.init(alloc, .{});
    defer server.deinit();
...
test "storage.hot_standby http admin accepts legacy /admin/v1/ha typed routes as aliases for /admin/v1/standby" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby admin typed operation bypasses legacy request dispatch" {
...
test "storage.hot_standby http admin holds state lock through mutation hook" {
...
test "storage.hot_standby http admin sheds requests while state transition is busy" {
    const alloc = std.testing.allocator;
    var mutex: std.atomic.Mutex = .unlocked;
...
test "storage.hot_standby http admin reports unsafe promotion as conflict" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin rejects invalid rejoin fence receipt" {
    const alloc = std.testing.allocator;
    var server = Server.init(alloc, .{});
    defer server.deinit();
...
test "storage.hot_standby http admin rejects invalid rejoin node identifiers" {
    const alloc = std.testing.allocator;
    var server = Server.init(alloc, .{});
    defer server.deinit();
...
test "storage.hot_standby http admin enforces optional bearer token on typed admin routes" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin bearer authorization requires exact token" {
...
test "storage.hot_standby watchdog proof remains available outside HA state mutex" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin empty configured bearer token fails closed" {
    const alloc = std.testing.allocator;
    for ([_][]const u8{ "", " \t\r\n " }) |configured_token| {
...
test "storage.hot_standby http admin required bearer token fails closed when unconfigured" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin trims configured bearer token before comparing" {
    const alloc = std.testing.allocator;
    var server = Server.initWithOptions(alloc, .{}, .{ .bearer_token = " secret-token\n" });
    defer server.deinit();
...
test "storage.hot_standby http admin rejects invalid fence request identity and bounds" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin allocates fence generation when omitted from typed request" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin accepts whole instance identity" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin promotes from operation-specific fence request body" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin serves typed base backup seed endpoints" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin validates typed seed manifest paths before file access" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin returns route method and command errors" {
    const alloc = std.testing.allocator;
    var server = Server.init(alloc, .{});
    defer server.deinit();
...
test "storage.hot_standby http admin requires node id before typed action receipts" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin rejects invalid primary node id before typed action receipts" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin rejects invalid fenced primary node id in typed write check" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin rejects invalid standby node id in typed status" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin applies typed standby upstream swap" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin reports unchanged standby upstream as idempotent" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin rejects standby upstream identity mismatch as conflict" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin rejects standby upstream without a wired hook" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin rejects standby upstream when node is not a standby" {
    const alloc = std.testing.allocator;
    var server = Server.init(alloc, .{});
    defer server.deinit();
...
test "storage.hot_standby http admin rejects malformed standby upstream requests" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin returns method errors for generated admin routes" {
    const alloc = std.testing.allocator;
    var server = Server.init(alloc, .{});
    defer server.deinit();
...
test "storage.hot_standby http admin handles every generated admin route method" {
    const alloc = std.testing.allocator;
    var server = Server.init(alloc, .{});
    defer server.deinit();
...
test "storage.hot_standby http admin implemented admin routes are documented" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin accepts absolute URIs" {
    const alloc = std.testing.allocator;
    var server = Server.init(alloc, .{});
    defer server.deinit();
...
test "storage.hot_standby http admin decodes sync policy query values" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http admin decodes OpenAPI ALL sync policy" {
...
test "storage.hot_standby http admin preserves omitted commit append shard and table defaults" {
