...
test "storage.hot_standby http client accepts zero LSN promotion assessment" {
...
test "storage.hot_standby http client round trips admin commands" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http client falls back to legacy admin paths and remembers the style" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http client round trips typed commit operations" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http client round trips typed gate operations" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http client round trips typed seed operations" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http client round trips typed standby upstream operations" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http client round trips typed safety operations" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http client accepts authoritative former primary tail in rejoin assessment" {
    const alloc = std.testing.allocator;
    var executor = StaticJsonExecutor{
        .body =
...
test "storage.hot_standby http client rejects mismatched rejoin admin responses" {
    const alloc = std.testing.allocator;
    {
        var executor = StaticJsonExecutor{
            .body =
...
test "storage.hot_standby http client rejects mismatched promotion admin responses" {
    const alloc = std.testing.allocator;
    var executor = StaticJsonExecutor{
        .body =
...
test "storage.hot_standby http client accepts empty fence receipt reason" {
    const alloc = std.testing.allocator;
    var executor = StaticJsonExecutor{
        .body =
...
test "storage.hot_standby http client validates live upgraded fence boundary and topology binding" {
...
test "storage.hot_standby http client accepts already applied idempotent admin receipts" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http client rejects invalid local admin inputs" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http client rejects invalid typed admin responses" {
    const alloc = std.testing.allocator;
...
test "storage.hot_standby http client maps admin errors" {
    const alloc = std.testing.allocator;
    var server = http_admin.Server.init(alloc, .{});
    defer server.deinit();
    var client = Client.init(alloc, server.executor());
