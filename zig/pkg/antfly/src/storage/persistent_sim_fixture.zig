...
pub const Options = struct {
    ...
    expected_doc_count: ?u32 = null,
    expected_segment_count: ?usize = null,
    expected_alpha_hits: ?usize = null,
    expected_beta_hits: ?usize = null,
    expected_gamma_hits: ?usize = null,
...
pub fn parseFixture(allocator: std.mem.Allocator, contents: []const u8) !ReplayFixture {
    ...
    const fixture_mode: Mode = if (std.mem.eql(u8, mode, "persistent"))
        .replay
    else if (std.mem.eql(u8, mode, "persistent_crash"))
        .crash
    else
    ...
    fixture.opts.main_no_sync = try parseOptionalBool(&raw_fixture, "main_no_sync", false);
    fixture.opts.main_no_meta_sync = try parseOptionalBool(&raw_fixture, "main_no_meta_sync", false);
    fixture.opts.wal_no_sync = try parseOptionalBool(&raw_fixture, "wal_no_sync", false);
    fixture.opts.main_commit_backend = try parseOptionalCommitBackend(&raw_fixture, "main_commit_backend", .sync);
    fixture.opts.wal_commit_backend = try parseOptionalCommitBackend(&raw_fixture, "wal_commit_backend", .adaptive);
    fixture.opts.expected_doc_count = try sim_fixture.parseOptionalUnsignedExtraField(u32, &raw_fixture, "expected_doc_count");
    fixture.opts.expected_segment_count = try sim_fixture.parseOptionalUnsignedExtraField(usize, &raw_fixture, "expected_segment_count");
    fixture.opts.expected_alpha_hits = try sim_fixture.parseOptionalUnsignedExtraField(usize, &raw_fixture, "expected_alpha_hits");
    fixture.opts.expected_beta_hits = try sim_fixture.parseOptionalUnsignedExtraField(usize, &raw_fixture, "expected_beta_hits");
    fixture.opts.expected_gamma_hits = try sim_fixture.parseOptionalUnsignedExtraField(usize, &raw_fixture, "expected_gamma_hits");
...
pub fn renderReplayArtifact(
    ...
    var fixture = try initArtifactFixture(allocator, "persistent", opts, case_label, seed, expectation_note);
...
pub fn renderCrashArtifact(
    ...
    var fixture = try initArtifactFixture(allocator, "persistent_crash", opts, case_label, seed, expectation_note);
    defer fixture.deinit(allocator);
...
pub fn parseAction(line: []const u8) !Action {
    ...
    if (std.mem.eql(u8, fields[0], "index_segment")) {
        if (fields.len != 2) return error.InvalidFixture;
        return .{ .index_segment = std.meta.stringToEnum(SegmentSpec, fields[1]) orelse return error.InvalidFixture };
    }
    return error.InvalidFixture;
}
...
pub fn renderAction(allocator: std.mem.Allocator, action: Action) ![]u8 {
    return switch (action) {
        ...
        .index_segment => |spec| std.fmt.allocPrint(allocator, "index_segment {s}", .{@tagName(spec)}),
    };
}
...
fn initArtifactFixture(
    ...
    try appendBoolField(allocator, &fixture, "main_no_sync", opts.main_no_sync, false);
    try appendBoolField(allocator, &fixture, "main_no_meta_sync", opts.main_no_meta_sync, false);
    try appendBoolField(allocator, &fixture, "wal_no_sync", opts.wal_no_sync, false);
    try appendCommitBackendField(allocator, &fixture, "main_commit_backend", opts.main_commit_backend, .sync);
    try appendCommitBackendField(allocator, &fixture, "wal_commit_backend", opts.wal_commit_backend, .adaptive);
    try sim_fixture.appendOptionalUnsignedExtraField(allocator, &fixture, "expected_doc_count", opts.expected_doc_count);
    try sim_fixture.appendOptionalUnsignedExtraField(allocator, &fixture, "expected_segment_count", opts.expected_segment_count);
    try sim_fixture.appendOptionalUnsignedExtraField(allocator, &fixture, "expected_alpha_hits", opts.expected_alpha_hits);
    try sim_fixture.appendOptionalUnsignedExtraField(allocator, &fixture, "expected_beta_hits", opts.expected_beta_hits);
    try sim_fixture.appendOptionalUnsignedExtraField(allocator, &fixture, "expected_gamma_hits", opts.expected_gamma_hits);
...
fn appendBoolField(
    ...
    try fixture.extra_fields.append(allocator, .{
        ...
        .value = try allocator.dupe(u8, if (value) "true" else "false"),
    });
}

fn appendCommitBackendField(
    allocator: std.mem.Allocator,
    fixture: *sim_fixture.Fixture,
    key: []const u8,
    value: CommitBackend,
    default_value: CommitBackend,
) !void {
    if (value == default_value) return;
    try fixture.extra_fields.append(allocator, .{
        .key = try allocator.dupe(u8, key),
        .value = try allocator.dupe(u8, @tagName(value)),
    });
}

fn parseOptionalBool(fixture: *const sim_fixture.Fixture, key: []const u8, default_value: bool) !bool {
    const value = sim_fixture.extraFieldValue(fixture, key) orelse return default_value;
    if (std.mem.eql(u8, value, "true")) return true;
    if (std.mem.eql(u8, value, "false")) return false;
    return error.InvalidFixture;
}

fn parseOptionalCommitBackend(fixture: *const sim_fixture.Fixture, key: []const u8, default_value: CommitBackend) !CommitBackend {
    const value = sim_fixture.extraFieldValue(fixture, key) orelse return default_value;
    return std.meta.stringToEnum(CommitBackend, value) orelse error.InvalidFixture;
}
