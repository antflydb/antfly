...
extern fn runtime_callback_test_routed_write() *const anyopaque;

test "callback archive boundary preserves optional write results and routing failures" {
    ...
    const write: @FieldType(VTable, "routed_write") = @ptrCast(@alignCast(runtime_callback_test_routed_write()));
    try std.testing.expect((try boundary.call("routed_write", foreign, write, .{@as(u8, 0)})) != null);
    try std.testing.expect((try boundary.call("routed_write", foreign, write, .{@as(u8, 1)})) == null);
    ...
    for (failures, 2..) |failure, mode| {
        try std.testing.expectError(failure, boundary.call("routed_write", foreign, write, .{@as(u8, @intCast(mode))}));
        try std.testing.expectEqualStrings(@errorName(failure), @errorName(error_abi.errorFromStatus(error_abi.statusFromError(failure))));
