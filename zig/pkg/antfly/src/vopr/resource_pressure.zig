...
pub const Scenario = struct {
    ...
    const State = struct {
        ...
        fn readerHolder(self: *@This()) void {
            ...
            var memory = self.resources.reserve(.dense_search_working_set, 50) catch |err| {
                self.fail(err);
                return;
            };
            ...
            while (!self.release_holders) self.condition.waitUncancelable(io, &self.mutex);
        ...
        fn writerHolder(self: *@This()) void {
            ...
            defer request.release();
            var memory = self.resources.reserve(.lsm_wal_write_working_set, 50) catch |err| {
                self.fail(err);
                return;
            };
            defer memory.release();
        ...
        fn denseWithContext(
            ...
            defer request.release();
            var memory = self.resources.reserve(.inference_scratch_working_set, 40) catch |err| {
                return err;
            };
            defer memory.release();
            self.mutex.lockUncancelable(io);
            ...
            self.condition.broadcast(io);
            self.condition.wait(io, &self.mutex) catch |err| switch (err) {
                error.Canceled => {
        ...
        fn exerciseDb(self: *@This()) !void {
            ...
            var db = try db_mod.DB.open(self.allocator, "resource-db", options);
            defer db.close();
            try db.batch(.{
                .writes = &.{.{ .key = "doc:pressure", .value = "{\"title\":\"shared-envelope\"}" }},
                .sync_level = .write,
            });
            var result = (try db.lookup(self.allocator, "doc:pressure", .{})) orelse
        ...
        fn backgroundRun(raw: *anyopaque) anyerror!void {
            ...
            var reservation = self.resources.reserve(.lsm_compaction_work, 40) catch |err| switch (err) {
        ...
        fn yieldUntil(self: *@This(), predicate: *const fn (*State) bool) !void {
            while (!predicate(self)) try std.Io.sleep(self.runtimeIo(), .zero, .awake);
        }
        ...
        fn verifyPressure(self: *@This(), cache: *lake.PersistentObjectRangeCache) !void {
            const io = self.runtimeIo();
            const memory = self.resources.memoryStats();
            if (memory.used_bytes != 100 or memory.used_bytes > memory_hard)
            ...
            if (self.resources.reserve(.inference_scratch_working_set, 40)) |unexpected| {
            ...
            if (io.concurrent(noOp, .{})) |unexpected| {
                ...
                error.ConcurrencyUnavailable => self.task_denied = true,
                else => return err,
            }
            ...
            var first_probe = try std.Io.Dir.cwd().createFile(io, "descriptor-probe-a", .{});
            defer first_probe.close(io);
            if (std.Io.Dir.cwd().createFile(io, "descriptor-probe-b", .{})) |unexpected| {
                var file = unexpected;
                file.close(io);
                return error.FileQuotaUnexpectedlyAdmitted;
            } else |err| switch (err) {
                error.ProcessFdQuotaExceeded => self.file_denied = true,
                else => return err,
            }

            try self.submitBackground();
            try self.yieldUntil(firstBackgroundDone);
        ...
        fn verifyRecovery(self: *@This(), cache: *lake.PersistentObjectRangeCache) !void {
            const io = self.runtimeIo();
            if (self.resources.memoryStats().used_bytes != 0) return error.MemoryDidNotRecover;
            ...
            var provider_memory = try self.resources.reserve(.inference_scratch_working_set, 40);
            provider_memory.release();
            self.progress_after_pressure += 1;

            try self.submitBackground();
            try self.yieldUntil(secondBackgroundDone);
            ...
            try self.yieldUntil(providerReady);
            provider.cancel(io);
        ...
        fn runRootInner(self: *@This()) !void {
            ...
            try self.yieldUntil(holdersReady);
            try self.verifyPressure(&cache);
            self.releasePressure();
            reader.await(io);
            reader_live = false;
            writer.await(io);
            writer_live = false;
            try self.verifyRecovery(&cache);
        ...
        fn ownershipClean(self: *const @This()) bool {
            if (!self.complete) return true;
            const runtime = self.sim.resourceSnapshot();
            return self.resources.memoryStats().used_bytes == 0 and
    ...
    fn resourceOptions(allocator: std.mem.Allocator) resource_manager.Options {
        var options = resource_manager.Options{
            ...
            .memory_budget = .{ .soft_limit_bytes = 90, .hard_limit_bytes = memory_hard },
        ...
        }) |slice| options.budgets[@intFromEnum(slice)] = .{ .soft_limit_bytes = 48, .hard_limit_bytes = 64 };
        return options;
    }
    ...
    pub fn init(allocator: std.mem.Allocator) !World {
        ...
        var sim = try vopr.vopr_io.VoprIo.init(.{
            ...
            .tasks = .{ .max_tasks = 4 },
            .files = .{ .max_open_handles = 2, .capacity_bytes = 1024 * 1024 },
        ...
        state.* = .{
            .allocator = allocator,
            .sim = sim,
            .resources = resource_manager.ResourceManager.init(resourceOptions(allocator)),
        ...
        errdefer state.resources.deinit(allocator);
        state.durable_jobs = vopr_durable_job_lane.Lane.init(allocator, state.sim.executor());
    ...
    pub fn deinit(world: *World, allocator: std.mem.Allocator) void {
        ...
        state.resources.deinit(allocator);
        state.sim.deinit();
    ...
    pub fn observe(world: *World, builder: *vopr.observation.Builder, allocator: std.mem.Allocator) !void {
        ...
        try builder.addNamed(allocator, name ++ ".memory-used", @intCast(state.resources.memoryStats().used_bytes));
    ...
    pub fn evaluate(world: *World, sink: *vopr.property.Sink, allocator: std.mem.Allocator) !void {
        const state = world.state;
        const runtime = state.sim.resourceSnapshot();
        const memory = state.resources.memoryStats();
        try sink.check(allocator, bounded_id, state.sound and memory.used_bytes <= memory_hard and
            runtime.open_file_handles <= 2 and runtime.open_sockets <= 3 and runtime.active_tasks <= 4);
...
test "cross-service resource pressure VOPR exact replays overload cancellation cleanup and recovery" {
    ...
    for (0..8) |ordinal| {
        ...
        var artifact = try vopr.runner.run(Scenario, std.testing.allocator, seeded.source(), .{
            .system = "antfly",
