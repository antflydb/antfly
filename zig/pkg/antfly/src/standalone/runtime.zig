...
const LocalStandaloneMetadata = struct {
    ...
    const PersistedCatalog = struct {
        epoch: u64 = 1,
        databases: []const antfly.metadata.DatabaseRecord = &.{},
        namespaces: []const antfly.metadata.NamespaceRecord = &.{},
        tablespaces: []const antfly.metadata.TablespaceRecord = &.{},
        tables: []const antfly.metadata.TableRecord = &.{},
        ranges: []const antfly.metadata.RangeRecord = &.{},
        sequences: []const antfly.metadata.SequenceRecord = &.{},
        foreign_key_ref_ranges: []const antfly.metadata.ForeignKeyReferenceRangeRecord = &.{},
        unique_constraint_ranges: []const antfly.metadata.UniqueConstraintRangeRecord = &.{},
        secondary_index_rebuild_ranges: []const antfly.metadata.SecondaryIndexRebuildRangeRecord = &.{},
        schema_rewrite_jobs: []const antfly.metadata.SchemaRewriteJobRecord = &.{},
        table_emptying_jobs: []const antfly.metadata.TableEmptyingJobRecord = &.{},
    ...
    fn beginCatalogMutationLocked(self: *LocalStandaloneMetadata) !CatalogMutation {
        ...
        defer self.manager.freeDatabases(self.alloc, databases);
        const namespaces = try self.manager.listNamespaces(self.alloc);
        defer self.manager.freeNamespaces(self.alloc, namespaces);
        const tablespaces = try self.manager.listTablespaces(self.alloc);
        defer self.manager.freeTablespaces(self.alloc, tablespaces);
        ...
        const foreign_key_ref_ranges = try self.manager.listForeignKeyReferenceRanges(self.alloc);
        defer self.manager.freeForeignKeyReferenceRanges(self.alloc, foreign_key_ref_ranges);
        const unique_constraint_ranges = try self.manager.listUniqueConstraintRanges(self.alloc);
        defer self.manager.freeUniqueConstraintRanges(self.alloc, unique_constraint_ranges);
        const secondary_index_rebuild_ranges = try self.manager.listSecondaryIndexRebuildRanges(self.alloc);
        defer self.manager.freeSecondaryIndexRebuildRanges(self.alloc, secondary_index_rebuild_ranges);
        const schema_rewrite_jobs = try self.manager.listSchemaRewriteJobs(self.alloc);
        defer self.manager.freeSchemaRewriteJobs(self.alloc, schema_rewrite_jobs);
        const table_emptying_jobs = try self.manager.listTableEmptyingJobs(self.alloc);
        defer self.manager.freeTableEmptyingJobs(self.alloc, table_emptying_jobs);

        for (databases) |record| try manager.upsertDatabase(record);
        for (namespaces) |record| try manager.upsertNamespace(record);
        for (tablespaces) |record| try manager.upsertTablespace(record);
        for (sequences) |record| try manager.upsertSequence(record);
        _ = try manager.replaceProjectedTopologyWithDerivedWork(
            tables,
            ranges,
            foreign_key_ref_ranges,
            unique_constraint_ranges,
            secondary_index_rebuild_ranges,
            schema_rewrite_jobs,
            table_emptying_jobs,
        );
        return .{
    ...
    fn catalogSource(self: *LocalStandaloneMetadata) catalog_source.CatalogSource {
        return .{
            .ptr = self,
            .vtable = &.{
    ...
    fn catalogAdminSnapshotLocked(self: *LocalStandaloneMetadata) !antfly.metadata_api.AdminSnapshot {
        const tables = try self.manager.listTables(self.alloc);
    ...
    pub fn adminSnapshot(self: *LocalStandaloneMetadata) !antfly.metadata_api.AdminSnapshot {
        lockAtomic(&self.mutex);
        defer self.mutex.unlock();
        return try self.catalogAdminSnapshotLocked();
    }

    pub fn freeAdminSnapshot(self: *LocalStandaloneMetadata, snapshot: *antfly.metadata_api.AdminSnapshot) void {
        catalogFreeAdminSnapshot(self, snapshot);
    }

    pub fn upsertTable(self: *LocalStandaloneMetadata, record: antfly.metadata.TableRecord) !void {
        lockAtomic(&self.mutex);
        defer self.mutex.unlock();
    ...
    pub fn upsertSequence(self: *LocalStandaloneMetadata, record: antfly.metadata.SequenceRecord) !void {
        lockAtomic(&self.mutex);
        defer self.mutex.unlock();
        var mutation = try self.beginCatalogMutationLocked();
        defer mutation.deinit(self);
    ...
    pub fn removeSequence(self: *LocalStandaloneMetadata, sequence_id: u64) !void {
        lockAtomic(&self.mutex);
        defer self.mutex.unlock();
        var mutation = try self.beginCatalogMutationLocked();
        defer mutation.deinit(self);
    ...
    pub fn promoteTableEmptyingBarrier(self: *LocalStandaloneMetadata, request: antfly.metadata.TableEmptyingBarrierPromotionRequest) !void {
        lockAtomic(&self.mutex);
        defer self.mutex.unlock();
        var mutation = try self.beginCatalogMutationLocked();
        defer mutation.deinit(self);
    ...
    pub fn resetIdentityAllocatorsForTableEmptyingBarrier(self: *LocalStandaloneMetadata, request: antfly.metadata.table_manager.TableEmptyingIdentityAllocatorResetRequest) !void {
        lockAtomic(&self.mutex);
        defer self.mutex.unlock();
        var mutation = try self.beginCatalogMutationLocked();
        defer mutation.deinit(self);
    ...
    pub fn upsertTableEmptyingJob(self: *LocalStandaloneMetadata, record: antfly.metadata.TableEmptyingJobRecord) !void {
        lockAtomic(&self.mutex);
        defer self.mutex.unlock();
        var mutation = try self.beginCatalogMutationLocked();
        defer mutation.deinit(self);
    ...
    pub fn upsertRange(self: *LocalStandaloneMetadata, record: antfly.metadata.RangeRecord) !void {
        lockAtomic(&self.mutex);
        defer self.mutex.unlock();
        var mutation = try self.beginCatalogMutationLocked();
        defer mutation.deinit(self);
    ...
    pub fn removeRange(self: *LocalStandaloneMetadata, group_id: u64) !void {
        lockAtomic(&self.mutex);
        defer self.mutex.unlock();
        var mutation = try self.beginCatalogMutationLocked();
        defer mutation.deinit(self);
    ...
    pub fn upsertSchemaRewriteJob(self: *LocalStandaloneMetadata, record: antfly.metadata.SchemaRewriteJobRecord) !void {
        lockAtomic(&self.mutex);
        defer self.mutex.unlock();
        var mutation = try self.beginCatalogMutationLocked();
        defer mutation.deinit(self);
    ...
    pub fn listProjectedTables(self: *LocalStandaloneMetadata, allocator: std.mem.Allocator) ![]antfly.metadata.TableRecord {
        lockAtomic(&self.mutex);
        defer self.mutex.unlock();
        return try self.manager.listTables(allocator);
    }

    pub fn freeProjectedTables(self: *LocalStandaloneMetadata, allocator: std.mem.Allocator, records: []antfly.metadata.TableRecord) void {
        self.manager.freeTables(allocator, records);
    }

    pub fn listProjectedRanges(self: *LocalStandaloneMetadata, allocator: std.mem.Allocator) ![]antfly.metadata.RangeRecord {
        lockAtomic(&self.mutex);
        defer self.mutex.unlock();
        return try self.manager.listRanges(allocator);
    }

    pub fn freeProjectedRanges(self: *LocalStandaloneMetadata, allocator: std.mem.Allocator, records: []antfly.metadata.RangeRecord) void {
        self.manager.freeRanges(allocator, records);
    }

    pub fn listProjectedStores(self: *LocalStandaloneMetadata, allocator: std.mem.Allocator) ![]antfly.metadata.StoreRecord {
        lockAtomic(&self.mutex);
        defer self.mutex.unlock();
    ...
    pub fn freeProjectedStores(_: *LocalStandaloneMetadata, allocator: std.mem.Allocator, records: []antfly.metadata.StoreRecord) void {
        for (records) |record| antfly.metadata.table_manager.freeStore(allocator, record);
        allocator.free(records);
    }

    pub fn listProjectedPlacementIntents(self: *LocalStandaloneMetadata, allocator: std.mem.Allocator) ![]antfly.raft.PlacementIntent {
        lockAtomic(&self.mutex);
        defer self.mutex.unlock();
    ...
    pub fn freeProjectedPlacementIntents(_: *LocalStandaloneMetadata, allocator: std.mem.Allocator, intents: []antfly.raft.PlacementIntent) void {
        allocator.free(intents);
    }

    pub fn listProjectedSchemaRewriteJobs(self: *LocalStandaloneMetadata, allocator: std.mem.Allocator) ![]antfly.metadata.SchemaRewriteJobRecord {
        lockAtomic(&self.mutex);
        defer self.mutex.unlock();
        return try self.manager.listSchemaRewriteJobs(allocator);
    }

    pub fn freeProjectedSchemaRewriteJobs(self: *LocalStandaloneMetadata, allocator: std.mem.Allocator, records: []antfly.metadata.SchemaRewriteJobRecord) void {
        self.manager.freeSchemaRewriteJobs(allocator, records);
    }

    pub fn listProjectedSplitTransitions(_: *LocalStandaloneMetadata, allocator: std.mem.Allocator) ![]antfly.metadata.SplitTransitionRecord {
        return try allocator.alloc(antfly.metadata.SplitTransitionRecord, 0);
    }

    pub fn freeProjectedSplitTransitions(_: *LocalStandaloneMetadata, allocator: std.mem.Allocator, records: []antfly.metadata.SplitTransitionRecord) void {
        allocator.free(records);
    }

    pub fn listProjectedMergeTransitions(_: *LocalStandaloneMetadata, allocator: std.mem.Allocator) ![]antfly.metadata.MergeTransitionRecord {
        return try allocator.alloc(antfly.metadata.MergeTransitionRecord, 0);
    }

    pub fn freeProjectedMergeTransitions(_: *LocalStandaloneMetadata, allocator: std.mem.Allocator, records: []antfly.metadata.MergeTransitionRecord) void {
        allocator.free(records);
    }

    pub fn observeSplitTransition(_: *LocalStandaloneMetadata, _: u64) !?antfly.metadata.SplitObservation {
        return null;
    }

    pub fn observeMergeTransition(_: *LocalStandaloneMetadata, _: u64) !?antfly.metadata.MergeObservation {
        return null;
    }

    pub fn applyReconciliationPlan(self: *LocalStandaloneMetadata, plan: *const antfly.metadata.ReconciliationPlan) !void {
        if (plan.split_upserts.len != 0 or
            ...
            plan.clear_reallocation_request != null)
        {
        ...
        const changed = plan.table_upserts.len != 0 or
            plan.range_upserts.len != 0 or
            plan.table_removals.len != 0 or
            plan.range_removals.len != 0;
        if (!changed) return;
        var mutation = try self.beginCatalogMutationLocked();
        defer mutation.deinit(self);
    ...
    pub fn proposeTransitionCommand(_: *LocalStandaloneMetadata, _: anytype) !void {}
    ...
    fn createTable(ptr: *anyopaque, alloc: std.mem.Allocator, table_name: []const u8, req: catalog_table_ddl.CreateTableRequest) !void {
        ...
        const table = try deriveStandaloneTableRecord(self.storage_engine, table_name, normalized_req);
    ...
    fn adoptEmbeddedLiteRootIfNeeded(self: *LocalStandaloneMetadata, backend: *antfly.lite.backend.Handle) !void {
        ...
        const ranges = try catalog_table_ddl.deriveInitialRanges(self.alloc, table);
        defer {
    ...
    fn loadPersistedCatalog(self: *LocalStandaloneMetadata) !void {
        ...
        for (parsed.value.databases) |record| try self.manager.upsertDatabase(record);
        for (parsed.value.namespaces) |record| try self.manager.upsertNamespace(record);
        for (parsed.value.tablespaces) |record| try self.manager.upsertTablespace(record);
        _ = try self.manager.replaceProjectedTopologyWithDerivedWork(
            parsed.value.tables,
            parsed.value.ranges,
            parsed.value.foreign_key_ref_ranges,
            parsed.value.unique_constraint_ranges,
            parsed.value.secondary_index_rebuild_ranges,
            parsed.value.schema_rewrite_jobs,
            parsed.value.table_emptying_jobs,
        );
    ...
    fn persistLocked(self: *LocalStandaloneMetadata) !void {
        ...
        const encoded = try std.json.Stringify.valueAlloc(self.alloc, PersistedCatalog{
            ...
            .schema_rewrite_jobs = schema_rewrite_jobs,
            .table_emptying_jobs = table_emptying_jobs,
            .extension_packages = extension_packages,
            .installed_extensions = installed_extensions,
            .extension_members = extension_members,
            .extension_dependencies = extension_dependencies,
...
fn deriveStandaloneTableRecord(
    ...
    var table = catalog_table_ddl.deriveTableRecord(table_name, req);
...
pub fn runFromIterator(
    ...
    try public_api_surface.startPgwireOptional(.{
        ...
        .auth_enabled = auth_enabled,
        .auth_error_message = "standalone pgwire listener does not support auth yet; disable --auth or omit --pgwire-port",
    });
    // ---------------------------------------------------------------
...
fn resolveMetadataRaftListener(
    ...
    local_node_id: u64,
    cfg: ?*const antfly.common.config.Config,
) metadata_runtime.ListenerConfig {
    if (cfg) |loaded| {
        if (metadata_runtime.metadataClusterPeerUrl(loaded, local_node_id)) |url| {
            return metadata_runtime.parseHostPort(url) catch .{ .bind_host = cli.bind_host orelse "127.0.0.1", .bind_port = 0 };
        }
    }
    return metadata_runtime.resolveListener(cli.bind_host, null, cfg);
}
...
fn resolveMetadataApiListener(
    ...
    if (cfg) |loaded| {
        if (metadata_runtime.metadataOrchestrationPeerUrl(loaded, local_node_id)) |url| {
            return metadata_runtime.parseHostPort(url) catch .{ .bind_host = fallback_host, .bind_port = 0 };
        }
    }
...
fn resolveMetadataClusterPeers(
    alloc: std.mem.Allocator,
    cfg: ?*const antfly.common.config.Config,
) ![]metadata_runtime.MetadataClusterPeer {
    if (cfg) |loaded| return try metadata_runtime.metadataClusterPeersFromConfig(alloc, loaded);
    return &.{};
}

fn resolvePublicListener(cli: CliConfig) metadata_runtime.ListenerConfig {
    return .{
...
test "standalone runtime registers public SQL route under db api prefix once" {
...
test "standalone Lite adoption preserves deterministic embedded document identity" {
    ...
    const default_range = catalog_table_ddl.deriveInitialRange(default_table);
...
test "standalone local metadata drop table cascade removes child foreign keys" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    ...
    var metadata = try LocalStandaloneMetadata.init(
        alloc,
        1,
        1,
        "http://127.0.0.1:8080",
...

    {
        ...
        var applied = try LocalStandaloneMetadata.applyRelationalSqlDdlPlanWithSession(&metadata, alloc, &durable_plan, catalog_resources.SqlCatalogSession.default());
    ...
    }
    {
        ...
        try std.testing.expectError(
            ...
            LocalStandaloneMetadata.applyRelationalSqlDdlPlanWithSession(&metadata, alloc, &durable_plan, catalog_resources.SqlCatalogSession.default()),
        );
    }
    {
...
test "standalone metadata rolls back an undurable catalog mutation" {
    ...
    try metadata.manager.ensureDefaultCatalog();
    const kept_table = catalog_table_ddl.deriveTableRecord("kept", .{});
    try metadata.manager.upsertTable(kept_table);
    const sequence_id = antfly.metadata.table_manager.deriveSequenceId(
        antfly.metadata.table_manager.default_database_name,
        antfly.metadata.table_manager.default_namespace_name,
        "kept_seq",
    );
    try metadata.manager.upsertSequence(.{
...

    {
        ...
        const table = catalog_table_ddl.deriveTableRecord("docs", .{});
    ...
    try std.testing.expect(metadata.findTableByNameLocked("kept") != null);
    const sequences = try metadata.manager.listSequences(alloc);
    defer metadata.manager.freeSequences(alloc, sequences);
    try std.testing.expectEqual(@as(usize, 1), sequences.len);
