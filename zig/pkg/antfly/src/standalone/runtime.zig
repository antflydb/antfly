...
const ha_wal = @import("../storage/wal.zig");
...
const LocalStandaloneMetadata = struct {
    ...
    fn applyHACatalogCreate(ptr: *anyopaque, record: antfly.ha.replication_record.RecordView) !void {
        const self: *LocalStandaloneMetadata = @ptrCast(@alignCast(ptr));
    ...
    fn replayHACatalog(self: *LocalStandaloneMetadata, primary: *antfly.ha.primary.Primary) !void {
    ...
    fn replayHACatalogEntry(self: *LocalStandaloneMetadata, entry: ha_wal.WalEntry) !ha_wal.WAL.ScanAction {
        const record = try antfly.ha.replication_record.decode(entry.data);
