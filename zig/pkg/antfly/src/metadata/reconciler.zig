...
pub const Reconciler = struct {
    ...
    pub fn computePlan(
        ...
        plan.foreign_key_ref_range_upserts = try foreign_key_ref_range_upserts.toOwnedSlice(self.alloc);
        plan.unique_constraint_range_upserts = try unique_constraint_range_upserts.toOwnedSlice(self.alloc);
        plan.secondary_index_rebuild_range_upserts = try secondary_index_rebuild_range_upserts.toOwnedSlice(self.alloc);
        ...
        plan.foreign_key_ref_range_removals = try foreign_key_ref_range_removals.toOwnedSlice(self.alloc);
        plan.unique_constraint_range_removals = try unique_constraint_range_removals.toOwnedSlice(self.alloc);
        plan.secondary_index_rebuild_range_removals = try secondary_index_rebuild_range_removals.toOwnedSlice(self.alloc);
    ...
    fn init(
        ...
        try self.occupied_group_ids.ensureTotalCapacity(
            ...
            @intCast(sorted_ranges.len +
                current.foreign_key_ref_ranges.len +
                current.unique_constraint_ranges.len +
                current.secondary_index_rebuild_ranges.len +
        ...
        for (current.foreign_key_ref_ranges) |record| {
            try self.occupied_group_ids.put(alloc, record.group_id, {});
        }
        for (current.unique_constraint_ranges) |record| {
            try self.occupied_group_ids.put(alloc, record.group_id, {});
        }
...
test "metadata reconciler automatic planning reserves auxiliary owner groups" {
    const foreign_key_ref_ranges = [_]table_manager.ForeignKeyReferenceRangeRecord{.{
        .group_id = 101,
        .child_table_id = 1,
    ...
    const unique_constraint_ranges = [_]table_manager.UniqueConstraintRangeRecord{.{
        .group_id = 102,
        .table_id = 1,
        .constraint_name = "child_email_key",
        .start_encoded_value = "",
    }};
    const secondary_index_rebuild_ranges = [_]table_manager.SecondaryIndexRebuildRangeRecord{.{
        .group_id = 103,
        .table_id = 1,
        .index_name = "child_status_idx",
        .index_generation = 1,
        .start_row_key = "",
    }};
    var index = try AutomaticPlanningIndex.init(std.testing.allocator, .{
    ...
    try std.testing.expect(index.groupExists(101));
    try std.testing.expect(index.groupExists(102));
    try std.testing.expect(index.groupExists(103));
}
...
fn schemaMigrationReady(
    ...
    if (hosting_node_ids.items.len == 0) {
        for (current.schema_progresses) |record| {
            if (record.table_id != table_id or record.schema_version != target_version) continue;
            if (containsU64(hosting_node_ids.items, record.node_id)) continue;
            try hosting_node_ids.append(alloc, record.node_id);
        }
    }
...
test "metadata reconciler finalizes schema migration from schema progress when placement intents are absent" {
    var manager = table_manager.TableManager.init(std.testing.allocator);
    defer manager.deinit();
    ...
    const progress = [_]table_manager.SchemaProgressRecord{
        .{ .table_id = 32, .node_id = 7, .schema_version = 1 },
    };
