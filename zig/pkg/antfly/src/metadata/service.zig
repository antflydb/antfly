...
fn runtimeStatusProtocolVersionReady(service: anytype, required_version: u16) bool {
    ...
    if (comptime @hasDecl(Service, "runtimeStatusProtocolReady"))
        return service.runtimeStatusProtocolReady(required_version);
    // Narrow adapter for topology-only test/service implementations that
    // predate versioned negotiation. The legacy capability represented the
    // same current repair format; unknown future versions still fail closed.
    if (required_version == metadata_runtime_status_protocol.repair_status_record_version) {
        if (comptime @hasDecl(Service, "runtimeStatusRepairProtocolReady"))
...
fn SharedStoreReports(comptime T: type, comptime free: anytype) type {
    return struct {
        ...
        refs: std.atomic.Value(usize) = .init(1),
        items: []T,
        leaves: std.AutoHashMapUnmanaged(u64, *Leaf) = .empty,

        // Payload ownership transfers only at commit, after both components
        // and the enclosing lease have been allocated successfully.
        fn init(alloc: std.mem.Allocator, incoming: []T, prior: ?*@This(), changed: ?[]const u64) !@This() {
            ...
            if (changed) |ids| {
                var removed: std.AutoHashMapUnmanaged(u64, void) = .empty;
                for (ids) |id| try removed.put(a, id, {});
                var it = prior.?.leaves.iterator();
                while (it.next()) |entry| {
                    if (removed.contains(entry.key_ptr.*)) continue;
                    try out.leaves.put(alloc, entry.key_ptr.*, entry.value_ptr.*);
                    _ = entry.value_ptr.*.refs.fetchAdd(1, .monotonic);
                }
            }
            ...
            while (it.next()) |entry| {
                ...
                try out.leaves.put(alloc, entry.key_ptr.*, leaf);
            }
            const ids = try a.alloc(u64, out.leaves.count());
            var keys = out.leaves.keyIterator();
            var count: usize = 0;
            for (ids) |*id| {
                id.* = keys.next().?.*;
                count += out.leaves.get(id.*).?.items.len;
            }
            std.mem.sort(u64, ids, {}, std.sort.asc(u64));
            out.items = try alloc.alloc(T, count);
            var offset: usize = 0;
            for (ids) |id| {
                const items = out.leaves.get(id).?.items;
                @memcpy(out.items[offset..][0..items.len], items);
                offset += items.len;
            }
            return out;
        }
        fn commit(self: *@This(), alloc: std.mem.Allocator, incoming: []T) void {
            ...
            while (it.next()) |leaf| if (!leaf.*.owned) {
                leaf.*.owned = true;
            };
            alloc.free(incoming);
        }
        fn deinit(self: *@This(), alloc: std.mem.Allocator) void {
            var it = self.leaves.valueIterator();
            while (it.next()) |leaf| leaf.*.release(alloc);
...
const StoreProjectionLease = struct {
    ...
    fn createWithChanges(alloc: std.mem.Allocator, record: metadata_table_manager.StoreRecord, prior: ?*StoreProjectionLease, share_groups: bool, share_runtime: bool, changed: ?[]const u64) !*StoreProjectionLease {
        ...
        if (share_runtime) runtime_capabilities = prior.?.runtime_capabilities else {
            var it = runtime.leaves.valueIterator();
            while (it.next()) |leaf| runtime_capabilities.flags.setUnion(leaf.*.capabilities.flags);
        }
...
const CoreProjectionChanges = struct {
    const StoreChange = struct {
        ...
        reports: bool,
        runtime: bool = true,
        group_ids: [32]u64 = undefined,
        group_count: ?usize = 0,
        fn mergeGroups(self: *StoreChange, signal: metadata_storage.raft_apply_store.ProjectionSignal) void {
            if (!signal.store_reports_changed or self.group_count == null) return;
            const ids = signal.store_group_ids orelse {
                self.group_count = null;
                return;
            };
            for (ids) |id| {
                if (std.mem.indexOfScalar(u64, self.group_ids[0..self.group_count.?], id) != null) continue;
                if (self.group_count.? == self.group_ids.len) {
                    self.group_count = null;
                    return;
                }
                self.group_ids[self.group_count.?] = id;
                self.group_count.? += 1;
            }
        }
    };
    ...
    fn mark(self: *@This(), signal: metadata_storage.raft_apply_store.ProjectionSignal) void {
        ...
        self.pending.stores[self.pending.store_count].mergeGroups(signal);
...
const StoreReadView = struct {
    ...
    fn capture(alloc: std.mem.Allocator, snapshot: *const ProjectedCoreSnapshot) !StoreReadView {
        const stores = try alloc.dupe(metadata_table_manager.StoreRecord, snapshot.stores);
...
fn captureLocalProjectionInputs(self: *MetadataHttpService) !LocalProjectionInputs {
    ...
    var stores = try StoreReadView.capture(self.alloc, core);
...
pub const MetadataService = struct {
    ...
    pub fn listProjectedStores(self: *MetadataService, alloc: std.mem.Allocator) ![]metadata_table_manager.StoreRecord {
        const store = self.projectedStore() orelse return error.MissingMetadataStore;
        const records = try store.listStores(alloc, self.metadata_group_id);
...
pub const MetadataHttpService = struct {
    ...
    fn runtimeStatusProtocolReadyVersion(self: *MetadataHttpService) u16 {
        const activated_version = self.runtimeStatusProtocolActivationVersion();
        ...
        const incarnation = (self.metadataIncarnation() catch return activated_version) orelse
            return activated_version;
        const membership_fingerprint = runtimeStatusProtocolMembershipFingerprint(
            raft_status.conf_state,
            self.reallocation_protocol_peers,
    ...
    fn nativeRestoreIdentityProtocolReady(self: *MetadataHttpService) bool {
        ...
        // admitted only after a V15 store transition has durably activated the
    ...
    fn scheduleRuntimeStatusProtocolProbe(
        ...
        const JobContext = struct {
            ...
            fn run(ptr: *anyopaque) !void {
                ...
                const common_version = ctx.service.probeRuntimeStatusProtocol(
                    ctx.membership_fingerprint,
                    ctx.incarnation,
                    ctx.required_version,
                );
                if (common_version) |version| ctx.service.storeRuntimeStatusProtocolFingerprint(
                    ctx.membership_fingerprint,
                    version,
                );
                ...
                // immediately project optional fields to it while the newer
                // capability remains on the bounded re-probe schedule.
    ...
    fn runtimeStatusProtocolFingerprintReadyVersion(
        ...
        return self.runtime_status_protocol_ready_version;
    }
    ...
    fn probeRuntimeStatusProtocol(
        ...
        const final_raft_status = self.raft.host.http_host.host.raftStatus(self.metadata_group_id) orelse return null;
        const final_incarnation = (self.metadataIncarnation() catch return null) orelse return null;
        const final_membership_fingerprint = runtimeStatusProtocolMembershipFingerprint(
            final_raft_status.conf_state,
            self.reallocation_protocol_peers,
            final_incarnation,
        );
        if (!std.mem.eql(u8, &final_incarnation, &incarnation) or
            !std.mem.eql(
                u8,
    ...
    fn buildAdminSnapshot(self: *MetadataHttpService, include_detailed_status: bool) !metadata_api.AdminSnapshot {
        ...
        var store_view: StoreReadView = undefined;
        {
            self.lockRuntime();
            defer self.unlockRuntime();
            self.catalog_projection_reader.lock();
            ...
            const core = try self.projectedCoreSnapshotLocked();
            store_view = try StoreReadView.capture(self.alloc, core);
            errdefer store_view.deinit(self.alloc);
            const store = self.projectedStore() orelse return error.MissingMetadataStore;
    ...
    fn captureTransitionReadinessInputs(self: *MetadataHttpService) !TransitionReadinessInputs {
        ...
        defer self.unlockRuntime();
        const core = try self.projectedCoreSnapshotLocked();
        var view = try StoreReadView.capture(self.alloc, core);
        errdefer view.deinit(self.alloc);
        return .{
            .stores = view.stores,
            .store_view = view,
    ...
    fn refreshProjectedStoreLocked(self: *MetadataHttpService, snapshot: *ProjectedCoreSnapshot, change: CoreProjectionChanges.StoreChange) !void {
        ...
        const changed_ids: ?[]const u64 = if (position != null and change.group_count != null) change.group_ids[0..change.group_count.?] else null;
    ...
    pub fn listProjectedStores(self: *MetadataHttpService, alloc: std.mem.Allocator) ![]metadata_table_manager.StoreRecord {
        ...
        const view = blk: {
            defer self.unlockRuntime();
            const snapshot = try self.projectedCoreSnapshotLocked();
            break :blk try StoreReadView.capture(self.alloc, snapshot);
        };
        var owned = view;
        defer owned.deinit(self.alloc);
        return try self.cloneProjectedStoresWithActivityLocked(alloc, view.stores);
    }
    ...
    ///
    /// Requires a pinned read view or `runtime_mutex` to retain the durable
...
test "metadata service transition commands negotiate runtime status payload versions" {
    ...
    var native_service = FakeService{
        .alloc = std.testing.allocator,
        .ready_version = metadata_runtime_status_protocol.native_restore_identity_record_version,
    };
    var owned_native_store: ?metadata_table_manager.StoreRecord = null;
    defer if (owned_native_store) |record| metadata_table_manager.freeStore(std.testing.allocator, record);
    const native_command = try runtimeStatusProtocolSafeCommand(
        &native_service,
        .{ .upsert_store = current_store },
        &owned_native_store,
    );
    try std.testing.expect(storeHasRuntimeRepairStatus(native_command.upsert_store));
    try std.testing.expect(!storeHasRuntimeEmbeddingActivity(native_command.upsert_store));
    try std.testing.expectEqual(
        metadata_table_manager.native_generation_restore_protocol_version,
        native_command.upsert_store.native_generation_restore_version,
    );
    ...
    defer if (unexpectedly_owned) |record| metadata_table_manager.freeStore(std.testing.allocator, record);
...
test "metadata service projects optional activity without freezing older status" {
    const FakeService = struct {
        ...
        fn upsertStore(self: *@This(), record: metadata_table_manager.StoreRecord) !void {
            self.upserts += 1;
            self.proposed_capacity_bytes = record.capacity_bytes;
            self.proposed_repair_status = storeHasRuntimeRepairStatus(record);
            self.proposed_activity = storeHasRuntimeEmbeddingActivity(record);
            self.proposed_native_restore_version = record.native_generation_restore_version;
        }
    };
    ...
    observed_indexes[0].embedding_activity.embeddings_computed = 13;
    ...
    try std.testing.expectEqual(@as(u64, 200), service.proposed_capacity_bytes);
    try std.testing.expect(service.proposed_repair_status);
    try std.testing.expect(!service.proposed_activity);
    try std.testing.expectEqual(
        metadata_table_manager.native_generation_restore_protocol_version,
...
test "metadata service activity cache is versioned TTL-bound and incarnation scoped" {
    ...
    var observed_runtime = committed_runtime;
    observed_runtime[0].indexes = observed_indexes[0..];
    ...
    cache.overlay(alloc, &current, 101);
    ...
    legacy_report.embedding_activity_protocol_version = 0;
...
fn reportStoreStatusesWithCapabilities(
    ...
    for (candidates, changed) |record, was_changed| {
        ...
        if (runtimeStatusMandatoryRecordVersion(record) > supported_version) continue;
        if (runtimeStatusRequiredRecordVersion(record) <= supported_version) {
            try service.upsertStore(record);
            continue;
        }
    ...
    // accepted. A follower or failed request must not retain an observation
...
test "metadata service sparse leases preserve pinned groups through allocation failure" {
    const Case = struct {
        ...
        fn run(a: std.mem.Allocator) !void {
            ...
            try std.testing.expectEqualStrings("new", next.record.runtime_statuses[1].table_name);
            ...
            try std.testing.expectEqual(@as(usize, 1), removed.record.runtime_statuses.len);
...
test "metadata service sparse invalidations retain IDs and fall back at capacity" {
    var changes: CoreProjectionChanges = .{};
    _ = changes.take();
    ...
    try std.testing.expectEqualSlices(u64, &.{ 1, 2, 3 }, pending.stores[0].group_ids[0..pending.stores[0].group_count.?]);
    var ids: [33]u64 = undefined;
    for (&ids, 0..) |*id, i| id.* = i;
    changes.mark(.{ .kind = .store, .metadata_group_id = 1, .store_id = 20, .store_group_ids = &ids });
    try std.testing.expect(changes.take().stores[0].group_count == null);
}
    ...
    for ([_]usize{ 100, 1000, 10000 }) |count| {
        ...
        for ([_]bool{ false, true }) |sparse| {
            ...
            for (&elapsed) |*sample| {
                ...
                try std.testing.expectEqual(count, next.record.runtime_statuses.len);
                try std.testing.expectEqual(runtimes[0].doc_count, next.record.runtime_statuses[0].doc_count);
                next.release(a);
                sample.* = platform_time.monotonicNs() - start;
                allocations = counter.allocations;
                try std.testing.expectEqual(counter.allocated_bytes, counter.freed_bytes);
