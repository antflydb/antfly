...
pub const MetadataHttpServer = struct {
    ...
    fn handleWithAllocator(self: *MetadataHttpServer, alloc: std.mem.Allocator, req: http_common.HttpRequest) !http_common.HttpResponse {
        _ = self.cfg;
        if (try self.forwardMutationToLeader(alloc, req)) |resp| return resp;
        switch (req.method) {
            .GET => {
            ...
            .POST => {
                ...
                if (routes.Routes.matchInternalSchemaRewriteJobPause(req.uri)) |job_id| {
                    return try self.handleSchemaRewriteJobControl(alloc, req, job_id, .pause);
                }
                if (routes.Routes.matchInternalSchemaRewriteJobResume(req.uri)) |job_id| {
                    return try self.handleSchemaRewriteJobControl(alloc, req, job_id, .@"resume");
                }
                if (routes.Routes.matchInternalSchemaRewriteJobRetry(req.uri)) |job_id| {
                    return try self.handleSchemaRewriteJobControl(alloc, req, job_id, .retry);
                }
                if (routes.Routes.matchInternalSchemaRewriteJobCancel(req.uri)) |job_id| {
                    return try self.handleSchemaRewriteJobControl(alloc, req, job_id, .cancel);
                }
                if (routes.Routes.matchInternalTableEmptyingJobPause(req.uri)) |job_id| {
                    return try self.handleTableEmptyingJobControl(alloc, req, job_id, .pause);
                }
                if (routes.Routes.matchInternalTableEmptyingJobResume(req.uri)) |job_id| {
                    return try self.handleTableEmptyingJobControl(alloc, req, job_id, .@"resume");
                }
                if (routes.Routes.matchInternalTableEmptyingJobRetry(req.uri)) |job_id| {
                    return try self.handleTableEmptyingJobControl(alloc, req, job_id, .retry);
                }
                if (routes.Routes.matchInternalTableEmptyingJobCancel(req.uri)) |job_id| {
                    return try self.handleTableEmptyingJobControl(alloc, req, job_id, .cancel);
                }
                ...
                if (routes.Routes.matchInternalForeignKeyReferenceRangeRebuildBegin(req.uri)) |table| {
                    var fk_req = parseForeignKeyReferenceRangeSelectorRequest(alloc, req.body) catch return try textResponse(alloc, 400, "invalid foreign key reference range request");
                    defer fk_req.deinit(alloc);
                    self.source.beginForeignKeyReferenceRangeRebuild(alloc, table.table_name, fk_req) catch |err| switch (err) {
                        error.TableNotFound, error.RangeNotFound => return try textResponse(alloc, 404, "not found"),
                        error.UnsupportedOperation => return try textResponse(alloc, 405, "unsupported operation"),
                        else => return try textResponse(alloc, 400, "invalid foreign key reference range request"),
                    };
                    return try textResponse(alloc, 202, "accepted");
                }
                if (routes.Routes.matchInternalForeignKeyReferenceRangeRebuildFinish(req.uri)) |table| {
                    var fk_req = parseForeignKeyReferenceRangeSelectorRequest(alloc, req.body) catch return try textResponse(alloc, 400, "invalid foreign key reference range request");
                    defer fk_req.deinit(alloc);
                    self.source.finishForeignKeyReferenceRangeRebuild(alloc, table.table_name, fk_req) catch |err| switch (err) {
                        error.TableNotFound, error.RangeNotFound => return try textResponse(alloc, 404, "not found"),
                        error.UnsupportedOperation => return try textResponse(alloc, 405, "unsupported operation"),
                        else => return try textResponse(alloc, 400, "invalid foreign key reference range request"),
                    };
                ...
                if (routes.Routes.matchInternalForeignKeyReferenceRangeSplitBegin(req.uri)) |table| {
                    var fk_req = parseForeignKeyReferenceRangeSplitRequest(alloc, req.body) catch return try textResponse(alloc, 400, "invalid foreign key reference range request");
                    defer fk_req.deinit(alloc);
                    self.source.beginForeignKeyReferenceRangeSplit(alloc, table.table_name, fk_req) catch |err| switch (err) {
                        error.TableNotFound, error.RangeNotFound => return try textResponse(alloc, 404, "not found"),
                        error.UnsupportedOperation => return try textResponse(alloc, 405, "unsupported operation"),
                        else => return try textResponse(alloc, 400, "invalid foreign key reference range request"),
                    };
                ...
                if (routes.Routes.matchInternalForeignKeyReferenceRangeSplitFinish(req.uri)) |table| {
                    var fk_req = parseForeignKeyReferenceRangeSplitRequest(alloc, req.body) catch return try textResponse(alloc, 400, "invalid foreign key reference range request");
                    defer fk_req.deinit(alloc);
                    self.source.finishForeignKeyReferenceRangeSplit(alloc, table.table_name, fk_req) catch |err| switch (err) {
                        error.TableNotFound, error.RangeNotFound => return try textResponse(alloc, 404, "not found"),
                        error.UnsupportedOperation => return try textResponse(alloc, 405, "unsupported operation"),
                        else => return try textResponse(alloc, 400, "invalid foreign key reference range request"),
                    };
                ...
                if (routes.Routes.matchInternalForeignKeyReferenceRangeMergeBegin(req.uri)) |table| {
                    var fk_req = parseForeignKeyReferenceRangeMergeRequest(alloc, req.body) catch return try textResponse(alloc, 400, "invalid foreign key reference range request");
                    defer fk_req.deinit(alloc);
                    self.source.beginForeignKeyReferenceRangeMerge(alloc, table.table_name, fk_req) catch |err| switch (err) {
                        error.TableNotFound, error.RangeNotFound => return try textResponse(alloc, 404, "not found"),
                        error.UnsupportedOperation => return try textResponse(alloc, 405, "unsupported operation"),
                        else => return try textResponse(alloc, 400, "invalid foreign key reference range request"),
                    };
                ...
                if (routes.Routes.matchInternalForeignKeyReferenceRangeMergeFinish(req.uri)) |table| {
                    var fk_req = parseForeignKeyReferenceRangeMergeRequest(alloc, req.body) catch return try textResponse(alloc, 400, "invalid foreign key reference range request");
                    defer fk_req.deinit(alloc);
                    self.source.finishForeignKeyReferenceRangeMerge(alloc, table.table_name, fk_req) catch |err| switch (err) {
                        error.TableNotFound, error.RangeNotFound => return try textResponse(alloc, 404, "not found"),
                        error.UnsupportedOperation => return try textResponse(alloc, 405, "unsupported operation"),
                        else => return try textResponse(alloc, 400, "invalid foreign key reference range request"),
                    };
    ...
    fn handleSchemaRewriteJobControl(
        ...
        const control = parseSchemaRewriteJobControlRequest(alloc, job_id, req.body) catch {
            return try textResponse(alloc, 400, "invalid schema rewrite job control request");
        };
        defer freeSchemaRewriteJobControlRequest(alloc, control);
        (switch (action) {
        ...
        }) catch |err| return try schemaRewriteJobControlErrorResponse(alloc, err);
        return try textResponse(alloc, 202, "accepted");
    }
    ...
    fn handleTableEmptyingJobControl(
        ...
        const control = parseTableEmptyingJobControlRequest(alloc, job_id, req.body) catch {
            return try textResponse(alloc, 400, "invalid table emptying job control request");
        };
        defer freeTableEmptyingJobControlRequest(alloc, control);
        (switch (action) {
        ...
        }) catch |err| return try tableEmptyingJobControlErrorResponse(alloc, err);
        return try textResponse(alloc, 202, "accepted");
    }

    fn forwardMutationToLeader(self: *MetadataHttpServer, alloc: std.mem.Allocator, req: http_common.HttpRequest) !?http_common.HttpResponse {
        if (req.source_node_id != null) return null;
        switch (req.method) {
            .POST, .PUT, .DELETE => {},
            .GET => return null,
        }
        return try self.source.forwardMetadataRequest(alloc, req);
    }
...
fn parseCreateTableRequest(alloc: std.mem.Allocator, body: []const u8) !catalog_table_ddl.CreateTableRequest {
    return try catalog_table_ddl.parseStoredCreateTableRequest(alloc, body);
}
...
test "metadata http server returns retryable authority response when reconcile lease is not held" {
    const FakeSource = struct {
        ...
        fn createTable(
            ...
            _: std.mem.Allocator,
            table_name: []const u8,
            _: catalog_table_ddl.CreateTableRequest,
        ) !void {
            const self: *@This() = @ptrCast(@alignCast(ptr));
            try std.testing.expectEqualStrings("docs", table_name);
            self.create_calls += 1;
