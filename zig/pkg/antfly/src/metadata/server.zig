...
test "metadata admin distinguishes non-admission from ambiguous authority loss on table mutations" {
    ...
    const FakeSource = struct {
        ...
        fn createTable(_: *anyopaque, _: std.mem.Allocator, _: []const u8, _: tables_api.CreateTableRequest) !void {
            // A pre-admission rejection: the mutation never reached Raft.
            return error.NotLeader;
        }

        fn dropTable(_: *anyopaque, _: std.mem.Allocator, _: []const u8) !void {
            // Authority moved after the mutation may have been admitted.
...
test "metadata public status source routes table ddl through local and follower paths" {
    const raft_engine = @import("raft_engine");
    ...
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    const replica_root = try std.fmt.allocPrint(std.testing.allocator, ".zig-cache/tmp/{s}/routed-ddl-root", .{tmp.sub_path});
    defer std.testing.allocator.free(replica_root);
    const replica_catalog_path = try std.fmt.allocPrint(std.testing.allocator, ".zig-cache/tmp/{s}/routed-ddl-catalog.txt", .{tmp.sub_path});
    defer std.testing.allocator.free(replica_catalog_path);
    const snapshot_root = try std.fmt.allocPrint(std.testing.allocator, ".zig-cache/tmp/{s}/routed-ddl-snapshots", .{tmp.sub_path});
    ...
    // The single voter elected itself, so the production route is local.
    ...
    const follower_replica_root = try std.fmt.allocPrint(std.testing.allocator, ".zig-cache/tmp/{s}/routed-ddl-follower-root", .{tmp.sub_path});
    defer std.testing.allocator.free(follower_replica_root);
    const follower_replica_catalog_path = try std.fmt.allocPrint(std.testing.allocator, ".zig-cache/tmp/{s}/routed-ddl-follower-catalog.txt", .{tmp.sub_path});
    defer std.testing.allocator.free(follower_replica_catalog_path);
    const follower_snapshot_root = try std.fmt.allocPrint(std.testing.allocator, ".zig-cache/tmp/{s}/routed-ddl-follower-snapshots", .{tmp.sub_path});
