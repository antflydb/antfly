...
test "transition command validation rejects metadata group ids in data group fields" {
    ...
    const commands = [_]TransitionCommand{
        ...
        .{ .admit_split_transition = .{ .expected_source_epoch = 0, .record = .{ .transition_id = 1, .attempt_epoch = 1, .source_group_id = metadata_group_id, .destination_group_id = 2, .split_key = "m" } } },
...
test "metadata transition tags remain append-only across the catalog merge" {
    try std.testing.expectEqual(@as(u8, 41), @intFromEnum(TransitionTag.upsert_foreign_key_ref_range));
    try std.testing.expectEqual(@as(u8, 95), @intFromEnum(TransitionTag.save_secondary_index_rebuild_range_progress));
    try std.testing.expectEqual(@as(u8, 96), @intFromEnum(TransitionTag.upsert_restore_job));
    try std.testing.expectEqual(@as(u8, 97), @intFromEnum(TransitionTag.remove_restore_job));
    try std.testing.expectEqual(@as(u8, 98), @intFromEnum(TransitionTag.remove_restore_jobs));
    try std.testing.expectEqual(@as(u8, 99), @intFromEnum(TransitionTag.admit_split_transition));
    try std.testing.expectEqual(@as(u8, 100), @intFromEnum(TransitionTag.initialize_metadata_incarnation));
    try std.testing.expectEqual(@as(u8, 101), @intFromEnum(TransitionTag.complete_restore_range));
    try std.testing.expectEqual(@as(u8, 102), @intFromEnum(TransitionTag.compare_and_replace_table));
    try std.testing.expectEqual(@as(u8, 103), @intFromEnum(TransitionTag.claim_replication_source_cutover));
    try std.testing.expectEqual(@as(u8, 104), @intFromEnum(TransitionTag.complete_replication_source_retirement));
}
...
pub const RaftApplyStore = struct {
    ...
    const metadata_snapshot_projections = [_]MetadataSnapshotProjectionDescriptor{
        ...
        .{ .projection = .database, .key = .{ .prefix = databasePrefixForGroup } },
        .{ .projection = .namespace, .key = .{ .prefix = namespacePrefixForGroup } },
        .{ .projection = .tablespace, .key = .{ .prefix = tablespacePrefixForGroup } },
        .{ .projection = .sequence, .key = .{ .prefix = sequencePrefixForGroup } },
        ...
        .{ .projection = .foreign_key_ref_range, .key = .{ .prefix = foreignKeyReferenceRangePrefixForGroup } },
        .{ .projection = .unique_constraint_range, .key = .{ .prefix = uniqueConstraintRangePrefixForGroup } },
        .{ .projection = .secondary_index_rebuild_range, .key = .{ .prefix = secondaryIndexRebuildRangePrefixForGroup } },
        .{ .projection = .schema_rewrite_job, .key = .{ .prefix = schemaRewriteJobPrefixForGroup } },
        .{ .projection = .table_emptying_job, .key = .{ .prefix = tableEmptyingJobPrefixForGroup } },
    ...
    fn transitionCommandProjectionMask(tag: std.meta.Tag(TransitionCommand)) u32 {
        return switch (tag) {
            ...
            .upsert_database, .remove_database => metadataSnapshotProjectionBit(.database),
            .upsert_namespace, .remove_namespace => metadataSnapshotProjectionBit(.namespace),
            .upsert_tablespace, .remove_tablespace => metadataSnapshotProjectionBit(.tablespace),
            .upsert_sequence, .remove_sequence, .compare_and_swap_sequence => metadataSnapshotProjectionBit(.sequence),
            ...
            => metadataSnapshotProjectionBit(.foreign_key_ref_range),
            ...
            => metadataSnapshotProjectionBit(.unique_constraint_range),
            ...
            => metadataSnapshotProjectionBit(.secondary_index_rebuild_range),
            ...
            => metadataSnapshotProjectionBit(.schema_rewrite_job),
            .apply_table_catalog_update_with_schema_rewrite_jobs,
            .apply_table_catalog_batch_update_with_schema_rewrite_jobs,
            => metadataSnapshotProjectionBit(.table) |
                metadataSnapshotProjectionBit(.schema_rewrite_job),
            .apply_table_catalog_drop_with_schema_rewrite_jobs => metadataSnapshotProjectionBit(.table) |
                metadataSnapshotProjectionBit(.schema_rewrite_job) |
                metadataSnapshotProjectionBit(.range) |
                metadataSnapshotProjectionBit(.sequence),
            ...
            => metadataSnapshotProjectionBit(.table_emptying_job),
            .promote_table_emptying_barrier => metadataSnapshotProjectionBit(.table_emptying_job) |
                metadataSnapshotProjectionBit(.table) |
                metadataSnapshotProjectionBit(.sequence),
            .promote_secondary_index_ready => metadataSnapshotProjectionBit(.table),
            .compare_and_swap_table_schema => metadataSnapshotProjectionBit(.table) |
                metadataSnapshotProjectionBit(.schema_rewrite_job),
    ...
    fn applyTableCatalogBatchUpdateWithSchemaRewriteJobsTxn(
        ...
        for (request.tables) |table| {
            if ((try self.loadTableTransitionFenceTxn(txn, group_id, table.table_id)).active()) return;
        }
        for (request.tables) |table| try self.applyTableUpsertTxn(txn, group_id, table);
    ...
    fn applyTableCatalogDropWithSchemaRewriteJobsTxn(
        ...
        if ((try self.loadTableTransitionFenceTxn(txn, group_id, request.table_id)).active()) return;
        for (request.table_updates) |table| {
            if ((try self.loadTableTransitionFenceTxn(txn, group_id, table.table_id)).active()) return;
        }
        for (request.table_updates) |table| try self.applyTableUpsertTxn(txn, group_id, table);
        for (request.schema_rewrite_jobs) |job| try self.putSchemaRewriteJobTxn(txn, group_id, job);
...
const TransitionTag = enum(u8) {
    ...
    upsert_restore_job = 96,
    remove_restore_job = 97,
    remove_restore_jobs = 98,
    admit_split_transition = 99,
    initialize_metadata_incarnation = 100,
    complete_restore_range = 101,
    compare_and_replace_table = 102,
    claim_replication_source_cutover = 103,
    complete_replication_source_retirement = 104,
};
...
test "metadata.table record decoder round-trips relational index access method metadata" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, body.method_config_json.?, "\"snippet\":true") != null);
}
