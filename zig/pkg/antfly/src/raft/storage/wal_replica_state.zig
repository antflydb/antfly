    ...
    fn decodeIntoStore(self: *WalReplicaState, bytes: []const u8) !void {
        ...
        if (entry_count > 0) {
            if (entry_count > (bytes.len - cursor) / min_encoded_entry_len)
                return error.InvalidReplicaState;
            const entries = try decodeEntriesAlloc(self.alloc, bytes, &cursor, entry_count);
...
test "wal replica state decoder frees partially decoded ownership" {
    ...
    try std.testing.expectError(
        ...
        WalReplicaState.decodeConfState(std.testing.allocator, &bytes, &cursor),
    );
}
