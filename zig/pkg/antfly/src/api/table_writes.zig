...
test "foreign key schema controller maintenance repairs unvalidated local constraint" {
    ...
    var second_summary = (try bound.source().foreignKeyIntegritySchemaControllerMaintenancePass(alloc, .{
        .action = .repair,
        .worker_id = "worker:fk-maintenance",
        .max_tables = 4,
        .max_work_units_per_table = 1,
...
test "foreign key schema controller maintenance reports failed durable action job" {
    ...
    var retry_summary = (try bound.source().foreignKeyIntegritySchemaControllerMaintenancePass(alloc, .{
        .action = .repair,
        .worker_id = "worker:fk-action-controller",
        .max_tables = 4,
        .max_jobs = 4,
