...
pub const TableRuntimeSnapshotCache = struct {
    ...
    pub fn publishGroupAtConsistentBoundary(
        ...
        return self.publishGroupsAtBoundary(token, table_name, null, null, &.{status}, true);
    }
    ...
    fn publishGroupsForStructuralTarget(
        ...
        return self.publishGroupsAtBoundary(token, table_name, target_index_name, transition_token, statuses, false);
    }

    fn publishGroupsAtBoundary(
        self: *@This(),
        sampled_token: PublicationToken,
        table_name: []const u8,
        target_index_name: ?[]const u8,
        transition_token: ?TargetedIndexTransitionToken,
        statuses: []const LocalTableRuntimeStatus,
        owned_consistent: bool,
    ) !PublishResult {
        var token = sampled_token;
        ...
        lockAtomic(&self.mutex);
        {
            ...
            if (owned_consistent) {
                if (state.epoch.root_generation != sampled_token.table_epoch.root_generation)
                    return .stale_table;
                // The operation owner may cross visibility invalidation, never
                // a root replacement. Preserve the sampled target revision so
                // this cannot borrow a newer targeted-index authority.
                token.table_epoch = state.epoch;
                token.observation_generation = self.takeObservationGenerationLocked();
            }
    ...
    fn applyTargetObservationAuthorityLocked(
        ...
        for (status.stats.indexes) |*item| {
            ...
            if (state.required_target_observation_revisions.get(group_id)) |required| {
                if (item.runtime_coverage_source_sequence) |sequence|
            ...
            const coverage_sequence = item.runtime_coverage_source_sequence orelse item.replay_target_sequence;
            const includes_target = coverage_sequence >= index_required.source_target_sequence;
            if (observed_revision >= index_required.event_revision and includes_target) {
                index_required.observed = true;
                index_required.observed_source_target_sequence = @max(index_required.observed_source_target_sequence, coverage_sequence);
            }
            item.runtime_target_observation_complete = item.runtime_target_observation_complete and index_required.observed and includes_target and
                coverage_sequence <= index_required.observed_source_target_sequence;
        }
    }
};
...
fn mergeCachedStatusWithNonAuthoritativePlaceholder(
    ...
    for (merged.stats.indexes) |*dst| {
        ...
        std.mem.swap(db_mod.types.DBIndexStats, dst, &retained.stats.indexes[cached_index]);
        if (targeted) {
...
pub fn cloneDBStats(alloc: std.mem.Allocator, stats: db_mod.types.DBStats) !db_mod.types.DBStats {
    ...
    for (stats.indexes, 0..) |item, i| {
        ...
        indexes[i] = .{
            ...
            .index_repair_trigger = stableStatusName(item.index_repair_trigger, &.{ "none", "corrupt_local_repair_state", "incomplete_bulk_publish", "operator_generation_rebuild", "root_generation_rebuild", "artifact_coverage_mismatch", "artifact_counter_missing", "projection_generation_invalid", "operator_generation_validation", "replay_artifact_unavailable", "storage_format_migration", "catalog_admission" }),
            .index_repair_phase = stableStatusName(item.index_repair_phase, &.{ "none", "detected", "preflight", "building", "catching_up", "ready", "waiting_for_convergence", "activating", "validating", "cleanup", "terminal", "rolling_back" }),
            .index_repair_automation = stableStatusName(item.index_repair_automation, &.{ "none", "enabled", "paused" }),
            ...
            .index_repair_wait_reason = stableStatusName(item.index_repair_wait_reason, &.{ "none", "paused", "terminal", "backoff", "rollback", "convergence" }),
            ...
            .projection_checkpoint_status = stableStatusName(item.projection_checkpoint_status, &.{ "clean", "rebuilding", "degraded", "repair_required" }),
    ...
    return .{
        .columnar_maintenance = stats.columnar_maintenance,
        .schema_epoch = stats.schema_epoch,
        .row_format_version = stats.row_format_version,
        .table_catalog_generation = stats.table_catalog_generation,
        .schema_index_state = stableStatusName(stats.schema_index_state, &.{ "none", "pending", "building", "ready", "failed" }),
...
pub fn observedSourceTargetSequence(stats: db_mod.types.DBStats) u64 {
    var target = @max(
    ...
    const Suite = struct {
        ...
        test "synthetic status merge owns retained index errors after previous snapshot retires" {
            ...
            defer if (merged) |*value| value.deinit(alloc);
            {
                ...
                merged = try mergeCachedStatusWithNonAuthoritativePlaceholder(alloc, previous, placeholder, 1, null, false);
        ...
        test "synthetic relabel cannot reuse cached catch up serviceability" {
            ...
            for ([_]RuntimeStatusFreshness{ .stale, .catching_up }) |freshness| {
                ...
                var merged = try mergeCachedStatusWithNonAuthoritativePlaceholder(std.testing.allocator, previous, placeholder, 100, null, false);
...
// These borrowed fields are a bounded status vocabulary in DBStats. Re-intern
// them at the JSON ownership boundary; cloning pointer values would retain
// slices into a released response arena. Unknown versions stay explicit.
fn stableStatusName(value: []const u8, comptime names: []const []const u8) []const u8 {
    inline for (names) |name| if (std.mem.eql(u8, value, name)) return name;
    return "unknown";
}
fn cloneEnrichmentStats(stats: db_mod.types.EnrichmentStats) db_mod.types.EnrichmentStats {
    var result = stats;
    result.projection_checkpoint_status = stableStatusName(stats.projection_checkpoint_status, &.{ "clean", "rebuilding", "degraded", "repair_required" });
    result.stall_reason = stableStatusName(stats.stall_reason, &.{ "", "worker_missing", "model_loading", "publishing_overdue", "embedding_overdue" });
    result.active_phase = stableStatusName(stats.active_phase, &.{ "idle", "loading_model", "tokenizing", "executing", "serializing", "publishing" });
    return result;
}
