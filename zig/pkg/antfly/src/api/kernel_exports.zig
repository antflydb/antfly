...
pub fn create(context: *const CreateContext) callconv(.c) c_int {
    ...
    const reads: *const ?table_read_source.TableReadSource = @ptrCast(@alignCast(context.table_reads));
    const writes: *const ?table_write_source.TableWriteSource = @ptrCast(@alignCast(context.table_writes));
