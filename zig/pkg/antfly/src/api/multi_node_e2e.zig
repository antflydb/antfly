...
test "public api multi-node e2e routes CRUD from a non-host node" {
    ...
    var created_table = try std.json.parseFromSlice(metadata_openapi.TableStatus, std.heap.page_allocator, created.body, .{ .ignore_unknown_fields = true });
    ...
    try std.testing.expectEqual(.rebuilding, parsed_stable_table_detail.value.migration.?.state);
