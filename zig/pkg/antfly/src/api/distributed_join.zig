...
pub fn rewriteJoinedBaseQueryBodyAlloc(
    ...
    defer out.deinit();
    // Re-admit the rewritten body as a public query. Optional nullable fields
    // that were omitted by the caller must stay omitted: emitting them as JSON
    // null changes public presence semantics (notably for graph_queries).
    try std.json.Stringify.value(base_request, .{ .emit_null_optional_fields = false }, &out.writer);
    return .{
