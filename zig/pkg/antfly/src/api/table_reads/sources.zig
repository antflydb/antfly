...
const graphHydrateSearchRequest = table_read_graph.graphHydrateSearchRequest;
const graphHydrateOnOpenDb = table_read_graph.graphHydrateOnOpenDb;
const graphHydrateOnPreparedDb = table_read_graph.graphHydrateOnPreparedDb;
...
const requiresDistributedGraphCoordinator = table_read_graph.requiresDistributedGraphCoordinator;
const currentIdentityReadGenerationForDb = table_read_document_sql.currentIdentityReadGenerationForDb;
const requireTableIdentityNamespaceForGroup = table_read_cache.requireTableIdentityNamespaceForGroup;

const ProvisionedConsistencyRequest = union(enum) {
    search: db_mod.types.SearchRequest,
    lookup: struct {
        key: []const u8,
...
fn prepareProvisionedGroupConsistency(
    ...
    (switch (request) {
        ...
        .scan => |scan| reads.reads.prepareScanWithConsistency(group_id, scan.from_key, scan.to_key, scan.opts, consistency),
    }) catch |err| switch (err) {
        error.NotLeader => {
            if (!fallback_to_stale_on_not_leader or consistency == .stale) return err;
            switch (request) {
                .search => |req| try reads.reads.prepareSearchWithConsistency(group_id, req, .stale),
                .lookup => |lookup| try reads.reads.prepareLookupWithConsistency(group_id, lookup.key, lookup.opts, .stale),
                .scan => |scan| try reads.reads.prepareScanWithConsistency(group_id, scan.from_key, scan.to_key, scan.opts, .stale),
            }
        },
        else => return err,
    };
...
const ManagedReadRuntimeConfig = table_read_core.ManagedReadRuntimeConfig;
const ResidentDbLease = table_read_core.ResidentDbLease;
const ResidentDbSource = table_read_core.ResidentDbSource;
...
pub const BoundTableReadSource = struct {
    ...
    fn documentArtifactManifestGroupLocal(
        ...
        return try documentArtifactManifest(ptr, alloc, table_name, doc_key, artifact_name, consistency);
    }
    ...
    fn documentArtifactManifestsGroupLocal(
        ...
        return try documentArtifactManifests(ptr, alloc, table_name, doc_key, consistency);
    }
    ...
    fn graphExpandGroupLocal(
        ...
        return try graphExpandWithSearch(Context, alloc, table_name, req, .{ .source = self, .consistency = consistency }, Context.search);
    }
...
const aggregationContextForCapturedResultDb = table_read_document_sql.aggregationContextForCapturedResultDb;
...
const aggregationCanUseCurrentResult = table_read_document_sql.aggregationCanUseCurrentResult;
const aggregationFullResultRequest = table_read_document_sql.aggregationFullResultRequest;
const requireCompleteAggregationFullResult = table_read_document_sql.requireCompleteAggregationFullResult;
const checkQueryDeadline = table_read_document_sql.checkQueryDeadline;
const loadTableAggregationTextAnalysis = table_read_cache.loadTableAggregationTextAnalysis;
...
pub const ProvisionedTableReadSource = struct {
    ...
    const PreparedKeyRead = struct {
        ...
        fn releaseActivity(self: *PreparedKeyRead) void {
    ...
    pub fn source(self: *ProvisionedTableReadSource) TableReadSource {
        return .{
            ...
            .vtable = &.{
                ...
                .document_artifact_manifest_group_local = ProvisionedTableReadSource.documentArtifactManifestGroupLocal,
                .document_artifact_manifests_group_local = ProvisionedTableReadSource.documentArtifactManifestsGroupLocal,
                .preflight_query = preflightQuery,
                .preflight_query_group_local = preflightQueryGroupLocal,
                .lookup_group_local = lookupGroupLocal,
                .relational_unique_owner_lookup = relationalUniqueOwnerLookup,
    ...
    fn prepareResidentGroupsForReadRetry(
        self: *ProvisionedTableReadSource,
        alloc: std.mem.Allocator,
        table_name: []const u8,
        group_ids: []const u64,
    ) !void {
        const resident = self.resident_db orelse return error.ReadUnavailable;
    ...
    fn prepareGroupsForReadAdmission(
        self: *ProvisionedTableReadSource,
        alloc: std.mem.Allocator,
        group_ids: []const u64,
        request: ProvisionedConsistencyRequest,
        ...
        // Never hold read admission while waiting for Raft apply: apply takes
        // exclusive operation admission and would otherwise wait on the read
        ...
        const errors = try alloc.alloc(?anyerror, group_ids.len);
        ...
        var start: usize = 0;
        while (start < group_ids.len) : (start += plan.width) {
    ...
    fn scan(
        ...
        retry: while (attempt < topology_read_attempt_limit) : (attempt += 1) {
            ...
            if (plans[0].topology_epoch != prepared.topology_epoch) return error.TopologyChanged;
            ...
            for (plans) |plan| {
                var group_opts = opts;
                group_opts.inclusive_from = group_opts.inclusive_from or plan.inclusive_start;
                if (opts.limit > 0) {
                    if (emitted >= opts.limit) break;
                    group_opts.limit = opts.limit - emitted;
                }

                var result = (scanProvisionedHostedLocal(self.resident_db, self.cache, self.replica_root_dir, self.catalog, self.requester, alloc, plan.group_id, self.visibleRootGeneration(plan.group_id), self.backend_runtime, table_name, plan.start_key, plan.end_key, group_opts, .stale, true) catch |err| switch (err) {
                    error.ResidentDbRetryRequired => {
                        prepared.releaseActivity();
                        try self.prepareResidentGroupsForReadRetry(alloc, table_name, prepared.group_ids);
    ...
    fn queryAttempt(
        ...
        if (requiresDistributedGraphCoordinator(group_ids.len, req)) {
            ...
            // retain the source table's admission while acquiring a target
            // table: opposing A -> B and B -> A traversals can otherwise form
    ...
    fn graphExpandGroupLocal(
        ...
        try self.ensureHAReadAllowed(consistency);
        var worker_ctx = ProvisionedGraphWorkerContext.init(self);
        return try executeProvisionedGraphExpand(&worker_ctx, alloc, group_id, table_name, req, consistency);
    }
    ...
    fn graphHydrateGroupLocal(
        ...
        try self.ensureHAReadAllowed(consistency);
        var worker_ctx = ProvisionedGraphWorkerContext.init(self);
        return try executeProvisionedGraphHydrate(&worker_ctx, alloc, group_id, table_name, req, consistency);
    }
    ...
    fn graphEdgesGroupLocal(
        ...
        try self.ensureHAReadAllowed(consistency);
        var worker_ctx = ProvisionedGraphWorkerContext.init(self);
        return try executeProvisionedGraphGetEdges(&worker_ctx, alloc, group_id, table_name, req, consistency);
    }
    ...
    fn localRuntimeStatuses(
        ...
        return self.localRuntimeStatusesFromResolvedGroups(alloc, table_name) catch |err| switch (err) {
    ...
    fn localRuntimeStatusesFromResolvedGroups(
        self: *ProvisionedTableReadSource,
        alloc: std.mem.Allocator,
        table_name: []const u8,
    ) !?runtime_status.LocalTableRuntimeStatuses {
        ...
        for (group_ids) |group_id| {
            const path = try metadata_mod.groupDbPathFromReplicaRoot(alloc, self.replica_root_dir, group_id);
            defer alloc.free(path);
            var db = try self.openRuntimeStatusDbForGroup(alloc, path, group_id, table_name);
            defer db.close();
            var stats = try db.runtimeStatusStatsConsistent(alloc);
            errdefer db_mod.types.freeDBStats(alloc, stats);
            try overlayPersistedRebuildStateStatus(alloc, &db, &stats);
            items[initialized] = .{
                .group_id = group_id,
                .stats = stats,
            };
    ...
    fn openRuntimeStatusDbForGroup(
        self: *ProvisionedTableReadSource,
        alloc: std.mem.Allocator,
        path: []const u8,
        group_id: u64,
        ...
        return openProvisionedQueryDbForTableWithRuntime(
            alloc,
            path,
            self.catalog,
            table_name,
            ...
            error.DocIdentityNamespaceMismatch => try openProvisionedQueryDbForTableWithCache(
                alloc,
                path,
                self.catalog,
                table_name,
                if (self.cache) |cache| cache.lsm_cache else null,
                if (self.cache) |cache| cache.hbc_cache else null,
                self.visibleRootGeneration(group_id),
                if (self.cache) |cache| cache.resource_manager else null,
    ...
    fn overlayPersistedRebuildStateStatus(
        alloc: std.mem.Allocator,
        db: *db_mod.DB,
        stats: *db_mod.types.DBStats,
    ) !void {
        for (stats.indexes) |*item| {
            const rebuild_root_path = switch (item.kind) {
                .full_text => if (db.core.index_manager.textIndexEntry(item.name)) |entry| entry.rebuild_root_path else continue,
                .sparse_vector => if (db.core.index_manager.sparseIndex(item.name)) |entry| entry.rebuild_root_path else continue,
                .graph => if (db.core.index_manager.graphIndex(item.name)) |entry| entry.rebuild_root_path else continue,
                .dense_vector, .algebraic => continue,
            };
            const rebuild_state = db_mod.backfill_state.RebuildState.init(rebuild_root_path);
            const resume_key = (try rebuild_state.check(alloc)) orelse continue;
            alloc.free(resume_key);
            item.backfill_active = true;
            if (item.backfill_progress >= 1.0) item.backfill_progress = 0.999;
        }
    }
    ...
    fn observedDynamicFieldCapabilitySets(
        ...
        var read_activity = self.beginPreparedRead(table_name, .general);
        defer if (read_activity) |*activity| activity.deinit();
        return try observedDynamicFieldCapabilitySetsProvisionedAlloc(
            alloc,
            self.catalog,
            self.replica_root_dir,
            self.backend_runtime,
...
pub const HostedProvisionedTableReadSource = struct {
    ...
    pub fn source(self: *HostedProvisionedTableReadSource) TableReadSource {
        return .{
            ...
            .vtable = &.{
                ...
                .document_artifact_manifest_group_local = HostedProvisionedTableReadSource.documentArtifactManifestGroupLocal,
                .document_artifact_manifests_group_local = HostedProvisionedTableReadSource.documentArtifactManifestsGroupLocal,
                .preflight_query = preflightQuery,
                .preflight_query_group_local = preflightQueryGroupLocal,
                .lookup_group_local = lookupGroupLocal,
                .relational_unique_owner_lookup = relationalUniqueOwnerLookup,
    ...
    fn scan(
        ...
        for (plans) |plan| {
            ...
            var result = switch (route) {
                .local => try scanProvisionedHostedLocal(null, null, self.replica_root_dir, self.catalog, self.requester, alloc, plan.group_id, self.visibleRootGeneration(plan.group_id), self.backend_runtime, table_name, plan.start_key, plan.end_key, group_opts, consistency, false),
    ...
    fn observedDynamicFieldCapabilitySets(
        ...
        return try observedDynamicFieldCapabilitySetsForGroupsAlloc(
            alloc,
            self.catalog,
            self.replica_root_dir,
            self.backend_runtime,
    ...
    fn graphExpandGroupLocal(
        ...
        return switch (route) {
            .local => blk: {
                ...
                const Context = struct {
                    ...
                    fn search(ctx: @This(), allocator: std.mem.Allocator, search_req: db_mod.types.SearchRequest) anyerror!db_mod.types.SearchResult {
                        return try queryHostedLocal(
                            ...
                            .{ .backend_runtime = ctx.source.backend_runtime },
                            ctx.table_name,
                            search_req,
                            ctx.consistency,
                        );
                ...
                break :blk try graphExpandWithSearch(Context, alloc, table_name, req, .{
                    .source = self,
                    .group_id = group_id,
                    .table_name = table_name,
...
fn observedDynamicRootGeneration(group_visible_root_generation: ?GroupVisibleRootGenerationSource, group_id: u64) u64 {
    return if (group_visible_root_generation) |generation_source| generation_source.visibleRootGenerationForGroup(group_id) else backend_current_root_generation;
}

fn observedDynamicFieldCapabilitySetsProvisionedAlloc(
    alloc: std.mem.Allocator,
    catalog: table_catalog.CatalogSource,
    replica_root_dir: []const u8,
    backend_runtime: ?*db_mod.background_runtime.BackendRuntime,
    group_visible_root_generation: ?GroupVisibleRootGenerationSource,
    table_name: []const u8,
) !?[]ObservedDynamicFieldCapabilitySet {
    ...
    return try observedDynamicFieldCapabilitySetsForGroupsAlloc(
        ...
        replica_root_dir,
        backend_runtime,
        group_visible_root_generation,
        table_name,
        group_ids,
    );
}

fn observedDynamicFieldCapabilitySetsForGroupsAlloc(
    alloc: std.mem.Allocator,
    catalog: table_catalog.CatalogSource,
    replica_root_dir: []const u8,
    backend_runtime: ?*db_mod.background_runtime.BackendRuntime,
    ...
    for (group_ids) |group_id| {
        ...
        var db = try openProvisionedWarmStatusDbForTable(
            alloc,
            path,
            observedDynamicRootGeneration(group_visible_root_generation, group_id),
            backend_runtime,
            identity_namespace,
        );
        defer db.close();
...
fn mergeObservedFieldCapability(
    ...
    const next_doc_value_coverage = storage_schema.conservativeDocValueCoverage(existing.doc_value_coverage, incoming.doc_value_coverage);
    const next_queryability_state = storage_schema.conservativeQueryabilityState(existing.queryability_state, incoming.queryability_state);
    const next_sort_lifecycle_state = storage_schema.conservativeSortLifecycleState(existing.sort_lifecycle_state, incoming.sort_lifecycle_state);
    const next_missing_null_policy = if (std.mem.eql(u8, existing.missing_null_policy, incoming.missing_null_policy))
        existing.missing_null_policy
    else
        "mixed";

    var doc_value_coverage_clone: ?[]u8 = null;
    errdefer if (doc_value_coverage_clone) |value| alloc.free(value);
    if (!std.mem.eql(u8, existing.doc_value_coverage, next_doc_value_coverage)) {
        doc_value_coverage_clone = try alloc.dupe(u8, next_doc_value_coverage);
    }
    var queryability_state_clone: ?[]u8 = null;
    errdefer if (queryability_state_clone) |value| alloc.free(value);
    if (!std.mem.eql(u8, existing.queryability_state, next_queryability_state)) {
        queryability_state_clone = try alloc.dupe(u8, next_queryability_state);
    }
    var sort_lifecycle_state_clone: ?[]u8 = null;
    errdefer if (sort_lifecycle_state_clone) |value| alloc.free(value);
    if (!std.mem.eql(u8, existing.sort_lifecycle_state, next_sort_lifecycle_state)) {
        sort_lifecycle_state_clone = try alloc.dupe(u8, next_sort_lifecycle_state);
    }
    var missing_null_policy_clone: ?[]u8 = null;
    errdefer if (missing_null_policy_clone) |value| alloc.free(value);
    if (!std.mem.eql(u8, existing.missing_null_policy, next_missing_null_policy)) {
        missing_null_policy_clone = try alloc.dupe(u8, next_missing_null_policy);
    }
    ...
    if (doc_value_coverage_clone) |value| {
        alloc.free(existing.doc_value_coverage);
        existing.doc_value_coverage = value;
        doc_value_coverage_clone = null;
    }
    if (queryability_state_clone) |value| {
        alloc.free(existing.queryability_state);
        existing.queryability_state = value;
        queryability_state_clone = null;
    }
    if (sort_lifecycle_state_clone) |value| {
        alloc.free(existing.sort_lifecycle_state);
        existing.sort_lifecycle_state = value;
        sort_lifecycle_state_clone = null;
    }
    if (missing_null_policy_clone) |value| {
        alloc.free(existing.missing_null_policy);
        existing.missing_null_policy = value;
        missing_null_policy_clone = null;
    }
...
test "table read distributed sorted merge uses catalog runtime schema and rejects incomplete shard windows" {
    ...
    try std.testing.expectEqual(@as(u64, 3), sort_profile.selected_count);
...
test "provisioned auxiliary reads publish resident databases outside read admission" {
    ...
    const ResidentSource = struct {
        ...
        fn iface(self: *@This()) ResidentDbSource {
            return .{
                .ptr = self,
                .lease_group = leaseGroup,
                .prepare_group_for_read_retry = prepareGroupForReadRetry,
            };
        }
        ...
        fn leaseGroup(
            ...
            return .{ .ptr = self, .db = self.db, .release_fn = release };
        }

        fn prepareGroupForReadRetry(
            ptr: *anyopaque,
            _: std.mem.Allocator,
            table_name: []const u8,
            group_id: u64,
            ...
            try std.testing.expectEqual(@as(u64, 7001), group_id);
            try std.testing.expectEqual(@as(u64, 9), lsm_root_generation);
            try std.testing.expectEqual(@as(usize, 0), self.active_reads.*);
            self.preparations += 1;
        }
        ...
        fn failNext(self: *@This()) void {
            self.fail_next = true;
        }
    };

    const ReadTracker = struct {
        active_reads: *usize,
        begins: usize = 0,
        ends: usize = 0,
    ...
    const RootGeneration = struct {
        fn source() GroupVisibleRootGenerationSource {
            return .{ .ptr = undefined, .visible_root_generation_for_group = visibleRootGeneration };
        }
    ...
    try std.testing.expect((try reads.documentArtifactManifest(alloc, "docs", "doc:a", "missing", .stale)) == null);
    resident.failNext();
    if (try reads.documentArtifactManifestsGroupLocal(alloc, 7001, "docs", "doc:a", .stale)) |manifests| {
        var owned = manifests;
        owned.deinit(alloc);
    }
    resident.failNext();
    var scan = (try reads.scan(alloc, "docs", "", "", .{}, .stale)).?;
    scan.deinit(alloc);
    resident.failNext();
    var preflight = (try reads.preflightQuery(alloc, "docs", .{}, .stale, 0)).?;
    preflight.deinit(alloc);
    resident.failNext();
    var text_stats = (try reads.textStatsGroupLocal(alloc, 7001, "docs", "{\"fields\":[]}")).?;
    text_stats.deinit(alloc);
    resident.failNext();
    const expr = algebraic_ir.TensorExpr{
        .fragment = .reduce,
        .input_dims = &.{ .doc, .scalar },
        .output_dims = &.{.bucket},
        .semantic_id = "missing_materialization",
        .layout = .materialized_expr,
        .law_id = .sum,
    };
    var algebraic_plan = (try algebraic_ir.planMaterializedExpressionAlloc(alloc, expr)).?;
    defer algebraic_plan.deinit(alloc);
    const partials_body = try encodeAlgebraicExpressionPartialsRequest(alloc, "missing", &.{algebraic_plan.access_path}, &.{expr});
    defer alloc.free(partials_body);
    if (reads.algebraicPartialsGroupLocal(alloc, 7001, "docs", partials_body)) |maybe_partials| {
        if (maybe_partials) |partials_value| {
            var partials = partials_value;
            partials.deinit(alloc);
        }
    } else |err| switch (err) {
        error.ResidentDbRetryRequired, error.StorageReadTemporarilyUnavailable => return err,
        else => {},
    }

    try std.testing.expectEqual(@as(usize, 12), resident.leases);
    try std.testing.expectEqual(@as(usize, 6), resident.releases);
    try std.testing.expectEqual(@as(usize, 6), resident.preparations);
    try std.testing.expectEqual(@as(usize, 0), active_reads);
    try std.testing.expectEqual(tracker.begins, tracker.ends);
}
...
test "provisioned graph hydrate completes consistency before resident read admission" {
    ...
    const ResidentSource = struct {
        ...
        releases: usize = 0,
        preparations: usize = 0,
        observed_read_activity_held: bool = false,
        ...
        fn leaseGroup(
            ...
            try std.testing.expectEqual(@as(u64, 9), lsm_root_generation);
            self.leases += 1;
            self.observed_read_activity_held = options.read_activity_held;
            if (self.leases == 1) return error.ResidentDbRetryRequired;
            return .{ .ptr = self, .db = self.db, .release_fn = release };
        }
        ...
        fn prepareGroupForReadRetry(
            ...
            try std.testing.expectEqual(@as(u64, 7001), group_id);
            try std.testing.expectEqual(@as(u64, 9), lsm_root_generation);
            // The retry preparation is permitted to wait for or create a
            // writer only after the failed attempt releases admission.
            try std.testing.expectEqual(@as(usize, 0), self.active_reads.*);
            self.preparations += 1;
        }
    ...
    const GateTracker = struct {
        ...
        fn requestReadableLease(ptr: *anyopaque, group_id: u64, _: []const u8) !void {
            ...
            // committed apply waiting for exclusive admission can deadlock the
    ...
    try std.testing.expectEqual(@as(usize, 2), resident.leases);
    try std.testing.expectEqual(@as(usize, 1), resident.releases);
    ...
    // Each graph callback owns exactly one short-lived table admission.
    response = try executeProvisionedGraphHydrate(
        &worker_ctx,
        alloc,
        7001,
        "docs",
    ...
    // A resident source can be configured without an outer read-preparation
    // hook. In that case the worker must tell the resident owner to acquire its
    // own admission rather than falsely claiming that one is already held.
...
test "provisioned table read source managed runtime config carries inference url" {
    ...
    try std.testing.expectEqualStrings("https://inference.antfly.example", source.inference_api_url.?);
}
...
fn queryProvisionedAcrossGroups(
    ...
    for (group_ids, 0..) |group_id, i| {
        shard_results[i] = try queryHostedLocal(self.resident_db, self.cache, self.replica_root_dir, self.catalog, self.requester, alloc, group_id, self.visibleRootGeneration(group_id), self.managedReadRuntimeConfig(), table_name, fan_in_shard_req.req, consistency);
        initialized += 1;
    }
...
fn queryHostedAcrossGroups(
    ...
    for (group_ids, 0..) |group_id, i| {
        ...
        shard_results[i] = switch (route) {
            .local => try queryHostedLocal(null, null, self.replica_root_dir, self.catalog, self.requester, alloc, group_id, self.visibleRootGeneration(group_id), .{ .backend_runtime = self.backend_runtime }, table_name, fan_in_shard_req.req, consistency),
...
const ProvisionedGraphWorkerContext = struct {
    ...
    fn init(source: *ProvisionedTableReadSource) @This() {
...
fn provisionedGraphFanoutIo(ptr: *anyopaque) ?std.Io {
    ...
    const io_impl = ctx.source.io_impl orelse return null;
    return io_impl.io();
}

fn provisionedGraphFanoutWidthCap(ptr: *anyopaque) usize {
    const ctx: *ProvisionedGraphWorkerContext = @ptrCast(@alignCast(ptr));
    const io_impl = ctx.source.io_impl orelse return 1;
...
fn executeProvisionedGraphExpandAttempt(
    ...
    for (req.frontier, 0..) |item, i| {
        ...
        var graph_result = graph_result_blk: {
            var result = try db_owner.db().search(alloc, search_req);
            defer result.deinit();
            var filtered = if (result.graph_results.len > 0)
            ...
            for (filtered.hits) |*hit| hit.deinit(alloc);
            if (filtered.hits.len > 0) alloc.free(filtered.hits);
            filtered.hits = @constCast((&[_]db_mod.types.SearchHit{})[0..]);
...
fn executeProvisionedGraphHydrateAttempt(
    ...
    defer db_owner.deinit();
    try validateGraphHydrateResolvedDocFilterForDb(req, db_owner.db());
...
fn lookupRelationalTemporalUniqueOwnerProvisionedHostedLocal(
    ...
    return lookupRelationalTemporalUniqueOwnerProvisionedLocal(resident_db, cache, replica_root_dir, catalog, requester, alloc, group_id, lsm_root_generation, backend_runtime, table_name, constraint_name, encoded_value, encoded_point, consistency) catch |err| switch (err) {
        error.NotLeader => if (consistency == .stale) err else try lookupRelationalTemporalUniqueOwnerProvisionedLocal(resident_db, cache, replica_root_dir, catalog, requester, alloc, group_id, lsm_root_generation, backend_runtime, table_name, constraint_name, encoded_value, encoded_point, .stale),
        else => err,
    };
}
...
fn lookupRelationalTemporalUniqueOverlapOwnerProvisionedHostedLocal(
    ...
    return lookupRelationalTemporalUniqueOverlapOwnerProvisionedLocal(resident_db, cache, replica_root_dir, catalog, requester, alloc, group_id, lsm_root_generation, backend_runtime, table_name, constraint_name, encoded_value, encoded_start, encoded_end, consistency) catch |err| switch (err) {
        error.NotLeader => if (consistency == .stale) err else try lookupRelationalTemporalUniqueOverlapOwnerProvisionedLocal(resident_db, cache, replica_root_dir, catalog, requester, alloc, group_id, lsm_root_generation, backend_runtime, table_name, constraint_name, encoded_value, encoded_start, encoded_end, .stale),
        else => err,
    };
}
...
fn lookupRelationalUniqueOwnerProvisionedHostedLocal(
    ...
    return lookupRelationalUniqueOwnerProvisionedLocal(resident_db, cache, replica_root_dir, catalog, requester, alloc, group_id, lsm_root_generation, backend_runtime, table_name, constraint_name, encoded_value, consistency) catch |err| switch (err) {
        error.NotLeader => if (consistency == .stale) err else try lookupRelationalUniqueOwnerProvisionedLocal(resident_db, cache, replica_root_dir, catalog, requester, alloc, group_id, lsm_root_generation, backend_runtime, table_name, constraint_name, encoded_value, .stale),
        else => err,
    };
}
...
fn profiledDenseQuery(req: db_mod.types.SearchRequest) ?ProfiledDenseQuery {
    ...
    if (req.pruner != null) return null;
...
fn queryLocalDetailed(
    ...
    // does not map the same potentially multi-gigabyte index a second time.
    // Cold writer publication is handled by the caller's admission-free retry
...
fn queryDbDetailed(
    ...
    try checkQueryDeadline(snapshot_req);
    if (profiledDenseQuery(snapshot_req)) |dense| {
        var profiled = try db.searchDenseProfiled(alloc, dense.req, dense.query);
        errdefer profiled.result.deinit();
        try checkQueryDeadline(snapshot_req);
        return .{
            .request = snapshot_req,
            .result = profiled.result,
    ...
    var result = try db.search(alloc, snapshot_req);
...
fn collectProvisionedHostedLocalTextStats(
    ...
    return switch (parsed) {
        .background_fields => blk: {
            const stats = try collectBackgroundTextStatsFromDbForRequest(alloc, owner.db(), parsed);
            defer db_mod.aggregations.deinitDistributedBackgroundTextStats(alloc, stats);
            break :blk .{ .json = try encodeBackgroundTextStatsResponse(alloc, stats) };
        },
        else => blk: {
            const stats = try collectTextStatsFromDbForRequest(alloc, owner.db(), parsed);
            defer distributed_stats_mod.deinitTextFieldStats(alloc, stats);
            break :blk .{ .json = try encodeTextStatsResponse(alloc, stats) };
        },
    };
...
fn collectProvisionedHostedLocalAlgebraicPartials(
    ...
    var owner = try provisionedLocalQueryDbOwner(resident_db, cache, replica_root_dir, catalog, alloc, group_id, lsm_root_generation, backend_runtime, table_name, read_activity_held);
    defer owner.deinit();
    const partials = try collectAlgebraicPartialsFromDbForRequest(alloc, owner.db(), parsed);
    defer db_mod.algebraic.distributed.freePartials(alloc, partials);
    return .{ .json = try encodeAlgebraicPartialsResponse(alloc, partials) };
}
...
fn remoteDocumentArtifactChildRangesAlloc(
    alloc: std.mem.Allocator,
    remote: []const RemoteDocumentArtifactManifest.ChildRange,
) ![]db_mod.types.DocumentArtifactChildRange {
...
fn remoteDocumentArtifactManifestAlloc(
    alloc: std.mem.Allocator,
    remote: RemoteDocumentArtifactManifest,
) !db_mod.types.DocumentArtifactManifest {
...
fn documentArtifactManifestRemote(
    ...
    defer response.deinit(alloc);
    return try parseRemoteDocumentArtifactManifest(alloc, response.body);
}
...
fn documentArtifactManifestsRemote(
    ...
    defer response.deinit(alloc);
    return try parseRemoteDocumentArtifactManifests(alloc, response.body);
}

test "remote document artifact manifest parser owns storage manifest fields" {
    const alloc = std.testing.allocator;
    var manifest = try parseRemoteDocumentArtifactManifest(alloc,
        \\{
        \\  "document_id":"doc:a",
        \\  "artifact_name":"units",
        ...
        \\  }],
        \\  "merge_status":"merged",
        \\  "merge_operation_granularity":"range",
        \\  "merge_operation_count":5,
    ...
    try std.testing.expectEqualStrings("units", manifest.artifact_name);
    try std.testing.expectEqualStrings("{\"ok\":true}", manifest.manifest_json);
    try std.testing.expectEqualStrings("{\"state\":\"ready\"}", manifest.state_json.?);
    try std.testing.expectEqual(@as(u64, 3), manifest.manifest_version);
    try std.testing.expectEqual(@as(usize, 1), manifest.child_ranges.len);
    try std.testing.expectEqual(@as(?u64, 7001), manifest.child_ranges[0].owner_group_id);
    try std.testing.expectEqual(@as(?usize, 17), manifest.child_ranges[0].text_bytes);
    try std.testing.expectEqualStrings("E_TEST", manifest.last_error_code.?);
}

test "remote document artifact manifest list parser owns artifact array" {
    const alloc = std.testing.allocator;
    var list = try parseRemoteDocumentArtifactManifests(alloc,
        \\{
        \\  "document_id":"doc:a",
        \\  "artifacts":[{
        \\    "document_id":"doc:a",
    ...
    try std.testing.expectEqualStrings("doc:a", list.document_id);
    try std.testing.expectEqual(@as(usize, 1), list.artifacts.len);
    try std.testing.expectEqualStrings("units", list.artifacts[0].artifact_name);
    try std.testing.expectEqualStrings("{}", list.artifacts[0].manifest_json);
}
...
fn expectNativeQueryTotal(alloc: std.mem.Allocator, body: []const u8, expected_total: i64) !void {
    try expectNativeQueryTotalRelation(alloc, body, expected_total, .exact);
}

fn expectNativeQueryTotalRelation(
    alloc: std.mem.Allocator,
    body: []const u8,
    expected_total: i64,
    expected_relation: metadata_openapi.QueryHitsTotalRelation,
) !void {
    var parsed = try parseJsonTestBody(metadata_openapi.QueryResponses, alloc, body);
    defer parsed.deinit();
    try std.testing.expectEqual(@as(usize, 1), parsed.value.responses.?.len);
    const total = parsed.value.responses.?[0].hits.?.total.?;
    try std.testing.expectEqual(expected_total, total.value);
    try std.testing.expectEqual(expected_relation, total.relation);
}
...
test "hosted lookup fallback skips draining placements when a serving peer exists" {
    var draining = raft_reconciler.PlacementIntent{
        .record = .{ .group_id = 7001, .replica_id = 1, .local_node_id = 10 },
        .serving_state = .draining,
    };
    var serving_peer = raft_reconciler.PlacementIntent{
        .record = .{ .group_id = 7001, .replica_id = 2, .local_node_id = 11 },
        .serving_state = .serving,
    };
    var other_group_serving = raft_reconciler.PlacementIntent{
        .record = .{ .group_id = 7002, .replica_id = 3, .local_node_id = 12 },
        .serving_state = .serving,
    };
    var planned = raft_reconciler.PlacementIntent{
        .record = .{ .group_id = 7001, .replica_id = 4, .local_node_id = 13 },
        .serving_state = .planned,
    };

    const with_serving_peer = [_]*const raft_reconciler.PlacementIntent{ &draining, &serving_peer };
    try std.testing.expect(!HostedProvisionedTableReadSource.placementRefReadableWithPeers(&with_serving_peer, &draining));
    try std.testing.expect(HostedProvisionedTableReadSource.placementRefReadableWithPeers(&with_serving_peer, &serving_peer));

    const without_serving_peer = [_]*const raft_reconciler.PlacementIntent{ &draining, &other_group_serving };
    try std.testing.expect(HostedProvisionedTableReadSource.placementRefReadableWithPeers(&without_serving_peer, &draining));

    const planned_only = [_]*const raft_reconciler.PlacementIntent{&planned};
    try std.testing.expect(!HostedProvisionedTableReadSource.placementRefReadableWithPeers(&planned_only, &planned));
}
...
test "api.table_reads.docid lowered document sql read plans execute native lookup and bounded scan" {
    ...
    try std.testing.expectEqualStrings("doc:a", native_scan_rows[0]._id);
    try std.testing.expectEqualStrings("alpha", native_scan_rows[0].title);
    try std.testing.expectEqualStrings("doc:b", native_scan_rows[1]._id);
    ...
    for (native_residual_rows) |row| {
        ...
        switch (native_residual_matches) {
            0 => {
                try std.testing.expectEqualStrings("doc:a", row._id);
                try std.testing.expectEqualStrings("alpha", row.title);
            },
            1 => {
                try std.testing.expectEqualStrings("doc:c", row._id);
                try std.testing.expectEqualStrings("alpha", row.title);
            },
            else => return error.TestUnexpectedResult,
        }
    ...
    try expectNativeQueryTotalRelation(alloc, native_full_text_alpha.json, 2, .exact);
...
test "api.table_reads.docid lowered document sql unnest read plan matches native scan expansion" {
    ...
    while (lines.next()) |line| {
        ...
        defer parsed.deinit();
        const key = parsed.value.object.get("_id").?.string;
...
test "api.table_reads.docid document sql view mapping runtime results match native document reads" {
    ...
    defer native_full_text.deinit(alloc);
    try expectNativeQueryTotalRelation(alloc, native_full_text.json, 3, .exact);
...
test "provisioned local runtime statuses reconcile empty managed embeddings indexes" {
    ...
    // Query-cache handles are not authoritative status sources. Publish the
    // reconciled writer-owned DB status as the production refresher does.
...
test "remote simple vector query uses vector worker route" {
    ...
    const ExecutorState = struct {
        ...
        fn execute(ptr: *anyopaque, alloc_inner: std.mem.Allocator, req: http_common.HttpRequest) !http_common.HttpResponse {
            ...
            if (std.mem.endsWith(u8, req.uri, "/internal/v1/groups/11/tables/docs/vector-worker")) {
                ...
                self.query_calls += 1;
                if (std.mem.indexOf(u8, req.body, "\"graph_metric\"") != null) {
                    try std.testing.expect(std.mem.indexOf(u8, req.body, "\"metric\":\"pagerank\"") != null);
                    try std.testing.expect(std.mem.indexOf(u8, req.body, "\"_identity_read_generation\":99") != null);
                } else {
    ...
    var graph_metric_result = try queryRemote(state.iface(), alloc, "http://remote.test", 11, "docs", .{
        .index_name = "dense_idx",
        .query = .{ .dense_knn = .{ .vector = &.{ 0.25, 0.5 }, .k = 7 } },
        .graph_metric_queries = &graph_metric_queries,
        .identity_read_generation = 99,
    });
    defer graph_metric_result.deinit();
    try std.testing.expectEqual(@as(usize, 1), state.vector_worker_calls);
    try std.testing.expectEqual(@as(usize, 2), state.query_calls);
    try std.testing.expectEqual(@as(?u64, 99), graph_metric_result.identity_read_generation);
}
...
test "api.table_reads.docid algebraic partial request fails closed when lifecycle is stale" {
    ...
    defer parsed.deinit(alloc);
    // Fence the request to this snapshot so the assertion isolates the
    // planner lifecycle gate from identity-generation drift.
...
test "hosted cross-range graph query expands explicit local start keys" {
    ...
    const right_path = try metadata_mod.groupDbPathFromReplicaRoot(alloc, path, 7002);
    defer alloc.free(right_path);
    const entity_path = try metadata_mod.groupDbPathFromReplicaRoot(alloc, path, 8002);
    defer alloc.free(entity_path);
    ...
    {
        var entity_db = try db_mod.DB.open(alloc, entity_path, .{
            .identity_namespace = .{ .table_id = 8, .shard_id = 8002, .range_id = 8002 },
        });
        defer entity_db.close();
        try entity_db.batch(.{
            .writes = &.{.{ .key = "entity:ada", .value = "{\"name\":\"Ada\"}" }},
            .sync_level = .write,
        });
    }
    ...
    const FakeCatalog = struct {
        const statuses = [_]metadata_reconciler.MergedGroupStatus{
            ...
            .{
                .group_id = 8002,
                .doc_identity = .{
                    .namespace_table_id = 8,
                    .namespace_shard_id = 8002,
                    .namespace_range_id = 8002,
                    .next_ordinal = 2,
                    .allocated_ordinals = 1,
                    .state_rows = 1,
                    .live_ordinals = 1,
        ...
        fn adminSnapshot(_: *anyopaque) !metadata_api.AdminSnapshot {
            return .{
                ...
                .ranges = @constCast((&[_]metadata_table_manager.RangeRecord{
                    ...
                    .{ .group_id = 8002, .table_id = 8, .range_id = 8002, .start_key = "", .end_key = null },
                })[0..]),
                .stores = @constCast((&[_]metadata_table_manager.StoreRecord{})[0..]),
                .placement_intents = @constCast((&[_]raft_reconciler.PlacementIntent{})[0..]),
                .split_transitions = @constCast((&[_]metadata_transition_state.SplitTransitionRecord{})[0..]),
...
test "provisioned storage inspection uses table read admission" {
    ...
    try std.testing.expect((try source.source().lsmStorageStats("docs")) == null);
