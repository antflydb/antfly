...
pub fn parseBackupFormat(maybe_value: anytype) !BackupFormat {
    const value = maybe_value orelse return .portable;
    return switch (@typeInfo(@TypeOf(value))) {
        .@"enum" => switch (value) {
            .native => .native,
            .portable => .portable,
        },
        .pointer => std.meta.stringToEnum(BackupFormat, value) orelse error.UnsupportedBackupFormat,
        else => error.UnsupportedBackupFormat,
    };
}
...
pub fn parseClusterRestoreRequest(alloc: std.mem.Allocator, body: []const u8) !ClusterRestoreRequest {
    ...
    const table_names = try cloneOptionalStringSlice(alloc, parsed.value.table_names);
    errdefer if (table_names) |values| freeStringSlice(alloc, values);
    const restore_mode = if (parsed.value.restore_mode) |value|
        try alloc.dupe(u8, clusterRestoreModeToken(value))
    else
        null;
...
fn clusterRestoreModeToken(value: anytype) []const u8 {
    return switch (@typeInfo(@TypeOf(value))) {
        .@"enum" => switch (value) {
        ...
        .pointer => value,
        else => @compileError("unsupported cluster restore mode type"),
    };
}
...
test "backup request parser accepts native and portable formats" {
    var native = try parseBackupRequest(
        std.testing.allocator,
        "{\"backup_id\":\"snap-native\",\"location\":\"file:///tmp/antfly-backups\",\"connection\":\"archive-writer\",\"format\":\"native\"}",
    );
    native.deinit();

    var portable = try parseBackupRequest(
        std.testing.allocator,
        "{\"backup_id\":\"snap-portable\",\"location\":\"file:///tmp/antfly-backups\",\"connection\":\"archive-writer\",\"format\":\"portable\"}",
    );
    portable.deinit();
}
