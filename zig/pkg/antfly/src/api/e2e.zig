...
const FakeEmbeddingProvider = struct {
    ...
    fn appendEmbedding(alloc: std.mem.Allocator, buf: *std.ArrayListUnmanaged(u8), input: std.json.Value, index: usize) !void {
    ...
    fn encodeFakeOpenAiEmbeddingResponse(alloc: std.mem.Allocator, input: std.json.Value) ![]u8 {
        ...
        switch (input) {
            .array => |items| {
                for (items.items, 0..) |item, i| try appendEmbedding(alloc, &buf, item, i);
            },
            else => try appendEmbedding(alloc, &buf, input, 0),
        }
