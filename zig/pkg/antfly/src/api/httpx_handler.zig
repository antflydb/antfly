...
pub const AntflyApiHandler = struct {
    ...
    pub fn backup(self: *AntflyApiHandler, ctx: *httpx.Context) !httpx.Response {
        ...
        if (try self.authorizeRequest(ctx, &authenticated_identity)) |resp| return resp;
        const body_data = (try ctx.body()) orelse "";
        var resp = try cluster_api_http.handleClusterBackup(ctx.allocator, body_data, self.api_server.clusterApi(), self.api_server.cfg.secret_store, self.api_server.cfg.node_config);
    ...
    pub fn restore(self: *AntflyApiHandler, ctx: *httpx.Context) !httpx.Response {
        ...
        if (try self.authorizeRequest(ctx, &authenticated_identity)) |resp| return resp;
        const body_data = (try ctx.body()) orelse "";
        var resp = try cluster_api_http.handleClusterRestore(ctx.allocator, body_data, self.api_server.clusterApi(), self.api_server.cfg.secret_store, self.api_server.cfg.node_config);
        return respondOwnedApiResponse(ctx, &resp);
    }
    ...
    pub fn listBackups(self: *AntflyApiHandler, ctx: *httpx.Context, params: metadata_openapi.server.ListBackupsParams) !httpx.Response {
        ...
        defer if (authenticated_identity) |*identity| identity.deinit(self.api_server.alloc);
        if (try self.authorizeRequest(ctx, &authenticated_identity)) |resp| return resp;
        var resp = try cluster_api_http.handleClusterBackupList(ctx.allocator, params.location, params.connection, self.api_server.clusterApi(), self.api_server.cfg.secret_store, self.api_server.cfg.node_config);
        return respondOwnedApiResponse(ctx, &resp);
    }
    ...
    pub fn backupTable(self: *AntflyApiHandler, ctx: *httpx.Context, table_name: []const u8) !httpx.Response {
        ...
        defer ctx.allocator.free(decoded_table_name);
        const body_data = (try ctx.body()) orelse "";
        var resp = try public_table_http.handleTableBackup(ctx.allocator, decoded_table_name, body_data, self.api_server.tableApi(), self.api_server.cfg.secret_store, self.api_server.cfg.node_config);
        return respondOwnedApiResponse(ctx, &resp);
    }
    ...
    pub fn restoreTable(self: *AntflyApiHandler, ctx: *httpx.Context, table_name: []const u8) !httpx.Response {
        ...
        defer ctx.allocator.free(decoded_table_name);
        const body_data = (try ctx.body()) orelse "";
        var resp = try public_table_http.handleTableRestore(ctx.allocator, decoded_table_name, body_data, self.api_server.tableApi(), self.api_server.cfg.secret_store, self.api_server.cfg.node_config);
        return respondOwnedApiResponse(ctx, &resp);
    }
    ...
    pub fn updateSchema(self: *AntflyApiHandler, ctx: *httpx.Context, table_name: []const u8) !httpx.Response {
        ...
        const table_before = try self.api_server.loadOwnedTableRecord(decoded_table_name);
