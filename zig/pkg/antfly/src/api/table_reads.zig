    ...
    fn graphExpandGroupLocal(
        ...
        for (req.frontier, 0..) |item, i| {
            ...
            expansions[i] = .{
                ...
                .frontier_key = try alloc.dupe(u8, item.key),
                .graph_result = graph_result_blk: {
                    var result = try queryHostedLocal(self.cache, self.replica_root_dir, self.catalog, self.requester, alloc, group_id, self.visibleRootGeneration(group_id), self.backend_runtime, self.antfly_provider, self.inference_api_url, self.secret_store, self.remote_content, table_name, search_req, consistency);
                    defer result.deinit();
                    var graph_result = if (result.graph_results.len > 0)
                        try distributed_graph.filterGraphSearchResult(alloc, result.graph_results[0], req.exclude_keys, req.exclude_edges)
                    else
                ...
                for (req.frontier, 0..) |item, i| {
                    ...
                    expansions[i] = .{
                        ...
                        .graph_result = graph_blk: {
                            var result = try queryHostedLocal(null, self.replica_root_dir, self.catalog, self.requester, alloc, group_id, self.visibleRootGeneration(group_id), self.backend_runtime, null, null, null, null, table_name, search_req, consistency);
                            defer result.deinit();
                            var graph_result = if (result.graph_results.len > 0)
                                try distributed_graph.filterGraphSearchResult(alloc, result.graph_results[0], req.exclude_keys, req.exclude_edges)
                            else
...
fn queryProvisionedAcrossGroups(
    ...
    for (group_ids, 0..) |group_id, i| {
        shard_results[i] = try queryHostedLocal(self.cache, self.replica_root_dir, self.catalog, self.requester, alloc, group_id, self.visibleRootGeneration(group_id), self.backend_runtime, self.antfly_provider, self.inference_api_url, self.secret_store, self.remote_content, table_name, shard_req, consistency);
        initialized += 1;
    }
...
fn queryHostedAcrossGroups(
    ...
    for (group_ids, 0..) |group_id, i| {
        ...
        shard_results[i] = switch (route) {
            .local => try queryHostedLocal(null, self.replica_root_dir, self.catalog, self.requester, alloc, group_id, self.visibleRootGeneration(group_id), self.backend_runtime, null, null, null, null, table_name, shard_req, consistency),
...
fn openProvisionedQueryDbForTable(
    ...
    return try openProvisionedQueryDbForTableWithCache(alloc, path, catalog, table_name, null, null, lsm_root_generation, null, null, null, null, null, null, null);
}
...
fn openProvisionedQueryDbForTableWithCache(
    ...
    const createEnrichments = struct {
        fn run(
            ...
            return .{
                .dense = try managed_embedder.ManagedEmbedder.createDenseEmbedderWithOptions(allocator, raw_indexes_json, .{ .antfly_provider = local_provider, .secret_store = store, .remote_content = remote, .inference_api_url = inference_url }),
                .sparse = try managed_embedder.ManagedEmbedder.createSparseEmbedderWithOptions(allocator, raw_indexes_json, .{ .antfly_provider = local_provider, .secret_store = store, .remote_content = remote, .inference_api_url = inference_url }),
