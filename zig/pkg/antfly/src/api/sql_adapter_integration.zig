...
test "postgres sql adapter relational full text read matches rows API array predicate" {
    const alloc = std.testing.allocator;
    const schema_json =
        \\{"version":1,"storage_mode":"relational","default_type":"row","enforce_types":true,"document_schemas":{"row":{"schema":{"type":"object","properties":{"id":{"type":"keyword"},"title":{"type":"text"},"status":{"type":"keyword"},"tags":{"type":"array","items":{"type":"keyword"}}},"required":["id","title","status","tags"],"additionalProperties":false}}},"primary_key":{"columns":["id"]},"relational_indexes":[{"name":"ft_v1","owner_kind":"relational_column","owner_name":"title","access_method":"text_search","method_config":{"type":"full_text","field":"title","analyzer":"standard"},"columns":["title"],"lifecycle":"ready","generation":7,"schema_fingerprint":"secondary-index-v1:ft_v1","generation_record":{"generation":7,"owner_ranges":[],"lifecycle":"ready","lag":0,"ready_watermark":0}}]}
    ;
    var parsed = try schema_api.parseValidatedTableSchema(alloc, schema_json);
    defer parsed.deinit(alloc);
    const schema = try schema_api.deriveRuntimeTableSchema(alloc, parsed);
...
test "postgres sql adapter relational full text read matches rows API JSON predicate" {
    const alloc = std.testing.allocator;
    const schema_json =
        \\{"version":1,"storage_mode":"relational","default_type":"row","enforce_types":true,"document_schemas":{"row":{"schema":{"type":"object","properties":{"id":{"type":"keyword"},"title":{"type":"text"},"status":{"type":"keyword"},"metadata":{"type":"json"}},"required":["id","title","status","metadata"],"additionalProperties":false}}},"primary_key":{"columns":["id"]},"relational_indexes":[{"name":"ft_v1","owner_kind":"relational_column","owner_name":"title","access_method":"text_search","method_config":{"type":"full_text","field":"title","analyzer":"standard"},"columns":["title"],"lifecycle":"ready","generation":7,"schema_fingerprint":"secondary-index-v1:ft_v1","generation_record":{"generation":7,"owner_ranges":[],"lifecycle":"ready","lag":0,"ready_watermark":0}}]}
    ;
    var parsed = try schema_api.parseValidatedTableSchema(alloc, schema_json);
    defer parsed.deinit(alloc);
    const schema = try schema_api.deriveRuntimeTableSchema(alloc, parsed);
