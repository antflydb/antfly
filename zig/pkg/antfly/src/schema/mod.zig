...
pub fn deriveRuntimeTableSchema(alloc: std.mem.Allocator, schema: ParsedTableSchema) !storage_schema.TableSchema {
    ...
    const dynamic_template_count = schema.dynamic_templates.len + embedded_dynamic_template_count + document_field_templates.len;
...
fn runtimeDynamicTemplateFromParsed(
    ...
    return .{
        ...
        .mapping = .{
            ...
            .doc_values = template.doc_values orelse runtimeMappingUsesDocValues(field_type, sortable, do_index),
            .sortable = sortable,
...
fn appendRuntimeDocumentFieldTemplates(
    ...
    if (property.embedded_schema) |embedded_schema| {
        try appendRuntimeDocumentFieldTemplates(alloc, out, path, embedded_schema.*);
    }
    if (property.item) |item| {
...
test "deriveRuntimeTableSchema rejects invalid document join cardinality proofs" {
    ...
    var valid_relational_scalar_index = try parseValidatedTableSchema(alloc,
    ...
    defer valid_relational_scalar_index.deinit(alloc);
    try std.testing.expectEqual(@as(usize, 1), valid_relational_scalar_index.relational_indexes.len);
...
test "runtime schema lowers document field mappings to exact declared fields" {
    ...
    var parsed = try parseValidatedTableSchema(alloc,
        ...
        \\        "type": "object",
        \\        "properties": {
        \\          "created_at": {"type": "string","format": "date-time","x-antfly-field": {"type":"date","sortable":true}},
        \\          "meta": {"type": "object","properties": {"rank": {"type":"number","x-antfly-field":{"type":"number","sortable":true}}}},
        \\          "title": {"type": "string","x-antfly-field": {"type":"text","fields": {"keyword": {"type":"keyword","sortable":true}}}},
        \\          "status": {"type": "string","x-antfly-field": {"type":"keyword","sortable":true}},
        \\          "location": {"type": "object","x-antfly-field": {"type":"geo_point"}}
        \\        }
        \\      }
        \\    }
        \\  }
    ...
    const status_field = findRuntimeFullTextField(runtime.full_text_documents[0], "status") orelse return error.TestExpectedEqual;
...
test "runtime schema derives and validates index sort metadata" {
    ...
    var parsed = try parseValidatedTableSchema(alloc,
        ...
        \\    {"name":"rank","path_match":"rank","mapping":{"type":"numeric","sortable":false}}
        \\  ],
        \\  "index_sort": [{"field":"created_at","order":"desc"}]
        \\}
    );
    defer parsed.deinit(alloc);
    ...
    var explicit_id = try parseValidatedTableSchema(alloc,
        \\{"dynamic_templates":[{"name":"created","path_match":"created_at","mapping":{"type":"datetime","sortable":true}}],"index_sort":[{"field":"created_at","order":"asc"},{"field":"_id","order":"asc"}]}
    );
    ...
    var match_mapping_type = try parseValidatedTableSchema(alloc,
        \\{"dynamic_templates":[{"name":"dates","path_match":"meta.*_at","match_mapping_type":"date","mapping":{"type":"datetime","sortable":true}}],"index_sort":[{"field":"meta.created_at","order":"desc"}]}
    );
    ...
    var unsortable = try parseValidatedTableSchema(alloc,
        \\{"dynamic_templates":[{"name":"rank","path_match":"rank","mapping":{"type":"numeric","sortable":false}}],"index_sort":[{"field":"rank","order":"asc"}]}
    );
    ...
    var id_not_final = try parseValidatedTableSchema(alloc,
        \\{"dynamic_templates":[{"name":"created","path_match":"created_at","mapping":{"type":"datetime","sortable":true}}],"index_sort":[{"field":"_id","order":"asc"},{"field":"created_at","order":"asc"}]}
    );
