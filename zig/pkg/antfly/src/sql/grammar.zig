...
test "sql adapter grammar parses ddl generated row expressions" {
    const alloc = std.testing.allocator;
    ...
    const lower = try parseDdlGeneratedRowExpressionAlloc(alloc, parser.Cursor.init(lower_tokens.items, &lower_pos));
    defer runtime_schema.freeRelationalRowsExpression(alloc, lower);
    try std.testing.expectEqual(runtime_schema.RelationalRowsExpressionKind.lower, lower.kind);
    try std.testing.expectEqual(@as(usize, 1), lower.operands.len);
    try std.testing.expectEqualStrings("email", lower.operands[0].field);
    ...
    var md5_pos: usize = 0;
    const md5 = try parseDdlGeneratedRowExpressionAlloc(alloc, parser.Cursor.init(md5_tokens.items, &md5_pos));
    defer runtime_schema.freeRelationalRowsExpression(alloc, md5);
    try std.testing.expectEqual(runtime_schema.RelationalRowsExpressionKind.md5, md5.kind);
    try std.testing.expectEqualStrings("request_id", md5.operands[0].field);
    ...
    const concat_ws = try parseDdlGeneratedRowExpressionAlloc(alloc, parser.Cursor.init(concat_ws_tokens.items, &concat_ws_pos));
    defer runtime_schema.freeRelationalRowsExpression(alloc, concat_ws);
    try std.testing.expectEqual(runtime_schema.RelationalRowsExpressionKind.concat_ws, concat_ws.kind);
    try std.testing.expectEqual(@as(usize, 3), concat_ws.operands.len);
    try std.testing.expectEqualStrings("tenant_id", concat_ws.operands[1].field);
    try std.testing.expectEqualStrings("status", concat_ws.operands[2].field);
    ...
    const replace = try parseDdlGeneratedRowExpressionAlloc(alloc, parser.Cursor.init(replace_tokens.items, &replace_pos));
    defer runtime_schema.freeRelationalRowsExpression(alloc, replace);
    try std.testing.expectEqual(runtime_schema.RelationalRowsExpressionKind.replace, replace.kind);
    try std.testing.expectEqual(@as(usize, 3), replace.operands.len);
    ...
    const arithmetic = try parseDdlGeneratedRowExpressionAlloc(alloc, parser.Cursor.init(arithmetic_tokens.items, &arithmetic_pos));
    defer runtime_schema.freeRelationalRowsExpression(alloc, arithmetic);
    try std.testing.expectEqual(runtime_schema.RelationalRowsExpressionKind.round, arithmetic.kind);
    try std.testing.expectEqual(runtime_schema.RelationalRowsExpressionKind.mul, arithmetic.operands[0].kind);
    ...
    const json_extract = try parseDdlGeneratedRowExpressionAlloc(alloc, parser.Cursor.init(json_extract_tokens.items, &json_extract_pos));
    defer runtime_schema.freeRelationalRowsExpression(alloc, json_extract);
    try std.testing.expectEqual(runtime_schema.RelationalRowsExpressionKind.json_extract, json_extract.kind);
    try std.testing.expect(json_extract.json_as_text);
    try std.testing.expectEqualStrings("source", json_extract.json_path);
    ...
    try std.testing.expectError(error.UnsupportedSqlShape, parseDdlGeneratedRowExpressionAlloc(alloc, parser.Cursor.init(volatile_tokens.items, &volatile_pos)));
}
...
test "sql adapter grammar parses row claim clauses" {
    ...
    const exclusive_clause = try parseCheckedForRowClaimClauseAlloc(alloc, exclusive_tokens.items, &exclusive_pos, &.{"jobs"});
    ...
    try std.testing.expect(exclusive_clause.mode.isExclusiveWriteClaim());
    ...
    const shared_clause = try parseCheckedForRowClaimClauseAlloc(alloc, shared_tokens.items, &shared_pos, &.{"jobs"});
    try std.testing.expect(!shared_clause.mode.isExclusiveWriteClaim());
}
...
test "sql adapter grammar parses owned identifiers and normalized objects" {
    const alloc = std.testing.allocator;
    ...
    var object = try lexer.tokenizeAlloc(alloc, "tenant_1.audit_records");
    defer lexer.freeTokens(alloc, &object);
    var object_pos: usize = 0;
    const object_name = try parseSqlObjectIdentifierOwnedAlloc(alloc, object.items, &object_pos);
    defer alloc.free(object_name);
    try std.testing.expectEqual(object.items.len, object_pos);
    try std.testing.expectEqualStrings("tenant_1.audit_records", object_name);
