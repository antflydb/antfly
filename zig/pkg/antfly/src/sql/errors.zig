...
pub fn describe(err: anyerror) Diagnostic {
    return switch (err) {
        ...
        error.SqlTruncateGraphDependency => .{ .code = "0A000", .message = "TRUNCATE cannot prove a complete graph dependency boundary.", .hint = "Use guarded DELETE while graph-generation retirement is unavailable.", .retryable = false },
