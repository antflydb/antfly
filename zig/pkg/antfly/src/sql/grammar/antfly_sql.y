...
%token ALL ALTER ANALYZE AND ANY ARRAY AS ASC ASYMMETRIC BEGIN BETWEEN BY CASCADE CASE CAST CLOSE CLUSTER COMMIT COMMENT CONFLICT CONSTRAINT CONTINUE CURRENT CURRENT_DATE CURRENT_TIMESTAMP
...
%token ISNULL JOIN KEY LAST LATERAL LEFT LIKE LIMIT LISTEN LOCK MATCHED MATERIALIZED MERGE METRIC MOVE NOT NULL NOTIFY NOTNULL NULLS ON OR ORDER OUTER OVER PARTITION PRECEDING PREPARE PRIMARY PUBLIC
