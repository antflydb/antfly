...
%token ALL ALTER ANALYZE AND ANY ARRAY AS ASC ASYMMETRIC BEGIN BETWEEN BY CASCADE CASE CAST COMMIT CONFLICT CONSTRAINT CONTINUE CURRENT
...
%token NEXT NOTHING OFFSET ONLY QUERY RANGE RECURSIVE RESET RESTART RESTRICT RETURNING RIGHT ROLLBACK ROW ROWS SCHEMA SELECT SET SHOW SOME TABLE TIMESTAMP TIMESTAMPTZ TO TRUNCATE
