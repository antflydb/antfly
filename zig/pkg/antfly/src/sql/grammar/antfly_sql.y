...
%token ALL ALTER ANALYZE AND ANY ARRAY AS ASC ASYMMETRIC BEGIN BETWEEN BY CASCADE CALL CASE CAST CHECKPOINT CLOSE CLUSTER COMMIT COMMENT CONFLICT CONSTRAINT CONTINUE CURRENT CURRENT_DATE CURRENT_TIMESTAMP
%token CREATE COPY DATABASE DEALLOCATE DECLARE DEFAULT DELETE DESC DISCARD DISTINCT DO DROP EXECUTE
%token ELSE END ESCAPE EXPLAIN EXISTS EXTENSION EXTRACT FALSE FETCH FILTER FIRST FOLLOWING FROM FULL GRANT GRAPH GROUP HAVING IDENTITY IF ILIKE INCLUDE IN INDEX INNER INSERT INTERVAL INTO IS
%token ISNULL JOIN KEY LABEL LAST LATERAL LEFT LIKE LIMIT LISTEN LOAD LOCK MATCHED MATERIALIZED MERGE METRIC MOVE NOT NULL NOTIFY NOTNULL NULLS ON OR ORDER OUTER OVER PARTITION PRECEDING PREPARE PRIMARY PUBLIC
%token NEXT NOTHING OFFSET ONLY QUERY RANGE RECURSIVE REFRESH REINDEX RELEASE RESET RESTART RESTRICT RETURNING REVOKE RIGHT ROLLBACK ROW ROWS SAVEPOINT SCHEMA SECURITY SELECT SET SHOW SOME TABLE TIMESTAMP TIMESTAMPTZ TO TRUNCATE
%token SYMMETRIC THEN TRUE UNION UNIQUE UNKNOWN UNLISTEN UPDATE USING VACUUM VALUES VIEW WHEN WHERE WINDOW WITH WITHIN
