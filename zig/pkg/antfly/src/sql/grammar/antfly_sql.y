...
%token ALL ALTER ANALYZE AND ANY AS ASC BEGIN BETWEEN BY CASCADE COMMIT CONFLICT CONSTRAINT CONTINUE CURRENT
%token CREATE DATABASE DEALLOCATE DEFAULT DELETE DESC DISCARD DISTINCT DO DROP EXECUTE
%token EXPLAIN EXISTS EXTENSION FALSE FETCH FIRST FOLLOWING FROM FULL GRAPH GROUP HAVING IDENTITY IF ILIKE IN INDEX INSERT INTO IS
%token JOIN KEY LATERAL LIKE LIMIT MATCHED MERGE METRIC NOT NULL ON OR ORDER OVER PARTITION PRECEDING PREPARE PRIMARY PUBLIC
%token NEXT NOTHING OFFSET ONLY QUERY RANGE RECURSIVE RESET RESTART RESTRICT RETURNING ROLLBACK ROW ROWS SCHEMA SELECT SET SHOW SOME TABLE TO TRUNCATE
%token THEN TRUE UNION UNKNOWN UPDATE USING VALUES WHEN WHERE WINDOW WITH
