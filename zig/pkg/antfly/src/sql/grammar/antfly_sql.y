...
%token ELSE END ESCAPE EXPLAIN EXISTS EXTENSION EXTRACT FALSE FETCH FILTER FIRST FOLLOWING FOREIGN FROM FULL GRANT GRAPH GROUP HAVING IDENTITY IF ILIKE INCLUDE IN INDEX INNER INSERT INTERVAL INTO IS
%token ISNULL JOIN KEY LABEL LAST LATERAL LEFT LIKE LIMIT LISTEN LOAD LOCK MATCHED MATERIALIZED MERGE METRIC MOVE NOT NULL NOTIFY NOTNULL NULLS ON OR ORDER OUTER OVER PARTITION POLICY PRECEDING PREPARE PRIMARY PUBLIC PUBLICATION
%token NEXT NOTHING OFFSET ONLY QUERY RANGE RECURSIVE REFRESH REINDEX RELEASE RESET RESTART RESTRICT RETURNING REVOKE RIGHT ROLLBACK ROW ROWS RULE SAVEPOINT SCHEMA SECURITY SELECT SERVER SET SHOW SOME SUBSCRIPTION TABLE TIMESTAMP TIMESTAMPTZ TO TRUNCATE
%token SYMMETRIC THEN TRUE TRIGGER UNION UNIQUE UNKNOWN UNLISTEN UPDATE USING VACUUM VALUES VIEW WHEN WHERE WINDOW WITH WITHIN
