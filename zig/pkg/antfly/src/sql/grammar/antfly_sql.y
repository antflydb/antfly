...
%token ALL ALTER ANALYZE AND AS ASC BEGIN BETWEEN BY CASCADE COMMIT CONFLICT CONSTRAINT CONTINUE CURRENT
...
%token NEXT NOTHING OFFSET ONLY QUERY RANGE RECURSIVE RESET RESTART RESTRICT RETURNING ROLLBACK ROW ROWS SCHEMA SELECT SET SHOW TABLE TO TRUNCATE
