...
- [ ] Native subquery semantics
  - [ ] Extend scalar subquery expression nodes beyond the promoted generated SELECT projection and predicate paths into generated expressions, defaults, remaining assignment families, and remaining expression entrypoints. Generated SELECT scalar-subquery projections already lower from retained child read ASTs into typed scalar-subquery projections, including mixed field plus scalar-subquery projection lists, `ORDER BY` alias/ordinal binding, and execution with single-column validation, zero-row `NULL`, multi-row cardinality errors, malformed retained-AST fail-closed coverage, runtime coverage, and SQL/API parity. `INSERT ... SELECT` source assignments now retain scalar-subquery source projections, bind the scalar alias as a typed source field for assignment/type validation, preserve runtime cardinality behavior, and have focused DML lowering coverage.
...
- [ ] Function and expression semantic completion
  - [ ] Make casts, type coercion, arrays, temporal literals, and generic scalar function calls generated-owned across projections, predicates, assignments, defaults, generated expressions, aggregate/window payloads, and document SQL expression entrypoints; remove token-layout fallback where retained generated payloads are available, or fail closed with typed unsupported diagnostics.
  - [ ] Complete executable semantics from generated operator metadata for arithmetic, comparison, boolean, null-safe, pattern, regex, JSON/path, containment, overlap, and array operators; each operator should have public SQL/API parity or a typed unsupported row plus retained-AST corruption coverage for stale operator kind, child range, and child-expression metadata.
  - [ ] Complete aggregate/function planning for generated argument lists, aggregate-local ordering, `FILTER`, `WITHIN GROUP`, ordered-set aggregates, `OVER`, named/inline windows, frame payloads, variadic functions, casts, and type coercion; cover both executable row shape and deterministic unsupported diagnostics for admitted-but-unimplemented shapes.
  - [ ] Audit every expression-bearing public entrypoint and either lower directly from retained generated expression/list payloads or reject malformed/stale payloads before legacy expression parsing: SELECT projections, WHERE/HAVING, GROUP BY, ORDER BY, DML assignments, INSERT sources/defaults, generated expressions, RETURNING, document SQL filters/projections, aggregate payloads, and window payloads.
