...
pub fn expectRuntimeRotation(comptime init_store: anytype) !void {
    ...
    var store = try init_store(alloc, &.{ primary.path, fallback.path });
    defer store.deinit();
...
pub fn expectRuntimeWrites(comptime init_store: anytype) !void {
    ...
    for ([_]bool{ false, true }) |relative| {
        ...
        var store = try init_store(alloc, &.{ path, fallback.path });
        defer store.deinit();
        ...
        inline for (.{ "..2026_01", "..2026_02" }, 0..) |dir, index| {
            ...
            var target = try secrets.FileStore.init(alloc, target_path);
            defer target.deinit();
