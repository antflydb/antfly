        ...
        for (0..num_params) |_| {
            ...
            if (has_state == 1) {
                ...
                ps_gop.value_ptr.* = .{
                    .m = m,
                    .v = v,
                    // Seed the per-param step count from the restored global step
                    // (read at the top of this function). optimizers.step() bumps
                    // ps.step_count and uses it for Adam bias correction; without
                    // this seed, resume would restart bias correction at t=1 and
                    // mis-scale the first ~tens of updates.
