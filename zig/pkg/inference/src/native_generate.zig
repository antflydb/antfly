...
fn runServerGenerate(allocator: std.mem.Allocator, io: std.Io, opts: Options, quiet_errors: bool) !void {
    ...
    const request = api.GenerateRequest{
        ...
        .cache_dtype = try generateRequestCacheDtype(opts.cache_dtype),
        .cache_compaction_ratio = opts.cache_compaction_ratio,
        .backend = generateBackendOverrideForChoice(opts.backend),
        .mode = serverGenerateModeValue(opts),
        .compiled_target = serverGenerateCompiledTargetValue(opts),
    };
...
fn generateRequestCacheDtype(value: ?[]const u8) !?api.GenerateRequestCacheDtype {
    const name = value orelse return null;
    if (std.mem.eql(u8, name, "f16")) return .f16;
    if (std.mem.eql(u8, name, "f32")) return .f32;
    if (std.mem.eql(u8, name, "int8")) return .int8;
    if (std.mem.eql(u8, name, "fp8")) return .fp8;
    if (std.mem.eql(u8, name, "int4")) return .int4;
    return error.InvalidCacheDtype;
}

fn serverGenerateModeValue(opts: Options) ?api.GenerateRequestMode {
    const mode = opts.mode orelse {
        if (opts.backend == .metal) return .compiled;
        return null;
    };
    return switch (mode) {
        .eager => .eager,
...
fn serverGenerateCompiledTargetValue(opts: Options) ?api.GenerateRequestCompiledTarget {
    const target = opts.compiled_target orelse {
        if (opts.backend == .metal and (opts.mode == null or opts.mode.? != .eager)) return .whole_model;
        return null;
    };
