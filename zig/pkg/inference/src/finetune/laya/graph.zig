...
pub fn build(b: *B, cfg: modern.Config, l: Layout, head_dropout: f32) !Built {
    ...
    var x = try trunk.encoder(b, &result.sites, cfg, .{ .batch = l.batch, .sequence = l.sequence }, .{
    ...
    for (0..lc.head_layers) |layer| {
        ...
        const attn = try trunk.attention(b, &result.sites, try b.sliceLastDim(qkv, 0, h), try b.sliceLastDim(qkv, h, h * 2), try b.sliceLastDim(qkv, h * 2, h * 3), result.inputs.head_bias, .{ .batch = l.batch, .sequence = l.sequence }, hh, 64, head_dropout);
