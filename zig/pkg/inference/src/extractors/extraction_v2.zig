...
pub fn errorDetails(err: anyerror) ?ErrorDetails {
    ...
    return switch (err) {
        error.InvalidLayaQuestion => .{ .status = 400, .code = "INVALID_EXTRACTION_SCHEMA", .message = "Laya requires named single, ordinal, or boolean questions with instructions and 2 to 20 distinct labels (255 for candidate-packed models); boolean labels must be false, true" },
