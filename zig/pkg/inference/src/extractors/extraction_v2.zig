...
test "span v2 feature rejections are client errors" {
    for ([_]anyerror{ error.UnsupportedGlinerSpanV2Task, error.UnsupportedGlinerSpanLongDocument }) |err| {
        const details = errorDetails(err) orelse return error.TestUnexpectedResult;
        try std.testing.expectEqual(@as(u16, 400), details.status);
        try std.testing.expectEqualStrings("UNSUPPORTED_EXTRACTION_FEATURE", details.code);
    }
}
