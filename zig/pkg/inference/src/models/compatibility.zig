...
fn assessWithFacts(
    ...
    return assessWithRuntimeFacts(man, architecture, expert_count, false, .none);
}
...
fn assessWithRuntimeFacts(
    ...
    if (man.embedding_style != .none and !man.isLastTokenDecoderEmbedder()) {
        return makeIncompatible(
            architecture,
            .unsafe_runtime,
            "decoder embedding style requires an embedder role with last-token pooling",
        );
    }
...
test "qwen3-embedding style embedder is compatible despite decoder arch" {
    ...
    // An explicit style cannot override the mathematical pooling contract.
    var contradictory = manifest_mod.ModelManifest{ .allocator = std.testing.allocator };
    contradictory.model_type = .embedder;
    contradictory.pooling = .mean;
    contradictory.embedding_style = .qwen3_embedding;
    const contradictory_result = assess(&contradictory, "qwen3");
    try std.testing.expectEqual(Level.incompatible, contradictory_result.level);
    try std.testing.expect(!contradictory_result.allowed(true));
}
