...
The projector holds a named request-scoped weight cache across every layer
and image for the lifetime of a request: it explicitly retires only its own
dynamic slots before freeing weights, and permits cleared slots to be
reclaimed by later requests. This lifetime is correctness-required, not just
a performance choice — keying the cache by transient tensor addresses instead
allows allocator reuse to execute one vision layer with an earlier layer's
prepared weights. Moving this cache to a model-scoped projector session and
eliminating redundant host mirrors remains an open performance/residency
follow-up, not grounds to restore a shorter-lived cache identity.
...
## Metal kernel enablement

Several Qwen3-VL Metal kernel routes are opt-in and model-neutral, each with
a matching `TERMITE_METAL_DISABLE_*` rollback switch:

- `TERMITE_METAL_ENABLE_Q4_K_HIGH_ROW_MM=1` accelerates the Q4_K attention
  and gate/up projections.
- `TERMITE_METAL_ENABLE_Q6_K_HIGH_ROW_MM=1` accelerates the Q6_K FFN-down
  projection.
- `TERMITE_METAL_ENABLE_VISION_SDPA_HD64_FLASH_Q32=1` enables the Qwen vision
  attention shape.
- `TERMITE_METAL_ENABLE_QWEN3VL_PREFILL_SG_ATTENTION=1` enables direct
  prefill K/V.
- `TERMITE_METAL_ENABLE_QWEN3VL_PREPARED_FFN=1` runs the prepared gated-FFN
  residual path after the whole-block fast path declines. It has a hard
  dependency on two other flags: enabling only this one is a no-op unless
  both `TERMITE_METAL_ENABLE_QWEN3VL_PREPARED_SLOTS=1` and
  `TERMITE_METAL_ENABLE_QWEN3VL_PREFILL_FRAME=1` are also set.

A complete opt-in qualification profile enables all of the above together;
qualification and performance reports must record the full environment and
assert the expected dispatch/success counts and zero fallbacks.
...
The tokenizer must recognize Qwen's exact isolated Split regex rather than
applying GPT-2 boundaries, to preserve the official `?\n` and `<Document`
BPE merges. Batched reranking must also supply explicit axis-major M-RoPE
positions rather than relying on the scalar RoPE API, which cannot infer
batch size independently from head count in a flattened tensor.
...
> **Relocated:** The dated 2026-08-29/08-30 qualification campaign (2B
