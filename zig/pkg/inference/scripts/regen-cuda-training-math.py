...
def compile_ptx(library, source):
    ...
    check(
        nvrtc.nvrtcCreateProgram(
            c.byref(program), source, b"gliner25_training_math.cu", 0, None, None
        )
    )
    try:
        ...
        if code:
            raise RuntimeError(
                f"NVRTC compilation failed ({code}): {log.value.decode()}"
            )
        ...
        if (
            COMPILER not in ptx
            or b".version 8.7" not in ptx
            or b".target sm_75" not in ptx
        ):
...
def source_bytes():
    ...
    return source.replace(
        marker, (ARTIFACTS.parent / "kernels/gliner25_softmax.cuh").read_bytes()
    )
...
def artifacts(source, ptx):
    ...
    return {
        "gliner25_training_math.ptx": ptx,
        "gliner25_training_math.json": (
    ...
    for name, content in outputs.items():
        ...
        try:
            with tempfile.NamedTemporaryFile(
                dir=directory, prefix=".training-math-", delete=False
            ) as f:
...
def main():
    ...
    parser.add_argument(
        "--nvrtc", default=os.environ.get("ANTFLY_CUDA128_NVRTC", "libnvrtc.so.12")
    )
