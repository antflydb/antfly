...
def execute_batch(model, case, schema_json):
    ...
    if case["kind"] == "extract":
        return model.batch_extract(
            texts,
            oracle.build_extract_schema(specification),
            batch_size=len(texts),
            num_workers=0,
            threshold=0.5,
            include_confidence=True,
            include_spans=True,
            max_len=contract.MAX_WORDS,
        )
...
def capture_batch(model, torch, device, case):
    ...
    def capture(_module, positional, keyword):
        ...
        mask = keyword.get(
            "attention_mask", positional[1] if len(positional) > 1 else None
        )
        if not torch.is_tensor(ids) or not torch.is_tensor(mask):
            raise reference.WorkerError(
                "validation encoder requires actual IDs and attention mask"
            )
        observed = {}
        ...
        def visit(value, path, depth=0):
            ...
            if torch.is_tensor(value):
                ...
                if (
                    ...
                    raise reference.WorkerError(
                        "encoder input device or floating dtype differs"
                    )
        ...
        if (
            list(ids.shape) != expected["input_shape"]
            or list(mask.shape) != expected["input_shape"]
        ):
            raise reference.WorkerError(
                "encoder did not execute the declared single true batch"
            )
        result = {
        ...
        if len(captured) > 1:
            raise reference.WorkerError(
                "source split the declared batch into multiple encoder calls"
            )
...
def serve(
    ...
    *,
    ...
    schemas = {
        name: json.dumps(row["schema"], ensure_ascii=False)
    ...
    while line := source.readline(reference.MAX_COMMAND_BYTES + 1):
        if (
            len(line) > reference.MAX_COMMAND_BYTES
        ...
        if (
            ...
            or not {"op", "request_id"} <= cmd.keys()
            or not cmd.keys() <= {"op", "request_id", "case_id"}
            or type(cmd["request_id"]) is not int
            or cmd["request_id"] <= previous
        ):
        ...
        if cmd["op"] == "stop":
            ...
            if oracle.verify_model_dir(args.model, args.model_dir) != bundle:
                raise reference.WorkerError(
                    "model bundle changed during batch extraction"
                )
        ...
        if (
            cmd["op"] not in ("validate", "run")
            or not isinstance(name, str)
            or name not in cases
        ):
        ...
        try:
            ...
            if model.strict_extraction is not True:
                raise reference.WorkerError(
                    "strict_extraction changed during extraction"
                )
            ...
            if validating:
                ...
                raise reference.WorkerError(
                    "run requires successful actual batch validation"
                )
            capture = (
                capture_batch(model, torch, args.device, case)
                if validating
                else contextlib.nullcontext([])
            )
            with reference.reject_mps_fallback(), capture as observed:
                raw, duration = reference.timed_call(
                    torch,
                    args.device,
                    lambda: execute(model, case, schemas[name]),
                    clock=clock,
                )
            if validating and len(observed) != 1:
                raise reference.WorkerError(
                    "validation requires exactly one encoder execution"
                )
            ...
            emit(
                {
                    ...
                    **(
                        ...
                        if observed
                            ...
                            "input_shape": [
                                len(case["items"]),
                                max(map(len, case["expected_input_ids"])),
                            ],
                            "input_device": None,
                            "encoder_input_devices": None,
                        }
                    ),
...
def worker(args):
    ...
    with reference.reject_mps_fallback():
        ...
        with reference.reject_mps_fallback():
            ...
            ready = reference.ready_event(
                args, model, torch, bundle, provenance, inputs["requests_sha256"]
            )
            ready.update(
                scope=contract.SCOPE,
                workload="scaling",
                cases_sha256=inputs["pins"]["native_inputs"]["sha256"],
                scaling_input_pins=inputs["pins"],
                max_batch_items=8,
                max_text_words=512,
            )
            bench.emit(ready)
