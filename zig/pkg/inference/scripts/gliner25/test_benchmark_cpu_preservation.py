...
class CPUPreservationTests(unittest.TestCase):
    def test_fresh_cpu_binary_uses_cpu_reference_only_and_all_actual_pair_samples(self):
        ...
        class Worker:
            ...
            def receive(self, timeout):
                return {
                    "event": "ready",
                    "arm": self.arm,
                    "scope": campaign.cpu.SCOPE,
                    "timing_boundary": campaign.cpu.TIMING_BOUNDARY,
                    **{
                        "model_id": fixture["bundle"]["model_id"],
            ...
            def request(self, op, name="", timeout=35):
                ...
                return {
                    "event": "result",
                    "duration_ns": 100 if self.arm == "native" else 200,
                    "input_ids": [1, 2, 3],
                    "output": copy.deepcopy(fixture["expected"][name]),
                }
        ...
        args = types.SimpleNamespace(
            native_bin=Path("/new/frozen/cpu-binary"),
            ...
            startup_timeout=10,
            max_rss_mib=512,
            execution_policy="reference_v1",
            legacy_reference_bin=True,
            purpose="benchmark",
        )
        with (
            tempfile.TemporaryDirectory() as temporary,
            mock.patch.object(campaign, "Worker", Worker),
            mock.patch.object(campaign, "ResourceGuard", FakeGuard),
            mock.patch.object(
                campaign,
                "hardware_receipt",
                return_value={"power_source": "ac", "low_power_mode": "0"},
            ),
            mock.patch.object(
                campaign.oracle, "verify_model_dir", return_value=fixture["bundle"]
            ),
            mock.patch.object(campaign.oracle, "verify_upstream_checkout"),
        ):
            result = campaign.run_pair(
                args,
                fixture,
                "cpu",
                Path(temporary) / "pair",
        ...
        self.assertEqual(
            35 * len(fixture["cases"]) * 2, sum(op == "run" for _, op, _ in actions)
        )
    ...
    def model(self):
        ...
        comparison = {
            "sample_pairs": 30,
            "native_ns": campaign.paired_benchmark.distribution([100] * 30),
            "python_ns": campaign.paired_benchmark.distribution([200] * 30),
            "native_over_python_latency": campaign.paired_benchmark.paired_log_ratio_ci(
                [(100, 200)] * 30, samples=2000
            ),
        }
        for case in fixture["cases"]:
            ...
            rows[case["id"]] = {
                ...
                "expected": expected,
                "outputs": {"native": expected, "python": expected},
                "input_ids": [1, 2, 3],
            }
            pairs.extend(
                {
                    ...
                    "native": {"duration_ns": 100},
                    "python": {"duration_ns": 200},
                }
        ...
        return {
            ...
            "validation": rows,
            "pairs": pairs,
            "comparisons": {name: copy.deepcopy(comparison) for name in rows},
            "workers": {
                "native": {
                    **ready,
                    "arm": "native",
                    "build_mode": "ReleaseFast",
                    "scheduler": "serial_io",
                    "cases_sha256": campaign.oracle.sha256_file(
                        campaign.cpu.case_fixture(variant)
                    ),
                },
                "python": {
                    **ready,
    ...
    def test_audit_rederives_ci_and_rejects_pair_loss_output_drift_or_foreign_model(
        self,
    ):
        model = self.model()
        ...
        mutations = (
            ...
            lambda value: next(iter(value["comparisons"].values()))[
                "native_over_python_latency"
            ].update(upper_95=0.1),
            lambda value: next(iter(value["validation"].values())).update(
                confidence_absolute_tolerance=0.01
            ),
        )
        for mutate in mutations:
            ...
            with (
                ...
                self.assertRaises(
                    (campaign.cpu.BenchmarkError, campaign.BenchmarkError)
                ),
            ):
