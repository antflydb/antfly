...
class CrossNERTests(unittest.TestCase):
            ...
            [
                (
                    ...
                    [
                        {
                            "type": "person",
                            "span": {"start": 0, "end": 6, "text": "İ 李"},
                        },
                        {
                            "type": "person",
                            "span": {"start": 7, "end": 10, "text": "李"},
                        },
                    ],
                    4,
                )
    ...
    def test_split_policy_uses_text_not_gold_and_keeps_test_multiplicity(self):
        ...
        parsed = {
            "train": [row, ("only train", [], 2)],
            "dev": [normalized_equivalent, ("only dev", [], 2)],
            "test": [row, row],
        }
        ...
        self.assertEqual(
            crossner.separate_splits(changed_gold)[1]["excluded_ids"],
            audit["excluded_ids"],
        )
    ...
    def test_prediction_conversion_keeps_absent_queries_and_exact_coordinates(self):
        ...
        python["entities"]["person"] = [
            {"text": "李", "start": 2, "end": 3, "confidence": 0.9}
        ]
        native = {"entities": [{"name": name, "values": []} for name in names]}
        native["entities"][names.index("person")]["values"] = [
            {
                "text": "李",
                "source": {"unit": "utf8_bytes", "start": 3, "end": 6},
                "confidence": 0.9,
            }
        ]
    ...
    def test_preparation_pins_source_audit_and_blinds_requests(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            corpus = root / "corpus"
            files = {
                "ner_data/ai/train.txt": b"Train\tB-person\n",
            ...
            manifest.update(
                entity_types=["person"],
                source_documents={"train": 1, "dev": 1, "test": 1},
                excluded_ids=[],
            )
            manifest["files"] = []
