...
def ready(arm, value, inputs, execution_policy):
    if (
        value.get("scope") != contract.SCOPE
        or value.get("workload") != "scaling"
...
def outputs(response, expected_count):
    ...
    if "output" in response:
        cpu.require_equal(
            actual[0],
            cpu.canonical_result(response["output"]),
            "compatibility_first_output",
        )
    return actual
        ...
        if span is not None:
            if (
                ...
                raise cpu.BenchmarkError(
                    "batch output has invalid original source coordinates"
                )
...
def result(arm, response, case, expected, *, validation, phase, native_receipt=None):
    ...
    if runtime.integer(response.get("duration_ns"), "duration_ns") == 0:
        raise cpu.BenchmarkError(
            "scaling response has no completed extraction duration"
        )
    if arm == v1.NATIVE:
    ...
    if validation:
        ...
        response.get("input_shape")
        != contract.packet(case["expected_input_ids"])["input_shape"]
    ):
    ...
    if arm != v1.NATIVE:
        ...
        if validation and reference.normalized_device(
            response.get("input_device")
...
def sample_receipt(response):
    ...
    result["outputs_sha256"] = hashlib.sha256(
        json.dumps(
