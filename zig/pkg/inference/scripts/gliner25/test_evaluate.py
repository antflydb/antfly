...
def corpus(root: Path):
    ...
    files = {
        "ner_data/ai/train.txt": b"Train\tO\n",
...
class EvaluationRunnerTests(unittest.TestCase):
    def args(self, root, backend, source=None, native=None, suffix=None):
        return argparse.Namespace(
            ...
            startup_timeout=10,
            response_timeout=10,
            lock=root / "locked/lock.json",
            prepared_dir=root / "locked/prepared",
            model="small",
            model_dir=root / "model",
            source_reference_report=source,
            native_reference_report=native,
            output_dir=root / (suffix or backend),
    ...
    def test_full_blinded_protocol_cpu_metal_and_complete_metrics(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                mock.patch.object(adapter, "MANIFEST", manifest),
                mock.patch.object(runner, "model_artifact", return_value=artifact),
                mock.patch.object(bench, "Worker", side_effect=start),
            ):
                adapter.prepare(raw, root / "locked")
                source = runner.run(self.args(root, "python"))
                native = runner.run(
                    self.args(root, "native", root / "python/report.json")
                )
                metal = runner.run(
                    self.args(
                        root,
                ...
                for report in (source, native, metal):
                    ...
                    self.assertEqual(
                        report["metrics"]["entity_type/algorithm"][
                            "zero_gold_documents"
                        ],
                        3,
                    )
                ...
                self.assertEqual(
                    metal["source_fp32_quality_delta"]["entity_exact"]["micro_f1_loss"],
                    0,
                )
                ...
                with self.assertRaisesRegex(
                    evaluation.EvaluationError, "identical bundle"
                ):
                    runner.load_reference(
                        root / "native/report.json",
                        fixture,
                        root / "locked/prepared",
                        "native",
                        wrong_artifact,
                    )
                ...
                with self.assertRaisesRegex(evaluation.EvaluationError, "reproduce"):
                    runner.prepare_fixture(
                        root / "locked/lock.json", root / "locked/prepared", "small"
                    )
    ...
    def test_token_drift_blocks_scoring_and_confidence_drift_never_widens_parity(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                ...
                self.assertEqual(
                    drift["required_parity"]["confidence_absolute_tolerance"], 5e-4
                )
            ...
            with mock.patch.object(adapter, "MANIFEST", manifest):
                ...
                for change in ("gold", "schema", "options", "limit"):
                    ...
                    if change == "gold":
                        ...
                        broken["cases"][0]["options"]["threshold"] = 0.4
                    else:
                        broken["cases"] *= 342
                    with (
    ...
    def test_python_limits_are_checked_before_encoder_execution(self):
        ...
        processor.collate_fn_inference = lambda *_args, **_kwargs: SimpleNamespace(
            text_tokens=[["word"]], input_ids=SimpleNamespace(shape=(1, 513))
        )
        with mock.patch.object(
            oracle,
            "build_extract_schema",
            return_value=SimpleNamespace(build=lambda: {}),
        ):
            with self.assertRaisesRegex(
                evaluation.EvaluationError, "BoundarySequenceLimitExceeded"
            ):
                runner.execute_python_case(
                    SimpleNamespace(processor=processor), case, None
                )
    ...
    def test_quality_loss_is_a_signed_measured_delta_not_a_pass(self):
        source = {
            "records": 3,
            "lock_sha256": "a" * 64,
            "metrics": {"entities": {"micro_f1": 0.8, "support": 7}},
        }
        actual = copy.deepcopy(source)
        actual["metrics"]["entities"]["micro_f1"] = 0.7
        delta = runner.quality_delta(source, actual)["entities"]
        self.assertAlmostEqual(delta["micro_f1_loss"], 0.1)
        self.assertAlmostEqual(delta["micro_f1_delta"], -0.1)
