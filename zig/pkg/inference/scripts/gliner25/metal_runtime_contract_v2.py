...
COUNTERS = (
    "actual_weight_upload_bytes",
    "scope_submissions",
    "peak_pending_device_bytes",
...
class NativeReceipt:
    ...
    def check_identity(self, event: dict[str, Any]) -> None:
        if self.legacy:
            if any(
                ...
                raise v1.BenchmarkError(
                    "legacy runtime changed receipt policy after ready"
                )
            return
        if (
            type(event.get("runtime_contract_version")) is not int
        ...
        if "runtime_policy" in event and not policy_matches(
            event["runtime_policy"], self.execution_policy
        ):
        ...
        *,
        ...
        if self.legacy:
            ...
            return v1.checked_result(
                response, expected, v1.NATIVE, validation=validation
            )
    ...
    def observe_ownership(
        self, response: dict[str, Any], *, validation: bool, phase: str | None = None
    ) -> None:
        ...
        if (
            response.get("gpu_work_submitted") is not True
            or response.get("external_frame") is not False
        ):
            raise v1.BenchmarkError("native request lacks strict owned GPU execution")
        dispatches = mapping(
            mapping(response.get("request_stats"), "request_stats").get("encoder"),
            "encoder",
        )
        if (
            integer(dispatches.get("device_dispatches"), "encoder.device_dispatches")
            == 0
        ):
            raise v1.BenchmarkError("native request lacks encoder dispatches")
        fallback = mapping(
            response.get("host_fallback_evidence"), "host_fallback_evidence"
        )
        ...
        if validation:
            ...
            if (
                ...
                or not 1 <= len(ids) <= v1.oracle.MAX_ENCODED_TOKENS
                or any(
                    type(token) is not int or not 0 <= token <= (1 << 32) - 1
                    for token in ids
                )
            ):
        ...
        for position in ("before_live_bytes", "after_live_bytes"):
            if (
                integer(model.get(position), f"model.{position}")
                != self.model_live_bytes
            ):
                raise v1.BenchmarkError(
                    "model-owned residency changed after common preparation"
                )
            if integer(transient.get(position), f"transient.{position}") != 0:
                raise v1.BenchmarkError(
                    "request boundary retains transient device memory"
                )
        if (
            integer(
                transient.get("pending_after_bytes"), "transient.pending_after_bytes"
            )
            != 0
        ):
            raise v1.BenchmarkError(
                "request returned before pending device ownership completed"
            )
        workspace = mapping(
            memory.get(
                "workspace",
                {"before_live_bytes": 0, "after_live_bytes": 0, "generation": 0},
            ),
            "owned_memory.workspace",
        )
        ...
        if (
            wb != self.workspace_live_bytes
            or not wb <= wa <= self.workspace_capacity_bytes
            or generation < self.workspace_generation
            or (wa != wb and generation == self.workspace_generation)
        ):
            raise v1.BenchmarkError(
                "workspace growth is discontinuous, unversioned or over its declared bound"
            )
        ...
        for position, workspace_bytes in (("before", wb), ("after", wa)):
            ...
            if (
                integer(
                    snapshot.get("device_owned_live_bytes"),
                    f"{position}.device_owned_live_bytes",
                )
                != self.model_live_bytes + workspace_bytes
            ):
                raise v1.BenchmarkError(
                    "aggregate device residency disagrees with owner receipts"
                )
            if (
                integer(
                    snapshot.get("host_mirror_live_bytes"),
                    f"{position}.host_mirror_live_bytes",
                )
                != 0
            ):
        ...
        for created, released, net in (
            ...
            for key in (created, released):
                delta = integer(after.get(key), f"after.{key}") - integer(
                    before.get(key), f"before.{key}"
                )
                if delta < 0:
                    raise v1.BenchmarkError(
                        "native cumulative allocation counters moved backwards"
                    )
                deltas.append(delta)
            observed = deltas[0] - deltas[1]
            if (net is not None and observed != net) or (
                net is None and wa == wb and observed != 0
            ):
                raise v1.BenchmarkError(
                    "native allocation counters do not reconcile with owner changes"
                )
        stats = mapping(response.get("runtime_stats"), "runtime_stats")
        checked = {
            key: integer(stats.get(key), f"runtime_stats.{key}") for key in COUNTERS
        }
        ...
        if stats.get("phase_timings_ns") is not None:
            timings = mapping(
                stats["phase_timings_ns"], "runtime_stats.phase_timings_ns"
            )
            for key in PHASE_TIMINGS:
                integer(timings.get(key), f"runtime_stats.phase_timings_ns.{key}")
            if phase in ("warmup", "measurement"):
                raise v1.BenchmarkError(
                    "instrumented phases cannot enter latency acceptance"
                )
        if checked["pending_device_bytes"] != 0:
            raise v1.BenchmarkError("runtime has pending device bytes after extraction")
        workspace_new = (
            self.execution_policy == "optimized_v2"
            and self.workspace_capacity_bytes > 0
        )
        if (
            integer(
                stats.get("workspace_pending_bytes", None if workspace_new else 0),
                "runtime_stats.workspace_pending_bytes",
            )
            != 0
        ):
            raise v1.BenchmarkError(
                "runtime has pending workspace bytes after extraction"
            )
        if "workspace_peak_pending_bytes" in stats or workspace_new:
            peak = integer(
                stats.get("workspace_peak_pending_bytes"),
                "runtime_stats.workspace_peak_pending_bytes",
            )
            if peak > self.workspace_capacity_bytes:
                raise v1.BenchmarkError(
                    "workspace pending peak exceeded its admitted capacity"
                )
        if (
            integer(
                stats.get("workspace_live_bytes", 0),
                "runtime_stats.workspace_live_bytes",
            )
            != wa
        ):
            raise v1.BenchmarkError(
                "workspace runtime counter disagrees with owned memory"
            )
        if self.execution_policy == "optimized_v2":
            ...
            if checked["scope_submissions"] == 0:
                raise v1.BenchmarkError(
                    "optimized request has no owned submission evidence"
                )
    ...
    def stopped(self, event: dict[str, Any]) -> dict[str, Any]:
        ...
        for key in ("model_live_bytes", "transient_live_bytes", "pending_device_bytes"):
            if integer(event.get(key), f"stopped.{key}") != 0:
                raise v1.BenchmarkError(
                    "native stopped before all model/request/device ownership was released"
                )
        if (
            integer(
                event.get("workspace_live_bytes", 0), "stopped.workspace_live_bytes"
            )
            != 0
        ):
            raise v1.BenchmarkError(
                "native stopped before its warm workspace was released"
            )
...
def native_ready(
    ...
    if scope == v1.SCOPE and version is None:
        if execution_policy != "reference_v1":
            raise v1.BenchmarkError(
                "optimized execution cannot accept a v1 runtime receipt"
            )
        if "execution_policy" in ready or "runtime_policy" in ready:
            raise v1.BenchmarkError(
                "legacy readiness contains ambiguous v2 policy fields"
            )
    ...
    v1.checked_ready(
        v1.NATIVE, {**ready, "scope": v1.SCOPE}, bundle, variant, case_path
    )
    ...
    if (execution_policy == "reference_v1" and model_bytes != 0) or (
        ...
        raise v1.BenchmarkError(
            "ready residency disagrees with the selected execution policy"
        )
    capacity = integer(
        ready.get("workspace_capacity_bytes", 0), "ready.workspace_capacity_bytes"
    )
    workspace = integer(
        ready.get("workspace_live_bytes", 0), "ready.workspace_live_bytes"
    )
    generation = integer(
        ready.get("workspace_generation", 0), "ready.workspace_generation"
    )
    if workspace > capacity or (
        execution_policy == "reference_v1" and (capacity or workspace or generation)
    ):
        raise v1.BenchmarkError(
            "ready workspace differs from the execution policy or cap"
        )
    return NativeReceipt(
        execution_policy, VERSION, model_bytes, capacity, workspace, generation
    )
...
def reference_ready(
    ...
    if ready.get("scope") != SCOPE and not (
        execution_policy == "reference_v1" and ready.get("scope") == v1.SCOPE
    ):
...
def competitiveness(
    intervals: list[dict[str, Any]], *, numerator: str
) -> dict[str, Any]:
    ...
    passed = complete and all(
        interval["lower_95"] >= 5 / 6
        if numerator == "python_over_metal"
        else interval["upper_95"] < 1
        for interval in intervals
    )
    return {
        "complete_three_repetitions": complete,
        "passed": passed,
        "criterion": (
            "each_lower_95_python_over_metal_at_least_5_over_6"
            if numerator == "python_over_metal"
