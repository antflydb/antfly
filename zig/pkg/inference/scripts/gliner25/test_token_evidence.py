...
class TokenEvidenceTest(unittest.TestCase):
    def test_all_thirty_sequences_remain_bound_to_source_and_ordered_requests(self):
        ...
        self.assertEqual(
            "completed_cpu_benchmark_encoder_token_evidence", value["scope"]
        )
        ...
        self.assertEqual(
            oracle.sha256_file(Path(evidence.__file__)), value["generator_sha256"]
        )
        self.assertEqual(manifest["upstream"]["commit"], value["source_commit"])
        self.assertEqual(manifest["runtime"], value["runtime"])
        self.assertEqual(
            {"small", "base", "multi"}, {row["model"] for row in value["models"]}
        )
        ...
        for row in value["models"]:
            ...
            self.assertEqual(
                {
                    name: {key: item[key] for key in ("sha256", "size_bytes")}
            ...
            self.assertEqual(
                {case["id"] for case in fixture["cases"]}, set(row["validation"])
            )
    ...
    def test_report_substitution_fails_before_reading_profile_or_tokens(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertRaisesRegex(
                oracle.ContractError, "exact completed benchmark report"
            ):
