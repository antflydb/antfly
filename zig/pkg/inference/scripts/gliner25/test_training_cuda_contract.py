...
class TrainingContractTests(unittest.TestCase):
    def test_reference_cublas_identity_checks_vendor_status_and_version(self):
        ...
        for status, value in ((0, 120800), (1, 120800), (0, 0)):
            ...
            with patch.object(
                ctypes, "CDLL", return_value=SimpleNamespace(cublasGetVersion_v2=query)
            ) as loader:
    ...
    def test_snapshot_reports_signed_zero_bits_without_changing_numeric_acceptance(
        self,
    ):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertTrue(
                report["tensors"]["classifier.weight"]["m"]["bitwise_equal"]
            )

    def test_compiled_snapshot_names_follow_parameter_objects_and_reject_inventory_changes(
        self,
    ):
    ...
    def test_negative_query_sampling_is_disabled_without_changing_other_settings(self):
        ...
        model = SimpleNamespace(
            boundary_head=SimpleNamespace(settings=original), boundary_settings=original
        )
        ...
        self.assertEqual(
            model.boundary_head.settings.max_negative_queries_per_batch, 64
        )
    ...
    def test_adapter_rejects_negative_and_out_of_range_offsets(self):
        original = json.loads(
            (bench.oracle.FIXTURES / "training_job_small_v1" / "train.jsonl")
            .read_text()
            .splitlines()[0]
        )
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for updates, validation_only in ((-1, False), (513, False), (1, True)):
                args = SimpleNamespace(
                    ...
                    validate_only=validation_only,
                )
                with self.assertRaisesRegex(
                    bench.common.BenchmarkError, "qualification update"
                ):
                    bench.run(args)
    ...
    def test_quality_adapter_preserves_record_schema_and_all_gold_tasks(self):
        row = json.loads(
            (bench.oracle.FIXTURES / "training_job_small_v1" / "validation.jsonl")
            .read_text()
            .splitlines()[0]
        )
        request = quality.evaluation_request(row)
        self.assertEqual(
            bench.common.adaptation.schema_for(request)["structures"],
            {
                name: {
                    **spec,
                    "fields": {
                        key: {
                            ("type" if k == "dtype" else k): v for k, v in field.items()
                        }
        ...
        self.assertEqual(
            {name: sum(values.values()) for name, values in facts.items()},
            {"entities": 2, "classifications": 1, "records": 1, "relations": 1},
        )

    def test_encoder_work_limit_rejects_invalid_values_before_loading_or_launching(
        self,
    ):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            ...
            for limit in (0, -1, True, 1.5, "8589934592", 1024 * 1024**3 + 1):
                ...
                with patch.object(bench.oracle, "verify_dependencies") as dependencies:
                    with self.assertRaisesRegex(
                        bench.common.BenchmarkError, "logical encoder"
                    ):
                        bench.run(args)
    ...
    def test_cublas_selection_rejects_missing_or_relative_library_before_launch(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for library in (Path("libcublas.so.12"), root / "missing.so"):
                ...
                with patch.object(bench.oracle, "verify_dependencies") as dependencies:
                    with self.assertRaisesRegex(
                        bench.common.BenchmarkError, "existing absolute"
                    ):
                        bench.run(args)
                    dependencies.assert_not_called()
            self.assertFalse((root / "report").exists())
    ...
    def test_evaluation_snapshot_validation_precedes_weight_mutation(self):
        ...
        parameter = Parameter()
        model = SimpleNamespace(
            named_parameters=lambda: [("classifier.weight", parameter)]
        )
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            model.named_parameters = lambda: [
                ("classifier._orig_mod.weight", parameter)
            ]
    ...
    def test_quality_evaluation_restores_model_mode_on_failure(self):
        ...
        model = SimpleNamespace(
            training=True,
            eval=lambda: modes.append(False),
            train=lambda value: modes.append(value),
        )
        row = json.loads(
            (bench.oracle.FIXTURES / "training_job_small_v1" / "validation.jsonl")
            .read_text()
        ...
        with patch.object(
            quality.common, "execute_python", side_effect=RuntimeError("device failed")
        ):
            with self.assertRaisesRegex(RuntimeError, "device failed"):
                quality.evaluate(
                    model, [row], SimpleNamespace(inference_mode=nullcontext)
                )
    ...
    def test_invalid_adam_epsilon_is_rejected_before_loading_or_launch(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for epsilon in (0, -1, float("nan"), float("inf"), 1.01):
                ...
                with self.assertRaisesRegex(
                    bench.common.BenchmarkError, "Adam epsilon"
                ):
                    bench.run(args)
            self.assertFalse((root / "report").exists())
    ...
    def test_adapter_preserves_all_fixture_tasks_and_absent_labels(self):
        ...
        self.assertEqual(
            schema["relations"], [{"works_for": {"head": "Ada", "tail": "Acme"}}]
        )
        self.assertEqual(
            schema["json_structures"],
            [{"employment": {"employee": "Ada", "employer": "Acme"}}],
        )
        self.assertEqual(
            schema["record_metadata"]["employment"]["fields"]["employer"][
                "cardinality"
            ],
            "required_one",
        )
    ...
    def snapshot(self, directory, arm, values, **changes):
        ...
        slot = {
            "canonical_name": "classifier.weight",
            "shape": [2],
            "elements": 2,
            "offset": 0,
            "present": True,
        ...
        return {
            ...
            "slots": [slot],
            "identity": {"optimizer_step": 1, "microbatch_step": 2},
    ...
    def test_snapshot_checks_gradients_and_moments_even_when_weights_match(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for index, field in ((2, "gradient"), (4, "m"), (6, "v")):
                ...
                self.assertTrue(
                    any("." + field + ":" in item for item in result["failures"])
                )
    ...
    def test_snapshot_reports_worst_coordinate_across_chunks(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                weight["worst_element"],
                {"flat_index": n - 1, "native": 0.0, "python": -0.5},
            )
            self.assertEqual(
                weight["state_at_worst"]["python"],
    ...
    def test_dropped_auxiliary_loss_fails_even_if_total_is_close(self):
        native = {
            "report": {
                "zero_loss_fallback": False,
                "examples": 2,
                "terms": {"total": 1, "relation": 0},
                "optimizer": {
...
class LossTraceContract(unittest.TestCase):
    def test_module_trace_compares_retained_input_without_inventing_an_output(self):
        ...
        torch = SimpleNamespace(
            nn=SimpleNamespace(Linear=Linear, LayerNorm=type("LayerNorm", (), {}))
        )
        with tempfile.TemporaryDirectory() as directory:
