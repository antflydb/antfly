...
def capture_cpu(args, inputs, selected, directory):
    ...
    command = [
        sys.executable,
        str(campaign.HERE / "metal_scaling_python_worker.py"),
        "--device",
        "cpu",
        "--model",
        inputs["model"],
        "--model-dir",
        str(args.model_root / inputs["model"]),
        "--upstream",
        str(args.upstream),
        "--prepared",
        str(args.prepared),
    ...
    with (directory / "events.jsonl").open("w", encoding="utf-8") as journal:
        ...
        try:
            ...
            for name in selected:
                ...
                try:
                    ...
                    report["cases"][name] = {
                        ...
                        "outputs": output,
                        **contract.packet(
                            inputs["source_cases"][name]["expected_input_ids"]
                        ),
                    ...
                    if response.get("recoverable") is not True or response.get(
                        "device_unsafe"
                    ):
                        raise BenchmarkError(
                            "unsafe CPU reference error prevents further cases"
                        ) from error
            stopped = worker.request("stop", timeout=args.startup_timeout)
            record(stopped)
            if (
                stopped.get("scope") != contract.SCOPE
                or stopped.get("requests_sha256") != inputs["requests_sha256"]
            ):
            ...
            report["status"] = (
                "complete"
                if all(row["status"] == "complete" for row in report["cases"].values())
                else "partial"
            )
            ...
            if worker is not None:
                ...
                if (
                    not isinstance(report["cleanup"], dict)
                    or report["cleanup"].get("complete") is not True
                ):
                    report.update(
                        status="failed",
                        cleanup_error="CPU reference cleanup is incomplete",
                    )
            ...
            for completed in report["resource_guard"].get("completed_workers", []):
                ...
                if not isinstance(receipt, dict) or receipt.get("complete") is not True:
                    report.update(
                        status="failed",
                        cleanup_error="partial-constructor worker cleanup is incomplete",
                    )
    ...
    for value in inputs:
        for name in value["cases"]:
            ...
            def completed(phase, status):
                return any(
                    item["phase"] == phase
                    and item["case_status"] == status
                    and item["run_status"] in ("complete", "partial")
                    for item in evidence
                )

            validated = completed("source_cpu_capture", "complete") and completed(
                "preflight", "validated"
            )
            smoke = value["source_cases"][name]["profile"]["mode"] == "smoke"
            status = "blocked"
            if any(
                item["case_status"] == "failed"
                or (
                    item["run_status"] in ("failed", "interrupted")
                    and item["case_status"] not in ("pending", "unprocessed")
                )
                for item in evidence
            ):
                status = "failed"
...
def driver(args):
    if (
        ...
        or args.execution_policy not in campaign.runtime.POLICIES
        or not 256 <= args.max_rss_mib <= 8192
        or not 1 <= args.timeout_ms <= 60_000
        or not 1 <= args.startup_timeout <= 300
    ):
        raise BenchmarkError(
            "requires fresh output, existing binary and bounded worker profile"
        )
    args.repetitions, args.warmup, args.pairs, args.legacy_reference_bin = (
        1,
        1,
        3,
        False,
    ...
    for variant in variants:
        ...
        value["workload"], value["bundle"] = (
            "scaling",
            oracle.verify_model_dir(variant, args.model_root / variant),
        )
        chosen = args.cases or value["cases"]
        if (
            not chosen
            or len(set(chosen)) != len(chosen)
            or any(name not in value["cases"] for name in chosen)
        ):
            raise BenchmarkError(
                "scaling selection must name unique manifest-approved cases"
            )
    ...
    report = {
        ...
        "source_files_sha256": snapshot["source_files_sha256"],
        ...
        "contracts": [
            {"model": value["model"], "cases": value["cases"], "pins": value["pins"]}
            for value in inputs
        ],
        "source_captures": [],
        "runs": [],
    ...
    try:
        ...
        for value in inputs:
            captured = capture_cpu(
                args,
                value,
                value["cases"],
                args.output / f"source-cpu-{value['model']}",
            )
            report["source_captures"].append(captured)
            if captured.get("cleanup_error"):
                raise BenchmarkError(
                    "source process cleanup failed; refusing any further model"
                )
            value["expected"] = {
                name: row["outputs"]
                for name, row in captured["cases"].items()
        ...
        for value in inputs:
            ...
            if not selected:
                ...
                continue
            run = campaign.run_pair(
                args,
                value,
                "mps",
                args.output / f"preflight-{value['model']}",
                repetition=0,
                phase="preflight",
                selected=selected,
            )
            ...
            if run.get("cleanup_errors"):
                raise BenchmarkError(
                    "paired process cleanup failed; refusing any further model"
                )
            eligible[value["model"]] = [
                name
                for name in selected
                if run["status"] in ("complete", "partial")
                and run["case_status"][name]["status"] == "validated"
            ]
            campaign.write_json(args.output / "report.json", report)
        if args.purpose != "validation":
            for repetition in range(1):
                for value in campaign.rotate(inputs, repetition):
                    selected = [
                        name
                        for name in eligible[value["model"]]
                        if value["source_cases"][name]["profile"]["mode"] != "smoke"
                    ]
                    if not selected:
                        continue
                    phase = (
                        "measurement" if args.purpose == "benchmark" else "diagnostic"
                    )
                    run = campaign.run_pair(
                        args,
                        value,
                    ...
                    if run.get("cleanup_errors"):
                        raise BenchmarkError(
                            "paired cleanup failed; refusing any further model"
                        )
        for value in inputs:
            if contract.load(args.prepared, value["model"])["pins"] != value["pins"]:
                raise BenchmarkError("scaling inputs changed during campaign")
        if (
            oracle.sha256_file(args.native_bin) != native["sha256"]
            or campaign.source_snapshot() != snapshot
        ):
        ...
        complete = (
            ...
            and all(
                len(eligible[value["model"]]) == len(value["cases"]) for value in inputs
            )
            and (
                args.purpose != "benchmark"
                or all(
                    row["status"] in ("complete", "correctness_only")
                    for row in report["summary"]
                )
            )
        )
        report.update(
            status="complete" if complete else "partial", parity_validated=complete
        )
    except BaseException as error:
        report.update(
            status="interrupted"
...
def main(argv=None):
    ...
    parser.add_argument(
        "--model-root", type=Path, default=Path("/private/tmp/antfly-gliner25-models")
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--execution-policy", choices=tuple(campaign.runtime.POLICIES), required=True
    )
    parser.add_argument(
        "--purpose",
        choices=("validation", "diagnostic", "benchmark"),
        default="benchmark",
    )
