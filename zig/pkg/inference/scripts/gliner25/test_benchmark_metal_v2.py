...
from test_benchmark_metal import (
    FakeGuard,
    contract as fixture_contract,
    ready as ready_v1,
)
...
class DriverTests(unittest.TestCase):
                ...
                if self.arm == bench.NATIVE:
                    if legacy:
                        ...
                        event.update(
                            ...
                            runtime_policy=copy.deepcopy(
                                bench.runtime.POLICIES[policy]
                            ),
    ...
    def test_optimized_policy_forwarded_and_final_cleanup_required(self):
        ...
        self.assertEqual(
            ["--execution-policy", "optimized_v2"], commands[bench.NATIVE][-2:]
        )
    ...
    def test_diagnostics_are_separate_and_never_produce_latency_comparisons(self):
        ...
        self.assertFalse(
            any(row["stage"] in ("measurement", "warmup") for row in journal)
        )
        self.assertEqual(["--diagnostics", "true"], commands[bench.NATIVE][-2:])

    def test_diagnostic_campaign_preflight_uses_instrumentation_but_legacy_never_does(
        self,
    ):
        _, _, commands, _ = self.run_fake(
            purpose="preflight", campaign_purpose="diagnostic"
        )
        self.assertEqual(["--diagnostics", "true"], commands[bench.NATIVE][-2:])
        _, _, commands, _ = self.run_fake(
            purpose="preflight",
            campaign_purpose="diagnostic",
            policy="reference_v1",
            legacy=True,
        )
    ...
    def test_schema_or_output_failure_does_not_admit_earlier_case_samples(self):
        ...
        self.assertEqual(
            2, len([row for row in result["pairs"] if row["case_id"] == first])
        )
    ...
    def test_policy_cannot_omit_native_flag_without_explicit_reference(self):
        args = types.SimpleNamespace(
            execution_policy="optimized_v2", legacy_reference_bin=True
        )
    ...
    def test_workspace_growth_on_rejected_case_keeps_later_case_measurable(self):
        ...
        def mutate(arm, op, case, event):
            ...
            if arm == bench.NATIVE and op != "stop":
                ...
                event.update(
                    owned_memory=owner["owned_memory"],
                    runtime_stats=owner["runtime_stats"],
                )
                live = 1024
                if case == first:
        ...
        result, events, _, journal = self.run_fake(
            mutate=mutate, workspace_capacity=2048
        )
        self.assertEqual("partial", result["status"])
        ...
        self.assertTrue(
            any(
                row["stage"] == "validation" and row["response"].get("case_id") == first
                for row in journal
            )
        )
...
class AggregationTests(unittest.TestCase):
    def report(self, repetitions=(0, 1, 2)):
        ...
        return {
            "contracts": [{"model": "small", "cases": ["case"]}],
            "baselines": ["mps"],
            ...
            "runs": [
                {
                    ...
                    "comparisons": {"case": copy.deepcopy(comparison)},
    ...
    def test_case_comparison_cannot_override_its_worker_repetition_identity(self):
        ...
        self.assertEqual(
            [0, 0, 0], [value["repetition"] for value in row["repetitions"]]
        )
        self.assertFalse(row["milestone"]["passed"])

    def test_one_failing_independent_interval_cannot_hide_behind_two_winners(self):
        report = self.report()
        report["runs"][1]["comparisons"]["case"]["python_over_metal_speedup"][
            "lower_95"
        ] = 0.82
        row = bench.aggregate(report)[0]
        self.assertEqual("complete", row["status"])
