...
TOLERANCES = {"weight": (2e-6, 2e-5), "gradient": (2e-4, 5e-3), "m": (2e-5, 5e-3), "v": (2e-7, 1e-2)}


def response(worker, op, case, timeout):
    result = worker.request(op, case, timeout)
    if result["event"] == "error":
        raise common.BenchmarkError(f"{worker.arm}: {result.get('error_type')}: {result.get('message')}")
    return result


def compare_snapshots(left, right):
    """Compare complete FP32 tensors in bounded chunks, including Adam state."""
    if left["identity"] != right["identity"] or left["accumulated_microbatches"] != right["accumulated_microbatches"]:
        raise common.BenchmarkError("training state counters differ")
    maps = [{slot["canonical_name"]: slot for slot in receipt["slots"]} for receipt in (left, right)]
    if any(len(mapping) != len(receipt["slots"]) for mapping, receipt in zip(maps, (left, right))) or maps[0].keys() != maps[1].keys():
        raise common.BenchmarkError("trainable parameter sets differ")
    arrays = []
    for receipt in (left, right):
        path = Path(receipt["snapshot"])
        if path.stat().st_size != receipt["size_bytes"] or receipt["size_bytes"] > 4 * 1024**3:
    ...
    for name, ls in maps[0].items():
        ...
        for index, field in enumerate(FIELDS):
            ...
            for first in range(0, n, 262144):
                ...
                for arr, slot in zip(arrays, (ls, rs)):
                    ...
                    if len(piece) != count or not np.isfinite(piece).all():
                        raise common.BenchmarkError("non-finite or truncated training tensor")
                    pieces.append(piece)
                bitwise_mismatches += int(np.count_nonzero(
                ...
                if worst is None or local_error > max_error:
                    worst = {"flat_index": first + local_index,
            ...
            passed = max_error <= allowed and math.sqrt(square_error) <= absolute * math.sqrt(n) + relative * math.sqrt(square_reference)
            metrics[field] = {"max_absolute_error": max_error, "reference_max_absolute": max_reference,
                              "l2_error": math.sqrt(square_error), "reference_l2": math.sqrt(square_reference),
                              "worst_element": worst, "passed": passed,
                              "bitwise_equal": bitwise_mismatches == 0,
                              "bitwise_mismatches": bitwise_mismatches}
            if not passed:
                failures.append(f"{name}.{field}: max_error={max_error:g}, allowed={allowed:g}")
        if not metrics["weight"]["passed"]:
            ...
            metrics["weight"]["state_at_worst"] = {
                arm: {field: float(arr[slot["offset"] // 4 + index * n + coordinate])
                      for index, field in enumerate(FIELDS)}
                for arm, arr, slot in zip(("native", "python"), arrays, (ls, rs))}
        summary[name] = metrics
    del arrays
    return {"passed": not failures, "parameters": len(summary), "elements_per_state_field": sum(x["elements"] for x in maps[0].values()),
            "tolerances_absolute_relative": TOLERANCES, "failures": failures, "tensors": summary}


def compare_steps(native, python):
    report = native["report"]
    if report["zero_loss_fallback"] or report["terms"] is None or report["examples"] != python["examples"]:
        raise common.BenchmarkError("training step lost supervision or changed batch size")
    counts = native["cuda_transfers"]
    memory = native.get("cuda_allocations")
    if memory is not None and not 0 <= memory["live"] <= memory["peak"] <= memory["limit"]:
    ...
    if (optimizer["optimizer_stepped"] != python["optimizer_stepped"]
            or optimizer["identity"] != {"optimizer_step": python["optimizer_step"], "microbatch_step": python["microbatch_step"]}):
        raise common.BenchmarkError("optimizer update identity differs")
    terms = report["terms"]
    if terms.keys() != python["terms"].keys():
        raise common.BenchmarkError("training loss components differ; an auxiliary loss may have been dropped")
    differences = {}
    for name, expected in python["terms"].items():
        ...
        differences[name] = {"native": actual, "python": expected, "absolute_error": abs(actual - expected),
                             "passed": abs(actual - expected) <= 2e-4 + 1e-3 * abs(expected)}
    return {"passed": all(x["passed"] for x in differences.values()), "terms": differences}


def run(args):
    if args.output.exists() or not args.native_bin.is_file() or args.batch_size not in (1, 2, 4, 8) or not 2 <= args.pairs <= 30 or not 0 <= args.warmup <= 5:
    ...
    if not 0 <= args.qualification_updates <= 512 or (args.validate_only and args.qualification_updates):
    ...
    if type(encoder_forward_bytes) is not int or not 0 < encoder_forward_bytes <= 1024 * 1024**3:
    ...
    if cublas_library is not None and (not cublas_library.is_absolute() or not cublas_library.is_file()):
        raise common.BenchmarkError("training cuBLAS library must be an existing absolute file")
    cublas_identity = ({"path": str(cublas_library), "sha256": oracle.sha256_file(cublas_library)}
                       if cublas_library is not None else None)
    ...
    config = {"version": 1, "source_dir": str(args.model_dir), "train_file": str(dataset),
              "output_dir": str(args.output / "unused-job-output"), "execution": "resident_cuda",
              "run": {"mode": args.mode, "epochs": 2048, "batch_size": args.batch_size, "accumulation": 2,
                      "encoder_lr": 1e-5, "task_lr": 5e-4, "weight_decay": 0.01, "beta1": 0.9, "beta2": 0.999,
                      "epsilon": args.adam_epsilon, "max_grad_norm": 1, "scheduler": "constant", "warmup_steps": 0, "seed": 42, "shuffle": False},
              "gold_start": 1, "gold_end": 1,
              # The multilingual tokenizer needs the source owner's normal
              # auxiliary allowance; the small/base fixture override is lower.
              "source_limits": {"max_auxiliary_bytes": (384 if args.model == "multi" else 128) * 1024**2},
              "memory": {"host_bytes": 5 * 1024**3},
              "training_limits": {"encoder": {"max_forward_tensor_bytes": encoder_forward_bytes},
                                  "differentiation": {"max_tape_bytes": 2 * 1024**3}, "resident": {"program": {"max_working_bytes": 2 * 1024**3}}}}
    config_path = args.output / "config.json"
    oracle.write_json(config_path, config)
    env = dict(os.environ)
    env.update({name: "1" for name in common.THREAD_ENV})
    env.update(PYTHONDONTWRITEBYTECODE="1", TOKENIZERS_PARALLELISM="false", HF_HUB_OFFLINE="1", TRANSFORMERS_OFFLINE="1", TORCHINDUCTOR_COMPILE_THREADS="1")
    ...
    snapshots = {arm: args.snapshot_root / f"{args.output.name}-{arm}.bin" for arm in ("native", "python")}
    if any(p.exists() for p in snapshots.values()):
        raise common.BenchmarkError("snapshot destination already exists")
    commands = {"native": [str(args.native_bin), "--config", str(config_path), "--snapshot", str(snapshots["native"])],
                "python": [sys.executable, str(HERE / "training_cuda_worker.py"), "--config", str(config_path), "--snapshot", str(snapshots["python"]),
                           "--upstream", str(args.upstream), "--profile", args.python_profile, "--model", args.model]}
    evaluation_layout = args.output / "evaluation-layouts.json"
    if args.evaluate:
        commands["python"] += ["--evaluation-layout", str(evaluation_layout),
                               "--validation-file", str(validation_path), "--validation-sha256", oracle.sha256_file(validation_path)]
    guard = ResourceGuard(11 * 1024**3, max_processes=1024)
    workers, report = {}, {"scope": SCOPE, "status": "running", "performance_qualified": False,
                          "profile": args.python_profile, "model": args.model, "mode": args.mode, "batch_size": args.batch_size, "accumulation": 2,
                          "qualification_updates": args.qualification_updates,
                          "heldout_scope": ("user-supplied validation" if args.training_file else "two existing synthetic validation examples") + "; both weight sets evaluated in pinned Python CUDA" if args.evaluate else None,
                          "validation_fixture_sha256": oracle.sha256_file(validation_path),
                          "dataset_paths": {"training": str(original), "validation": str(validation_source)},
                          "dataset_counts": {"training": len(source_rows), "validation": len(validation_rows)},
                          "optimizer_config": {key: config["run"][key] for key in
                                               ("encoder_lr", "task_lr", "weight_decay", "beta1", "beta2", "epsilon", "max_grad_norm")},
                          "native_resource_limits": {key: config[key] for key in ("memory", "source_limits", "training_limits")},
                          "native_cublas_library": cublas_identity,
                          "deterministic_overrides": {"dropout": 0, "schema_augmentation": False, "negative_query_sampling": False, "gold_injection_probability": 1, "shuffle": False},
                          ...
                          "excluded": ["loading", "initial_graph_compilation", "snapshots", "JSON", "checkpoint_IO"],
                          "source": provenance, "model_bundle": bundle, "fixture_sha256": oracle.sha256_file(original),
                          "derived_dataset_sha256": oracle.sha256_file(dataset), "commands": commands,
                          "native_binary_sha256": oracle.sha256_file(args.native_bin),
                          "helpers_sha256": {name: oracle.sha256_file(HERE / name) for name in (Path(__file__).name, "training_cuda_worker.py", "training_cuda_quality.py", "training_cuda_trace.py", "oracle.py", "metal_python_worker.py", "metal_benchmark_supervisor.py", "benchmark_cpu.py")},
                          "diagnostic_throughput": args.diagnostic_throughput,
                          "parity_failures": [], "readiness": {}, "validation": [], "pairs": [], "cleanup": {}}
    def save():
    ...
    def pair_step(op, order=("native", "python")):
        ...
        for arm in order:
            ...
            with (args.output / "steps.jsonl").open("a") as journal:
                journal.write(json.dumps({"op": op, "response": results[arm]}, allow_nan=False) + "\n")
        comparison = compare_steps(results["native"], results["python"])
        return {"responses": results, "comparison": comparison}
    def inspect(label):
        receipts = {arm: response(workers[arm], "validate", "snapshot", args.timeout) for arm in workers}
        ...
        if args.evaluate and label in ("initial", "final", "microbatch-2"):
            ...
            if label == "initial":
                report["initial_quality"] = {arm: quality[arm]["metrics"] for arm in ("native", "python")}
            quality["regressions_from_initial"] = [f"{arm}.{task}" for arm in ("native", "python")
                                                    for task, metric in quality[arm]["metrics"].items()
                                                    if metric["f1"] < report["initial_quality"][arm][task]["f1"]]
            report.setdefault("heldout", {})[label] = {"metrics": {arm: quality[arm]["metrics"] for arm in ("native", "python")},
                                                       "regressions_from_initial": quality["regressions_from_initial"]}
            try:
                common.require_equal(quality["python"]["outputs"], quality["native"]["outputs"])
                quality["passed"] = True
            ...
            if not quality["passed"]:
                parity_failure(f"{label} held-out extraction parity failed: {quality['error']}")
        for path in snapshots.values():
            path.unlink()
        if not comparison["passed"]:
            parity_failure(f"{label} tensor parity failed: " + "; ".join(comparison["failures"][:5]))
    ...
    try:
        for arm in ("python", "native"):
            ...
            if ready.get("scope") != SCOPE or ready.get("dtype") != "float32" or ready.get("dropout") != 0 or ready.get("dataset_sha256") != report["derived_dataset_sha256"] or ready.get("config_sha256") != oracle.sha256_file(config_path):
            ...
            if arm == "native":
                if cublas_identity is not None and (type(ready.get("cublas_version")) is not int or ready["cublas_version"] <= 0):
                    raise common.BenchmarkError("native worker did not identify the configured cuBLAS runtime")
                expected_source = {"backbone": args.model, "precision": "fp32", "weight": bundle["files"]["model.safetensors"],
                                   "sidecars": [bundle["files"][name] for name in ("config.json", "encoder_config/config.json", "tokenizer.json", "tokenizer_config.json")]}
                if ready.get("source") != expected_source:
                    raise common.BenchmarkError("native source differs from the pinned Python bundle")
            elif ready.get("model_bundle") != bundle:
                raise common.BenchmarkError("Python source differs from the pinned bundle")
            report["readiness"][arm] = ready
            save()
        report["initial_state_parity"] = inspect("initial")
        for micro in range(2):
            inputs = {arm: response(workers[arm], "validate", "inputs", args.timeout) for arm in workers}
            oracle.write_json(args.output / f"inputs-{micro}.json", inputs)
            if inputs["native"]["input_ids"] != inputs["python"]["input_ids"] or inputs["native"]["encoder_shape"] != inputs["python"]["encoder_shape"]:
            ...
            report[f"validation_{micro}_tensor_parity"] = inspect(f"microbatch-{micro + 1}")
        ...
        if not args.validate_only:
            ...
            for number in range(1, args.pairs + 1):
                order = common.paired_benchmark.balanced_pair_order(number, "native", "python")
                samples = [pair_step("run", order) for _ in range(2)]
                row = {"pair": number, "order": order, "microbatches": samples,
                       **{arm + "_ns": sum(x["responses"][arm]["duration_ns"] for x in samples) for arm in workers}}
            ...
            for update in range(1, args.qualification_updates + 1):
                for _ in range(2):
                    ...
                    if not result["comparison"]["passed"]:
                        parity_failure(f"qualification update {update} loss parity failed")
                if update % 10 == 0:
                    report[f"qualification_{update}_tensor_parity"] = inspect(f"qualification-{update}")
                    save()
            report["final_state_parity"] = inspect("final")
            pairs = [(row["native_ns"], row["python_ns"]) for row in report["pairs"]]
            report["comparison"] = {"native_over_python_latency": common.paired_benchmark.paired_log_ratio_ci(pairs, samples=2000),
                                    **{arm + "_examples_per_second": common.paired_benchmark.distribution(2 * args.batch_size * 1e9 / row[arm + "_ns"] for row in report["pairs"]) for arm in workers}}
        for w in workers.values():
            w.request("stop", timeout=args.timeout)
        if oracle.sha256_file(args.native_bin) != report["native_binary_sha256"] or any(oracle.sha256_file(HERE / name) != value for name, value in report["helpers_sha256"].items()):
            raise common.BenchmarkError("benchmark implementation changed during measurement")
        if cublas_identity is not None and oracle.sha256_file(cublas_library) != cublas_identity["sha256"]:
            raise common.BenchmarkError("configured training cuBLAS library changed during measurement")
        passed = not report["parity_failures"]
        report.update(status="complete" if passed else "diagnostic_complete", parity_validated=passed)
        if "comparison" in report:
            report["comparison"]["interpretation"] = "parity_passed" if passed else "diagnostic_only_parity_failed"
    except BaseException as exc:
        report.update(status="failed", parity_validated=False, error=f"{type(exc).__name__}: {exc}")
        raise
    finally:
        cleanup_errors = []
        ...
        if cleanup_errors:
            report.update(status="failed", parity_validated=False, cleanup_errors=cleanup_errors)
        save()
...
def main():
    ...
    p.add_argument("--qualification-updates", type=int, default=0,
                   help="additional untimed optimizer updates, 0–512, with complete state checks every ten")
    p.add_argument("--evaluate", action="store_true", help="compare held-out extraction and gold metrics using the existing validation fixture")
    p.add_argument("--training-file", type=Path, help="optional bounded annotated JSONL; requires a disjoint validation file")
    p.add_argument("--validation-file", type=Path)
    p.add_argument("--timeout", type=float, default=600)
    p.add_argument("--validate-only", action="store_true")
    p.add_argument("--diagnostic-throughput", action="store_true",
