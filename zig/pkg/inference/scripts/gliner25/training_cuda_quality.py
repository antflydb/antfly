...
def require_disjoint(training, validation):
    ...
    def texts(rows):
        return {
            " ".join(unicodedata.normalize("NFKC", row["text"]).casefold().split())
            for row in rows
        }

    if {row["id"] for row in training} & {row["id"] for row in validation} or texts(
        training
    ) & texts(validation):
...
def canonical_parameters(model, inventory=None):
    ...
    if (
        len({name for name, _ in canonical}) != len(canonical)
...
def load_weights(model, receipt, torch, inventory=None):
    ...
    if (
        ...
        or not 0 < receipt["size_bytes"] <= 4 * 1024**3
    ):
        raise ValueError("invalid evaluation snapshot size")
    selected = {
        name: value
        for name, value in canonical_parameters(model, inventory)
        if value.requires_grad
    }
    slots = receipt["slots"]
    if (
        len(slots) != len(selected)
        or {s["canonical_name"] for s in slots} != selected.keys()
    ):
    ...
    with path.open("rb") as source:
        ...
        for slot in slots:
            ...
            n = parameter.numel()
            if (
                slot["shape"] != list(parameter.shape)
                or slot["elements"] != n
                or slot["offset"] != offset
                or offset + n * 16 > receipt["size_bytes"]
            ):
                raise ValueError("invalid evaluation snapshot layout")
            for start in range(offset // 4, offset // 4 + n, 262144):
                if not np.isfinite(
                    data[start : min(start + 262144, offset // 4 + n)]
                ).all():
        ...
        with torch.no_grad():
            for slot in slots:
                values = np.array(
                    data[slot["offset"] // 4 : slot["offset"] // 4 + slot["elements"]],
                    copy=True,
                )
                selected[slot["canonical_name"]].copy_(
                    torch.from_numpy(values).reshape(slot["shape"])
                )
        del data
...
def gold_facts(row):
    ...
    result = {
        name: Counter()
        for name in ("entities", "classifications", "records", "relations")
    }
    ...
    for item in row.get("relations", []):
        result["relations"][
            (
                item["type"],
                entities[item["head"]["entity"]],
                entities[item["tail"]["entity"]],
            )
        ] += 1
    return result
...
def predicted_facts(output):
        ...
        for record in group["instances"]:
            fields = tuple(
                sorted(
                    (
                        field["name"],
                        tuple(sorted(value["text"] for value in field["values"])),
                    )
    ...
    for edge in output["relations"]:
        result["relations"][
            (edge["name"], edge["head"]["text"], edge["tail"]["text"])
        ] += 1
    return result
            ...
            "structures": {
                name: {
                    **spec,
                    "fields": [
                        {"name": field, **settings}
                        for field, settings in spec["fields"].items()
                    ],
                }
    ...
    try:
        ...
        with torch.inference_mode():
            for row in rows:
                ...
                output = common.canonical_python(
                    request,
                    common.execute_python(
                        model, request, json.dumps(request["schema"])
                    ),
                )
