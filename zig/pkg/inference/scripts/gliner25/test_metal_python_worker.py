...
class Model:
    def __init__(self, device="mps", events=None):
        ...
        self.buffers = [
            ("positions", Tensor(device=device, dtype="int64")),
...
class MetalPythonWorkerTest(unittest.TestCase):
    def setUp(self):
        ...
        self.args = types.SimpleNamespace(
            ...
            upstream=Path("/unused/source"),
            max_commands=100,
        )
        self.bundle = {
            "model_id": "pinned/small",
            "revision": "abc",
            "files": {"model": {"sha256": "1" * 64}},
        }
        self.requests, self.requests_sha256 = worker.read_requests(
    ...
    def test_import_time_flags_reject_late_torch_and_inherited_relaxations(self):
        ...
        self.assertEqual(
            {
                **{key: "0" for key in worker.MPS_ENV},
        ...
        for flag in worker.MPS_ENV + bench.THREAD_ENV:
            with self.subTest(flag=flag), self.assertRaises(worker.WorkerError):
                worker.configure_environment(
                    {flag: "1" if flag in worker.MPS_ENV else "8"}, {}
                )
        ...
        with (
            mock.patch.dict(os.environ, {"PYTORCH_ENABLE_MPS_FALLBACK": "1"}),
            self.assertRaises(worker.WorkerError),
        ):
            worker.check_environment()

    def test_load_is_local_strict_fp32_eval_explicit_device_without_quantization_or_compile(
        self,
    ):
        for device in ("cpu", "mps"):
            ...
            loaded = worker.load_model(
                types.SimpleNamespace(from_pretrained=loader),
                Path("/unused/model"),
                device,
            )
    ...
    def test_ready_corrects_cpu_provenance_and_reports_actual_policy_and_allocator(
        self,
    ):
        provenance = {"device": "cpu", "runtime": {"torch": "pinned"}}
        ready = worker.ready_event(
            self.args, Model(), Torch(), self.bundle, provenance, self.requests_sha256
        )
        ...
        self.assertEqual(
            {
                "current_allocated_bytes": 32,
                "driver_allocated_bytes": 64,
                "recommended_max_bytes": 128,
            },
            ready["mps_memory"],
        )

    def test_readiness_rejects_wrong_devices_dtypes_training_and_missing_parameters(
        self,
    ):
        for tensor_set, tensor in (
            ("parameters", Tensor(device="cpu")),
            ("buffers", Tensor(device="cpu", dtype="int64")),
            ("buffers", Tensor(dtype="float64")),
            ("parameters", Tensor(dtype="float16")),
            ("parameters", Tensor(dtype="int64")),
            ("buffers", Tensor(dtype="complex64")),
        ):
            model = Model()
            setattr(model, tensor_set, [("wrong", tensor)])
            with (
                self.subTest(
                    tensors=tensor_set, dtype=tensor.dtype, device=tensor.device
                ),
                self.assertRaises(worker.WorkerError),
            ):
    ...
    def test_mps_clock_includes_decode_and_last_fence_but_excludes_first_fence_and_memory(
        self,
    ):
        events = []
        torch = Torch(events)
        ...
        def extract():
            ...
            events.extend(
                ("schema", "encoder", "heads", "host_decode", "temporaries_released")
            )
            return {"value": 7}
    ...
    def test_cpu_clock_never_touches_mps(self):
        ...
        output, duration = worker.timed_call(
            torch, "cpu", lambda: "host result", clock=lambda: next(ticks)
        )
    ...
    def test_failed_case_is_drained_before_reuse_but_has_no_timing_sample(self):
        ...
        with self.assertRaises(NotImplementedError):
            worker.timed_call(
                torch, "mps", unsupported, clock=lambda: events.append("clock") or 1
            )
        self.assertEqual(
            [
                "inference_enter",
        ...
        torch.mps.synchronize = mock.Mock(
            side_effect=RuntimeError("command buffer failed")
        )
        with self.assertRaises(worker.DeviceSynchronizationError):
            worker.timed_call(torch, "mps", lambda: None, clock=lambda: 1)

    def test_fallback_warning_including_unconditional_svd_is_fatal_but_host_decode_is_allowed(
        self,
    ):
        for operator in ("aten::embedding_renorm_", "aten::linalg_svd", "aten::other"):
            ...
            with (
                ...
                self.assertRaises(UserWarning) as caught,
            ):
        ...
        with warnings.catch_warnings(record=True) as captured:
            with worker.reject_mps_fallback():
                warnings.warn(
                    "Using eager attention because SDPA is unavailable", UserWarning
                )
    ...
    def test_validation_captures_direct_encoder_ids_devices_and_removes_all_hooks(self):
        for positional in (False, True):
            ...
            with worker.capture_validation_inputs(model, torch, "mps") as (
                captured,
                devices,
            ):
                if positional:
    ...
    def test_input_device_rejection_precedes_validation_readback_and_bad_tokens_do_not_pass(
        self,
    ):
        for tensor in (
            Tensor(device="cpu", dtype="int64"),
            Tensor(device="mps:1", dtype="int64"),
            ...
            model = Model()
            with (
                self.subTest(device=tensor.device, dtype=tensor.dtype),
                self.assertRaises(worker.WorkerError),
            ):
                with worker.capture_validation_inputs(model, Torch(), "mps"):
        ...
        with self.assertRaises(worker.WorkerError):
            with worker.capture_validation_inputs(model, Torch(), "mps"):
                model.encoder(
                    input_ids=Tensor(dtype="int64"),
                    attention_mask=Tensor(device="cpu", dtype="bool"),
                )
    ...
    def serve(self, commands, *, execute=None, model=None, torch=None, canonical=None):
        ...
        with (
            mock.patch.object(
                oracle, "verify_model_dir", return_value=self.bundle
            ) as model_pin,
            mock.patch.object(oracle, "verify_upstream_checkout") as source_pin,
        ):
            code = worker.serve_commands(
                self.args,
                model,
                torch,
                self.bundle,
                ...
                emit=events.append,
                execute=execute,
                canonical=canonical or (lambda request, output: output),
                clock=lambda: next(ticks),
            )
        return code, events, model_pin, source_pin

    def test_validate_run_stop_protocol_reuses_source_execution_and_omits_timed_hooks(
        self,
    ):
        ...
        self.assertEqual(
            ["result", "result", "stopped"], [event["event"] for event in events]
        )
    ...
    def test_recoverable_unsupported_case_retains_identity_and_does_not_drop_later_cases(
        self,
    ):
        ...
        self.assertEqual(0, code)
        self.assertEqual(
            ["error", "result", "stopped"], [row["event"] for row in events]
        )
        self.assertEqual(
            (1, bad, "unsupported_operation", True),
            tuple(
                events[0][key]
                for key in ("request_id", "case_id", "category", "recoverable")
            ),
        )
        self.assertNotIn("duration_ns", events[0])

    def test_device_oom_fallback_and_nonfinite_output_never_become_successful_samples(
        self,
    ):
        case = self.requests[0]["id"]
    ...
    def test_case_cannot_run_before_input_validation_or_when_strict_extraction_was_disabled(
        self,
    ):
        case = self.requests[0]["id"]
        code, events, _, _ = self.serve(
            [{"op": "run", "request_id": 1, "case_id": case}]
        )
        self.assertEqual(1, code)
        ...
        model.strict_extraction = False
        code, events, _, _ = self.serve(
            [{"op": "validate", "request_id": 1, "case_id": case}], model=model
        )
        self.assertEqual(1, code)
    ...
    def test_protocol_invalid_identity_duplicate_keys_overflow_or_eof_fail_closed(self):
        for raw in (
            ...
            with (
                ...
                self.assertRaises((bench.BenchmarkError, oracle.ContractError)),
            ):
        ...
        with self.assertRaises(worker.WorkerError):
            self.serve(
                [
                    {"op": "validate", "request_id": 2, "case_id": case},
                    {"op": "run", "request_id": 2, "case_id": case},
                ]
            )
    ...
    def test_stop_rehashes_model_source_and_exact_requests_without_reporting_stopped_on_tamper(
        self,
    ):
        for changed in ("model", "source", "requests"):
            ...
            with (
                mock.patch.object(
                    oracle,
                    "verify_model_dir",
                    return_value={} if changed == "model" else self.bundle,
                ),
                mock.patch.object(
                    oracle,
                    "verify_upstream_checkout",
                    side_effect=oracle.ContractError("dirty source")
                    if changed == "source"
                    else None,
                ),
                mock.patch.object(
                    ...
                    return_value=(
                        self.requests,
                        "0" * 64 if changed == "requests" else self.requests_sha256,
                    ),
                ),
            ):
                with (
                    self.subTest(changed=changed),
                    self.assertRaises((worker.WorkerError, oracle.ContractError)),
                ):
                    worker.serve_commands(
                        self.args,
                        Model(),
    ...
    def test_fixed_requests_are_bounded_and_do_not_allow_duplicate_case_identity(self):
        ...
        self.assertEqual(
            oracle.sha256_file(oracle.FIXTURES / "requests.json"), self.requests_sha256
        )
    ...
    def test_unchanged_canonical_adapter_covers_all_thirty_pinned_task_outputs(self):
        ...
        for variant in ("small", "base", "multi"):
            capture = oracle.read_json(
                oracle.FIXTURES / f"{variant}_reference/capture.json"
            )
            expected = {
                row["id"]: row["expected"]
                for row in oracle.read_json(bench.case_fixture(variant))["cases"]
            }
            for row in capture["requests"]:
                with self.subTest(model=variant, case=row["id"]):
                    ...
                    bench.require_equal(
                        bench.canonical_result(expected[row["id"]]), output
                    )
