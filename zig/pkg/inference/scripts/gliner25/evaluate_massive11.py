...
def python_runtime_identity():
    ...
    if configuration.exists():
        with configuration.open("rb") as source:
            ...
            execution.check(
                stat.S_ISREG(before.st_mode) and 0 < before.st_size <= 65536,
                "Python environment configuration exceeds its byte budget",
            )
            data = source.read(65537)
            after = os.fstat(source.fileno())
        execution.check(
            len(data) == before.st_size == after.st_size
    ...
    execution.check(
        prefix == base_prefix or configuration_pin is not None,
        "virtual environment is missing pyvenv.cfg",
    )
    return {
        "scope": "gliner25_python_invocation/v1",
        "invocation": str(invocation),
        "executable": str(executable),
        "executable_sha256": oracle.sha256_file(executable),
        "prefix": prefix,
        "base_prefix": base_prefix,
        "pyvenv_cfg": configuration_pin,
        "environment": {
...
def python_worker_command(fixture_path, model_dir, upstream, identity):
    return [
        ...
        contract.digest(contract.encoded(identity)),
    ]
...
def offline_network():
    ...
    original_connect, original_connection = (
...
def execute_python_case(model, fixture, case, torch):
    ...
    if (
        len(list(model.processor.word_splitter(text, lower=False)))
        > execution.POLICY["max_words"]
    ):
        raise contract.EvaluationError("BoundaryTextLimitExceeded")
    ...
    if constrained:
        ...
        config = ClassificationConfig(
            ...
            batch_size=1,
            max_len=None,
            exact_node_budget=execution.LIMITS["exact_node_budget"],
            beam_size=execution.LIMITS["beam_width"],
            candidate_threshold=0.5,
            max_candidates_per_task=execution.LIMITS["max_candidates_per_task"],
    ...
    with bench.capture_encoder_input_ids(model) as captured, torch.inference_mode():
        if constrained:
            output = classifier.decode(
                classifier.score(text, schema, config=config), schema, config=config
            ).to_dict()
        else:
    ...
    execution.check(
        captured == [expected_ids],
        "actual encoder differs from untruncated preparation",
    )
    return {
        "event": "result",
        "case_id": case["id"],
...
def python_worker(args):
    ...
    execution.check(
        contract.is_digest(args.python_runtime_sha256)
        and contract.digest(contract.encoded(python_runtime))
    ...
    for name in bench.THREAD_ENV:
        execution.check(
            os.environ.get(name) == "1", "worker thread budget must be explicit"
        )
    provenance, torch = oracle.prepare_runtime(args.upstream)
    torch.set_num_interop_threads(1)
    artifact = evaluate.model_artifact(fixture["model"], args.model_dir)
    execution.check(
        artifact["kind"] == "source_fp32", "Python requires original FP32 checkpoint"
    )
    ...
    with offline_network():
        ...
        bench.emit(
            {
                ...
                "word_splitter": fixture["options"]["word_splitter"],
        ...
        for case in fixture["cases"]:
            try:
                ...
                code = (
                    str(error)
                    if isinstance(error, contract.EvaluationError)
                    and str(error).startswith("Boundary")
            ...
            bench.emit(response)
    execution.check(
        evaluate.model_artifact(fixture["model"], args.model_dir) == artifact
        and oracle.sha256_file(args.evaluation_fixture) == fixture_sha
        and execution.contract_files() == closure
        and python_runtime_identity() == python_runtime,
...
def validate_ready(ready, backend, artifact, fixture, fixture_sha, python_runtime=None):
    expected = {
        ...
        "registry_sha256": fixture["registry_sha256"],
        "word_splitter": fixture["options"]["word_splitter"],
        "weight_precision": artifact["precision"],
        "activation_precision": "f32",
        "accumulation_precision": "f32",
        "head_precision": "f32",
        "math_policy": "torch_f32_cpu_v1"
        if backend == "python"
        else comparison.MATH_POLICY,
    }
    execution.check(
        isinstance(ready, dict)
        and all(ready.get(key) == value for key, value in expected.items()),
        "worker artifact, shard, splitter or math identity differs",
    )
    if backend == "python":
        execution.check(
            ready.get("threads") == ready.get("interop_threads") == 1
            and ready.get("provenance", {}).get("commit") == oracle.UPSTREAM_COMMIT,
            "oracle dependency/thread profile differs",
        )
        execution.check(
            isinstance(python_runtime, dict)
            and ready.get("python_runtime") == python_runtime,
            "worker Python invocation or environment differs",
        )
    else:
        execution.check(
            ready.get("build_mode") == "ReleaseFast",
            "evaluation requires complete ReleaseFast build graph",
        )
...
def write_line(target, value):
    ...
    execution.check(
        len(data) <= execution.MAX_EVENT_BYTES
        and target.tell() + len(data) <= execution.MAX_SHARD_BYTES,
...
def wait_for_exit(worker, guard):
    ...
    while worker.process.poll() is None:
        guard.check()
        execution.check(
            time.monotonic() < deadline, "worker failed to exit after completion"
        )
        time.sleep(0.05)
    execution.check(
        worker.process.returncode == 0
        and not worker.buffer
        and not worker.process.stdout.read(1),
...
def evidence_rows(path: Path, expected, limit):
    ...
    with source_path.open("rb") as source:
        while line := source.readline(execution.MAX_EVENT_BYTES + 1):
            size += len(line)
            execution.check(
                len(line) <= execution.MAX_EVENT_BYTES
                and line.endswith(b"\n")
                and size <= limit
                and len(rows) < execution.RECORDS,
                "evidence row/count budget exceeded",
            )
            digest.update(line)
            ...
            rows.append(value)
    execution.check(
        size == expected["size_bytes"] and digest.hexdigest() == expected["sha256"],
...
def reference_report(path, entry, root, expected_backend, artifact=None):
    ...
    execution.check(
        report.get("scope") == execution.AGGREGATE_SCOPE
        and report.get("status") == "complete"
        and report.get("qualification") is False
        and report.get("backend") == expected_backend
        and report.get("profile") == entry["profile"]["id"]
        and report.get("lock_sha256") == entry["lock_sha256"]
        and report.get("contract_files") == execution.contract_files()
        and report.get("denominator") == execution.RECORDS
        and report.get("policy") == execution.POLICY
        and report.get("limits") == execution.LIMITS
        and report.get("registry_sha256") == oracle.sha256_file(execution.REGISTRY)
        and report.get("prepared_sha256") == entry["prepared_sha256"],
        "reference aggregate is incomplete or has different execution identity",
    )
    execution.check(
        isinstance(report.get("shards"), list) and len(report["shards"]) == 3,
        "reference does not contain all three shard identities",
    )
    for index, shard in enumerate(report["shards"]):
        execution.check(
            set(shard) == {"index", "report_sha256", "transport"}
            and shard["index"] == index
            and contract.is_digest(shard["report_sha256"])
            and shard["transport"]
            == {
                "global_records": execution.RECORDS,
    ...
    responses_path = read_evidence(
        path, report["files"]["responses"], 3 * execution.MAX_SHARD_BYTES
    )
    predictions_path = read_evidence(
        path, report["files"]["predictions"], 3 * execution.MAX_SHARD_BYTES
    )
    responses = evidence_rows(
        path, report["files"]["responses"], 3 * execution.MAX_SHARD_BYTES
    )
    predictions = evidence_rows(
        path, report["files"]["predictions"], 3 * execution.MAX_SHARD_BYTES
    )
    requests = list(
        contract.rows(root / entry["profile"]["id"] / "prepared/requests.jsonl")
    )
    execution.check(
        len(responses) == len(predictions) == len(requests) == execution.RECORDS,
        "reference coverage differs",
    )
    errors = 0
    for response, prediction, row in zip(responses, predictions, requests, strict=True):
        ...
        expected, _ = execution.response_prediction(
            fixture, case, response, expected_backend
        )
        execution.check(
            prediction == expected, "reference predictions differ from raw outputs"
        )
        errors += response["event"] == "error"
    execution.check(errors == report["errors"], "reference error denominator differs")
    metrics = contract.score(
        root / entry["profile"]["id"] / "prepared", predictions_path
    )
    execution.check(
        metrics["metrics"] == report["metrics"],
        "reference metrics differ from raw predictions",
    )
    return {
...
def references(args, entry, artifact):
    ...
    if args.backend == "python":
        execution.check(
            args.source_reference_report is None
            and args.native_reference_report is None,
            "Python source capture cannot accept prediction references",
        )
    else:
        execution.check(
            args.source_reference_report is not None,
            "native and Metal runs require completed FP32 Python reference",
        )
        source = reference_report(
            args.source_reference_report, entry, args.prepared_root, "python"
        )
        execution.check(
            source["report"]["model"] == args.model, "source reference model differs"
        )
    if args.backend == "metal":
        execution.check(
            args.native_reference_report is not None,
            "Metal requires exact-artifact native aggregate",
        )
        native = reference_report(
            args.native_reference_report, entry, args.prepared_root, "native", artifact
        )
        execution.check(
            native["report"]["model"] == args.model, "native reference model differs"
        )
    else:
        execution.check(
            args.native_reference_report is None,
            "same-artifact native reference only applies to Metal",
        )
...
def run_shard(args):
    execution.check(
        256 <= args.max_rss_mib <= 8192
        and 1 <= args.startup_timeout <= 600
        and 121 <= args.response_timeout <= 180,
        "invalid bounded process limits",
    )
    ...
    execution.check(
        args.backend != "python" or artifact["kind"] == "source_fp32",
        "Python only supports original FP32",
    )
    source, native = references(args, entry, artifact)
    python_runtime = python_runtime_identity() if args.backend == "python" else None
    binary = (
        Path(python_runtime["executable"])
        if python_runtime is not None
        else args.binary
    )
    execution.check(
        binary is not None and binary.is_file(), "native evaluation binary is required"
    )
    binary = binary.resolve()
    binary_sha = oracle.sha256_file(binary)
    if native is not None:
        execution.check(
            native["report"]["binary_sha256"] == binary_sha,
            "cross-backend comparison requires the identical binary",
        )
    ...
    if args.backend == "python":
        command = python_worker_command(
            fixture_path, args.model_dir, args.upstream, python_runtime
        )
    else:
        command = [
            str(binary),
    ...
    report = {
        "scope": execution.SHARD_SCOPE,
        "status": "incomplete",
        "qualification": False,
        "backend": args.backend,
        "model": args.model,
        "artifact": artifact,
        "contract_files": closure,
        ...
        "transport": fixture["transport"],
        "denominator": len(fixture["cases"]),
        "processed": 0,
        "errors": 0,
        "resource_policy": {
            "max_worker_rss_bytes": args.max_rss_mib * 1024**2,
            "startup_timeout_seconds": args.startup_timeout,
            "response_timeout_seconds": args.response_timeout,
            "threads": 1,
        },
        "source_reference_report_sha256": source["sha256"] if source else None,
        "native_reference_report_sha256": native["sha256"] if native else None,
        "comparisons": [],
    }
    ...
    try:
        ...
        with (
            (output / "responses.jsonl").open("xb") as responses,
            (output / "predictions.jsonl").open("xb") as predictions,
        ):
            for local_index, case in enumerate(fixture["cases"]):
                response = worker.receive(args.response_timeout)
                prediction, _ = execution.response_prediction(
                    fixture, case, response, args.backend
                )
                ...
                row = {
                    "case_id": case["id"],
                    "input_ids_u32_le_sha256": None
                    if response["event"] == "error"
                    else contract.digest(
                        b"".join(
                            token.to_bytes(4, "little")
                            for token in response["input_ids"]
                        )
                    ),
                }
                global_index = fixture["transport"]["start"] + local_index
                for name, reference, backend in (
                    ("source_fp32", source, "python"),
                    ("same_artifact_native", native, "native"),
                ):
                    if reference is not None:
                        row[name] = execution.compare_response(
                            fixture,
                            case,
                            response,
                            args.backend,
                            reference["responses"][global_index],
                            backend,
                        )
        ...
        execution.check(
            evaluate.model_artifact(args.model, args.model_dir) == artifact
            and oracle.sha256_file(binary) == binary_sha
            and execution.contract_files() == closure
            and oracle.sha256_file(fixture_path) == fixture_sha,
            "model, binary, fixture or helper changed during execution",
        )
        execution.check(
            python_runtime is None or python_runtime_identity() == python_runtime,
        ...
        for path, reference in (
            (args.source_reference_report, source),
            (args.native_reference_report, native),
        ):
            if reference is not None:
                execution.check(
                    oracle.sha256_file(path) == reference["sha256"],
                    "reference report changed during execution",
                )
    ...
    report["files"] = {
        name: execution.pin(output / filename)
        for name, filename in (
            ("fixture", "requests.fixture.json"),
...
def shared_identity(report):
    keys = (
        ...
        "limits",
        "resource_policy",
        "source_reference_report_sha256",
...
def validate_shard_set(reports, entry):
    execution.check(
        len(reports) == len(execution.RANGES),
        "aggregate needs all three shards exactly once",
    )
    ...
    for index, report in enumerate(reports):
        ...
        execution.check(
            report.get("scope") == execution.SHARD_SCOPE
            and report.get("status") == "complete"
            and report.get("qualification") is False
            and shared_identity(report) == identity
            and report.get("transport") == expected
            and report.get("processed")
            == report.get("denominator")
            == expected["end"] - expected["start"]
            and report.get("unprocessed") == 0
            and type(report.get("errors")) is int
            and 0 <= report["errors"] <= report["denominator"],
            "incomplete, duplicated, reordered or differently configured shard",
        )
    execution.check(
        identity["contract_files"] == execution.contract_files()
        and identity["profile"] == entry["profile"]["id"]
        and identity["registry_sha256"] == oracle.sha256_file(execution.REGISTRY)
        and identity["lock_sha256"] == entry["lock_sha256"]
        and identity["prepared_sha256"] == entry["prepared_sha256"]
        and identity["policy"] == execution.POLICY
        and identity["limits"] == execution.LIMITS,
...
def aggregate(args):
    ...
    for key, reference in (
        ("source_reference_report_sha256", source),
        ("native_reference_report_sha256", native),
    ):
        execution.check(
            identity[key] == (reference["sha256"] if reference else None),
            "aggregate reference differs from shard execution",
        )
    output = args.output_dir.resolve()
    output.mkdir(mode=0o700, parents=True, exist_ok=False)
    ...
    try:
        with (
            (output / "responses.jsonl").open("xb") as responses_target,
            (output / "predictions.jsonl").open("xb") as predictions_target,
        ):
            for index, (path, report) in enumerate(
                zip(args.shard_report, reports, strict=True)
            ):
                report_sha = oracle.sha256_file(path)
                fixture = execution.read_json(
                    read_evidence(
                        path, report["files"]["fixture"], execution.MAX_FIXTURE_BYTES
                    ),
                    execution.MAX_FIXTURE_BYTES,
                )
                execution.check(
                    fixture
                    == execution.make_fixture(
                        entry, requests, cases, identity["model"], index
                    )
                    and report["fixture_sha256"]
                    == report["files"]["fixture"]["sha256"],
                    "shard fixture differs from approved global request range",
                )
                validate_ready(
                    report["ready"],
                    identity["backend"],
                ...
                response_path = read_evidence(
                    path, report["files"]["responses"], execution.MAX_SHARD_BYTES
                )
                prediction_path = read_evidence(
                    path, report["files"]["predictions"], execution.MAX_SHARD_BYTES
                )
                responses = evidence_rows(
                    path, report["files"]["responses"], execution.MAX_SHARD_BYTES
                )
                ...
                execution.check(
                    len(responses) == len(predictions) == len(fixture["cases"]),
                    "shard result coverage differs",
                )
                count = 0
                for local_index, (case, response, prediction) in enumerate(
                    zip(fixture["cases"], responses, predictions, strict=True)
                ):
                    expected, _ = execution.response_prediction(
                        fixture, case, response, identity["backend"]
                    )
                    execution.check(
                        prediction == expected,
                        "shard metrics were substituted after model execution",
                    )
                    ...
                    for target, value in (
                        ...
                        (predictions_target, prediction),
                    ):
                        data = contract.encoded(value) + b"\n"
                        execution.check(
                            target.tell() + len(data) <= 3 * execution.MAX_SHARD_BYTES,
                ...
                execution.check(
                    ...
                    == comparisons[
                        execution.RANGES[index][0] : execution.RANGES[index][1]
                    ],
                    "shard error accounting or parity evidence differs",
                )
                errors += count
                execution.check(
                    oracle.sha256_file(path) == report_sha
                    and oracle.sha256_file(response_path)
                    == report["files"]["responses"]["sha256"]
                    and oracle.sha256_file(prediction_path)
                    == report["files"]["predictions"]["sha256"],
                    "shard evidence changed while aggregating",
                )
                shards.append(
                    {
                        "index": index,
            ...
            durable_close(responses_target)
            durable_close(predictions_target)
        execution.check(
            len(comparisons) == execution.RECORDS, "aggregate omitted request results"
        )
        metrics = contract.score(
            args.prepared_root / args.profile / "prepared", output / "predictions.jsonl"
        )
        required = (
            "same_artifact_native"
            if native
            else "source_fp32"
            if source and identity["artifact"]["precision"] == "fp32"
            else None
        )
        result = {
            "scope": execution.AGGREGATE_SCOPE,
            "status": "complete",
            "qualification": False,
            **identity,
            "denominator": execution.RECORDS,
            "successful_results": execution.RECORDS - errors,
            "errors": errors,
            "error_codes": dict(sorted(error_codes.items())),
            "shards": shards,
            "metrics_scope": "all_locked_rows_empty_facts_for_explicit_error_events"
            if errors
            else "all_locked_rows_successful_outputs",
            "metrics": metrics["metrics"],
            "comparisons": comparisons,
            "required_token_parity": None
            if source is None
            else {
                "pass": errors == 0
                and all(
                    row[name].get("token_ids_equal") is True
                    for row in comparisons
                    for name in (
                        ("source_fp32", "same_artifact_native")
                        if native
                        else ("source_fp32",)
                    )
                ),
            ...
            if required is None
                ...
                "reference": required,
                "pass": errors == 0
                and all(row[required]["parity_pass"] for row in comparisons),
                "confidence_absolute_tolerance": comparison.CONFIDENCE_TOLERANCE,
            },
            "files": {
                name: execution.pin(output / (name + ".jsonl"))
                for name in ("responses", "predictions")
            },
        }
        if source:
            result["source_fp32_quality_delta"] = evaluate.quality_delta(
                source["metrics"], metrics
            )
        names = (
            "intent_exact",
            "scenario_exact",
            "joint_intent_scenario",
            "constraints_satisfied",
        )
        result["direct_document_rates"] = {
            name: metrics["metrics"][name]["document_exact_match"]
            for name in names
            if name in metrics["metrics"]
        }
        execution.admit_profile(args.prepared_root, args.profile)
        execution.check(
            execution.contract_files() == identity["contract_files"],
            "helpers changed while aggregating",
        )
        ...
        execution.atomic_json(
            output / "failure.json",
            {
                ...
                "qualification": False,
                "denominator": execution.RECORDS,
                "driver_error": {"type": type(error).__name__, "message": str(error)},
            },
        )
        raise
...
def prepare_fixtures(args):
    ...
    result = {
        "scope": "gliner25_massive11_execution_preflight/v1",
        "qualification": False,
        "no_model_execution": True,
        "profile": args.profile,
        "model": args.model,
        "denominator": execution.RECORDS,
        "registry_sha256": oracle.sha256_file(execution.REGISTRY),
        "contract_files": execution.contract_files(),
        "files": files,
    }
    ...
    for name in ("prepare", "run-shard", "aggregate"):
        ...
        if name != "aggregate":
            command.add_argument(
                "--model", choices=("small", "base", "multi"), required=True
            )
        if name != "prepare":
        ...
        if name == "run-shard":
            ...
            command.add_argument(
                "--backend", choices=("python", "native", "metal"), required=True
            )
...
def main():
    ...
    if (
        ...
        or report.get("required_parity")
        and not report["required_parity"]["pass"]
        or report.get("required_token_parity")
        and not report["required_token_parity"]["pass"]
    ):
        raise SystemExit(1)
