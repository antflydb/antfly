...
TOKEN_EVIDENCE_SHA256 = (
    "b6c8d4d7ff5f6a944eac36e26bb258bd177ad6e08b4171f11583d75cee65fdc8"
)
...
POLICY = {
    ...
    "regular_templates": list(REGULAR),
    "ragged_templates": list(RAGGED),
    "ragged_batch_size": 4,
    "batch8_smoke_width": 128,
    "word_splitter": "whitespace",
    "threshold": 0.5,
    "max_text_words": MAX_WORDS,
    "max_encoded_tokens": MAX_TOKENS,
    "max_queries": 64,
    "padding_token_id": 0,
...
def files_for(variant):
    return {
        name: {key: item[key] for key in ("size_bytes", "sha256")}
...
def case_specs():
    ...
    return [
        (f"{name}_s{width}_b{batch}_{mode}", name, width, batch, mode)
...
def packet(sequences):
    if (
        ...
        or not 1 <= len(sequences) <= MAX_BATCH
        or any(
            not isinstance(ids, list)
            or not 1 <= len(ids) <= MAX_TOKENS
    ...
    return {
        "input_shape": [len(sequences), width],
        "input_ids": [
            token for ids in sequences for token in ids + [0] * (width - len(ids))
        ],
        "attention_mask": [
            bit
            for ids in sequences
            for bit in [1] * len(ids) + [0] * (width - len(ids))
        ],
    }
...
def validate_pair(native, source, variant, source_sha256):
    ...
    if (
        ...
        or set(native) != expected_top
        or type(native["format_version"]) is not int
        or native["format_version"] != 1
        or native["scope"] != SCOPE
        or native["source_commit"] != oracle.UPSTREAM_COMMIT
    ...
    if (
        ...
        != {
            ...
            "model",
            "policy",
            "original_requests_sha256",
            "token_evidence_sha256",
            "cases",
        }
        or type(source["format_version"]) is not int
        or source["format_version"] != 1
        or source["scope"] != SOURCE_SCOPE
        or source["model"] != variant
        or source["source_commit"] != oracle.UPSTREAM_COMMIT
        or source["policy"] != POLICY
        or source["original_requests_sha256"]
        != oracle.sha256_file(oracle.FIXTURES / "requests.json")
        or source["token_evidence_sha256"] != TOKEN_EVIDENCE_SHA256
    ):
        raise cpu.BenchmarkError("scaling source input identity differs")
    originals = {
        row["id"]: row
        for row in oracle.read_json(oracle.FIXTURES / "requests.json")["requests"]
    }
    specs = case_specs()
    if (
        not isinstance(native["cases"], list)
        or not isinstance(source["cases"], list)
        or len(native["cases"]) != len(specs)
        or len(source["cases"]) != len(specs)
    ):
        raise cpu.BenchmarkError("scaling matrix is incomplete")
    import generate_pipeline_cases as adaptation

    for native_case, source_case, (name, template, width, batch, mode) in zip(
        native["cases"], source["cases"], specs
    ):
        if set(native_case) != {
            "id",
            "schema",
            "items",
            "expected_encoded_lengths",
            "encoded_width",
        } or set(source_case) != {
            "id",
            "template_id",
        ...
        if (
            native_case["id"] != name
            or source_case["id"] != name
            or source_case["template_id"] != template
            or source_case["kind"] != original["kind"]
            ...
            or source_case["profile"]
            != {"width": width, "batch_size": batch, "mode": mode}
        ...
        if (
            ...
            or len(items) != batch
            or not ordered_equal(items, native_case["items"])
        ):
        ...
        lengths = (
            [max(minimum, width * n // 4) for n in (4, 3, 2, 1)]
            if mode == "ragged"
            else [width] * batch
        )
        ...
        if (
            native_case["expected_encoded_lengths"] != lengths
        ...
        for index, item in enumerate(items):
            if (
                ...
                or not 1 <= len(item["text"].encode()) <= 65536
                or not item["text"].endswith(".")
                or original["text"] not in item["text"]
            ):
                raise cpu.BenchmarkError(
                    "scaling text/identity or original source sentence differs"
                )
...
def load(prepared: Path, variant: str):
    ...
    native_path, source_path = (
        prepared / f"scaling_{variant}.json",
        prepared / f"source_{variant}.json",
    )
    ...
    return {
        "model": variant,
        "case_path": native_path,
        "source_path": source_path,
        "prepared": prepared,
        "cases": list(cases),
        "source_cases": cases,
        "requests_sha256": source_pin["sha256"],
        "pins": {
            "manifest": manifest_pin,
            "native_inputs": native_pin,
            "source_inputs": source_pin,
        },
        "reference_input_ids": {
            name: packet(row["expected_input_ids"]) for name, row in cases.items()
        },
    }
