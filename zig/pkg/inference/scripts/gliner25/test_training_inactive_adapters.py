...
PINS = {
    "capture.json": (
        420293,
        "d0d5f8dbbd39446cf8050021623f2eda7db6992447bd65029970e93a3a1eff53",
    ),
    "tensors.safetensors": (
        207375,
        "9394123bccbaa959e6aa2b4a32905208e3f2df5917626824edff7af741e0192a",
    ),
}
...
def tensor_header(raw):
    ...
    for name, item in header.items():
        ...
        if (
            ...
            or not 0 <= offsets[0] <= offsets[1] <= payload
            or offsets[1] - offsets[0] != math.prod(shape) * widths[dtype]
        ):
...
class InactiveAdapterSourceContract(unittest.TestCase):
    def load_fixture(self):
        fixture_support.require_fixtures(
            "training_inactive_adapters/capture.json",
            "training_inactive_adapters/tensors.safetensors",
        )
        self.metadata = capture.step_capture.expand_metadata(
            capture.oracle.read_json(DIRECTORY / "capture.json")
        )
    ...
    def floats(self, key):
        ...
        if item["dtype"] != "F32":
            raise ValueError("expected fixture float32")
        start, end = item["data_offsets"]
        count = math.prod(item["shape"])
        if end - start != count * 4:
            raise ValueError("invalid fixture tensor geometry")
        return struct.unpack(
            "<" + str(count) + "f", self.raw[self.start + start : self.start + end]
        )

    def test_pins_tensor_headers_and_provenance_have_no_numerical_import(self):
        self.load_fixture()
        fixture_support.require_fixtures(
            "training_step/capture.json", "training_step/tensors.safetensors"
        )
        ...
        self.assertEqual(capture.SCOPE, value["scope"])
        ...
        self.assertEqual(
            capture.digest(Path(capture.step_capture.__file__)),
            value["tensor_storage_helper"],
        )
        self.assertEqual(
            contract["tensor_storage_helper"], value["tensor_storage_helper"]
        )
        ...
        for name, pin in value["baseline"].items():
            self.assertEqual(pin, capture.digest(capture.BASELINE / name))
        self.assertEqual({"file", "sha256", "size_bytes"}, set(value["tensors"]))
        self.assertEqual(
            capture.digest(DIRECTORY / value["tensors"]["file"]),
            {key: value["tensors"][key] for key in ("size_bytes", "sha256")},
        )
        for name, item in self.header.items():
            if item["dtype"] == "F32":
                self.assertTrue(all(math.isfinite(x) for x in self.floats(name)))
        for profile in value["profiles"]:
            ...
            for micro in profile["microbatches"]:
                self.assertTrue(
                    all(key in self.header for key in micro["inputs"].values())
                )
                slots.extend(
                    micro[name]
                    for name in (
        ...
        self.assertFalse(
            any(name in sys.modules for name in ("torch", "gliner2", "peft"))
        )
    ...
    def test_compact_capture_bindings_resolve_to_bounded_tensor_headers(self):
        for family in (
            "tasks",
            "training_step",
            "training_step_dropout",
            "training_inactive_adapters",
            "training_inactive_native_epoch",
        ):
            with self.subTest(family=family):
                names = [
                    name
                    for name in fixture_support.external_inventory()
                ...
                for name in ("weights", "tensors"):
                    ...
                    with (directory / binding["file"]).open("rb") as stream:
    ...
    def test_tensor_header_rejects_invalid_geometry_and_duplicate_keys(self):
        ...
        for key, value in (
            ...
            malformed["b"][key] = value
            with self.subTest(key=key, value=value), self.assertRaises(ValueError):
                tensor_header(encoded(malformed))
        duplicate = (
            b'{"a":{"dtype":"F32","dtype":"I32","shape":[1],"data_offsets":[0,4]}}'
        )
        for raw in (
            b"",
            struct.pack("<Q", 1024**2 + 1),
            encoded(valid, b"\0" * 9),
    ...
    def test_shared_metadata_preserves_absence_and_rejects_invalid_references(self):
            ...
            "profiles": [
                {
                    ...
                    "microbatches": [
                        {
                            "inputs_ref": 0,
                            "gradients_unscaled": {"weight": None},
                            "loss": -0.0,
                            "resumed": True,
                        }
                    ],
        ...
        for index in (-1, 1, True, None):
            ...
            changed["profiles"][0]["initial_ref"] = index
            with (
                self.subTest(index=index),
                self.assertRaises(capture.oracle.ContractError),
            ):
                capture.step_capture.expand_metadata(changed)
        for change in ("ambiguous", "missing_pool", "oversize", "version", "nested"):
            changed = copy.deepcopy(value)
            shared = changed["shared_metadata"]
            if change == "ambiguous":
                changed["profiles"][0]["initial"] = {}
            elif change == "missing_pool":
                shared.pop("bindings")
            elif change == "oversize":
                shared["bindings"] *= (
                    capture.step_capture.MAX_SHARED_METADATA_ENTRIES + 1
                )
            elif change == "version":
                shared["version"] = True
            else:
                shared["bindings"][0]["weight"] = {"ref": 0}
            with (
    ...
    def test_classifier_fallback_is_zero_present_and_zero_window_advances_all_slots(
        self,
    ):
        self.load_fixture()
        for profile in self.metadata["profiles"]:
            if not profile["id"].endswith(".classifier_only"):
                continue
            self.assertEqual([2, 3, 5], profile["flush_after"])
            self.assertEqual(5, len(profile["microbatches"]))
            ...
            for micro in profile["microbatches"]:
                ...
                for key in micro["gradients_scaled"].values():
                    ...
                    self.assertTrue(all(x == 0 for x in self.floats(key)))
            self.assertEqual(
                [2, 1, 2], [item["microbatches"] for item in profile["flushes"]]
            )
            self.assertEqual(
                [1, 2, 1],
                [item["partial_renormalization"] for item in profile["flushes"]],
            )
            for index, flush in enumerate(profile["flushes"], 1):
                ...
                for slot in flush["parameters"].values():
                    self.assertTrue(slot["state_present"])
                    self.assertEqual(index, slot["step"])
            self.assertEqual(0, profile["flushes"][-1]["grad_norm"])
            before, after = profile["flushes"][-2:]
            self.assertTrue(
                any(
                    self.floats(before["parameters"][name]["weight"])
                    != self.floats(slot["weight"])
                    for name, slot in after["parameters"].items()
                )
            )
            self.assertTrue(
                any(
                    self.floats(before["parameters"][name]["exp_avg"])
                    != self.floats(slot["exp_avg"])
                    for name, slot in after["parameters"].items()
                )
            )
    ...
    def test_optional_head_touch_keeps_nonzero_objective_and_present_zero_slots(self):
        ...
        selected = [
            p
            for p in self.metadata["profiles"]
            if p["id"].endswith((".record_only", ".relation_only"))
        ]
        self.assertEqual(4, len(selected))
        for profile in selected:
            micro = profile["microbatches"][0]
            self.assertFalse(micro["fallback"])
            self.assertTrue(micro["model_loss_requires_grad"])
            self.assertGreater(micro["reported_loss"], 0)
            self.assertEqual(micro["model_loss"], micro["reported_loss"])
            for key in micro["gradients_unscaled"].values():
                self.assertIsNotNone(key)
                self.assertTrue(all(x == 0 for x in self.floats(key)))
            for slot in profile["flushes"][0]["parameters"].values():
                self.assertEqual(1, slot["step"])

    def test_other_live_path_keeps_unsupervised_classifier_absent_without_losing_prior_accumulation(
        self,
    ):
        self.load_fixture()
        for profile in self.metadata["profiles"]:
            if not profile["id"].endswith(".encoder_classifier"):
                continue
            ...
            self.assertEqual(micro["model_loss"], micro["reported_loss"])
            absent = [
                name for name, key in micro["gradients_unscaled"].items() if key is None
            ]
            ...
            for name in absent:
                ...
                self.assertTrue(
                    profile["flushes"][0]["parameters"][name]["state_present"]
                )
