...
class InactiveNativeEpochContract(unittest.TestCase):
    def assert_tensor_bindings_equal(
        self, actual, expected, actual_tensors, expected_tensors, path=""
    ):
        ...
        if isinstance(expected, dict):
            ...
            return sum(
                self.assert_tensor_bindings_equal(
                    actual[key],
                    expected[key],
                    actual_tensors,
                    expected_tensors,
    ...
    def test_only_declared_epoch_changes_and_private_helper_preserves_control(self):
        ...
        self.assertEqual(
            epoch.expected_contract(), epoch.oracle.read_json(epoch.CONTRACT)
        )
        self.assertFalse(
    ...
    def test_jsonl_is_exact_ordered_case_schema_and_annotation_mapping(self):
        ...
        for family, profile in helper.PROFILES.items():
            ...
            for index, (row, case_id) in enumerate(
                zip(rows, profile["sequence"], strict=True)
            ):
                case = cases[case_id]
                ...
                self.assertEqual(
                    case["schema_json"],
                    json.dumps(row["schema"], separators=(",", ":")),
                )
                self.assertEqual("Ada", row["text"].encode()[0:3].decode())
                self.assertEqual(
                    case["annotations"]["entities"][0]["source"],
                    row["entities"][0]["span"],
                )
                labels = (
                    []
                    if case_id == "inactive"
                    else ["good" if case_id == "active_good" else "bad"]
                )
                self.assertEqual(
                    labels,
                    []
                    if "classifications" not in row
                    else row["classifications"][0]["labels"],
                )
            ...
            flushes = [
                i for i in range(1, len(rows) + 1) if i % 2 == 0 or i == len(rows)
            ]
    ...
    def test_contract_or_helper_tamper_fails_without_loading_numerical_runtime(self):
        ...
        for change in ("flush", "lr", "dropout", "source"):
            ...
            if change == "flush":
                altered["profiles"]["classifier_only"]["flush_after"] = [2, 3, 5]
            elif change == "lr":
                altered["task_lr"] = 0.02
            elif change == "dropout":
                altered["dropout"] = 0.1
            else:
            ...
            with tempfile.TemporaryDirectory() as temporary:
                ...
                with mock.patch.object(epoch, "CONTRACT", path):
                    with self.assertRaisesRegex(
                        epoch.oracle.ContractError, "native epoch contract differs"
                    ):
                        epoch.load_control()
        with mock.patch.object(
            epoch, "digest", return_value={"size_bytes": 1, "sha256": "0" * 64}
        ):
            with self.assertRaisesRegex(
                epoch.oracle.ContractError, "frozen control generator changed"
            ):
                epoch.load_control()
    ...
    def test_input_publication_is_atomic_bounded_and_does_not_overwrite(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch.object(epoch, "preflight", return_value=helper):
                ...
                self.assertLess(
                    sum(pin["size_bytes"] for pin in result["files"].values()),
                    64 * 1024,
                )
                ...
                with self.assertRaisesRegex(
                    epoch.oracle.ContractError, "refusing to overwrite"
                ):
    ...
    def test_captured_fixture_pins_native_rows_and_source_generator_provenance(self):
        fixture_support.require_fixtures(
            "training_inactive_native_epoch/capture.json",
            "training_inactive_native_epoch/tensors.safetensors",
        )
        directory = epoch.oracle.FIXTURES / "training_inactive_native_epoch"
        expected = {
            "capture.json": {
                "size_bytes": 423554,
                "sha256": "24cefafa4dde1f7067b3ba81e7b9209495a58a0cb31620c5805b00bec9abdc24",
            },
            "tensors.safetensors": {
                "size_bytes": 207343,
                "sha256": "eb8d7939308642c9587c73789070516e632078850473feecd3416f57d665246c",
            },
        }
        ...
        self.assertEqual(
            control.digest(Path(control.step_capture.__file__)),
            report["tensor_storage_helper"],
        )
        self.assertEqual(
            epoch.expected_contract()["tensor_storage_helper"],
            report["tensor_storage_helper"],
        )
        ...
        self.assertEqual(
            report["native_settings"],
            epoch.oracle.read_json(directory / "native_settings.json"),
        )
        for name, raw in epoch.input_files(helper).items():
            self.assertEqual(raw, (directory / name).read_bytes())

    def test_captured_epoch_changes_only_classifier_schedule_and_preserves_initial_state(
        self,
    ):
        fixture_support.require_fixtures(
            "training_inactive_native_epoch/capture.json",
        ...
        report = control.step_capture.expand_metadata(
            epoch.oracle.read_json(directory / "capture.json")
        )
        prior = control.step_capture.expand_metadata(
            epoch.oracle.read_json(original / "capture.json")
        )
        ...
        def tensors(path):
            ...
            return {
                name: (
                    ...
                    tuple(item["shape"]),
                    raw[
                        start + item["data_offsets"][0] : start
                        + item["data_offsets"][1]
                    ],
                )
        ...
        actual, expected = (
            tensors(directory / "tensors.safetensors"),
            tensors(original / "tensors.safetensors"),
        )
        profiles = {profile["id"]: profile for profile in prior["profiles"]}
        self.assertEqual(
            set(profiles), {profile["id"] for profile in report["profiles"]}
        )
        ...
        for key in (
            ...
            self.assert_tensor_bindings_equal(
                report[key], prior[key], actual, expected, key
            )
        unchanged = 0
        initial_states = 0
        for profile in report["profiles"]:
            ...
            if not profile["id"].endswith(".classifier_only"):
                self.assertGreater(
                    self.assert_tensor_bindings_equal(
                        profile, profiles[profile_id], actual, expected, profile_id
                    ),
                    0,
                )
                unchanged += 1
            ...
            self.assertEqual(
                [1, 1, 2], [f["partial_renormalization"] for f in profile["flushes"]]
            )
            self.assertEqual(
                [0, 0, 1, 1, 2],
                [m["global_step_before"] for m in profile["microbatches"]],
            )
            ...
            for index, flush in enumerate(profile["flushes"], 1):
                ...
                self.assertTrue(
                    all(
                        slot["state_present"] and slot["step"] == index
