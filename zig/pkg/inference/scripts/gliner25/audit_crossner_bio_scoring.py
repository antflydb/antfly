...
def load_scorer():
    ...
    checked(
        pin(raw) == {"size_bytes": 13425, "sha256": SCORER_SHA256},
        "official scorer pin differs",
    )
...
def spans(text, facts, ontology):
    checked(
        isinstance(text, str) and len(text.encode()) <= 1024 * 1024,
        "text exceeds audit geometry",
    )
    checked(
        isinstance(facts, list) and len(facts) <= MAX_FACTS,
        "entity count exceeds audit geometry",
    )
    raw = text.encode()
    boundaries = {0}
    position = 0
    ...
    for fact in facts:
        checked(
            ...
            and set(fact) == {"type", "span"}
            and fact["type"] in ontology,
            "entity type or fields differ",
        )
        span = fact["span"]
        checked(
            isinstance(span, dict) and set(span) == {"start", "end", "text"},
            "span fields differ",
        )
    ...
    for name in ontology:
        ...
        checked(
            all(left[1] <= right[0] for left, right in zip(typed, typed[1:])),
    ...
    cuts = sorted(
        {
            0,
            len(text.encode()),
            *[value for rows in (wanted, found) for row in rows for value in row[:2]],
        }
    )
...
def score_documents(documents, ontology, scorer=None):
    checked(
        1 <= len(documents) <= MAX_ROWS
        and isinstance(ontology, list)
        and 1 <= len(ontology) <= 128
        and all(
            isinstance(name, str)
            and 0 < len(name) <= 128
        ...
        and len(set(ontology)) == len(ontology),
    ...
    def lines():
        ...
        for text, gold, predicted in documents:
            wanted, found = (
                spans(text, gold, ontology),
                spans(text, predicted, ontology),
            )
            for name in ontology:
    ...
    def result(metric):
        return {
            ...
            "recall": metric.rec,
            "micro_f1": metric.fscore,
        }

    zero = scorer.calculate_metrics(0, 0, 0)
    return {
        "entity_exact": result(overall),
...
def check_report_identity(report, receipt, stored=None):
    checked(
        report.get("scope") == "gliner25_heldout_execution/v1"
        and report.get("qualification") is False
        and report.get("status") == "complete"
        and report.get("lock_sha256") == receipt["lock_sha256"]
        and report.get("denominator")
        == report.get("completed_results")
        == receipt["records"]
        == 431
        and report.get("errors") == report.get("unprocessed") == 0,
        "report identity or completion differs",
    )
    if stored is not None:
        checked(
            stored.get("lock_sha256") == receipt["lock_sha256"]
            and stored.get("records") == receipt["records"]
            and stored.get("scope") == "gliner25_heldout_exact_fact_metrics/v1"
            and stored.get("qualification") is False
            and report.get("metrics") == stored.get("metrics"),
            "embedded report metrics differ",
        )


def audit(prepared, reports):
    checked(
        1 <= len(reports) <= 16 and len(set(reports)) == len(reports),
        "invalid report inventory",
    )
    inputs = {}
    ...
    checked(
        pin(request_raw)["sha256"] == receipt["requests_sha256"]
        and pin(gold_raw)["sha256"] == receipt["gold_sha256"],
    ...
    scorer = load_scorer()
    remember(
        REFERENCE / "conll2002_metrics.py",
        {"size_bytes": 13425, "sha256": SCORER_SHA256},
    )
    results = []
    for path in reports:
        ...
        for name in ("predictions", "metrics"):
            checked(
                files[name]["path"]
                == name + (".jsonl" if name == "predictions" else ".json"),
                "non-canonical report path",
            )
        prediction_raw = remember(
            path.parent / "predictions.jsonl",
            {k: files["predictions"][k] for k in ("size_bytes", "sha256")},
        )
        predictions = rows(prediction_raw)
        stored = decode(
            remember(
                path.parent / "metrics.json",
                {k: files["metrics"][k] for k in ("size_bytes", "sha256")},
            )
        )
        ...
        for request, expected, actual in zip(requests, gold, predictions):
            checked(
                all(
                    request[key] == expected[key] == actual[key]
                    for key in ("request_id", "request_sha256")
                ),
                "request identity or ordering differs",
            )
            checked(
                request["request"]["schema"]["entities"] == ontology
                and request["request"]["offset_unit"] == "utf8_bytes"
                and request["request"]["options"]["overlap"] == "flat",
                "metric schema differs",
            )
            documents.append(
                (
                    request["request"]["text"],
                    expected["metrics"]["entity_exact"],
                    actual["metrics"]["entity_exact"],
                )
            )
        ...
        with tempfile.TemporaryDirectory(prefix="gliner25-bio-metric-") as temporary:
            ...
            for name, raw in (
                ("prepared.json", receipt_raw),
                ("requests.jsonl", request_raw),
                ("gold.jsonl", gold_raw),
                ("predictions.jsonl", prediction_raw),
            ):
        ...
        for name, value in official.items():
            for key, expected in value.items():
                ...
                checked(
                    actual == expected
                    if key in ("tp", "fp", "fn")
                    else abs(actual - expected) <= 1e-15,
                    "official BIO metric differs: " + name + "/" + key,
                )
        results.append(
            {
                "report": str(path),
                "report_pin": inputs[str(path)],
                "model": report["model"],
                "backend": report["backend"],
                "precision": report["artifact"]["precision"],
                "documents": len(documents),
                "metric_families": len(official),
    ...
    return {
        "scope": "gliner25_crossner_official_bio_metric_audit/v1",
        "status": "pass",
        "qualification": False,
        "model_execution": False,
        "source_revision": "2e7ba2a7798c961e3f29fbc51252c5a8d40224bf",
        "representation": "per_type_common_UTF8_boundary_refinement_with_explicit_type_and_document_sentinels",
        "limitations": [
            "Exact per-type flat span counts only; original BIO-token representability is not asserted.",
