...
with weight.open('rb') as source:
    for chunk in iter(lambda: source.read(8 * 1024 * 1024), b''):
        digest.update(chunk)
if digest.hexdigest() != '40a5a23ff860dc3dff426cecd1048cacdd29c648c96db209dad818e9686dc997':
...
def run():
    with torch.inference_mode():
        ...
        if max(('refund','technical_support','sales'), key=lambda label: scores.logit('intent',label)) != 'refund':
...
result = {
    'implementation': 'upstream_gliner2_python', 'device': args.device,
    'torch': torch.__version__, 'transformers': __import__('transformers').__version__,
    'gpu': torch.cuda.get_device_name(0) if args.device == 'cuda' else None,
    'threads': torch.get_num_threads(), 'warmup': args.warmup, 'reps': args.reps,
