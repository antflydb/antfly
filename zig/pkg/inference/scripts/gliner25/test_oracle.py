...
class ProvenanceTest(unittest.TestCase):
    ...
    def test_external_references_verify_present_bytes_and_report_missing_coverage(self):
        ...
        self.assertEqual(
            set(fixture_support.external_inventory()),
            set(report["files"]) | set(report["missing"]),
        )
    ...
    def test_external_policy_skips_only_declared_absence_and_rejects_corruption(self):
        with (
            ...
            mock.patch.object(oracle, "FIXTURES", Path(tmp)),
        ):
    ...
    def test_missing_file_never_becomes_a_skip(self):
        with tempfile.TemporaryDirectory() as tmp:
            with self.assertRaisesRegex(oracle.ContractError, "missing"):
                oracle.verify_file(
                    Path(tmp) / "absent", {"size_bytes": 1, "sha256": "0" * 64}
                )
    ...
    def test_snapshot_symlink_is_verified_by_content(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            oracle.verify_file(
                link, {"size_bytes": 5, "sha256": hashlib.sha256(b"fixed").hexdigest()}
            )
    ...
    def test_duplicate_and_nonfinite_json_are_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for value, reason in [
                ('{"id":1,"id":2}', "duplicate"),
                ('{"score":NaN}', "non-finite"),
            ]:
                path.write_text(value)
                with (
                    self.subTest(value=value),
                    self.assertRaisesRegex(oracle.ContractError, reason),
                ):
    ...
    def test_dependency_mismatch_is_rejected_before_import(self):
        ...
        with (
            mock.patch.object(
                oracle.platform, "python_version", return_value=expected["python"]
            ),
            mock.patch.object(
                oracle.importlib.metadata, "version", side_effect=versions.__getitem__
            ),
    ...
    def test_import_from_another_checkout_is_rejected(self):
        module = types.SimpleNamespace(
            __file__="/private/tmp/unpinned/gliner2/__init__.py"
        )
    ...
    def test_namespace_packages_must_have_only_pinned_search_locations(self):
        module = types.SimpleNamespace(
            __file__=None, __path__=["/private/tmp/pinned/gliner2/utils"]
        )
        oracle.verify_import_source(module, Path("/private/tmp/pinned"))
                    ...
                    "small": {
                        ...
                        "files": {
                            "config.json": {
                                ...
                                "sha256": oracle.sha256_file(config),
                            }
                        },
                    }
                }
...
class CheckoutTest(unittest.TestCase):
    def test_wrong_dirty_and_ignored_source_are_rejected(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                commit, oracle.verify_upstream_checkout(root, commit)["commit"]
            )
...
class PipelineAdaptationTest(unittest.TestCase):
    def test_each_published_variant_retains_its_own_bundle_and_edge_identity(self):
        for variant in ("small", "base", "multi"):
            with self.subTest(model=variant):
                result = generate_pipeline_cases.generate(
                    variant,
                ...
                self.assertEqual(
                    pinned["files"]["tokenizer.json"]["sha256"],
                    result["tokenizer_sha256"],
                )
                self.assertEqual(
                    pinned["files"]["model.safetensors"]["sha256"],
                    result["model_sha256"],
                )
                ...
                fixture_name = (
                    "pipeline_cases.json"
                    if variant == "small"
                    else f"pipeline_cases_{variant}.json"
                )
                ...
                for generated, stored in zip(result["cases"], checked_in["cases"]):
                    self.assertEqual(
                        json.dumps(generated["schema"], ensure_ascii=False),
                        json.dumps(stored["schema"], ensure_ascii=False),
                    )
                joint = next(
                    case for case in result["cases"] if case["id"] == "joint_ie"
                )
                if variant != "small":
    ...
    def test_variant_substitution_is_rejected_before_adapting_outputs(self):
        with self.assertRaisesRegex(oracle.ContractError, "different model variant"):
            generate_pipeline_cases.generate(
                "multi",
                oracle.FIXTURES / "base_reference",
                oracle.FIXTURES / "requests.json",
            )
    ...
    def test_modified_capture_tensor_is_rejected_by_content(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (directory / "capture.json").write_bytes(
                (original / "capture.json").read_bytes()
            )
    ...
    def test_output_adaptation_uses_explicit_retention_without_rewriting_capture(self):
        for variant in ("base", "multi"):
            with (
                ...
                retained = {
                    name: pin
                    for name, pin in generate_pipeline_cases.reference_inventory().items()
                    if not (
                        name.startswith(f"{variant}_reference/")
                        and name.endswith(".safetensors")
                    )
                }
                with mock.patch.object(
                    generate_pipeline_cases,
                    "reference_inventory",
                    return_value=retained,
                ):
                    generated = generate_pipeline_cases.generate(
                        variant, directory, oracle.FIXTURES / "requests.json"
                    )
                expected = oracle.read_json(
                    oracle.FIXTURES / f"pipeline_cases_{variant}.json"
                )
                self.assertEqual(
    ...
    def test_missing_retained_attachment_still_fails(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(
                oracle.ContractError, "required regular file is missing"
            ):
                generate_pipeline_cases.generate(
                    "small", directory, oracle.FIXTURES / "requests.json"
                )
    ...
    def test_optional_diagnostics_do_not_allow_modified_public_outputs(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(oracle.ContractError, "SHA256 mismatch"):
                generate_pipeline_cases.generate(
                    "base", directory, oracle.FIXTURES / "requests.json"
                )
    ...
    def test_retention_cannot_omit_capture_or_substitute_retained_tensor_identity(self):
        ...
        for name, value in (
            ("small_reference/capture.json", None),
            (
                "small_reference/mixed_tasks.safetensors",
            ...
            with (
                ...
                mock.patch.object(
                    generate_pipeline_cases, "reference_inventory", return_value=changed
                ),
                self.assertRaises(oracle.ContractError),
            ):
                generate_pipeline_cases.generate(
                    "small",
                    oracle.FIXTURES / "small_reference",
                    oracle.FIXTURES / "requests.json",
                )
    ...
    def test_generator_request_or_profile_drift_never_becomes_reference(self):
        ...
        for mutate in (
            lambda report: report.update(generator_sha256="0" * 64),
...
class LegacyParityDiscoveryTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        spec = importlib.util.spec_from_file_location(
            "legacy_gliner2_parity_discovery", path
        )
