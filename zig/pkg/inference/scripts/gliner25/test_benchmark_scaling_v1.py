...
def inputs_fixture():
    ...
    for name, batch, mode in zip(names, (2, 8), ("regular", "smoke")):
        rows[name] = {
            "id": name,
            "items": [{"id": f"{name}.{i}", "text": "Alice."} for i in range(batch)],
            "expected_input_ids": [[1, 2, 3]] * batch,
            "kind": "extract",
    ...
    return {
        **original,
        ...
        "source_cases": rows,
        "expected": {
            name: [copy.deepcopy(EMPTY) for _ in row["items"]]
            for name, row in rows.items()
        },
        "prepared": Path("/unused/prepared"),
        "requests_sha256": "a" * 64,
        "pins": {
            "native_inputs": {
                "sha256": campaign.oracle.sha256_file(original["case_path"])
            }
        },
        "reference_input_ids": {
            name: contract.packet(row["expected_input_ids"])
            for name, row in rows.items()
        },
    }
...
def readiness(arm, inputs):
    ...
    if arm == campaign.NATIVE:
        value.update(
            ...
            runtime_policy=copy.deepcopy(campaign.runtime.POLICIES["optimized_v2"]),
            model_live_bytes=4096,
        )
    else:
...
class ScalingDriverTests(unittest.TestCase):
    def test_real_pair_orchestration_uses_batch_outputs_and_descriptive_samples(self):
        ...
        class Worker:
            ...
            def request(self, op, name="", timeout=35):
                ...
                value.update(
                    scope=contract.SCOPE,
                    outputs=copy.deepcopy(inputs["expected"][name]),
                    output=copy.deepcopy(EMPTY),
                    input_device="mps",
        ...
        args = types.SimpleNamespace(
            native_bin=Path("/new/metal"),
            model_root=Path("/models"),
            upstream=Path("/source"),
            purpose="benchmark",
            execution_policy="optimized_v2",
            legacy_reference_bin=False,
            warmup=1,
            pairs=3,
            timeout_ms=1000,
            startup_timeout=10,
        ...
        with (
            ...
            mock.patch.object(
                campaign.oracle, "verify_model_dir", return_value=inputs["bundle"]
            ),
            ...
            result = campaign.run_pair(
                ...
                repetition=0,
                phase="measurement",
                selected=inputs["cases"][:1],
            )
        self.assertEqual("complete", result["status"])
        self.assertEqual(["--workload", "scaling"], commands[campaign.NATIVE][-2:])
        self.assertEqual(
            "metal_scaling_python_worker.py", Path(commands["fastino_mps"][1]).name
        )
    ...
    def run_fake_campaign(
        self,
        *,
        source_smoke_status="complete",
        paired_smoke_status="validated",
        ...
        def cpu_capture(args, values, selected, directory):
            calls.append(
                ("cpu", selected.copy(), args.repetitions, args.warmup, args.pairs)
            )
            result = {
                "status": "complete",
                "model": values["model"],
                ...
                "cases": {
                    name: {"status": "complete", "outputs": inputs["expected"][name]}
            ...
            if source_smoke_status != "complete":
                ...
                result["cases"][smoke] = {
                    "status": source_smoke_status,
                    "error": "source B8 failure evidence",
                }
            if cleanup_failed:
                result.update(
                    status="failed", cleanup_error="source owner was not reaped"
                )
            return result

        def pair(args, values, baseline, directory, *, repetition, phase, selected):
            calls.append(
                (phase, selected.copy(), args.repetitions, args.warmup, args.pairs)
            )
            result = {
                "status": "complete",
                "model": values["model"],
                ...
                "case_status": {
                    name: {
                        "status": "validated" if phase == "preflight" else "complete"
                    }
                    for name in selected
                },
                "comparisons": {name: {"sample_pairs": 3} for name in selected}
                if phase == "measurement"
                else {},
            }
            if (
                phase == "preflight"
                and smoke in selected
                and paired_smoke_status != "validated"
            ):
                result.update(status="partial")
                result["case_status"][smoke] = {
                    "status": paired_smoke_status,
                    "stage": "validation",
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                mock.patch.object(
                    contract, "load", side_effect=lambda *args: copy.deepcopy(inputs)
                ),
                ...
                mock.patch.object(
                    campaign,
                    "source_snapshot",
                    return_value={"source_files_sha256": "c" * 64},
                ),
                mock.patch.object(
                    driver.oracle, "verify_dependencies", return_value={}
                ),
                mock.patch.object(
                    driver.oracle, "verify_upstream_checkout", return_value={}
                ),
                mock.patch.object(driver.oracle, "verify_config_fixtures"),
                mock.patch.object(
                    driver.oracle, "verify_model_dir", return_value=inputs["bundle"]
                ),
            ):
    ...
    def test_default_campaign_is_one_by_one_by_three_and_b8_remains_correctness_only(
        self,
    ):
    ...
    def test_failed_and_unprocessed_b8_keep_denominator_and_original_error_evidence(
        self,
    ):
        for source_status, expected in (
            ...
            with self.subTest(source_status=source_status):
                exit_code, report, calls, inputs = self.run_fake_campaign(
                    source_smoke_status=source_status
                )
                self.assertEqual(2, exit_code)
                self.assertFalse(report["parity_validated"])
                self.assertEqual(
                    inputs["cases"], [row["case_id"] for row in report["summary"]]
                )
                ...
                self.assertEqual(
                    "source B8 failure evidence", smoke["evidence"][0]["error"]
                )
    ...
    def test_failed_b8_preflight_keeps_regular_samples_and_exact_failure_evidence(self):
        ...
        self.assertEqual(
            "native B8 output parity failure evidence", smoke["evidence"][1]["error"]
        )
    ...
    def test_early_cleanup_failure_still_reports_every_b8_and_stops_campaign(self):
        ...
        self.assertEqual(
            "source owner was not reaped",
            report["summary"][1]["evidence"][0]["run_cleanup_error"],
        )
    ...
    def test_later_batch_item_parity_failure_advances_only_valid_owner_receipt(self):
        ...
        value.update(
            outputs=copy.deepcopy(expected),
            output=copy.deepcopy(EMPTY),
            **inputs["reference_input_ids"][name],
        )
        # Sample zero remains identical: a later batch item must still reject.
        value["outputs"][1]["classifications"].append(
    ...
    def test_cpu_capture_unsafe_error_retains_unprocessed_rows_and_reaps_worker(self):
        ...
        with (
            tempfile.TemporaryDirectory() as temporary,
            mock.patch.object(driver, "Worker", Worker),
            mock.patch.object(driver, "ResourceGuard", FakeGuard),
        ):
            report = driver.capture_cpu(
                args, inputs, inputs["cases"], Path(temporary) / "capture"
            )
