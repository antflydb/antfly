...
def metrics(left, right):
    ...
    if a.shape != b.shape or not np.isfinite(a).all() or not np.isfinite(b).all():
        raise ValueError("diagnostic tensors differ in shape or contain non-finite values")
    difference = a - b
    index = int(np.argmax(np.abs(difference)))
    return {"elements": a.size, "max_absolute_error": float(abs(difference[index])),
            "l2_error": float(np.linalg.norm(difference)), "reference_l2": float(np.linalg.norm(b)),
            "worst_index": index, "native": float(a[index]), "python": float(b[index])}
...
def run(args):
    ...
    if args.modules and config.get("activation_profile", "retained_v1") != "retained_v1":
    ...
    scratch = Path(tempfile.mkdtemp(prefix="gliner25-loss-trace-", dir=args.snapshot_root))
    ...
    commands = {
        "native": [str(args.native_bin), "--config", str(args.config), "--snapshot", str(scratch / "native.bin")],
        "python": [sys.executable, str(HERE / "training_cuda_worker.py"), "--config", str(args.config),
                   "--snapshot", str(scratch / "python.bin"), "--upstream", str(args.upstream),
                   "--model", args.model, "--trace-native-input", str(native_inputs)],
    }
    if getattr(args, "records", False):
        commands["native"] += ["--trace-records"]
    if args.modules:
        commands["native"] += ["--trace-tensors", str(scratch / "native-tensors.bin")]
    report = {"scope": "diagnostic_loss_boundary", "production_qualified": False,
              "runtime_environment": {key: env[key] for key in ("LD_LIBRARY_PATH", "CUBLAS_WORKSPACE_CONFIG", *common.THREAD_ENV) if key in env},
              "config_sha256": oracle.sha256_file(args.config), "native_binary_sha256": oracle.sha256_file(args.native_bin),
              "helpers_sha256": {name: oracle.sha256_file(HERE / name) for name in
                                  (Path(__file__).name, "training_cuda_trace.py", "training_cuda_worker.py", "benchmark_training_cuda.py", "metal_benchmark_supervisor.py")},
              "commands": commands, "observations": [], "cleanup": {}}
    workers = {}
    try:
        ...
        for microbatch in range(1, microbatches + 1):
            inputs = {arm: training.response(w, "validate", "inputs", args.timeout) for arm, w in workers.items()}
            ...
            native = training.response(workers["native"], "validate", "trace_step", args.timeout)
            events = {event["kind"]: event for event in map(common.strict_json, native["trace_events"])}
            payload = dict(events["boundary_inputs"]["value"])
            if getattr(args, "records", False):
                # Multiple record groups share an event kind; preserve each.
                payload["_records"] = [event for event in map(common.strict_json, native["trace_events"]) if event["kind"] == "record_inputs"]
            if args.modules:
            ...
            python = training.response(workers["python"], "validate", "trace_step", args.timeout)
            oracle.write_json(args.output / f"trace-{microbatch}.json", {"native": native, "python": python})
            trace = python["loss_trace"]
            observation = {"microbatch": microbatch, "geometry_errors": trace.get("geometry_errors"),
                           "forward": {}, "gradient": {}, "same_logits_gradient": {}, "modules": trace.get("modules", [])}
            for key in ("starts", "ends", "inside", "pairs", "proposals", "nulls", "counts"):
                ...
                observation["gradient"][key] = metrics(actual, trace["python"]["gradients"][key])
                if "native_logits_python_backward" in trace:
                    observation["same_logits_gradient"][key] = metrics(actual, trace["native_logits_python_backward"]["gradients"][key])
            receipts = {arm: training.response(w, "validate", "snapshot", args.timeout) for arm, w in workers.items()}
            comparison = training.compare_snapshots(receipts["native"], receipts["python"])
            oracle.write_json(args.output / f"microbatch-{microbatch}-tensor-parity.json", comparison)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--microbatches", type=int, default=2, help="Bounded diagnostic length (1–128 microbatches)")
    parser.add_argument("--records", action="store_true", help="Capture bounded record logits and detached matching metadata")
    parser.add_argument("--modules", action="store_true", help="Compare retained Linear/LayerNorm inputs and outputs")
