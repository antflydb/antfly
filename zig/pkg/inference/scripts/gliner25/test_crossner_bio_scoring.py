...
def fact(text, start, end, kind="person"):
    return {
        "type": kind,
        "span": {"start": start, "end": end, "text": text.encode()[start:end].decode()},
    }
...
class OfficialBIOTests(unittest.TestCase):
    def test_reference_identity_and_license(self):
        ...
        self.assertEqual(
            receipt["revision"], "2e7ba2a7798c961e3f29fbc51252c5a8d40224bf"
        )
        self.assertEqual(receipt["license"], "MIT")
        self.assertEqual(receipt["modifications"], [])
        for entry in receipt["files"]:
            self.assertEqual(
                audit.pin((audit.REFERENCE / entry["path"]).read_bytes()),
                {key: entry[key] for key in ("size_bytes", "sha256")},
            )
        self.assertIn(
            "Copyright (c) 2020 Zihan Liu", (audit.REFERENCE / "LICENSE").read_text()
        )
    ...
    def test_exhaustive_flat_span_counts_with_adjacent_and_partial_word_entities(self):
        ...
        for gold, predicted in itertools.product(choices, repeat=2):
            with self.subTest(gold=gold, predicted=predicted):
                actual = audit.score_documents(
                    [(text, gold, predicted)], ontology, scorer
                )
                for name in [None, *ontology]:

                    def keys(rows):
                        return {
                            (row["span"]["start"], row["span"]["end"], row["type"])
                            for row in rows
                            if name is None or row["type"] == name
                        }

                    wanted, found = keys(gold), keys(predicted)
                    counts = actual[
                        "entity_exact" if name is None else "entity_type/" + name
                    ]
                    self.assertEqual(
    ...
    def test_utf8_empty_documents_duplicate_sets_and_explicit_boundaries(self):
        ...
        documents = [
            ("", [], []),
            (text, gold, predicted + predicted),
            ("x", [fact("x", 0, 1)], []),
        ]
        counts = audit.score_documents(documents, ["person", "absent"])
        self.assertEqual(
            {key: counts["entity_exact"][key] for key in ("tp", "fp", "fn")},
            {"tp": 1, "fp": 1, "fn": 2},
        )
        ...
        self.assertEqual(
            audit.score_documents([("ab", adjacent, adjacent)], ["person"])[
                "entity_exact"
            ]["tp"],
            2,
        )
    ...
    def test_invalid_spans_and_overlaps_fail_without_projection(self):
        ...
        with self.assertRaisesRegex(audit.evaluation.EvaluationError, "overlapping"):
            audit.score_documents(
                [("abc", [], [fact("abc", 0, 2), fact("abc", 1, 3)])], ["person"]
            )
        with self.assertRaises(audit.evaluation.EvaluationError):
    ...
    def test_cross_type_overlap_preserves_every_prediction(self):
        ...
        self.assertEqual(
            (
                actual["entity_exact"]["tp"],
                actual["entity_exact"]["fp"],
                actual["entity_exact"]["fn"],
            ),
            (1, 1, 0),
        )
...
"""
            child = subprocess.run(
                [sys.executable, "-c", worker, str(fifo)],
                cwd=audit.HERE,
    ...
    def test_foreign_report_and_embedded_metric_substitution_rejected(self):
        ...
        stored = {
            "scope": "gliner25_heldout_exact_fact_metrics/v1",
            "qualification": False,
            "lock_sha256": receipt["lock_sha256"],
            "records": 431,
            "metrics": metric,
        ...
        for key, value in {
            "scope": "unrelated-report/v100",
            "qualification": True,
            "lock_sha256": "0" * 64,
            "errors": 1,
            "completed_results": 430,
            "metrics": {"entity_exact": {"tp": 100}},
        }.items():
            with (
                self.subTest(key=key),
                self.assertRaises(audit.evaluation.EvaluationError),
            ):
                audit.check_report_identity({**report, key: value}, receipt, stored)
        with self.assertRaises(audit.evaluation.EvaluationError):
            audit.check_report_identity(
                report, receipt, {**stored, "lock_sha256": "0" * 64}
            )
