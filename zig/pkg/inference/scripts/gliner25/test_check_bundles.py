...
def ready(receipt, fixture_sha256, backend="native"):
    return {
        "event": "ready",
        "scope": "converted_bundle_diagnostic",
        "backend": backend,
        "math_policy": bundles.MATH_POLICY,
        "qualification": False,
        "receipt": receipt,
        "fixture_sha256": fixture_sha256,
        "weight_precision": receipt["precision"],
...
def reference():
    ...
    entry = {
        ...
        "receipt_sha256": "3" * 64,
        "fixture_sha256": "4" * 64,
        "ready": ready(receipt, "4" * 64),
        "cases": [
            {
                "case_id": case["id"],
                "token_ids_equal": True,
                "input_ids": tokens[case["id"]],
                "expected": benchmark_cpu.canonical_result(expected),
                "actual": benchmark_cpu.canonical_result(actual),
                "backend_output": backend,
                "source_fp32": compare(
                    benchmark_cpu.canonical_result(expected),
                    benchmark_cpu.canonical_result(actual),
                ),
            }
    ...
    report = {
        "format_version": 2,
        "scope": bundles.REPORT_SCOPE,
        "status": "complete",
        "qualification": False,
        "backend": "native",
        "math_policy": bundles.MATH_POLICY,
        "source_commit": oracle.UPSTREAM_COMMIT,
        "token_reference_report_sha256": "5" * 64,
        "driver_sha256": "6" * 64,
        "binary_sha256": "7" * 64,
        "bundles": [entry],
    }
...
class BundleComparisonTests(unittest.TestCase):
    ...
    def test_aligned_confidence_delta_is_separate_from_decisions(self):
        result = compare(
            {"text": "John", "confidence": 0.9}, {"text": "John", "confidence": 0.899}
        )
    ...
    def test_same_bundle_parity_is_independent_of_source_quality(self):
        ...
        found = bundles.matching_native_bundle(
            report, receipt, "3" * 64, "4" * 64, fixture, tokens
        )
        self.assertFalse(found["cases"][0]["source_fp32"]["decisions_equal"])
        result = bundles.compare_backends(
            found["cases"][0]["backend_output"], found["cases"][0]["backend_output"]
        )
        ...
        self.assertFalse(
            bundles.compare_backends(found["cases"][0]["backend_output"], changed)[
                "parity_pass"
            ]
        )
    ...
    def test_stale_native_profile_or_driver_is_rejected(self):
        ...
        for key, value in (
            ...
            ("math_policy", "legacy"),
            ("driver_sha256", "0" * 64),
            ("token_reference_report_sha256", "0" * 64),
        ...
        for key in (
            ...
            with (
                self.subTest(ready=key),
                self.assertRaises(benchmark_cpu.BenchmarkError),
            ):
                bundles.validate_native_reference(bad, "5" * 64, "6" * 64)
    ...
    def test_receipt_tokens_case_identity_and_output_evidence_are_bound(self):
        ...
        for name in (
            ...
            if name == "receipt":
                ...
                bad["bundles"][0]["cases"][0]["backend_output"]["entities"][0][
                    "values"
                ][0]["text"] = "forged"
            else:
            ...
            with (
                ...
                bundles.matching_native_bundle(
                    bad, bad_receipt, receipt_hash, fixture_hash, fixture, tokens
                )
    ...
    def test_serial_worker_protocol_routes_backend_and_requires_fresh_native_evidence(
        self,
    ):
        ...
        token_rows = next(
            item for item in token_report["models"] if item["model"] == "small"
        )["validation"]
        commands, workers = [], []
        receipt = {
            "family": "gliner_boundary_bundle/v1",
            "version": 1,
            "backbone": "small",
            "precision": "q8_0",
            "source_files": [
                {"path": name, **pin} for name, pin in fixture["model_files"].items()
            ],
            "files": [],
        }
        ...
        class FakeWorker:
            ...
            def __init__(self, arm, command, env, directory, guard):
                ...
                self.frames += [
                    {
                        "event": "result",
                        "case_id": case["id"],
                        "input_ids": token_rows[case["id"]]["input_ids"],
                        "output": lifted(case["expected"]),
                    }
                    for case in fixture["cases"]
                ]
                self.frames += [
                    {"event": "complete", "cases": 10, "qualification": False}
                ]
                ...
                self.process = type(
                    "Process",
                    (),
                    {"poll": lambda _: 0, "returncode": 0, "stdout": io.BytesIO()},
                )()
        ...
        with (
            ...
            patch.object(benchmark_cpu, "Worker", FakeWorker),
        ):
            root = Path(directory)
            model = root / "model"
            model.mkdir()
            ...
            native_dir.mkdir()
            metal_dir.mkdir()
            native = bundles.run_one(
                Path("synthetic-runner"), model, native_dir, token_report, 10, 256
            )
            self.assertEqual(native["status"], "complete")
            self.assertEqual(native["source_fp32"]["decisions_equal_cases"], 10)
            metal = bundles.run_one(
                Path("synthetic-runner"),
                model,
                metal_dir,
                token_report,
                10,
            ...
            self.assertTrue(
                all(worker.closed and not worker.frames for worker in workers)
            )
            with self.assertRaises(benchmark_cpu.BenchmarkError):
                bundles.run_one(
                    Path("synthetic-runner"),
