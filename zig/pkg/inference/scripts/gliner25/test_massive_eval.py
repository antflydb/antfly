...
def synthetic_fixture(profile_name="entities_en-US", index=0):
    ...
    options = {
        ...
        "overlap": "flat",
        "threshold": 0.5,
        "word_splitter": profile["word_splitter"],
    }
    ...
    for row in range(start, end):
        ...
        cases.append(
            {
                "id": facts.digest(
                    facts.encoded({"lock": entry["lock_sha256"], "id": source_id})
                ),
                "source_id": source_id,
    ...
    shard["request_ids_sha256"] = facts.digest(
        facts.encoded([row["id"] for row in cases])
    )
    shard["request_sha256s_sha256"] = facts.digest(
        facts.encoded([row["request_sha256"] for row in cases])
    )
    fixture = {
        "format_version": 2,
        "scope": contract.WORKER_SCOPE,
        "qualification": False,
        "source_commit": oracle.UPSTREAM_COMMIT,
        "model": "small",
        "source_files": registry["models"]["small"]["source_files"],
        "registry_sha256": oracle.sha256_file(contract.REGISTRY),
        ...
        **{
        ...
        "harness_sha256": registry["frozen_helpers"]["evaluation_contract.py"],
        "policy": contract.POLICY.copy(),
        "limits": contract.LIMITS.copy(),
        "schema": schema,
        "options": options,
        "offset_unit": "utf8_bytes",
        "transport": {
            "global_records": contract.RECORDS,
...
class EnvelopeTest(unittest.TestCase):
    def test_real_locked_first_request_hashes_keep_preparation_field_order(self):
        ...
        vectors = (
            (
                "entities_ar-SA",
                "صحيني خمسة الفجر هذا الأسبوع",
                "a92a4811ee2b0eba6a958880614e14ed5a8fada1f462758651b46722e279381c",
                "c55a2f60bbf2f0ed44935ba151a0cabed2e9fda375efd4941ad5e9e71c89e650",
            ),
            (
                "entities_zh-CN_char",
                "这周五点叫我起床",
                "5767e37345a928d05a02e998bf64ad90e1376d632ca50dd5ee59a4b231ece34a",
                "7a6998522d1a0909f9de8f88c8a94b81edc14e5248c4f5b3da05f6ec184e5793",
            ),
            (
                "intent_en-US",
                "wake me up at five am this week",
                "d228c0dda1eb1c72e8d6bad23558e6ed36b2f328522b721a0062c13d9726eab3",
                "72b979ae5253861c7521b874142b825f8d7c45d61b8511d81714282a038404d7",
            ),
            (
                "intent_scenario_en-US",
                "wake me up at five am this week",
                "4fcd65c921ea54db0610648336c8888795dda8e58979608ca9487f7dabab84b6",
                "8c37bfab8b28d352f26e3de121508f1f6fe3466a66a48d4ef17cc0b4c6fda748",
            ),
        )
        for name, text, expected_id, expected_request in vectors:
            entry = contract.profile_entry(name)
            schema, _ = massive.schema_and_metrics(
                entry["profile"], oracle.read_json(massive.MANIFEST)
            )
            fixture = {
                "schema": schema,
                "options": {
                    ...
                    "overlap": "flat",
                    "best_effort": False,
                    "word_splitter": entry["profile"]["word_splitter"],
                },
                "offset_unit": "utf8_bytes",
            }
            ...
            self.assertEqual(
                expected_request,
                facts.digest(
                    facts.encoded(contract.request_for(fixture, {"text": text}))
                ),
            )
    ...
    def test_all_ten_fixed_schemas_splitters_and_full_three_shard_ranges(self):
        ...
        self.assertEqual(
            {"char", "whitespace"},
            {row["profile"]["word_splitter"] for row in registry["profiles"]},
        )

    def test_mutation_rehash_does_not_authorize_new_text_schema_order_or_source_identity(
        self,
    ):
        ...
        fixture["cases"][0]["request_sha256"] = facts.digest(
            facts.encoded(contract.request_for(fixture, fixture["cases"][0]))
        )
        fixture["transport"]["request_sha256s_sha256"] = facts.digest(
            facts.encoded([row["request_sha256"] for row in fixture["cases"]])
        )
    ...
    def test_embedded_worker_pins_match_audited_registry(self):
        source = (
            contract.HERE.parents[1] / "src/bench/gliner25_evaluation_worker.zig"
        ).read_text()
        ...
        self.assertEqual(
            capture.zig_pins(
                contract.load_registry(), oracle.sha256_file(contract.REGISTRY)
            ),
            actual,
        )
    ...
    def test_frozen_crossner_identity_and_v1_fixture_unchanged(self):
        ...
        fixture = {
            "format_version": 1,
            "scope": evaluate.WORKER_SCOPE,
            "qualification": False,
            "source_commit": oracle.UPSTREAM_COMMIT,
            "model": "small",
            "source_files": evaluate.source_identity("small")["source_files"],
            **{
                key: "1" * 64
                for key in ("lock_sha256", "prepared_sha256", "requests_sha256")
            },
        ...
        request = {
            "text": "Alice",
            "schema": {
                "entities": oracle.read_json(evaluate.adapter.MANIFEST)["entity_types"]
            },
            "options": evaluate.REQUEST_OPTIONS.copy(),
            "offset_unit": "utf8_bytes",
        }
        fixture["cases"] = [
            {
...
class OutputTest(unittest.TestCase):
    def fixture(self, profile_name):
        ...
        schema, _ = massive.schema_and_metrics(
            profile, oracle.read_json(massive.MANIFEST)
        )
        return {
            "profile": profile_name,
            "schema": schema,
    ...
    def test_python_routes_ordinary_and_constrained_schemas_without_ignored_fields(
        self,
    ):
        class Builder:
            def __init__(self):
            ...
            def classification(self, **kwargs):
                self.calls.append(("classification", kwargs))

            def single(self, *args, **kwargs):
                self.calls.append(("single", args, kwargs))

            def constrain(self, value):
                self.calls.append(("constrain", value))

        modules = {
            "gliner2": SimpleNamespace(Schema=Builder),
            "gliner2.classification": SimpleNamespace(ClassificationSchema=Builder),
            "gliner2.classification.constraints": SimpleNamespace(
                constraint_from_dict=copy.deepcopy
            ),
        }
        with mock.patch.dict("sys.modules", modules):
            ...
            self.assertEqual(
                [
                    (
                        ...
                        {
                            "task": "intent",
                            "labels": ordinary["schema"]["classifications"][0][
                                "labels"
                            ],
                            "multi_label": False,
                            "cls_threshold": 0.5,
            ...
            self.assertEqual(
                constrained["schema"]["classification_constraints"],
                [row[1] for row in built.calls if row[0] == "constrain"],
            )
    ...
    def test_entity_unicode_source_offsets_and_confidence_parity(self):
        ...
        request = {
            "text": "😀明天",
            **{key: fixture[key] for key in ("schema", "options", "offset_unit")},
        }
        source = {"entities": {name: [] for name in fixture["schema"]["entities"]}}
        source["entities"]["date"] = [
            {"text": "明天", "start": 1, "end": 3, "confidence": 0.9}
        ]
        native = {
            "entities": [
                {"name": name, "values": []} for name in fixture["schema"]["entities"]
            ]
        }
        next(group for group in native["entities"] if group["name"] == "date")[
            "values"
        ] = [
            {
                "text": "明天",
                "source": {"start": 4, "end": 10, "unit": "utf8_bytes"},
                "confidence": 0.9,
            }
        ]
        ...
        native["entities"].pop()
        with self.assertRaises(ValueError):
            contract.canonical_output(request, native, "native", profile)
    ...
    def test_constrained_labels_and_real_solver_status_are_required(self):
        ...
        source = {
            "intent": {"value": "alarm_set", "confidence": 0.8},
            "scenario": {"value": "alarm", "confidence": 0.9},
            ...
            "classifications": [
                {
                    ...
                    "multi_label": False,
                    "labels": [
                        {"label": row["value"], "confidence": row["confidence"]}
                    ],
                }
        ...
        self.assertEqual(
            contract.canonical_output(request, source, "python", profile),
            contract.canonical_output(request, native, "native", profile),
        )
    ...
    def test_capacity_error_is_a_result_with_all_fixed_metrics_and_cannot_pass_parity(
        self,
    ):
        ...
        event = {
            ...
            "error_code": "BoundarySequenceLimitExceeded",
            "input_ids": None,
        }
        prediction, canonical = contract.response_prediction(
            fixture, case, event, "native"
        )
        self.assertIsNone(canonical)
        self.assertEqual(61, len(prediction["metrics"]))
        self.assertTrue(all(value == [] for value in prediction["metrics"].values()))
        self.assertFalse(
            contract.compare_response(fixture, case, event, "metal", event, "native")[
                "parity_pass"
            ]
        )
        event["input_ids"] = [1]
        with self.assertRaisesRegex(ValueError, "malformed"):
            contract.response_prediction(fixture, case, event, "native")
    ...
    def test_exact_token_and_unchanged_confidence_tolerance(self):
                ...
                "classifications": [
                    {
                        "name": "intent",
                        "multi_label": False,
                        "labels": [{"label": "alarm_set", "confidence": 0.9}],
                    }
                ]
            },
        }
        ...
        self.assertTrue(
            contract.compare_response(fixture, case, event, "metal", other, "native")[
                "parity_pass"
            ]
        )
        other["input_ids"] = [2, 4]
        ...
        other = copy.deepcopy(event)
        other["output"]["classifications"][0]["labels"][0]["confidence"] -= 0.00051
        self.assertFalse(
            contract.compare_response(fixture, case, event, "metal", other, "native")[
                "parity_pass"
            ]
...
class AggregateTest(unittest.TestCase):
    def reports(self):
        ...
        identity = {
            ...
            "resource_policy": {"threads": 1},
            "source_reference_report_sha256": None,
            "native_reference_report_sha256": None,
            "python_runtime": runner.python_runtime_identity(),
        }
        reports = [
            {
                "scope": contract.SHARD_SCOPE,
                "status": "complete",
                "qualification": False,
                **identity,
                "transport": {
                ...
                "denominator": row["end"] - row["start"],
                "processed": row["end"] - row["start"],
                "unprocessed": 0,
                "errors": 0,
            }
    ...
    def test_all_three_complete_shards_are_required_with_identical_execution(self):
        ...
        for changed in (
            ...
            with self.assertRaises(ValueError):
                runner.validate_shard_set(changed, entry)
        for key, value in (
            ("status", "incomplete"),
            ("binary_sha256", "b" * 64),
            ("artifact", {"precision": "q8_0"}),
            ("unprocessed", 1),
            ("processed", 0),
            ("resource_policy", {"threads": 2}),
            ...
            with self.subTest(key=key), self.assertRaises(ValueError):
                runner.validate_shard_set(changed, entry)
        reports[1]["errors"] = 1
        runner.validate_shard_set(
            reports, entry
        )  # complete transport; error remains in denominator
    ...
    def test_receipt_publish_never_overwrites_and_cleans_failed_staging(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaises(FileExistsError):
                contract.atomic_json(path, {"changed": True})
            self.assertEqual(before, path.read_bytes())
            self.assertEqual([path], list(path.parent.iterdir()))
            with mock.patch.object(
                os := contract.os, "fsync", side_effect=OSError("disk full")
            ):
                with self.assertRaises(OSError):
                    contract.atomic_json(
                        path.parent / "second.json", {"complete": True}
                    )
            self.assertEqual([path], list(path.parent.iterdir()))
    ...
    def test_consumed_evidence_hash_and_json_protocol_cannot_be_substituted(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                [{"event": "error"}],
                runner.evidence_rows(path.parent / "report.json", expected, 1024),
            )
            ...
            with self.assertRaises(ValueError):
                runner.evidence_rows(
                    path.parent / "report.json", contract.pin(path), 1024
                )
    ...
    def test_shard_output_budget_and_cli_profile_controls_are_explicit(self):
        ...
        with mock.patch.object(contract, "MAX_SHARD_BYTES", 4):
            with self.assertRaisesRegex(ValueError, "budget"):
                runner.write_line(target, {"too": "large"})
        ...
        self.assertEqual(
            ("entities_ja-JP_char", 2, "metal"),
            (args.profile, args.shard, args.backend),
        )
        self.assertEqual(125, args.response_timeout)


class PythonInvocationTest(unittest.TestCase):
    def test_worker_command_preserves_real_symlink_venv_and_hashes_executable_target(
        self,
    ):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                oracle.sha256_file(invocation.resolve()), identity["executable_sha256"]
            )
            self.assertEqual(str(environment), identity["prefix"])
            self.assertNotEqual(identity["prefix"], identity["base_prefix"])
            self.assertEqual(
                oracle.sha256_file(environment / "pyvenv.cfg"),
                identity["pyvenv_cfg"]["sha256"],
            )
            self.assertEqual(
                first["command"][-1], facts.digest(facts.encoded(identity))
            )
            second = run(first["command"][0])
            self.assertEqual(
                first, second
            )  # The exact production command retains the venv-only import.
            with (environment / "pyvenv.cfg").open("a") as target:
                target.write("# external configuration change\n")
            changed = run(first["command"][0])
            self.assertNotEqual(
                identity["pyvenv_cfg"], changed["identity"]["pyvenv_cfg"]
            )
            self.assertNotEqual(first["command"][-1], changed["command"][-1])
            self.assertEqual(
                identity["executable_sha256"], changed["identity"]["executable_sha256"]
            )
    ...
    def test_wrong_interpreter_identity_rejects_before_fixture_or_runtime_load(self):
        ...
        with (
            mock.patch.object(runner.oracle, "prepare_runtime") as runtime,
...
class TransportLifecycleTest(unittest.TestCase):
    def exercise(self, temporary: Path, *, cancel=False, corrupt=False):
        ...
        profile = entry["profile"]
        schema, metrics = massive.schema_and_metrics(
            profile, oracle.read_json(massive.MANIFEST)
        )
        options = {
            "best_effort": False,
        ...
        for index in range(3):
            ...
            common = {
                "request_id": facts.digest(
                    facts.encoded({"lock": entry["lock_sha256"], "id": source})
                ),
                "request_sha256": facts.digest(facts.encoded(request)),
            }
            ...
            gold.append(
                {
                    **common,
                    "family_id": f"test-family-{index}",
                    "language": "en",
                    "metrics": massive.gold_facts(
                        {"intent": "alarm_set"},
                        profile,
                        oracle.read_json(massive.MANIFEST),
                    ),
                }
            )
            cases.append(
            ...
            entry["shards"][index] = {
                ...
                "start": index,
                "end": index + 1,
                "shard_sha256": str(index) * 64,
                "request_ids_sha256": facts.digest(
                    facts.encoded([common["request_id"]])
                ),
                "request_sha256s_sha256": facts.digest(
                    facts.encoded([common["request_sha256"]])
                ),
            }
        for name, values in (("requests", requests), ("gold", gold)):
            (prepared / f"{name}.jsonl").write_bytes(
                b"".join(facts.encoded(row) + b"\n" for row in values)
            )
        entry["requests_sha256"] = oracle.sha256_file(prepared / "requests.jsonl")
        receipt = {
            "scope": "gliner25_blinded_evaluation/v1",
            "status": "complete",
            "qualification": False,
            "harness_sha256": registry["frozen_helpers"]["evaluation_contract.py"],
            "metrics": metrics,
            "requests_sha256": entry["requests_sha256"],
            "gold_sha256": oracle.sha256_file(prepared / "gold.jsonl"),
            "lock_sha256": entry["lock_sha256"],
            "records": 3,
        }
        ...
        class FakeWorker:
            def __init__(self, backend, command, env, directory, guard):
                ...
                self.process = SimpleNamespace(
                    poll=lambda: 0, returncode=0, stdout=io.BytesIO()
                )
                ...
                self.ready = {
                    ...
                    "backend": backend,
                    "qualification": False,
                    "artifact_kind": "source_fp32",
                    "receipt": None,
                    "source_files": fixture["source_files"],
                    "model": "small",
                    "source_commit": oracle.UPSTREAM_COMMIT,
                    "fixture_sha256": oracle.sha256_file(
                        directory / "requests.fixture.json"
                    ),
                    ...
                    "word_splitter": "whitespace",
                    "math_policy": "torch_f32_cpu_v1",
                    "weight_precision": "fp32",
                    "activation_precision": "f32",
                    "accumulation_precision": "f32",
                    "head_precision": "f32",
                    "threads": 1,
                    "interop_threads": 1,
                    "provenance": {"commit": oracle.UPSTREAM_COMMIT},
                    "python_runtime": runner.python_runtime_identity(),
                }
                ...
                self.response = {
                    ...
                    "request_sha256": case["request_sha256"],
                    "input_ids": [1, 2],
                    "output": {"intent": {"label": "alarm_set", "confidence": 0.9}},
                }
        ...
        with (
            mock.patch.object(contract, "RECORDS", 3),
            mock.patch.object(contract, "RANGES", ((0, 1), (1, 2), (2, 3))),
            mock.patch.object(contract, "load_registry", return_value=registry),
            mock.patch.object(
                contract, "admit_profile", return_value=(entry, requests, cases)
            ),
            mock.patch.object(evaluate, "model_artifact", return_value=artifact),
            mock.patch.object(runner.bench, "ResourceGuard", FakeGuard),
            mock.patch.object(runner.bench, "Worker", FakeWorker),
        ):
            reports = []
            ...
            if corrupt:
                ...
                value = oracle.read_json(path)
                value["metrics"]["intent_exact"][0]["label"] = "music_query"
                path.write_bytes(facts.encoded(value) + b"\n")
                report = oracle.read_json(reports[0])
                report["files"]["predictions"] = contract.pin(path)
            ...
            args = runner.parser().parse_args(
                [
                    ...
                    profile["id"],
                    "--output-dir",
                    str(temporary / "aggregate"),
                    *[
                        item
                        for path in reports
                        for item in ("--shard-report", str(path))
                    ],
                ]
            )
            if corrupt:
                ...
                reference = runner.reference_report(
                    temporary / "aggregate/report.json", entry, temporary, "python"
                )
