...
source_snapshot, load_contract, sample_receipt = (
    v1.source_snapshot,
    v1.load_contract,
    v1.sample_receipt,
)
...
def run_pair(
    ...
    *,
    ...
    phase: str,
    selected: list[str],
    native_backend: str = "metal",
) -> dict[str, Any]:
    if native_backend not in ("metal", "native") or (
        native_backend == "native" and baseline != "cpu"
    ):
    ...
    if scaling:
        ...
        if cpu_mode or args.legacy_reference_bin:
            raise BenchmarkError(
                "scaling requires the new Metal worker and separate source captures"
            )
    ...
    variant, reference = (
        contract["model"],
        "python" if cpu_mode else f"fastino_{baseline}",
    )
    ...
    commands = {
        ...
        reference: [
            sys.executable,
            str(HERE / "metal_python_worker_v2.py"),
            "--device",
            baseline,
            "--model",
            variant,
    ...
    run = {
        ...
        "execution_policy": "native_cpu_reference_v1"
        if cpu_mode
        else args.execution_policy,
        "model": variant,
        "baseline": baseline,
        "phase": phase,
    ...
    def checked(arm, response, name, *, validation, stage):
        if scaling:
            return scaling_runtime_v1.result(
                arm,
                response,
                contract["source_cases"][name],
        ...
        if cpu_mode:
            if response.get("event") == "error":
                raise cpu.BenchmarkError(
                    f"CPU worker error: {response.get('message', '')}"
                )
            ...
            if validation:
                ...
                if (
                    ...
                    or not 1 <= len(ids) <= oracle.MAX_ENCODED_TOKENS
        ...
        if arm == native_arm:
            ...
            return native_receipt.result(
                response, contract["expected"][name], validation=validation, phase=stage
            )
        if response.get("scope") != active_scope:
            raise BenchmarkError("Python result changed comparison scope")
        return v1.checked_result(
            response, contract["expected"][name], arm, validation=validation
        )
    ...
    try:
        for arm in paired_benchmark.balanced_pair_order(
            repetition + 1, reference, native_arm
        ):
            ...
            if scaling:
                receipt = scaling_runtime_v1.ready(
                    arm, ready, contract, args.execution_policy
                )
                if arm == native_arm:
                    native_receipt = receipt
            elif cpu_mode:
                cpu.checked_ready(
                    arm, ready, contract["bundle"], contract["case_path"], 1
                )
                if (
                    type(ready.get("threads")) is not int
                    or ready.get("qualification") is not False
                ):
                    raise BenchmarkError("CPU ready counter/qualification types differ")
            elif arm == native_arm:
                native_receipt = runtime.native_ready(
                    ready,
                    contract["bundle"],
                    variant,
                    contract["case_path"],
        ...
        for name in selected:
            ...
            try:
                normalized = {
                    arm: checked(
                        arm, responses[arm], name, validation=True, stage="validation"
                    )
                    for arm in arms
                }
                if (
                    responses[native_arm]["input_ids"]
                    != responses[reference]["input_ids"]
                ):
                    raise cpu.BenchmarkError(
                ...
                if scaling:
                    for response in responses.values():
                        scaling_contract_v1.check_packet(
                            response,
                            contract["source_cases"][name]["expected_input_ids"],
                        )
                    captured_ids = captured_ids["input_ids"]
                if (
                    captured_ids is not None
                    and responses[native_arm]["input_ids"] != captured_ids
                ):
                    raise cpu.BenchmarkError(
                        "encoder tokens differ from the frozen oracle capture"
                    )
                run["validation"][name] = {
                    "input_ids": responses[native_arm]["input_ids"],
                    "outputs": normalized,
                ...
                if scaling:
                    run["validation"][name].update(
                        attention_mask=responses[native_arm]["attention_mask"],
                ...
                for name in rotate(eligible.copy(), repetition + iteration):
                    try:
                        for arm in paired_benchmark.balanced_pair_order(
                            ...
                            checked(
                                arm, response, name, validation=False, stage="warmup"
                            )
                    except (cpu.BenchmarkError, KeyError, TypeError) as error:
                ...
                for name in rotate(eligible.copy(), repetition + pair - 1):
                    ...
                    try:
                        for arm in row["order"]:
                            ...
                            row[arm] = (
                                scaling_runtime_v1.sample_receipt(response)
                                if scaling
                                else sample_receipt(response)
                            )
        ...
        if (
            scaling
            and scaling_contract_v1.load(contract["prepared"], variant)["pins"]
            != contract["pins"]
        ):
        ...
        if phase == "measurement":
            for name in eligible:
                ...
                pairs = [
                    (row[reference]["duration_ns"], row[native_arm]["duration_ns"])
                    for row in rows
                ]
                run["comparisons"][name] = {
                    ...
                    "native_ns"
                    if cpu_mode
                    else "metal_ns": paired_benchmark.distribution(
                ...
                if scaling:
                    run["comparisons"][name].update(
                        descriptive_python_over_metal_ratio=statistics.median(
                            python / native for python, native in pairs
                        ),
                    ...
                    run["comparisons"][name][
                        "native_over_python_latency"
                        if cpu_mode
                        else "python_over_metal_speedup"
                    ] = paired_benchmark.paired_log_ratio_ci(
                        [(native, python) for python, native in pairs]
                        if cpu_mode
                        else pairs,
                        samples=2000 if cpu_mode else BOOTSTRAP_SAMPLES,
                        seed=SEED,
                    )
    ...
    for contract in report["contracts"]:
        for name in contract["cases"]:
            for baseline in report["baselines"]:
                ...
                exact_repetitions = all(
                    type(value) is int for value in repetition_ids
                ...
                if not exact_repetitions:
                    row["error"] = (
                        "measurement repetitions must have exact unique IDs from zero through count minus one"
                    )
                if (
                    exact_repetitions
                    and len(observations) == count
                    and same_power_profile
                ):
                    row.update(v1.completed_comparison(observations))
                row["execution_policy"] = report["execution_policy"]
                if baseline == "mps":
                    row["milestone"] = runtime.competitiveness(
                        [obs["python_over_metal_speedup"] for obs in observations]
...
def markdown_report(report: dict[str, Any]) -> str:
    return v1.markdown_report(
        report,
        title="GLiNER2.5 Metal comparison v2",
        details=f"Execution policy: **{report['execution_policy']}**. Purpose: **{report['purpose']}**.",
    )
...
def driver(args: argparse.Namespace) -> int:
    ...
    v1.validate_campaign_limits(args)
    ...
    report = {
        ...
        "legacy_reference_binary": args.legacy_reference_bin,
        "scope": SCOPE,
    ...
    try:
        v1.run_campaign(
            args, report, contracts, variants, baselines, run_pair, purpose=args.purpose
        )
        ...
        finished = (
            all(row["status"] == "complete" for row in report["summary"])
        ...
        report["metal_mps_latency_gate_passed"] = (
            args.purpose == "benchmark"
            and args.execution_policy == "optimized_v2"
            and variants == list(VARIANTS)
            and len(milestone_rows) == 30
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        "--execution-policy", choices=tuple(runtime.POLICIES), required=True
    )
    parser.add_argument(
        "--legacy-reference-bin",
        action="store_true",
        help="explicitly use an original v1 executable without the new CLI flag",
    )
    parser.add_argument(
        "--purpose", choices=("benchmark", "diagnostic"), default="benchmark"
    )
    v1.add_campaign_arguments(parser)
