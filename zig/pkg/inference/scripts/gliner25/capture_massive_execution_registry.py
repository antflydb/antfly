...
def zig_pins(registry: dict, registry_sha256: str) -> str:
    ...
    lines = [
        f'const massive_registry_sha256 = "{registry_sha256}";',
    ...
    for row in registry["profiles"]:
        ...
        for shard in row["shards"]:
            lines.append(
                f'            .{{ .shard = "{shard["shard_sha256"]}", .ids = "{shard["request_ids_sha256"]}", .requests = "{shard["request_sha256s_sha256"]}" }},'
            )
        lines.extend(["        },", "    },"])
    lines.append("};")
    return "\n".join(lines) + "\n"
...
def capture(root: Path) -> dict:
    ...
    contract.checked(
        oracle.sha256_file(root / "preparation.json") == evidence["preparation_sha256"],
    ...
    for profile, approved in zip(profiles, evidence["profiles"], strict=True):
        ...
        for name, key in (
            ...
            path = directory / name
            contract.checked(
                oracle.sha256_file(path) == approved[key],
                "audited profile bytes differ",
            )
        ...
        for shard in transport["shards"]:
            ...
            contract.checked(
                oracle.read_json(path)["requests"] == selected,
                "shard differs from ordered requests",
            )
            shards.append(
                {key: shard[key] for key in ("index", "start", "end")}
                | {
                    "shard_sha256": shard["file"]["sha256"],
                    "request_ids_sha256": contract.digest(
                        contract.encoded([row["request_id"] for row in selected])
                    ),
                    "request_sha256s_sha256": contract.digest(
                        contract.encoded([row["request_sha256"] for row in selected])
                    ),
                }
            )
        rows.append(
