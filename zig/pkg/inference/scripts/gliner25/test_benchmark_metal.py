...
def contract():
    ...
    return {
        ...
        "cases": names,
        "expected": {
            case["id"]: bench.cpu.canonical_result(case["expected"])
...
def ready(arm, fixture):
    ...
    if arm == bench.NATIVE:
        ...
        result.update(
            ...
            requests_sha256=bench.oracle.sha256_file(
                bench.oracle.FIXTURES / "requests.json"
            ),
...
class ContractTests(unittest.TestCase):
    def test_readiness_rejects_wrong_device_sync_artifact_and_build(self):
        ...
        for arm in (bench.NATIVE, "fastino_mps", "fastino_cpu"):
            ...
            bench.checked_ready(
                arm, correct, fixture["bundle"], "small", fixture["case_path"]
            )
            mutations = [
                ("revision", "wrong"),
                ("threads", 2),
            ...
            if arm == bench.NATIVE:
                ...
                mutations += [
                    ("device", "cpu" if arm == "fastino_mps" else "mps"),
                    ("provenance", {"device": "cuda"}),
                    ("synchronization_policy", "none"),
                    ("math_policy", "fallback_allowed"),
                    ("deterministic_algorithms", False),
                ]
            for key, value in mutations:
                with (
                    self.subTest(arm=arm, key=key),
    ...
    def test_frozen_token_evidence_covers_all_thirty_ordered_requests(self):
        ...
        for variant in bench.VARIANTS:
            with (
                ...
                mock.patch.object(bench.oracle, "verify_model_dir", return_value={}),
            ):
            ...
            self.assertEqual(
                {
                    name: packet["input_ids"]
                    for name, packet in saved["validation"].items()
                },
            ...
            self.assertTrue(
                all(
                    ids and all(type(token) is int for token in ids)
    ...
    def test_token_evidence_rejects_profile_bundle_request_and_value_drift(self):
        ...
        for index, mutate in enumerate(mutations):
            ...
            def read(path):
                return (
                    report
                    if Path(path).name == "token_evidence.json"
                    else original_read(path)
                )

            with (
                self.subTest(mutation=index),
                mock.patch.object(bench.oracle, "read_json", side_effect=read),
                self.assertRaises(bench.BenchmarkError),
            ):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch.object(bench.oracle, "FIXTURES", directory):
                with self.assertRaisesRegex(
                    bench.oracle.ContractError, "required regular file is missing"
                ):
                    bench.reference_input_ids("small", fixture, case_path)
                (directory / "token_evidence.json").write_bytes(raw[:-1] + b" ")
                with self.assertRaisesRegex(
                    bench.oracle.ContractError, "SHA256 mismatch"
                ):
                    bench.reference_input_ids("small", fixture, case_path)
        ...
        *,
        ...
        with tempfile.TemporaryDirectory() as temporary:
            args = types.SimpleNamespace(
                native_bin=Path("/unused/metal"),
                model_root=Path("/unused/models"),
                upstream=Path("/unused/upstream"),
                warmup=1,
                pairs=2,
                timeout_ms=1000,
                startup_timeout=10,
            ...
            with (
                mock.patch.object(bench, "Worker", FakeWorker),
                mock.patch.object(bench, "ResourceGuard", FakeGuard),
                mock.patch.object(
                    bench,
                    "hardware_receipt",
                    side_effect=[
                        ...
                        {
                            "power_source": "battery" if power_change else "ac",
                            "low_power_mode": "0",
                        },
                    ],
                ),
                mock.patch.object(
                    bench.oracle, "verify_model_dir", return_value=fixture["bundle"]
                ),
                mock.patch.object(bench.oracle, "verify_upstream_checkout"),
            ):
                result = bench.run_pair(
                    args,
                    fixture,
                    "mps",
                    directory,
    ...
    def test_validation_precedes_timing_and_pair_order_is_balanced(self):
        ...
        calls = [
            (arm, op) for arm, op, _ in events if op not in ("start", "close", "stop")
        ]
        self.assertEqual(
            [(bench.NATIVE, "validate"), ("fastino_mps", "validate")] * 2, calls[:4]
        )
        for name in contract()["cases"]:
            rows = [row for row in result["pairs"] if row["case_id"] == name]
            self.assertEqual(
                [(bench.NATIVE, "fastino_mps"), ("fastino_mps", bench.NATIVE)],
                [row["order"] for row in rows],
            )
            self.assertEqual(
                2.0, result["comparisons"][name]["python_over_metal_speedup"]["median"]
            )
    ...
    def test_recoverable_unsupported_case_is_preserved_without_discarding_other_rows(
        self,
    ):
        def unsupported(arm, op, name, count, response, fixture):
            if (
                arm == "fastino_mps"
                and name == fixture["cases"][0]
                and op == "validate"
            ):
                response.clear()
                response.update(
        ...
        self.assertTrue(
            any(event.get("response", {}).get("event") == "error" for event in journal)
        )

    def test_measured_parity_failure_invalidates_entire_case_but_retains_raw_samples(
        self,
    ):
        ...
        self.assertNotIn(failed, result["comparisons"])
        self.assertEqual(
            2, len([row for row in result["pairs"] if row["case_id"] == failed])
        )
        self.assertTrue(any(not row["valid"] for row in result["pairs"]))
        self.assertEqual(
            7, len([event for event in journal if event["stage"] == "measurement"])
        )
    ...
    def test_native_memory_leak_and_missing_dispatch_are_fatal(self):
        for failure in (
            ...
            def corrupt(arm, op, name, count, response, fixture):
                if arm == bench.NATIVE:
                    if failure == "leak":
                        response["owned_memory"]["after"]["device_owned_live_bytes"] = (
                            1024
                        )
                        ...
                        response["owned_memory"]["after"]["host_mirror_live_bytes"] = (
                            0.0
                        )
                    else:
                        response["host_fallback_evidence"][
    ...
    def test_power_change_retains_samples_but_invalidates_repetition(self):
        ...
        self.assertTrue(
            all(
                value["status"] == "blocked" for value in result["case_status"].values()
            )
        )
        self.assertEqual(
            8, len([event for event in journal if event["stage"] == "measurement"])
        )
    ...
    def run_driver_fake(self, *, cleanup_error=False, source_drift=False):
        ...
        final = (
            {**initial, "source_files_sha256": "1" * 64} if source_drift else initial
        )
        ...
        def pair(args, fixture, baseline, directory, *, repetition, phase, selected):
            ...
            run = {
                ...
                "case_status": {
                    name: {
                        "status": "complete" if phase == "measurement" else "validated"
                    }
            ...
            if phase == "measurement":
                run["comparisons"] = {
                    name: {
                        "metal_ns": {"median": 1e6},
                        "python_ns": {"median": 2e6},
                        "python_over_metal_speedup": {
                            "median": 2,
                            "lower_95": 1.5,
                            "upper_95": 2.5,
                        },
            ...
            if cleanup_error:
                run.update(
                    status="failed", cleanup_errors=["surviving owned descendant"]
                )
            return run
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                mock.patch.object(bench, "run_pair", side_effect=pair),
                mock.patch.object(bench, "load_contract", side_effect=load),
                mock.patch.object(
                    bench, "source_snapshot", side_effect=[initial, final]
                ),
                ...
                mock.patch.object(
                    bench.oracle, "verify_upstream_checkout", return_value={}
                ),
    ...
    def test_entire_preflight_matrix_precedes_fresh_rotating_measurement_repetitions(
        self,
    ):
        ...
        for repetition, order in enumerate(
            ...
            self.assertEqual(
                [model for model in order for _ in range(2)],
                [
                    event[2]
                    for event in events[6 + repetition * 6 : 12 + repetition * 6]
                ],
            )
    ...
    def test_repeatability_requires_all_intervals_and_all_repetitions(self):
        ...
        report = {
            "contracts": [{"model": "small", "cases": [name]}],
            "baselines": ["mps"],
            "repetitions": 3,
            "runs": [],
        }
