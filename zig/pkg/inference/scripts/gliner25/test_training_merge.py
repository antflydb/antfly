        ...
        return {
            name: (
                ...
                list(
                    struct.unpack(
                        "<" + "f" * (item["size_bytes"] // 4),
                        b"".join(file.chunks(item["offset"], item["size_bytes"])),
                    )
                ),
            )
            for name, item in exports.tensor_header(file).items()
        }
...
def scaffold(root, mode="lora"):
    ...
    pins["model.safetensors"] = fixtures.write_tensors(
        source / "model.safetensors", original
    )
    ...
    adapter_receipt.update(
        source=identity, frozen_weight_sha256=pins["model.safetensors"]["sha256"]
    )
    adapter_pin = fixtures.write_json(
        adapter / exports.ADAPTER_RECEIPT, adapter_receipt
    )
    ...
    configuration = {
        ...
        "output_dir": str((root / "merged").resolve()),
        "expected_source": identity,
        "expected_adapter": adapter_files,
    ...
    values["encoder.encoder.layer.0.weight"] = ([2, 2], [5.0, 6.0, 7.0, 8.0])
    merged_pins = {
        "model.safetensors": fixtures.write_tensors(
            merged / "model.safetensors", values
        )
    }
    ...
    receipt = {
        **contract["native_receipt"],
        ...
        "target_sha256": adapter_receipt["target_sha256"],
        "parameter_sha256": adapter_receipt["parameter_sha256"],
        "merged": exports.source_identity("small", merged_pins),
        "tensor_count": 334,
...
def prepared(mode="lora"):
    with tempfile.TemporaryDirectory() as temporary:
        ...
        with (
            mock.patch.object(exports, "expected_source", return_value=value.pins),
            mock.patch.object(
                exports, "published_inventory", return_value=value.inventory
            ),
        ):
            yield value
...
def args(value):
    return check.parser().parse_args(
        [
            ...
            str(value.config),
            "--output-dir",
            str(value.root / "report"),
            "--runtime",
            "--peft-wheel",
            str(value.root / "unused-wheel.whl"),
        ]
    )


class MergeIntegrityTests(unittest.TestCase):
    def test_both_modes_keep_full334_source_bias_sidecars_and_final_training_state(
        self,
    ):
        for mode in ("lora", "dora"):
            with self.subTest(mode=mode), prepared(mode) as value:
                ...
                self.assertEqual(
                    (334, 333, 1),
                    (
                        len(result["merged_tensors"]),
                        len(result["untouched_names"]),
                        len(result["adapted_names"]),
                    ),
                )
                self.assertTrue(result["untouched_and_bias_bytes_equal"])
                self.assertTrue(
                    result["training_export"]["job"]["state_digest_recomputed"]
                )
    ...
    def test_rehashed_untouched_weight_and_bias_mutations_are_rejected(self):
        for name in (
            "frozen.constant.000",
            "encoder.encoder.layer.0.bias",
            "classifier.weight",
        ):
            with self.subTest(name=name), prepared() as value:
                rewrite_merged(
                    value, lambda tensors: tensors[name][1].__setitem__(0, 0.25)
                )
                with self.assertRaisesRegex(ValueError, "untouched tensor or bias"):
                    audit(value)

    def test_complete_inventory_shape_nonfinite_and_sidecar_cannot_be_rehashed_away(
        self,
    ):
        mutations = (
            lambda tensors: tensors.pop("frozen.constant.000"),
            lambda tensors: tensors.__setitem__("frozen.extra", ([1], [0.0])),
            lambda tensors: tensors.__setitem__(
                "encoder.encoder.layer.0.weight", ([1, 4], [1.0, 2.0, 3.0, 4.0])
            ),
            lambda tensors: tensors["encoder.encoder.layer.0.weight"][1].__setitem__(
                0, float("inf")
            ),
        )
    ...
    def test_exact_job_bytes_schema_adapter_counts_and_math_policy_are_bound(self):
        ...
        with prepared() as value:
            ...
            with self.assertRaisesRegex(ValueError, "job bytes"):
                audit(value)
        with prepared() as value:
            configuration = json.loads(value.config.read_text())
            configuration["schema_sha256"] = [4] * 32
            receipt = json.loads((value.merged / check.RECEIPT).read_text())
            receipt["provenance"]["configuration"] = fixtures.write_json(
                value.config, configuration
            )
            fixtures.write_json(value.merged / check.RECEIPT, receipt)
            with self.assertRaisesRegex(ValueError, "job schema"):
                audit(value)
        for field, content in (
            ("version", True),
            ("source_dir", "/source/../other"),
            ("memory", {"job_bytes": 1.5}),
            ("merge_limits", {"adapter": {"max_rank": True}}),
        ):
    ...
    def test_static_run_publishes_narrow_scope_failure_is_retained_and_never_overwrites(
        self,
    ):
        with prepared() as value:
            selected = args(value)
        ...
        with prepared() as value:
            ...
            selected.runtime, selected.peft_wheel = False, None
            rewrite_merged(
                value, lambda tensors: tensors["classifier.bias"][1].__setitem__(0, 2.0)
            )
            with self.assertRaises(ValueError):
                check.run(selected)
...
class MergeNumericalTests(unittest.TestCase):
    def test_absolute_plus_relative_rule_nearzero_large_negative_nonfinite_and_full_count(
        self,
    ):
        row = check.numeric_comparison([0.0, -10.0, 1000.0], [1e-6, -10.0001, 1000.01])
        self.assertEqual((3, 0), (row["elements"], row["violations"]))
        row = check.numeric_comparison(
            [0.0, -10.0, 1000.0], [math.nextafter(1e-6, math.inf), -10.001, 1000.1]
        )
        self.assertEqual(
            (3, [0, 1, 2]), (row["violations"], row["first_violation_indices"])
        )
        for actual in ([float("nan")], [float("inf")], [True], []):
            with self.assertRaises(ValueError):
                check.numeric_comparison([0.0], actual)

    def test_threeway_complete_token_decision_metadata_probability_and_confidence_gates(
        self,
    ):
        request = {"id": "one"}
        output = {
            "intent": {
                "value": "keep",
                "confidence": 0.75,
                "probabilities": {"keep": 0.75, "delete": 0.25},
            },
            "_meta": {
                "objective": 1.0,
                "feasible": True,
        ...
        row = {
            "id": "one",
            "request_sha256": check.object_digest(request),
            "input_ids": [1, 2],
            "output": output,
        }
        ...
        self.assertTrue(
            all(result["pass"] for result in check.compare_phases(captures, [request]))
        )
        mutations = (
            lambda item: item["input_ids"].__setitem__(1, 3),
            lambda item: item["output"]["intent"].__setitem__("confidence", 0.751),
            lambda item: item["output"]["intent"]["probabilities"].__setitem__(
                "delete", 0.251
            ),
        ...
        for mutation in mutations:
            ...
            self.assertFalse(
                all(
                    result["pass"]
                    for result in check.compare_phases(changed, [request])
                )
            )
        del captures["native_merged"]
        with self.assertRaisesRegex(ValueError, "incomplete"):
            check.compare_phases(captures, [request])

    def test_comparison_summary_cannot_omit_untouched_or_change_denominators(self):
        audit = {
            "merged_tensors": {
                "x.weight": {"size_bytes": 8},
                "x.bias": {"size_bytes": 4},
            },
            "adapted_names": ["x.weight"],
        }
        rows = [
            {
                "name": "x.weight",
                "kind": "numerical",
                **check.numeric_comparison([0.0, 1.0], [0.0, 1.0]),
            },
        ...
        for changed in (
            rows[:1],
            [rows[0], rows[0]],
            [dict(rows[0], elements=1), rows[1]],
            [rows[0], dict(rows[1], bytes_equal=False)],
        ):
            with self.assertRaises(ValueError):
                check.validate_tensor_results(changed, audit)
...
class MergeLifecycleTests(unittest.TestCase):
    def exercise(self, value, failure=None):
        ...
        class FakeWorker:
            def __init__(self, arm, command, env, directory, guard):
                ...
                (Path(envelope["scratch_dir"]) / "partial-copy").write_bytes(
                    b"owned worker bytes"
                )
                self.messages = [
                    {
                        "event": "ready",
                ...
                for kind, phase, identifier in check.event_plan(requests):
                    ...
                    if kind == "case":
                        ...
                        record = {
                            ...
                            "request_sha256": check.object_digest(request),
                            "input_ids": [1, 2],
                            "output": {"value": "same", "confidence": 0.9},
                        }
                        if failure == "comparison" and phase == "native_merged":
                            record["output"]["confidence"] = 0.901
                        event["case"] = record
                ...
                result = {
                    ...
                    "loader_profile": "peft-0.18.0-export-v1",
                    "one_resident_model_owner": True,
                    "owners_released_before_next_load": True,
                    "network_allowed": False,
                    "missing_weight_fallback": False,
                    "quality_evaluation": False,
                    "requests": check.REQUEST_PIN,
                    "private_copy_bytes": 10,
                    "captures": captures,
                    "comparisons": check.compare_phases(captures, requests),
                    "tensor_comparisons": tensor_results,
                ...
                if failure == "substitution":
                    result["comparisons"][0]["pass"] = False
                self.messages.append({"event": "complete", "runtime": result})
                owners.append(self)

            def receive(self, _timeout):
                self.calls += 1
                if failure == "cancel" and self.calls == 3:
                    raise KeyboardInterrupt()
                if failure == "deadline" and self.calls == 3:
        ...
        with (
            mock.patch.object(check, "MergeGuard", FakeGuard),
            mock.patch.object(check.bench, "Worker", FakeWorker),
        ):
            if failure in ("cancel", "deadline", "omitted", "substitution"):
                with self.assertRaises((KeyboardInterrupt, TimeoutError, ValueError)):
                    check.supervise_runtime(
                        audited, selected, selected.output_dir, value.contract
                    )
            else:
                result = check.supervise_runtime(
                    audited, selected, selected.output_dir, value.contract
                )
                self.assertEqual(
                    "comparison_failed" if failure else "verified", result["status"]
                )
    ...
    def test_complete_threephase_protocol_and_numerical_failure_are_distinct(self):
        for failure in (None, "comparison"):
            with self.subTest(failure=failure), prepared() as value:
                self.exercise(value, failure)
    ...
    def test_runtime_owner_views_must_release_before_next_model(self):
        ...
        with (
            mock.patch.object(check, "load_unmerged", new=lambda *_: Model()),
        ...
        with (
            mock.patch.object(check, "load_unmerged", new=leaked),
            mock.patch.object(check, "capture_outputs", new=lambda *_: []),
        ):
    ...
    def test_real_worker_rejects_altered_envelope_before_loading_any_runtime(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            command = [
                check.python_identity()["invocation"],
