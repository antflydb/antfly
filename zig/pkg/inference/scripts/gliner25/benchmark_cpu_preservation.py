...
def checked_model(model, *, check_statistics=True):
    ...
    expected = {
        row["id"]: cpu.canonical_result(row["expected"]) for row in fixture["cases"]
    }
    ...
    if {key: actual_bundle.get(key) for key in bundle} != bundle:
        raise cpu.BenchmarkError(
            "CPU report model identity differs from the pinned FP32 bundle"
        )
    for arm in ("native", "python"):
        cpu.checked_ready(
            arm, model["workers"][arm], bundle, cpu.case_fixture(variant), 1
        )
    provenance = model["workers"]["python"].get("provenance", {})
    manifest = oracle.load_manifest()
    if (
        provenance.get("runtime") != manifest["runtime"]
        or provenance.get("commit") != manifest["upstream"]["commit"]
        or provenance.get("device") != "cpu"
        or provenance.get("dtype") != "float32"
    ):
        raise cpu.BenchmarkError(
            "CPU reference dependency/device/source provenance differs"
        )
    if set(model.get("validation", {})) != set(expected) or set(
        model.get("comparisons", {})
    ) != set(expected):
    ...
    if not isinstance(rows, list) or len(rows) != 30 * len(expected):
        raise cpu.BenchmarkError(
            "CPU report requires exactly thirty pairs per fixed case"
        )
    seen, pairs = set(), {name: [] for name in expected}
    for row in rows:
        case, number = row.get("case_id"), row.get("pair")
        if (
            case not in expected
            or type(number) is not int
            or not 1 <= number <= 30
            or (case, number) in seen
            or row.get("valid", True) is not True
            or sorted(row.get("order", [])) != ["native", "python"]
        ):
            raise cpu.BenchmarkError(
                "CPU pair identity/order/validity is incomplete or duplicated"
            )
        seen.add((case, number))
        values = [
            runtime.integer(row[arm].get("duration_ns"), f"{case}.{arm}.duration_ns")
            for arm in ("native", "python")
        ]
        if not all(values):
    ...
    for name in expected:
        ...
        if (
            validation.get("outputs_match_oracle") is not True
            or validation.get("confidence_absolute_tolerance") != 5e-4
        ):
            raise cpu.BenchmarkError(
                "CPU validation did not preserve the original confidence gate"
            )
        ...
        computed = {
            "native_ns": paired.distribution(n for n, _ in selected),
            "python_ns": paired.distribution(p for _, p in selected),
            "native_over_python_latency": paired.paired_log_ratio_ci(
                selected, samples=2000
            ),
        }
        if check_statistics and any(
            recorded.get(key) != value for key, value in computed.items()
        ):
            raise cpu.BenchmarkError(
                "CPU report statistics differ from the retained raw pairs"
            )
...
def audit_reports(paths, *, power_receipt=None):
    if len(paths) != 3 or len({str(path.resolve()) for path in paths}) != 3:
        raise cpu.BenchmarkError(
            "CPU preservation requires three distinct report files"
        )
    reports, pins, checked = [], [], []
    for path in paths:
        report, pin = read_pinned(path)
        if (
            report.get("scope") not in (cpu.SCOPE, SCOPE)
            ...
            raise cpu.BenchmarkError(
                "CPU report status/profile/scope is not eligible for preservation"
            )
        if (
            report.get("source", {}).get("commit")
            != oracle.load_manifest()["upstream"]["commit"]
        ):
            raise cpu.BenchmarkError("CPU report upstream commit differs")
        models = report.get("models", [])
        if len(models) != 3 or {m.get("model") for m in models} != set(
            campaign.VARIANTS
        ):
            raise cpu.BenchmarkError(
                "CPU report must contain every variant exactly once"
            )
    ...
    if any(
        report["driver_sha256"] != reports[0]["driver_sha256"] for report in reports
    ):
        raise cpu.BenchmarkError("CPU repetitions use different driver identities")
    closure = {
        name: oracle.sha256_file(campaign.HERE / name)
        for name in (
            "benchmark_cpu.py",
            "oracle.py",
            "oracle_manifest.json",
            "generate_pipeline_cases.py",
        )
    }
    closure["../paired_benchmark.py"] = oracle.sha256_file(Path(paired.__file__))
    expected_driver = (
        closure["benchmark_cpu.py"]
        if reports[0]["scope"] == cpu.SCOPE
        else oracle.sha256_file(Path(__file__))
    )
    ...
    if power_receipt is not None:
        ...
        if (
            ...
            raise cpu.BenchmarkError(
                "CPU power receipt lacks the three observed repetitions"
            )
        for row in observed:
            if (
                row.get("exit_code") != 0
                or type(row.get("exit_code")) is not int
                or Path(row.get("report", "")).resolve()
                != paths[row["repetition"] - 1].resolve()
                or any(
                    "Now drawing from 'AC Power'" not in row.get(key, "")
                    for key in ("power_before", "power_after")
                )
            ):
                raise cpu.BenchmarkError(
                    "CPU repetition power/exit/report binding differs"
                )
    else:
        profiles = [report.get("hardware_start") for report in reports]
        if not all(isinstance(profile, dict) for profile in profiles):
            raise cpu.BenchmarkError(
                "historical CPU reports require their separately pinned power receipt"
            )
        for report in reports:
            for model in report["models"]:
                if model.get("power_profile_stable") is not True or any(
                    model["hardware_start"].get(key)
                    != reports[0]["hardware_start"].get(key)
                    for key in ("power_source", "low_power_mode")
                ):
                    raise cpu.BenchmarkError(
                        "CPU power profile differs across the measured repetitions"
                    )
    summary = []
            ...
            summary.append(
                {
                    "model": variant,
                    "case_id": name,
                    "native_median_ms": statistics.median(
                        row["native_ns"]["median"] for row in observations
                    )
                    / 1e6,
                    "python_median_ms": statistics.median(
                        row["python_ns"]["median"] for row in observations
                    )
                    / 1e6,
                    "repetitions": intervals,
                    "acceptance": runtime.competitiveness(
                        intervals, numerator="native_over_python"
                    ),
                }
            )
    return {
        ...
        "summary": summary,
        "all_30_cases_preserve_native_cpu_advantage": all(
            row["acceptance"]["passed"] for row in summary
        ),
        "comparison": "native_cpu_vs_fastino_cpu",
        "source_update_identity": reports[0]["source"],
        "same_source_as_optimized_metal": False,
        "serving_qualified": False,
        "native_binary_source_binding_requires_separate_build_receipt": True,
        "performance_release_qualified": False,
        "audit_sha256": oracle.sha256_file(Path(__file__)),
    }
...
def run(args):
    if args.output.exists() or not args.native_bin.is_file():
        raise cpu.BenchmarkError(
            "requires a fresh directory and existing native CPU executable"
        )
    if (
        not 256 <= args.max_rss_mib <= 8192
        or not 1 <= args.timeout_ms <= 60_000
        or not 1 <= args.startup_timeout <= 300
    ):
        raise cpu.BenchmarkError(
            "CPU process RSS/deadline caps are outside the declared bounds"
        )
    ...
    contracts = {
        variant: campaign.load_contract(variant, args.model_root, None)
        for variant in campaign.VARIANTS
    }
    ...
    for repetition in range(3):
        ...
        report = {
            ...
            "hardware_start": campaign.hardware_receipt(),
        ...
        try:
            for variant in campaign.rotate(list(campaign.VARIANTS), repetition):
                ...
                result = campaign.run_pair(
                    ...
                    phase="measurement",
                    selected=contracts[variant]["cases"],
                ...
                if result.get("cleanup_errors"):
                    raise cpu.BenchmarkError(
                        "CPU worker cleanup incomplete; refusing another model process"
                    )
            if any(model["status"] != "complete" for model in report["models"]):
                raise cpu.BenchmarkError(
                    "CPU repetition contains a preserved failed case"
                )
    ...
    if (
        campaign.source_snapshot() != snapshot
        or oracle.verify_dependencies() != dependencies
    ):
        raise cpu.BenchmarkError(
            "CPU campaign source or Python dependency identity changed"
        )
            ...
            json.dumps(
                {
                    ...
                    "all_30_cases_preserve_native_cpu_advantage": result[
                        "all_30_cases_preserve_native_cpu_advantage"
                    ],
                }
            ),
            flush=True,
