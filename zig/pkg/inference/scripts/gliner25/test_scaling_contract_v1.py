...
def fixture():
    ...
    evidence = {
        "validation": {
            row["id"]: {
                "input_ids": [7, 8]
                + generator.body_ids(row["text"], splitter, tokenize)
            }
...
class ScalingContractTests(unittest.TestCase):
    def test_complete_separate_matrix_preserves_original_sentences_and_padding(self):
        ...
        self.assertTrue(
            all(
                "expected" not in row and "outputs" not in row
        ...
        cases = contract.validate_pair(
            native,
            source,
            "small",
            hashlib.sha256(contract.encoded(source)).hexdigest(),
        )
        self.assertTrue(
            all(
                len(set(map(len, row["expected_input_ids"]))) > 1
                for row in cases.values()
                if row["profile"]["mode"] == "ragged"
            )
        )
    ...
    def test_original_prefix_requires_actual_observed_suffix(self):
        evidence = {
            "validation": {
                name: {"input_ids": [1, 2, 3]}
                for name in (*contract.REGULAR, *contract.RAGGED)
            }
        }
        with self.assertRaisesRegex(
            oracle.ContractError, "original source token suffix"
        ):
            generator.generate_variant("small", evidence, splitter, tokenize)

    def test_first_sentence_remains_exact_when_fill_is_smaller_than_second_sentence(
        self,
    ):
    ...
    def test_schema_order_model_identity_and_missing_shardlike_case_fail_closed(self):
        mutations = [
            ...
            lambda native, source: source["cases"][0]["expected_input_ids"][0].append(
                9
            ),
        ]
        for mutate in mutations:
            ...
            with (
                ...
                self.assertRaises((oracle.ContractError, runtime.cpu.BenchmarkError)),
            ):
                contract.validate_pair(native, source, "small", digest)

    def test_true_batch_packet_rejects_serial_tokens_transpose_bool_and_padding_tamper(
        self,
    ):
        sequences = [[4, 5, 6], [7]]
    ...
    def test_native_scaling_checks_every_output_and_owned_memory(self):
        ...
        case = {
            "items": [{"text": "John works at Apple."}, {"text": "Empty."}],
        ...
        first["classifications"] = [
            {"name": "kind", "labels": [{"label": "one", "confidence": 0.8}]}
        ]
        event.update(
            outputs=[first, empty],
            output=first,
            **contract.packet(case["expected_input_ids"]),
        )
    ...
    def test_output_span_checks_original_unicode_and_rejects_synthetic_suffix(self):
        ...
        with self.assertRaises(runtime.cpu.BenchmarkError):
            runtime.check_source_spans(
                {"text": "東京.", "source": {"start": 5, "end": 9}}, text
            )
