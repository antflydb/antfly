...
class BenchmarkContractTest(unittest.TestCase):
    def test_encoder_capture_covers_direct_classifier_and_boundary_core_routes_and_restores_hooks(
        self,
    ):
        class Tensor:
        ...
        for execute in (
            lambda: model._encode_core(batch),
            lambda: encoder(input_ids=batch.input_ids),
            lambda: encoder(batch.input_ids),
        ):
        ...
        for variant in ("small", "base", "multi"):
            ...
            for row in captured["requests"]:
                with self.subTest(model=variant, case=row["id"]):
                    bench.require_equal(
                        bench.canonical_result(expected[row["id"]]),
                        bench.canonical_python(requests[row["id"]], row["output"]),
                    )

    def test_only_confidence_is_approximate(self):
        bench.require_equal({"confidence": 0.5}, {"confidence": 0.5004})
        for expected, actual in (
            ({"confidence": 0.5}, {"confidence": 0.501}),
    ...
    def test_pairing_is_balanced_and_validation_precedes_warmup_and_measurement(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                mock.patch.object(bench, "Worker", Worker),
                mock.patch.object(bench, "ResourceGuard", Guard),
                mock.patch.object(oracle, "verify_model_dir", return_value=bundle),
                mock.patch.object(oracle, "verify_upstream_checkout"),
            ):
        ...
        self.assertEqual(
            0.5,
            result["comparisons"][case["id"]]["native_over_python_latency"]["median"],
        )
