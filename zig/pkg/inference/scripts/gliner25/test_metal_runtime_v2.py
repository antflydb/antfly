...
def admitted(policy="optimized_v2", *, capacity=0):
    ...
    return contract.native_ready(
        event, fixture["bundle"], fixture["model"], fixture["case_path"], policy
    )
...
def response(policy="optimized_v2", *, wb=0, wa=0, generation=0):
    ...
    snapshot = {
        key: 0
        for key in (
            "device_owned_live_bytes",
            "host_mirror_live_bytes",
            "device_owned_buffers_created",
            "device_owned_buffers_released",
            "device_owned_bytes_created",
            "device_owned_bytes_released",
            "host_mirror_allocations",
            "host_mirror_frees",
        )
    }
    before, after = dict(snapshot), dict(snapshot)
    before.update(
        device_owned_live_bytes=model + wb, device_owned_bytes_created=model + wb
    )
    after.update(
        device_owned_live_bytes=model + wa, device_owned_bytes_created=model + wa
    )
    event = {
        "event": "result",
        "runtime_contract_version": 2,
        ...
        "output": copy.deepcopy(fixture["expected"][fixture["cases"][0]]),
        "input_ids": [1, 2, 3],
        ...
        "owned_memory": {
            ...
            "transient": {
                "before_live_bytes": 0,
                "after_live_bytes": 0,
                "pending_after_bytes": 0,
            },
            "workspace": {
                "before_live_bytes": wb,
                "after_live_bytes": wa,
                "generation": generation,
            },
        },
        "runtime_stats": {
            "actual_weight_upload_bytes": 0,
            "scope_submissions": 1,
            "peak_pending_device_bytes": 1024,
            "pending_device_bytes": 0,
            "workspace_live_bytes": wa,
            "workspace_pending_bytes": 0,
...
class RuntimeTests(unittest.TestCase):
    def test_phase_instrumentation_is_validated_and_excluded_from_acceptance(self):
        ...
        event["runtime_stats"]["phase_timings_ns"] = {
            name: 1 for name in contract.PHASE_TIMINGS
        }
        self.assertEqual(
            expected,
            admitted().result(event, expected, validation=True, phase="validation"),
        )
        self.assertEqual(
            expected,
            admitted().result(event, expected, validation=False, phase="diagnostic"),
        )
        for phase in ("warmup", "measurement"):
            with (
                self.subTest(phase=phase),
                self.assertRaises(contract.v1.BenchmarkError),
            ):
    ...
    def test_workspace_growth_cannot_hide_in_measurement_or_skip_a_generation(self):
        for values, phase in (
            ...
            ((0, 4096, 1), "warmup"),
            ((1024, 1024, 1), "warmup"),
            ((0, 1024, 1), "measurement"),
        ):
            event, expected = response(wb=values[0], wa=values[1], generation=values[2])
            with (
                self.subTest(values=values, phase=phase),
                self.assertRaises(contract.v1.BenchmarkError),
            ):
                admitted(capacity=2048).result(
                    event, expected, validation=False, phase=phase
                )
    ...
    def test_rejected_case_preserves_valid_owner_baseline_for_next_case(self):
        for malformed in (False, True):
            ...
            if malformed:
                ...
                event["output"]["classifications"].append(
                    {"name": "unexpected", "labels": []}
                )
            with (
                self.subTest(malformed=malformed),
                self.assertRaises((contract.v1.cpu.BenchmarkError, KeyError)),
            ):
                state.result(event, expected, validation=True)
            self.assertEqual(
                (1024, 1), (state.workspace_live_bytes, state.workspace_generation)
            )
    ...
    def test_invalid_owner_is_fatal_before_parity_and_never_advances_baseline(self):
        ...
        with self.assertRaisesRegex(contract.v1.BenchmarkError, "pending device bytes"):
            state.result(event, expected, validation=True)
        self.assertEqual(
            (0, 0), (state.workspace_live_bytes, state.workspace_generation)
        )
    ...
    def test_per_case_gate_does_not_average_away_one_bad_repetition(self):
        ...
        self.assertTrue(
            contract.competitiveness([passed] * 3, numerator="python_over_metal")[
                "passed"
            ]
        )
        bad = {**passed, "lower_95": 0.83}
        self.assertFalse(
            contract.competitiveness(
                [passed, bad, passed], numerator="python_over_metal"
            )["passed"]
        )
        self.assertFalse(
            contract.competitiveness([passed] * 2, numerator="python_over_metal")[
                "passed"
            ]
        )
        cpu = {"lower_95": 0.7, "median": 0.8, "upper_95": 0.9}
        self.assertTrue(
            contract.competitiveness([cpu] * 3, numerator="native_over_python")[
                "passed"
            ]
        )
        self.assertFalse(
            contract.competitiveness(
                [cpu, cpu, {**cpu, "upper_95": 1}], numerator="native_over_python"
            )["passed"]
        )
