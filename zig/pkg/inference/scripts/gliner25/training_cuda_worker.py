...
def cublas_version(torch):
    ...
    if (
        not handle
        or query(ctypes.c_void_p(handle), ctypes.byref(version)) != 0
        or version.value <= 0
    ):
...
def adapt_row(row):
    ...
    for record in row.get("records", []):
        ...
        for field in record["fields"]:
            ...
            if len(values) != 1 or len(values[0]["occurrences"]) != 1:
                raise ValueError(
                    "fixture adapter requires one explicit occurrence per field"
                )
    ...
    output["relations"] = [
        {
            edge["type"]: {
                "head": by_id[edge["head"]["entity"]],
                "tail": by_id[edge["tail"]["entity"]],
            }
        }
...
def worker(args):
    ...
    run = config["run"]
    if (
        run["mode"] not in ("heads", "full")
        or run["shuffle"]
        or run["scheduler"] != "constant"
        or run["warmup_steps"] != 0
    ):
    ...
    def create():
        model = (
            AutoExtractor.from_pretrained(config["source_dir"], local_files_only=True)
            .float()
            .cuda()
        )
        ...
        cfg = TrainingConfig(
            output_dir="/tmp/unused-gliner25-training-output",
            fp16=False,
            bf16=False,
            encoder_lr=run["encoder_lr"],
            task_lr=run["task_lr"],
            weight_decay=run["weight_decay"],
            adam_beta1=run["beta1"],
            adam_beta2=run["beta2"],
            adam_epsilon=run["epsilon"],
            max_grad_norm=run["max_grad_norm"],
            gradient_accumulation_steps=run["accumulation"],
            scheduler_type="constant",
            warmup_steps=0,
            warmup_ratio=0,
            fused_optimizer=True,
            gold_injection_start=1,
            gold_injection_end=1,
            log_proposal_metrics=False,
        ...
        trainer._planned_max_steps = (
            run["epochs"] * len(rows) // (run["batch_size"] * run["accumulation"])
        )
    ...
    def prepare():
        ...
        examples = [
            adapt_row(copy.deepcopy(row))
            for row in rows[start : start + run["batch_size"]]
        ]
        ...
        return trainer.model.processor.collate_fn_inference(
            ...
            build_targets=True,
            max_gold_per_query=trainer.model.boundary_head.settings.max_gold_per_query,
    ...
    for line in sys.stdin.buffer:
        ...
        if (
            set(c) != {"request_id", "op", "case_id"}
            or type(c["request_id"]) is not int
            or c["request_id"] <= previous
        ):
        ...
        try:
            if c["op"] == "stop":
                ...
                if (
                    oracle.verify_model_dir(args.model, Path(config["source_dir"]))
                    != bundle
                ):
            ...
            if c["case_id"] == "reset" and c["op"] == "validate":
                ...
                batch = prepare()
                result = {
                    "input_ids": batch.input_ids.detach().cpu().flatten().tolist(),
                    "encoder_shape": list(batch.input_ids.shape),
                    "query_layouts": repr(batch.query_layouts),
                    "record_specs": repr(batch.record_specs),
                }
                del batch
                duration = 1
                ...
                parameters = quality.canonical_parameters(
                    trainer.model, trainer.snapshot_parameters
                )
                        ...
                        slots.append(
                            {
                                ...
                                "adam_step": int(state.get("step", 0)),
                                "group": 0 if "encoder" in name else 1,
                            }
                        )
                        for value in (
                            parameter,
                ...
                result = {
                    ...
                    "identity": {
                        ...
                        "microbatch_step": trainer.microbatch,
                    },
                    "accumulated_microbatches": trainer.microbatch
                    % run["accumulation"],
                }
                duration = 1
                ...
                if (
                    validation is None
                    or oracle.sha256_file(validation) != args.validation_sha256
                ):
                ...
                if tracing and c["op"] != "validate":
                    raise ValueError(
                        "loss tracing is only supported outside throughput measurements"
                    )
                if tracing:
                ...
                with trace if tracing else nullcontext():
                    loss = trainer._backward_one(
                        batch, trainer.microbatch, False, torch.float32
                    )
                ...
                terms = {
                    name.removesuffix("_loss"): float(value.detach())
                    for name, value in trainer._last_train_outputs.losses.items()
            ...
            common.emit(
                {"event": "result", **identity, "duration_ns": duration, **result}
            )
        except Exception as exc:
            common.emit(
                {"event": "error", **identity, **runtime.error_details(exc, "cuda")}
            )
            raise
...
def main():
    ...
    parser.add_argument(
        "--profile", choices=("eager_fp32", "compile_fp32"), default="eager_fp32"
    )
