...
M = os.path.expanduser(sys.argv[1] if len(sys.argv) > 1 else "~/.antfly/inference/models/fastino/GLiNER2.5-Decide")
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(os.path.dirname(__file__), "..", "..", "testdata", "gliner25", "decide", "cases.json")
...
CASES = [
    ("support_intent", "My subscription renewed on April 15 for ¥5,400 after the service was already down. Can I get that charge refunded?",
    ...
    ("travel", "I need to move my Friday flight to Paris to Saturday morning, same cabin, and keep the aisle seat if you can.",
     {"request": ["book", "change", "cancel", "status", "seat_change", "refund", "baggage"]}),
    ("hotel_multi_task", "Guest in room 1408 says the AC has been out since yesterday and they want to move tonight or leave. They also asked for the incidentals hold to be released.",
     {"intent": ["maintenance", "room_change", "checkout", "billing", "complaint", "amenity_request"],
      "priority": ["low", "normal", "high", "urgent"],
      "needs_human": ["yes", "no"],
      "topics": {"labels": ["hvac", "billing", "housekeeping", "noise", "safety"], "multi_label": True, "cls_threshold": 0.4}}),
    ("passage_question", "The treaty was signed in Paris in 1992. It entered into force the following year, after the last signatory ratified it.",
     {"answer": {"labels": ["yes", "no"], "prompt": "Did the treaty enter into force in 1992?"}}),
    ("described_labels", "Please reset the card PIN. The new one never arrived and the old one is locked after three tries.",
    ...
    ("ordinal", "I finished it in two nights. The ending is earned, the middle drags, and I would still hand it to a friend.",
     {"rating": [str(i) for i in range(11)]}),
    ("book_genre", "She closed the ledger, blew out the lamp, and listened for the stair. The house had been empty since the winter the river took the bridge",
     {"genre": ["mystery", "romance", "history", "science_fiction", "literary_fiction", "cookbook"]}),
    ("sentiment_multi", "Great food but the service was painfully slow and our waiter was rude!",
     {"sentiment": ["positive", "negative", "mixed", "neutral"],
      "aspects": {"labels": ["food", "service", "price", "ambience"], "multi_label": True}}),
]
...
def to_v2(tasks):
    ...
    for name, cfg in tasks.items():
        ...
        if isinstance(labels, dict):
            entry["label_definitions"] = {k: {"description": v} for k, v in labels.items()}
...
with torch.no_grad():
    for name, text, tasks in CASES:
        ...
        cases.append({"name": name, "text": text, "tasks": tasks, "v2_schema": to_v2(tasks), "input_ids": ids[0][0],
                      "classifier_logits": logits, "result": result})
        print(name, json.dumps(result))

    # Entity extraction (span head + CountLSTM v1) reference.
    ent_cases = []
    for text, labels in [("My name is Clara and I live in Berkeley, California.", ["person", "location"]),
                         ("Apple CEO Tim Cook announced the iPhone 17 in Cupertino on September 9.", ["person", "organization", "product", "location", "date"])]:
...
json.dump({"model": "fastino/GLiNER2.5-Decide", "gliner2_version": "2.0.0",
           "classification": cases, "entities": ent_cases}, open(OUT, "w"), indent=1, ensure_ascii=False)
print("wrote", OUT)
