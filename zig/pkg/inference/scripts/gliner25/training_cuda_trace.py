...
class ModuleTrace:
            ...
            for name, module in self.model.named_modules():
                if name in ambiguous:
                    self.results.append(
                        {
                            "name": name,
                            "skipped": "multiple native invocations require explicit alignment",
                        }
                    )
                    continue
                if (name, "output") not in records and (name, "input") not in records:
                    continue
                if not isinstance(
                    module, (self.torch.nn.Linear, self.torch.nn.LayerNorm)
                ):
                    self.results.append(
                        {
                            "name": name,
                            "skipped": "unsupported replay module",
                            "type": type(module).__name__,
                        }
                    )
                    continue
                self.hooks.append(
                    module.register_forward_hook(self.callback(name, records))
                )
    ...
    def compare(native, python):
        ...
        return {
            ...
            "max_absolute_error": float(abs(delta[index])),
            "l2_error": float(np.linalg.norm(delta)),
            "worst_index": index,
            "native": float(actual[index]),
            "python": float(reference[index]),
        }
                ...
                if (name, "output") in records and raw.size == inputs[0].numel():
                    ...
                    try:
                        with self.torch.no_grad():
                            value = (
                                self.torch.from_numpy(np.array(raw, copy=True))
                                .to(inputs[0].device)
                                .reshape(inputs[0].shape)
                            )
                            replayed = module(value)
                            row["same_input_output"] = self.compare(
                                self.data[records[name, "output"]], replayed
                            )
                            if isinstance(module, self.torch.nn.Linear):
                                unfused = self.torch.nn.functional.linear(
                                    value, module.weight, None
                                )
                                if module.bias is not None:
                                    unfused = unfused + module.bias
                                row["same_input_unfused_output"] = self.compare(
                                    self.data[records[name, "output"]], unfused
                                )
                    finally:
                        self.replaying = False
...
class BoundaryTrace:
    ...
    def __enter__(self):
        if self.native is not None and self.native.get("_modules"):
            self.modules = ModuleTrace(
                self.model, self.torch, self.native["_modules"]
            ).__enter__()
    ...
    def capture(self, *args, **kwargs):
        ...
        leaves = dict(
            starts=marginal.start_logits,
            ends=marginal.end_logits,
            inside=marginal.inside_logits,
            pairs=kwargs["pooled_pair_logits"].transpose(1, 2),
            proposals=pooled.proposal_logits,
            nulls=nulls,
            counts=counts,
        )
        ...
        inputs.update(
            pool_mask=self.values(pooled.mask),
            query_mask=self.values(queries),
        ...
        def differentiate(native=None):
            ...
            for name, value in leaves.items():
                if value is None:
                    ...
                    item = torch.tensor(
                        raw, dtype=value.dtype, device=value.device
                    ).reshape(value.shape)
            ...
            copied_args[0] = replace(
                marginal,
                start_logits=independent["starts"],
                end_logits=independent["ends"],
                inside_logits=independent["inside"],
            )
            copied_args[2] = independent["pairs"]
            copied_args[7], copied_args[8] = independent["nulls"], independent["counts"]
            copied_kwargs = dict(
                kwargs,
                pooled=replace(pooled, proposal_logits=independent["proposals"]),
            ...
            if independent["pairs"].numel() <= 32768:
                weights = {
                    ...
                    "soft_iou_loss": self.head.settings.soft_iou_aux_weight
                    * self.head._soft_iou_scale,
                    "rerank_listwise_loss": self.head.settings.rerank_listwise_weight,
                    "consistency_loss": self.head.settings.consistency_loss_weight
                    * self.head._consistency_scale,
                }
                for name, weight in weights.items():
                    if losses[name].requires_grad:
                        ...
                        components[name] = self.values(
                            torch.zeros_like(independent["pairs"])
                            if component is None
                            else component
                        )
            return {
                "pair_gradient_components": components,
                "terms": {
                    name: float(value.detach()) for name, value in losses.items()
                },
                "gradients": {
                    name: self.values(
                        torch.zeros_like(independent[name])
                        if gradient is None
                        else gradient
                    )
        ...
        if self.native is not None:
            ...
            if len(native_spans) != len(python_spans) or any(
                a != b
                for a, b, valid in zip(native_spans, python_spans, inputs["pool_mask"])
                if valid
            ):
            ...
            native_gold = [
                (span["start"], span["end"]) for span in native["gold"]["spans"]
            ]
            python_gold = [
                tuple(span)
                for batch in inputs["gold_spans"]
                for query in batch
                for span in query
            ]
            if len(native_gold) != len(python_gold) or any(
                a != b
                for a, b, valid in zip(native_gold, python_gold, inputs["gold_mask"])
                if valid
            ):
