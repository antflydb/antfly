...
PINS = {
    "fixture_support.py": {
        "size_bytes": 2851,
        "sha256": "07df9f1240763dbf96125481893ae229a31e1b418c916587f2266b4e60da88a4",
    },
    "capture_training_attention.py": {
        "size_bytes": 20879,
        "sha256": "c67ee517899871b4ad36a11408ff321028ad10ab88f0941aa0c09ef48f6a1a6f",
    },
    "training_attention_contract_v1.json": {
        "size_bytes": 12057,
        "sha256": "7e512fe4418d006619088093ed7410ddd9a209dcb421954c9fbb14287bbecfcc",
    },
    "test_training_attention.py": {
        "size_bytes": 6003,
        "sha256": "32a1cea343fbca8f189aaff3a3a0f8f8d6ed1c44a71c2de52f726bfc7f419cc1",
    },
    "oracle.py": {
        "size_bytes": 28555,
        "sha256": "61bbf4544a21111dbfbd643ed2392b1d9723498493b3fa6832154d6e1dc42752",
    },
    "oracle_manifest.json": {
        "size_bytes": 4097,
        "sha256": "918005508a57d2c558c18a78167637f39c54e7c2c6d67814be15fc6714c4152f",
    },
}
SUPERVISOR_PIN = {
    "size_bytes": 12474,
    "sha256": "a937237975be2ed879f62afd285494ccb51a7c1d7cc1dff7160621fb26b87332",
}
LIMITS = {
    "timeout_seconds": 180,
    ...
    try:
        with os.scandir(descriptor) as entries:
            for entry in entries:
                checked(
                    entry.name in ARTIFACT_NAMES and len(sizes) < 3,
                    "unexpected attention artifact entry",
                )
                with os.fdopen(
                    os.open(
                        entry.name,
                ...
                checked(
                    ...
                    "unbounded or nonregular attention artifact",
                )
                sizes[entry.name] = info.st_size
                checked(
                    sum(sizes.values()) <= limit,
                    "attention artifact byte ceiling exceeded",
                )
    finally:
        os.close(descriptor)
    return sizes
    ...
    class ArtifactTree(supervisor.ProcessTree):
        ...
        def sample(self):
            ...
            if self.check_artifacts:
                ...
                self.artifact_peak_bytes = max(
                    self.artifact_peak_bytes, sum(sizes.values())
                )
            return result
        ...
        def receipt(self):
            return {
                **super().receipt(),
                "sampled_artifact_peak_bytes": self.artifact_peak_bytes,
...
def fresh_path(path):
    ...
    checked(
        ...
        "refusing to overwrite attention output: " + str(path),
    )
...
def python_identity():
    ...
    return {
        ...
        "python_executable": pin(executable),
...
def preflight(upstream):
    inputs = {
        name: require_pin(HERE / name, expected) for name, expected in PINS.items()
    }
    supervisor_raw = require_pin(SUPERVISOR, SUPERVISOR_PIN)
    generator = module_from_bytes(
        "attention_source_contract",
        HERE / "capture_training_attention.py",
        inputs["capture_training_attention.py"],
    )
    contract, source = generator.preflight(upstream)
    manifest_raw = inputs["oracle_manifest.json"]
    checked(
        json.loads(manifest_raw)["runtime"] == contract["runtime"],
        "oracle manifest dependency closure differs",
    )
    inputs["oracle_manifest.json"] = manifest_raw
    checked(
        not any(name in sys.modules for name in ("torch", "gliner2", "peft")),
        "supervisor preflight imported a numerical runtime",
    )
    return (
        inputs,
        supervisor_raw,
        {
            "scope": SCOPE,
            "qualification": False,
            "inputs": {name: pin(raw) for name, raw in inputs.items()},
            "supervisor": SUPERVISOR_PIN,
            "transformers_source": generator.digest(source),
            "runtime": contract["runtime"],
            **python_identity(),
            "limits": {**LIMITS, "artifact_bytes": MAX_ARTIFACT_BYTES},
            "source_commit": contract["source_commit"],
            "wrapper": pin(read(Path(__file__))),
        },
    )
...
def execute(upstream, output, evidence):
    ...
    command = [
        admitted["python_invocation"],
        "-B",
        str(snapshots / "capture_training_attention.py"),
        "--upstream",
        str(upstream),
        "--output-dir",
        str(output),
    ...
    supervisor = module_from_bytes(
        "pinned_attention_supervision", SUPERVISOR, supervisor_raw
    )
    result = supervisor.run(
        command,
        evidence / "stdout.jsonl",
        evidence / "stderr.log",
        **LIMITS,
        tree_factory=tree_factory(supervisor, output),
    )
    ...
    try:
        checked(
            ...
            "attention child or owned-tree cleanup failed",
        )
        sizes = artifact_sizes(output)
        ...
        checked(
            manifest["scope"] == "gliner25_projected_training_attention/v1"
            and manifest["qualification"] is False
            and len(manifest["cases"]) == 9
            and manifest["generator"] == PINS["capture_training_attention.py"]
            and manifest["contract"] == PINS["training_attention_contract_v1.json"],
            "attention artifact source identity differs",
        )
        checked(
            manifest["files"]
            == {
                name: final["artifacts"][name]
                for name in ("weights.safetensors", "tensors.safetensors")
            },
            "attention file receipt differs",
        )
        for name, raw in inputs.items():
            checked(
                read(HERE / name) == raw,
                "original attention helper changed during child execution",
            )
        for name in (
            "capture_training_attention.py",
            "training_attention_contract_v1.json",
            ...
            checked(
                read(snapshots / name) == inputs[name],
                "private attention helper changed",
            )
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Checkout of the pinned Fastino commit",
    )
    parser.add_argument("--preflight-only", action="store_true")
    parser.add_argument(
        "--output-dir", type=Path, help="Fresh absolute capture directory"
    )
    parser.add_argument(
        "--evidence-dir", type=Path, help="Fresh absolute process-receipt directory"
    )
    args = parser.parse_args(argv)
    if not args.preflight_only and (
        args.output_dir is None or args.evidence_dir is None
    ):
...
def main():
    ...
    if args.preflight_only:
        ...
        print(
            json.dumps(
                {**admitted, "status": "preflight_only", "model_execution": False},
                sort_keys=True,
            )
        )
    else:
        previous = {
            number: signal.signal(number, stop_signal)
            for number in (signal.SIGINT, signal.SIGTERM)
        }
        try:
            ...
            json.dumps(
                {
                    ...
                    "qualification": False,
                    "process_receipt": str(args.evidence_dir / "process.json"),
                },
                sort_keys=True,
            )
        )
