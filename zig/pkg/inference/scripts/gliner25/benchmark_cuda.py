...
def require_equal(
    ...
    path: str = "output",
    *,
    confidence_tolerance: float = 5e-4,
) -> None:
    ...
    if confidence_tolerance == 5e-3:
        ...
        def normalize(want, got, key=""):
            ...
            if isinstance(want, dict) and isinstance(got, dict):
                return {
                    k: normalize(want[k], v, k) if k in want else v
                    for k, v in got.items()
                }
            if isinstance(want, list) and isinstance(got, list):
                return [
                    normalize(want[i], v) if i < len(want) else v
                    for i, v in enumerate(got)
                ]
            if (
                key == "confidence"
                and type(got) in (int, float)
                and math.isfinite(got)
                and abs(want - got) <= confidence_tolerance
            ):
                return want
            return got
...
def checked_ready(
    ...
    *,
    ...
    cpu.checked_ready(
        arm, {**ready, "scope": cpu.SCOPE, "arm": arm}, bundle, cases_path, threads
    )
    if arm == "native":
        if (
            ready.get("backend") != "cuda"
            or ready.get("synchronization_policy")
            != "cuda_stream_before_start_and_after_extract_v1"
        ):
            raise BenchmarkError("native CUDA device or synchronization policy differs")
    elif (
        ready.get("device") != "cuda"
        or ready.get("parameter_device") != "cuda"
        ...
        or ready.get("encoder_backend")
        != ("flashdeberta" if flashdeberta else "transformers")
        or ready.get("math_policy")
        != (
            f"pytorch_cuda_amp_{profile.rsplit('_', 1)[-1]}_no_tf32_v1"
            if "_amp_" in profile
            else "pytorch_cuda_fp32_deterministic_no_tf32_v1"
        )
        or ready.get("synchronization_policy")
        != "torch_cuda_synchronize_before_start_and_after_extract_v1"
    ):
...
def checked_transfers(response):
    ...
    if (
        ...
        or counts["kernel_launches"] == 0
        or counts["host_fallback_calls"] != 0
    ):
...
def checked_output(response, batch_size=1):
    if response.get("event") == "error":
        raise BenchmarkError(
            f"{response.get('arm')}: {response.get('error_type')}: {response.get('message')}"
        )
...
def checked_batch_tokens(responses, batch_size):
    ...
    if (
        ...
        or not left
        or len(left) % batch_size
        or len(left) > batch_size * oracle.MAX_ENCODED_TOKENS
        or any(type(token) is not int for token in left)
        or left != right
    ):
...
def run_variant(args, variant, directory):
    ...
    regenerated = cpu.adaptation.generate(
        variant,
    ...
    if json.dumps(fixture, ensure_ascii=False) != json.dumps(
        ...
        raise BenchmarkError(
            "canonical fixture differs from verified oracle adaptation"
        )
    cases = {case["id"]: case for case in fixture["cases"]}
    request_kinds = {
        item["id"]: item["kind"]
        for item in oracle.read_json(oracle.FIXTURES / "requests.json")["requests"]
    }
    selected = args.cases or [
        name
        for name in cases
        if args.batch_size == 1 or request_kinds[name] == "extract"
    ]
    if args.batch_size > 1 and any(
        request_kinds.get(name) != "extract" for name in selected
    ):
    ...
    expected = {
        name: cpu.canonical_result(cases[name]["expected"]) for name in selected
    }
    ...
    commands = {
        ...
        + (["--flashdeberta"] if args.flashdeberta else [])
        + (["--compile-static"] if args.compile_static else []),
    }
    ...
    guard = ResourceGuard(
        ...
        max_processes=1024
        if args.python_profile.startswith("compile_")
        else MAX_PROCESSES,
    )
    ...
    def check_result(name, arm, response):
        ...
        require_equal(
            ...
            actual,
            f"{variant}.{name}.{arm}",
            confidence_tolerance=tolerance(arm),
        )
    ...
    def timeout(arm, warming=False):
        return (
            ...
            if warming
            and arm == "python"
            and (args.python_profile.startswith("compile_") or args.flashdeberta)
            else args.timeout_ms / 1000 + 5
        )

    try:
        for arm in ("python", "native"):
            workers[arm] = Worker(
                "fastino_cuda" if arm == "python" else arm,
                commands[arm],
                env,
                directory,
                guard,
            ...
            checked_ready(
                ...
                flashdeberta=args.flashdeberta,
        ...
        for name in selected:
            responses = {
                arm: workers[arm].request("validate", name, timeout(arm, True))
                for arm in ("python", "native")
            }
            oracle.write_json(
                directory / f"validation-{name}.json",
                {"expected": expected[name], "responses": responses},
            )
            ...
            validation[name] = {
                ...
                "confidence_absolute_tolerance": {
                    arm: tolerance(arm) for arm in workers
                },
                "expected": expected[name],
                **{
                    arm: checked_output(response, args.batch_size)
                    for arm, response in responses.items()
                },
            }
        for iteration in range(0 if args.validate_only else args.warmup):
            for name in selected:
                for arm in cpu.paired_benchmark.balanced_pair_order(
                    iteration + 1, "native", "python"
                ):
                    check_result(
                        name, arm, workers[arm].request("run", name, timeout(arm, True))
                    )
        rows = []
            ...
            for name in selected:
                ...
                for arm in row["order"]:
                    ...
                    row[arm] = {
                        ...
                        **(
                            {"cuda_transfers": response.get("cuda_transfers")}
                            if arm == "native"
                            else {}
                        ),
                    }
        ...
        for name in () if args.validate_only else selected:
            pairs = [
                (row["native"]["duration_ns"], row["python"]["duration_ns"])
                for row in rows
                if row["case_id"] == name
            ]
            comparisons[name] = {
                "batch_size": args.batch_size,
                "native_documents_per_second": cpu.paired_benchmark.distribution(
                    args.batch_size * 1e9 / n for n, _ in pairs
                ),
                "python_documents_per_second": cpu.paired_benchmark.distribution(
                    args.batch_size * 1e9 / p for _, p in pairs
                ),
                ...
                "native_over_python_latency": cpu.paired_benchmark.paired_log_ratio_ci(
                    pairs, samples=2000
                ),
            }
        return {
            ...
            "combined_peak_observed_rss_bytes": guard.peak_rss_bytes,
            "rss_sampling_period_seconds": 0.1,
            "max_combined_rss_bytes": guard.max_rss_bytes,
        ...
        if any(
            not receipt or not receipt.get("complete") for receipt in cleanup.values()
        ):
...
def driver(args):
    ...
    if (
        not 1 <= args.batch_size <= 64
        or args.threads != 1
        or not 1 <= args.compile_timeout <= 1800
    ...
    if args.output.exists() or not args.native_bin.is_file():
        raise BenchmarkError(
            "requires a new output directory and an existing native binary"
        )
    ...
    report = {
        ...
        "scope": CUDA_SCOPE,
        "backend": "cuda",
        "python_variant": args.python_profile,
        "flashdeberta": args.flashdeberta,
        "compile_static": args.compile_static,
        ...
        "batch_size": args.batch_size,
        "batch_composition": "repeated_same_fixture_document_and_schema",
        "threads": 1,
        "warmup_per_case": 0 if args.validate_only else args.warmup,
    ...
    try:
        for variant in (
            ("small", "base", "multi") if args.model == "all" else (args.model,)
        ):
        ...
        if oracle.sha256_file(Path(__file__)) != report["driver_sha256"] or any(
            oracle.sha256_file(HERE / name) != digest
            for name, digest in report["helpers_sha256"].items()
        ):
            raise BenchmarkError(
                "benchmark driver or helpers changed during measurement"
            )
...
def main():
    ...
    run.add_argument(
        "--compile-static",
        action="store_true",
        help="Compile fixed shapes instead of dynamic shapes; record as a separate candidate",
    )
    run.add_argument(
        "--validate-only",
        action="store_true",
        help="Check candidate parity and provenance without collecting timing samples",
    )
    run.add_argument("--compile-timeout", type=float, default=600)
    run.add_argument("--native-bin", type=Path, required=True)
    run.add_argument(
        "--model", choices=("small", "base", "multi", "all"), default="small"
    )
    ...
    run.add_argument(
        ...
        type=int,
        default=1,
        help="True homogeneous GPU batch, 1..64; batch >1 supports extract fixtures",
    )
