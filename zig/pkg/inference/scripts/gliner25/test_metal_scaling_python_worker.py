...
class BatchTensor(Tensor):
    ...
    def reshape(self, dimension):
        assert dimension == -1
        result = Tensor(
            [x for row in self.values for x in row],
            device=self.device,
            dtype=self.dtype,
        )
        return result
...
def encode(model, ids=None, mask=None):
    model.encoder(
        input_ids=BatchTensor(ids or [[1, 2, 3], [4, 0, 0]], dtype="int64"),
...
class ScalingWorkerTests(unittest.TestCase):
    ...
    def test_actual_batch_hook_is_removed_on_replay_split_padding_and_device_error(
        self,
    ):
        for mode in ("pass", "split", "padding", "device"):
            ...
            def exercise():
                with worker.capture_batch(model, torch, "mps", case()) as captured:
                    if mode == "split":
                        ...
                        model.encoder(
                            input_ids=BatchTensor(device="cpu", dtype="int64"),
            ...
            if mode == "pass":
                ...
                with (
                    ...
                    self.assertRaises(worker.bench.BenchmarkError),
                ):
                    exercise()
    ...
    def test_extract_uses_batch_api_once_with_complete_list_and_same_fixed_schema(self):
        ...
        model = types.SimpleNamespace(
            batch_extract=lambda *args, **kwargs: (
        ...
        with mock.patch.object(
            ...
            side_effect=lambda spec: ("compiled", spec),
        ):
    ...
    def test_serve_validation_then_run_then_stop_with_no_hook_in_timed_run(self):
        ...
        with (
            mock.patch.object(worker.oracle, "verify_model_dir", return_value={}),
        ...
        self.assertEqual(
            ["result", "result", "stopped"], [row["event"] for row in events]
        )
