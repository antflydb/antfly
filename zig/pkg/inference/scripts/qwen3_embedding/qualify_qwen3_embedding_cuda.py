...
def batch_document_case_ids(cases: dict[str, dict[str, Any]]) -> list[str]:
    """Select bounded document cases for batch-versus-single qualification.

    A truncated case exercises the full 8,192-token context on its singleton
    request. Including it in the mixed-length batch pads every short document
    to that width, turning a numerical consistency gate into an accidental
...
def needs_reduced_dimension_replay(case: dict[str, Any]) -> bool:
...
def run_qualification(
    ...
    for case_id in sorted(cases):
        ...
        if not needs_reduced_dimension_replay(case):
            continue
    ...
    document_ids = batch_document_case_ids(cases)
