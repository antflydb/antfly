...
def main():
    ...
    parser.add_argument('--cuda', type=Path, default=Path(os.environ.get('CUDA_HOME', '/usr/local/cuda')))
    ...
    for direction in DIRECTIONS:
        ...
        with tempfile.TemporaryDirectory(prefix='gliner25-boundary-') as temp:
            ...
            for suffix, arch in (('cubin', 'sm_89'), ('sm80.cubin', 'sm_80')):
                ...
                out = Path(temp) / name
                subprocess.run([str(nvcc), '-cubin', f'-arch={arch}', *OPTIONS, str(src), '-o', str(out)], check=True)
                outputs[name] = out.read_bytes()
    metadata = {'profile': 'gliner25_boundary_attention_native_d32_v1',
                'compiler': version.strip(), 'options': list(OPTIONS),
    ...
    for name, data in outputs.items():
        ...
        if args.check:
            ...
            with tempfile.NamedTemporaryFile(dir=args.output_dir, prefix='.boundary-', delete=False) as f:
