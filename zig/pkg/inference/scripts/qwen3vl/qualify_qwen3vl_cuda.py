...
bundle, frozen Transformers processor/tokenizer/M-RoPE parity, real CUDA
...
SCHEMA = "antfly.qwen3vl.cuda_qualification.v1"
    ...
    if platform.system() == "Linux":
        try:
            ...
            for line in Path("/proc/vmstat").read_text(encoding="utf-8").splitlines():
                ...
                if name in ("pswpout", "pgpgout"):
                    counters[name] = int(value)
            # pswpout is reported in pages. sysconf is the authoritative page size.
            return counters.get("pswpout", 0) * int(os.sysconf("SC_PAGE_SIZE"))
        except (OSError, ValueError) as exc:
            raise QualificationError("/proc/vmstat resource probe failed") from exc
    completed = subprocess.run(
...
def memory_free_percent() -> int:
    if platform.system() == "Linux":
        try:
            ...
            raise QualificationError("/proc/meminfo resource probe failed") from exc
    completed = subprocess.run(
...
def process_gpu_memory_mib(pid: int) -> float:
    ...
    if completed.returncode != 0:
        raise QualificationError("nvidia-smi GPU memory probe failed")
    total = 0.0
...
def cuda_command(args: argparse.Namespace, work_dir: Path) -> list[str]:
    ...
    if not image_paths:
        raise QualificationError("CUDA Qwen3-VL request requires at least one image")
    ...
    command.extend((
        ...
        "--print-token-count",
        "--json-timing",
        str(work_dir / "antfly_cuda_timing.json"),
        "--qwen3vl-parity-json",
...
def run_resource_monitored(
    ...
    *,
    ...
    max_rss_mib: float,
    max_gpu_memory_mib: float | None = None,
                ...
                try:
                    ...
                    if now >= next_system_probe:
                        if max_gpu_memory_mib is not None:
                            gpu_memory = process_gpu_memory_mib(process.pid)
                            max_gpu_memory = max(max_gpu_memory, gpu_memory)
                            if gpu_memory > max_gpu_memory_mib:
                                violation = (
                                    f"{label} GPU memory {gpu_memory:.1f} MiB exceeded "
                                    f"{max_gpu_memory_mib:.1f} MiB"
                                )
                                break
    ...
    resources = {
        ...
        "max_rss_mib": max_rss,
        "max_gpu_memory_mib": max_gpu_memory,
...
def run_cuda(args: argparse.Namespace, work_dir: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    command = cuda_command(args, work_dir)
    ...
    execution = run_resource_monitored(
        command,
        work_dir / "antfly_cuda.stdout.log",
        work_dir / "antfly_cuda.stderr.log",
    ...
    if execution["returncode"] != 0:
        raise QualificationError(
            f"Antfly CUDA exited {execution['returncode']}: "
            f"{execution['stderr'].strip()[-2000:]}"
        )
    parity = load_json(work_dir / "antfly_parity.json")
    timing = load_json(work_dir / "antfly_cuda_timing.json")
...
def run_weights_oracle(args: argparse.Namespace, work_dir: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    execution = run_resource_monitored(
        ...
        max_gpu_memory_mib=args.weights_max_gpu_memory_mib
        if args.weights_device == "cuda"
        else None,
        min_free_percent=args.min_free_percent,
        max_swap_growth_mib=args.max_swap_growth_mib,
        sample_interval_seconds=args.sample_interval_seconds,
...
def read_f32le(path: Path) -> list[float]:
    ...
    if len(payload) % 4 != 0:
        raise QualificationError(f"invalid f32le payload length for {path}: {len(payload)}")
...
def patch_metrics(reference_path: Path, actual_path: Path) -> dict[str, float | int | bool]:
    reference = read_f32le(reference_path)
    actual = read_f32le(actual_path)
    if not reference or len(actual) != len(reference):
        return {
            "size_match": False,
            "reference_value_count": len(reference),
            "actual_value_count": len(actual),
        }
    difference = [abs(candidate - expected) for expected, candidate in zip(reference, actual)]
    sorted_difference = sorted(difference)
    p99_index = math.ceil(0.99 * (len(sorted_difference) - 1))
    return {
        "size_match": True,
        "value_count": len(reference),
        "mean_abs": math.fsum(difference) / len(difference),
        "rmse": math.sqrt(math.fsum(value * value for value in difference) / len(difference)),
        "p99_abs": sorted_difference[p99_index],
...
def logit_metrics(reference_path: Path, actual_path: Path, top_k: int = 10) -> dict[str, Any]:
    ...
    finite = all(math.isfinite(value) for value in reference) and all(
        math.isfinite(value) for value in actual
    )
    if not finite:
        return {"size_match": True, "finite": False, "value_count": len(reference)}
    difference = [candidate - expected for expected, candidate in zip(reference, actual)]
    reference_order = sorted(range(len(reference)), key=lambda index: (-reference[index], index))
    actual_order = sorted(range(len(actual)), key=lambda index: (-actual[index], index))
    selected_top_k = min(top_k, len(reference))
    reference_top = reference_order[:selected_top_k]
    actual_top = actual_order[:selected_top_k]
    dot_product = math.fsum(expected * candidate for expected, candidate in zip(reference, actual))
    denominator = math.sqrt(math.fsum(value * value for value in reference)) * math.sqrt(
    ...
    covariance = math.fsum(
        (expected - reference_mean) * (candidate - actual_mean)
        for expected, candidate in zip(reference, actual)
    )
    correlation_denominator = math.sqrt(
        math.fsum((value - reference_mean) ** 2 for value in reference)
        * math.fsum((value - actual_mean) ** 2 for value in actual)
    )
    absolute_difference = [abs(value) for value in difference]
    return {
        "size_match": True,
        "finite": True,
        "value_count": len(reference),
        ...
        "mean_abs": math.fsum(absolute_difference) / len(absolute_difference),
        "rmse": math.sqrt(math.fsum(value * value for value in difference) / len(difference)),
        "max_abs": max(absolute_difference),
        "cosine_similarity": dot_product / denominator if denominator else 0.0,
...
def cuda_determinism_metrics(
    samples: list[tuple[dict[str, Any], dict[str, Any], dict[str, Any]]],
) -> dict[str, Any]:
    """Compare repeated identical CUDA requests at architecture boundaries."""
...
def fallback_counters_are_zero(timing: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
    ...
    if not isinstance(cuda, dict):
        return False, {"error": "missing CUDA timing object"}
    zero_keys = (
        "cross_backend_copies",
        "cross_backend_copy_bytes",
        "cross_backend_sync_fallbacks",
    )
    positive_keys = (
        "launch_qwen3vl_mrope",
        "launch_qwen3vl_vision_rope",
        "launch_qwen3vl_vision_attention",
    )
    zero_values = {key: cuda.get(key) for key in zero_keys}
    positive_values = {key: cuda.get(key) for key in positive_keys}
    return (
        all(value == 0 for value in zero_values.values())
        and all(isinstance(value, int) and value > 0 for value in positive_values.values())
    ), {
        "required_zero": zero_values,
        "required_positive": positive_values,
    }
...
def parity_gates(
    ...
    add_gate(
        gates,
        "cuda_backend_exact",
        timing.get("backend") == "cuda" and isinstance(timing.get("cuda"), dict),
        {"backend": timing.get("backend"), "device": timing.get("cuda", {}).get("device")},
    )
    fallback_pass, fallback_details = fallback_counters_are_zero(timing)
    add_gate(gates, "cuda_fallback_counters_zero", fallback_pass, fallback_details)
    ...
    add_gate(
        ...
        resources["max_gpu_memory_mib"] <= args.max_gpu_memory_mib,
        {
            "actual_mib": resources["max_gpu_memory_mib"],
            "limit_mib": args.max_gpu_memory_mib,
        },
    )
    add_gate(
        gates,
...
def validate_args(args: argparse.Namespace) -> None:
    if platform.system() != "Linux":
        raise QualificationError("real Qwen3-VL CUDA qualification requires Linux")
    ...
    if min(
        args.host_budget_mb,
        args.backend_budget_mb,
        args.combined_budget_mb,
        args.kv_budget_mb,
        args.scratch_budget_mb,
    ) <= 0:
        raise QualificationError("all runtime memory budgets must be positive")
    if args.max_gpu_memory_mib <= 0 or args.weights_max_gpu_memory_mib <= 0:
        raise QualificationError("GPU memory limits must be positive")
    if args.weights_dir is not None and args.weights_max_rss_mib <= args.max_rss_mib:
        raise QualificationError("BF16 oracle RSS limit must exceed the CUDA RSS limit")
    if not 2 <= args.cuda_repeat_count <= 5:
        raise QualificationError("cuda repeat count must be in [2, 5]")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--weights-device", choices=("cpu", "cuda"), default="cpu")
    parser.add_argument(
        "--weights-dtype",
    ...
    parser.add_argument("--weights-max-gpu-memory-mib", type=float, default=12288.0)
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="identical serial CUDA requests required for bitwise determinism",
    )
    ...
    parser.add_argument("--backend-budget-mb", type=int, default=8192)
    parser.add_argument("--combined-budget-mb", type=int, default=12288)
    parser.add_argument("--kv-budget-mb", type=int, default=256)
    parser.add_argument("--scratch-budget-mb", type=int, default=2048)
...
def main(argv: list[str] | None = None) -> int:
    ...
    report: dict[str, Any] = {
        ...
        "pass": False,
        "release_ready": False,
        "scope": "preprocess_and_bounded_single_request_cuda_acceptance",
    ...
    try:
        ...
        if args.weights_dir is not None:
            report["scope"] = "bf16_logits_preprocess_and_bounded_single_request_cuda_acceptance"
        ...
        parity, timing, cuda_run = run_cuda(args, work_dir)
        cuda_samples = [(parity, timing, cuda_run)]
        repeat_summaries: list[dict[str, Any]] = []
        repeat_gate_failures: list[dict[str, Any]] = []
        for repeat_index in range(2, args.cuda_repeat_count + 1):
            repeat_dir = work_dir / f"cuda_repeat_{repeat_index}"
            repeat_dir.mkdir()
            repeat_parity, repeat_timing, repeat_run = run_cuda(args, repeat_dir)
            cuda_samples.append((repeat_parity, repeat_timing, repeat_run))
        ...
        metrics = patch_metrics(
            ...
            Path(cuda_run["patches"]),
        )
        gates = parity_gates(args, oracle, parity, timing, cuda_run, metrics)
        determinism = cuda_determinism_metrics(cuda_samples)
        add_gate(
            gates,
            "all_cuda_repeats_pass_acceptance",
            not repeat_gate_failures,
            {"run_count": args.cuda_repeat_count, "failures": repeat_gate_failures},
        )
        add_gate(
            gates,
            "cuda_structure_bitwise_deterministic",
            determinism["structure_exact"],
            determinism,
        )
        add_gate(
            gates,
            "cuda_positioned_embeddings_bitwise_deterministic",
        ...
        if args.vision_trace_layer is not None:
            add_gate(
                gates,
                "cuda_vision_trace_bitwise_deterministic",
        ...
        add_gate(
            gates,
            "cuda_projector_bitwise_deterministic",
            determinism["projector_exact"],
            determinism,
        )
        add_gate(
            gates,
            "cuda_deepstack_bitwise_deterministic",
            determinism["deepstack_exact"],
            determinism,
        )
        add_gate(
            gates,
            "cuda_prefill_logits_bitwise_deterministic",
            determinism["prefill_logits_exact"],
            determinism,
        )
        add_gate(
            gates,
            "cuda_generated_tokens_deterministic",
        ...
        if weights_run is not None and weights_payload is not None:
            logits = logit_metrics(Path(weights_run["logits"]), Path(cuda_run["logits"]))
        ...
        report["cuda_run"] = {
            "command": cuda_run["command"],
            "environment": cuda_run["environment"],
            "stdout_path": cuda_run["stdout_path"],
            "stderr_path": cuda_run["stderr_path"],
            "resources": cuda_run["resources"],
            "timing": timing,
        }
        report["cuda_repeats"] = repeat_summaries
        report["parity"] = {
