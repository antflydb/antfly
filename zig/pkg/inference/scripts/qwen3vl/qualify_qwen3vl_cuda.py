...
def read_f32le(path: Path) -> list[float]:
    ...
    if len(payload) % 4 != 0:
        raise QualificationError(
            f"invalid f32le payload length for {path}: {len(payload)}"
        )
...
def patch_metrics(
    ...
    difference = [
        abs(candidate - expected) for expected, candidate in zip(reference, actual)
    ]
    ...
    return {
        ...
        "rmse": math.sqrt(
            math.fsum(value * value for value in difference) / len(difference)
        ),
...
def logit_metrics(
    ...
    difference = [
        candidate - expected for expected, candidate in zip(reference, actual)
    ]
    reference_order = sorted(
        range(len(reference)), key=lambda index: (-reference[index], index)
    )
    ...
    dot_product = math.fsum(
        expected * candidate for expected, candidate in zip(reference, actual)
    )
    denominator = math.sqrt(
        math.fsum(value * value for value in reference)
    ) * math.sqrt(math.fsum(value * value for value in actual))
...
def fallback_counters_are_zero(timing: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
    ...
    return (
        ...
        and all(
            isinstance(value, int) and value > 0 for value in positive_values.values()
        )
    ), {
...
def parity_gates(
    ...
    add_gate(
        ...
        timing.get("backend") == "cuda" and isinstance(timing.get("cuda"), dict),
        {
            ...
            "device": timing.get("cuda", {}).get("device"),
        },
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.weights_dir is not None:
            report["scope"] = (
                "bf16_logits_preprocess_and_bounded_single_request_cuda_acceptance"
            )
        ...
        if weights_run is not None and weights_payload is not None:
            logits = logit_metrics(
                Path(weights_run["logits"]), Path(cuda_run["logits"])
            )
            add_gate(
                gates,
