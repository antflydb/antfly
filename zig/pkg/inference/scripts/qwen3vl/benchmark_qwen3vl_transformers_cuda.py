...
from qualify_qwen3vl_cuda import (
    QualificationError,
    ResourceViolation,
    load_json,
    logit_metrics,
...
SCHEMA = "antfly.qwen3vl.transformers_cuda_benchmark.v1"
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.logits_to_keep < 1:
        raise QualificationError("CUDA benchmark logits to keep must be positive")
...
def oracle_command(args: argparse.Namespace, work_dir: Path) -> list[str]:
    command = [
        ...
        "--logits-output",
        str(work_dir / "cuda_last_logits.f32le"),
        "--output",
        str(work_dir / "cuda_oracle.json"),
    ]
...
def parity_quality_pass(metrics: dict[str, Any]) -> bool:
    return (
        ...
        >= CUDA_PARITY_LIMITS["min_cosine_similarity"]
        and metrics.get("pearson_correlation", -math.inf)
        >= CUDA_PARITY_LIMITS["min_pearson_correlation"]
        and metrics.get("mean_abs", math.inf) <= CUDA_PARITY_LIMITS["max_mean_abs"]
        and metrics.get("rmse", math.inf) <= CUDA_PARITY_LIMITS["max_rmse"]
        and metrics.get("max_abs", math.inf) <= CUDA_PARITY_LIMITS["max_max_abs"]
        and metrics.get("top_k_overlap", -1) >= CUDA_PARITY_LIMITS["min_top_10_overlap"]
    )
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--max-gpu-memory-mib", type=float, default=12288.0)
    ...
    try:
        ...
        try:
            execution = run_resource_monitored(
                command,
                work_dir / "cuda_oracle.stdout.log",
                work_dir / "cuda_oracle.stderr.log",
                ...
                env=environment,
                label="Transformers CUDA benchmark",
            )
        ...
        if execution["returncode"] != 0:
            raise QualificationError(
                f"CUDA oracle exited {execution['returncode']}: {execution['stderr'].strip()[-2000:]}"
            )
        candidate_path = work_dir / "cuda_oracle.json"
        candidate_logits = work_dir / "cuda_last_logits.f32le"
