...
pub fn exercise(maintenance: Maintenance) !void {
    ...
    const initial_tensors = metal.memoryStatsSnapshot();
    {
        ...
        // A completed HTTP request has already destroyed its managed compute
        // backend and tensor buffers. The mapped model and shared provider
        // remain owned by the cached session observed above.
        try expectRequestTensorWork(initial_tensors);
        ...
        try expectRequestTensorWork(before_reload);
