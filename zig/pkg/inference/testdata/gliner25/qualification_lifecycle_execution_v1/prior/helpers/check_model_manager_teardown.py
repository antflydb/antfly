...
MODES = (
    "cache", "ttl", "admission", "retired", "shutdown", "rollback", "escaped",
    "stderr-close", "stderr-return",
)
...
def run_case(binary, output, mode):
    ...
    with stdout_path.open("rb") as source:
        output_bytes = source.read(STREAM_BYTES)
    markers = error_bytes + output_bytes
    close_lease_entries = markers.count(b"teardown-fixture close-entered lease-held64\n")
    close_no_lease_entries = markers.count(b"teardown-fixture close-entered no-lease\n")
    close_entries = close_lease_entries + close_no_lease_entries
    cache_entries = markers.count(b"teardown-fixture cache-entered primary-and-optional-active lease-held64\n")
    if mode == "cache":
    ...
    watchdog = any(marker in error_bytes for marker in (
        b"uninterruptible inference request expired",
        b"uninterruptible inference teardown expired",
    ))
    stderr_lock_confirmed = error_bytes.count(b"teardown-fixture stderr-lock-held\n") == 1
    final_check_isolated = error_bytes.count(b"teardown-fixture final-check-isolated\n") == 1
    lease_release_reported = b"teardown-fixture lease-released\n" in markers
    expected_stderr_lock = mode in ("stderr-close", "stderr-return")
    expected_final_check = mode == "stderr-return"
    success = (
        failure is None and process.returncode == 86 and entered
        and started_expected_operation and not operation_error and not watchdog
        and not cleanup_returned and not lease_release_reported
        and stderr_lock_confirmed == expected_stderr_lock
        and final_check_isolated == expected_final_check
    )
    return {
        ...
        "watchdog_timeout_reported": watchdog,
        "terminal_evidence": "exit86_without_blocking_inprocess_diagnostic",
        "stderr_lock_confirmed": stderr_lock_confirmed,
        "final_release_check_isolated": final_check_isolated,
        "lease_release_reported": lease_release_reported,
...
def main():
    ...
    report = {
        "version": 2, "scope": "model_manager_bounded_teardown/no_model_stderr_v2",
