...
class CheckerTests(unittest.TestCase):
    def test_v2_preserves_exact_input_bytes_and_all_caps_except_new_paths(self):
        prep, inventory, resources = c.preparation()
        previous = Path('/private/tmp/antfly-gliner25-regional-all-small-native-v1')
        old = c.load(previous/'preparation.json', 64*1024)
        self.assertEqual(2, prep['campaign_version'])
        self.assertFalse(prep['model_execution'])
        self.assertFalse(prep['qualification'])
        self.assertEqual(old['inventory'], prep['inventory'])
        self.assertEqual(old['source'], prep['source'])
        self.assertEqual(old['data'], prep['data'])
        self.assertNotEqual(c.pin(c.read(previous/'preparation.json'))['sha256'], c.PREPARATION_SHA256)
        for name in old['configs']:
            self.assertEqual(c.read(c.ROOT/name), c.read(previous/name).replace(str(previous).encode(), str(c.ROOT).encode()))
            self.assertEqual(32*1024**2, c.load(c.ROOT/name)['memory']['optimizer_transaction_bytes'])
        old_resources = c.load(previous/'resource_plan.json', 64*1024)
        for name, value in old_resources.items():
            if name != 'code_formula_pins':
                self.assertEqual(value, resources[name], name)

    def test_v2_frozen_formula_inventory_rejects_stale_or_missing_restore_source(self):
        _, _, resources = c.preparation()
        recorded = {entry['path']: {key: entry[key] for key in ('size_bytes', 'sha256')} for entry in resources['code_formula_pins'].values()}
        c.validate_formula_inventory(recorded, resources)
        controller = resources['code_formula_pins']['controller']['path']
        old = c.load(Path('/private/tmp/antfly-gliner25-regional-all-small-native-v1/resource_plan.json'), 64*1024)['code_formula_pins']['controller']
        self.assertNotEqual(old['sha256'], recorded[controller]['sha256'])
        changed = copy.deepcopy(recorded); changed[controller] = {key: old[key] for key in ('size_bytes', 'sha256')}
        with self.assertRaisesRegex(ValueError, 'formula source changed'):
            c.validate_formula_inventory(changed, resources)
        changed = copy.deepcopy(recorded); del changed[controller]
        with self.assertRaisesRegex(ValueError, 'omits formula source'):
            c.validate_formula_inventory(changed, resources)
        changed = copy.deepcopy(recorded); changed[controller]['size_bytes'] = True
        with self.assertRaisesRegex(ValueError, 'invalid integer'):
            c.validate_formula_inventory(changed, resources)

    def test_v2_binding_rejects_previous_f482_before_any_build_lookup(self):
        old = 'f482989317fd34c0a8852d5a36d837fb78f6cc57862863f5a0b77981125f02b8'
        args = types.SimpleNamespace(binary='/unused/historical-binary', binary_sha256=old)
        with mock.patch.object(runner, 'artifact_argument', return_value={'path': args.binary, 'size_bytes': 10944672, 'sha256': old}) as artifact:
            with self.assertRaisesRegex(ValueError, 'historical executable'):
                runner.bind(args)
