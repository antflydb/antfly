        ...
        switch (tokenizer_type) {
            .huggingface => {
                hf_tok = if (man.gliner_architecture == .boundary) blk: {
                    ...
                    break :blk try hf_tokenizer.HfTokenizer.loadFromBytes(self.allocator, bytes);
...
test "gliner boundary cache pinned small Metal handle retention eviction and reload" {
    try @import("gliner_boundary_cache_lifecycle_test.zig").exercise(ModelManager.evictExpiredAt);
}
