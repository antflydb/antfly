...
fn executeOwned(a: Allocator, io: std.Io, config: Config, admission: *memory.AdmissionController, execution: Execution, outer_control: ?Control, budget: *Budget) !Result {
    ...
    var train = try dataset_mod.Dataset.open(a, config.train_file, .{ .limits = config.dataset_limits }, control, null);
    defer train.deinit();
    var calibration: ?dataset_mod.Dataset = if (config.calibration_file) |path| try dataset_mod.Dataset.open(a, path, .{ .limits = config.dataset_limits }, control, null) else null;
    defer if (calibration) |*value| value.deinit();
    var heldout: ?dataset_mod.Dataset = if (config.test_file) |path| try dataset_mod.Dataset.open(a, path, .{ .limits = config.dataset_limits }, control, null) else null;
