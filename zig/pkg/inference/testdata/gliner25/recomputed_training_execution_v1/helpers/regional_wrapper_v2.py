    ...
    class ObservedTree(supervisor.ProcessTree):
        ...
        def register(self, process, relation):
            ...
            try:
                ...
                if argv and Path(argv[0]).name == 'test' and '/pkg/inference/.zig-cache/o/' in argv[0]:
                    entry = dict(pid=process.pid, create_time=process.create_time(), argv=argv, executable=pin(Path(argv[0])))
