...
const TeardownTicket = struct {
    ...
    fn release(raw: *anyopaque) void {
        ...
        check(self) catch |err| {
            std.log.err("uninterruptible inference teardown expired; terminating supervised worker err={s}", .{@errorName(err)});
    ...
    fn close(raw: *anyopaque) void {
        ...
        if (self.block) {
            if (self.controller != null)
                std.debug.print("teardown-fixture close-entered lease-held64\n", .{})
            else
                std.debug.print("teardown-fixture close-entered no-lease\n", .{});
