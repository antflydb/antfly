...
pub fn planWithProfile(config: *const boundary.Config, layout: Layout, mode: Mode, profile: AttentionProfile, limits: Limits) !Plan {
    ...
    const attention_work = try mul(try mul(scores, e.num_hidden_layers), if (profile == .replay_tiled_v1 and mode == .train) 3 else @as(u64, 1));
    ...
    if (dropout_bytes > limits.max_dropout_mask_bytes) return error.ResourceLimitExceeded;
    ...
    const binding_bytes = try add(dropout_bytes, try mul(try add(attention_bindings, try mul(try add(bs, routed_rows), 2)), 4));
    ...
    const forward_bytes = try add(try add(binding_bytes, constant_bytes), try mul(try add(try mul(e.num_hidden_layers, per_layer), try mul(try mul(routed_rows, e.hidden_size), 4)), 4));
...
pub fn buildWithProfile(bld: *Builder, config: *const boundary.Config, layout: Layout, mode: Mode, profile: AttentionProfile, limits: Limits) !Built {
    const admission = try planWithProfile(config, layout, mode, profile, limits);
    const e = config.encoder;
    const bs = try rows(layout.batch, layout.sequence);
    const bh = try rows(layout.batch, e.num_attention_heads);
...
fn bindPreparedInternal(allocator: Allocator, built: *const Built, config: *const boundary.Config, prepared: *const processor.PreparedBatch, replay: Replay, include_dropout: bool) !BoundInputs {
    ...
    _ = try planWithProfile(config, layout, built.mode, built.attention_profile, built.limits);
