...
def test_catalog_relational_rows_keep_scope_and_identity_after_rename(stateful_api):
    ...
    projected = api._request(
        "POST", path + "/rows/query", payload={"fields": ["id", "count"]}
    )
    assert projected.status_code == 200, projected.text
    typed_row = json.loads(projected.text.strip())
    assert typed_row["row"] == row
    row = {"id": "row1", "count": 43}
    api.post(
        path + "/rows/mutate",
        {
            "schema_version": typed_row["schema_version"],
            "mutations": [
                {"key": "row1", "expected_version": typed_row["version"], "row": row}
            ],
            "sync_level": "full_index",
        },
    )
