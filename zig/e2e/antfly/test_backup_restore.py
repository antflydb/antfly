...
def _seed_cluster_docs_when_writable(
    ...
    # Only explicit pre-commit rejection/abort permits a fresh batch attempt;
...
def test_three_by_three_mixed_relational_restore_survives_coordinator_and_owner_crash(
    ...
    with requests.Session() as session:
        ...
        source_topologies = {table: cluster.table_topology(table) for table in tables}
        assert all(source_topologies.values())
        backup_dir = Path(
        ...
        for table in tables:
            ...
            current = cluster.table_topology(table)
