...
pub fn build(b: *std.Build) void {
    ...
    const lib_platform_test_step = b.step("lib-platform-test", "Run platform unit and native process-lifecycle tests");
    lib_platform_test_step.dependOn(&platform_tests.unit.step);
    if (platform_tests.process) |process| lib_platform_test_step.dependOn(process);
    // Preserve the release branch's existing public target name.
    b.step("platform-test", "Alias for lib-platform-test").dependOn(lib_platform_test_step);
    delegated_inference_steps.inference_test.dependOn(lib_platform_test_step);
