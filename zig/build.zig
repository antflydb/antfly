...
pub fn build(b: *std.Build) void {
    ...
    const api_http_runtime_default_filters = [_][]const u8{
        ...
        "idempotent table batch requires table write permission",
    ...
    const public_api_parity_default_filters = [_][]const u8{
        ...
        "httpx idempotent batch first response matches persisted pending receipt",
        "httpx idempotent batch keeps durable abort receipt failures typed and recoverable",
        "httpx idempotent batch terminalizes missing tables before every operation kind",
        "httpx idempotent batch continues after adopting an expired remote owner",
        "httpx post-start ambiguity preserves receipt and rejects interactive mutation",
        "httpx idempotent terminal replay is read-only while maintenance repairs acknowledgement debt",
        ...
        "httpx write admission rejects saturated table mutations",
        "httpx idempotent batch admission returns its documented JSON receipt error",
        "httpx idempotent batch parse failures use JSON error envelopes",
        "httpx inference connection uses the configured shared admission owner",
        "local inference connection admission is owned exactly once by its target",
        "httpx inference connection preserves upstream retry guidance",
        "request admission bounds positive capacity and preserves unlimited mode",
    ...
    const lib_api_session_maintenance_tests = b.addTest(.{
        ...
        .filters = &.{
            ...
            "interactive sessions and compact receipts have independent retention cutoffs",
            "isolated retention namespaces cannot starve completed sessions",
            "idempotent receipt transitions serialize rejection against execution start",
            "versioned idempotent receipt storage is invisible to legacy session scans",
            "lease renewal retries transient failure and excludes terminal receipts",
            "lease confirmation rejects windows consumed by storage latency",
            "lease cancellation observes deadline while renewal IO is blocked",
            "lease acquires renews and releases by owner",
            ...
            "terminal commit receipts join concurrent replay results monotonically",
            ...
            "transaction inventory work is principal bounded and legacy indexes self heal",
            ...
            "api http server paginates transaction session inventory",
            "receipt admission cleanup followers honor the cleanup budget",
            "stable transaction recovery heartbeat interval stays inside the lease",
            "standalone durable receipt is recovered after owner process restart without client replay",
            "stable transaction recovery retains bare decision conflict ambiguity and heartbeats its lease",
            "httpx idempotent batch first response matches persisted pending receipt",
            "httpx idempotent batch keeps durable abort receipt failures typed and recoverable",
            "httpx idempotent batch terminalizes missing tables before every operation kind",
            "httpx idempotent batch admission returns its documented JSON receipt error",
            "httpx idempotent batch reclaims expired receipt capacity before rejecting admission",
            "httpx idempotent batch continues after adopting an expired remote owner",
            "httpx post-start ambiguity preserves receipt and rejects interactive mutation",
            "httpx idempotent terminal replay is read-only while maintenance repairs acknowledgement debt",
    ...
    const lib_api_docid_tests = b.addTest(.{
        ...
        .filters = &.{
            ...
            "typed internal HTTP errors preserve conflict semantics",
            "distributed session ownership requires a nonzero route and executor",
            "internal transaction HTTP responses prove not-proposed only before decision",
            "internal transaction ingress establishes and validates pre-decision deadline",
    ...
    const api_transactions_docid_tests = b.addTest(.{
        ...
        .filters = &.{
            ...
            "receipt capacity is independent and uses one atomic ledger bootstrap",
            "interactive sessions and compact receipts have independent retention cutoffs",
            "shared session mutations and cleanup reject an adopted owner incarnation",
