...
const AntflyRootImports = struct {
    ...
    fn configure(self: @This(), b: *std.Build, mod: *std.Build.Module, include_lmdb_c: bool, link_libc: bool) void {
        ...
        // Real `lmdb.h` bindings only exist where the C sources are compiled and
        // libc is linked (`include_lmdb_c`); otherwise the stub stands in.
        antfly_storage_build.addLmdbCBindingsImport(b, mod, self.target, self.optimize, include_lmdb_c and link_libc);
...
pub fn build(b: *std.Build) void {
    ...
    addSpngBinding(b, lib_image_bench_mod, target, optimize, lib_image_spng_paths);
    ...
    addSpngBinding(b, lib_image_corpus_mod, target, optimize, lib_image_spng_paths);
    if (lib_image_spng_paths) |spng_paths| {
