...
const platform_time = wal.platform_time;
...
const Config = struct {
    ...
    no_sync: bool = false,
    commit_backend: wal.CommitBackend = .sync,
};
...
const Worker = struct {
    ...
    fn run(self: *@This(), total_threads: usize) void {
        ...
        while (i < self.appends) : (i += 1) {
            _ = self.wal_impl.append(self.payload) catch |err| {
                self.err = err;
                return;
            };
        }
...
fn runCase(alloc: std.mem.Allocator, cfg: Config, grouped: bool) !RunResult {
    ...
    return .{
        ...
        .stats = wal_impl.fullStatsSnapshot(),
    };
}
...
fn benchTmpPath(buf: []u8, suffix: []const u8) [*:0]const u8 {
    const base = "/tmp/antfly-wal-bench-";
    const ts = nowNs();
