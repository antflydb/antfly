...
| Ogg/Vorbis | Pure Zig, unvalidated | Shared Ogg page/packet parsing tracks BOS/EOS, packet boundaries, and page-end granule ownership, and the Vorbis lane decodes headers, floor0/floor1, residues, inverse coupling, IMDCT/windowing, and overlap/add. Its output does not yet match the reference decoder on the checked-in stereo `.ogg`/`.oga` fixtures (the corpus test marks them as a known decoder gap), so Vorbis is demuxed but not qualified. |
...
| Vorbis | Pure-Zig decode, output not validated | Header/floor/residue/coupling/IMDCT code exists and no longer reads freed floor scratch across channels, but the checked-in stereo `.ogg`/`.oga` fixtures decode to output that does not correlate with the reference. Validate against libvorbis stage by stage before claiming the lane. |
...
- No attempt to take on general Vorbis support casually; that is a real
