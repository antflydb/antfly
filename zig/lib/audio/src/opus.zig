...
fn silkDecodeIndices(
    ...
    out.signal_type = @fromBackingInt(@intCast(@as(u2, @intCast(@divTrunc(ix_type, 2)))));
    ...
    if (cond_coding == .conditionally) {
        ...
        out.gains_indices[0] = @intCast((try decoder.decodeIcdf(&silk_tables.gain_icdf[@backingInt(out.signal_type) >> 1])) << 3);
    ...
    const cb1_base: usize = (@as(usize, @backingInt(out.signal_type)) >> 1) * cb.n_vectors;
...
fn silkDecodeSigns(decoder: *RangeDecoder, pulses: []i16, signal_type: SilkSignalType, quant_offset_type: u1, sum_pulses: []const i32) !void {
    const base: usize = 7 * (@as(usize, quant_offset_type) + (@as(usize, @backingInt(signal_type)) << 1));
    var block: usize = 0;
...
fn silkDecodePulses(
    ...
    const signal_class: usize = @as(usize, @backingInt(signal_type)) >> 1;
...
fn silkDecodeFrameParameters(
    ...
    out.quant_offset_q10 = silk_tables.quantization_offsets_q10[@backingInt(front.indices.signal_type) >> 1][front.indices.quant_offset_type];
