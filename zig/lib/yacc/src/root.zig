...
pub const Grammar = struct {
    ...
    token_class_reference: []const u8 = "",
    tokens: std.ArrayListUnmanaged([]const u8) = .empty,
    token_classes: std.ArrayListUnmanaged(TokenClass) = .empty,
...
fn parseGrammar(allocator: std.mem.Allocator, source: []const u8) !Grammar {
    ...
    while (lines.next()) |raw_line| {
        ...
        if (std.mem.startsWith(u8, line, "%token_class_reference ")) {
            grammar.token_class_reference = try dupToken(allocator, line["%token_class_reference ".len..]);
            active_rule = null;
            pending_declaration = null;
            continue;
        }
        if (std.mem.startsWith(u8, line, "%token_class ")) {
            try parseTokenClass(allocator, &grammar, line["%token_class ".len..]);
            active_rule = null;
            pending_declaration = null;
            continue;
        }
...
fn parseTokenClass(allocator: std.mem.Allocator, grammar: *Grammar, declaration: []const u8) !void {
    var parts = std.mem.tokenizeAny(u8, declaration, " \t\r");
    const class = parts.next() orelse return error.MissingTokenClassName;
    var token_count: usize = 0;
    while (parts.next()) |token| {
        try grammar.token_classes.append(allocator, .{
            .token = try allocator.dupe(u8, token),
            .class = try allocator.dupe(u8, class),
        });
        token_count += 1;
    }
    if (token_count == 0) return error.MissingTokenClassTokens;
}
...
pub fn validateGrammar(grammar: Grammar) !void {
    ...
    if (grammar.token_class_reference.len != 0 and
        !std.mem.eql(u8, grammar.token_class_reference, grammar.postgres_commit))
    {
        return error.TokenClassReferenceMismatch;
    }
    for (grammar.token_classes.items, 0..) |token_class, index| {
        for (grammar.token_classes.items[0..index]) |previous| {
            if (std.mem.eql(u8, previous.token, token_class.token)) return error.DuplicateTokenClass;
        }
    }
...
fn emitZigMetadata(
    ...
    try appendFmt(allocator, &out,
        ...
        \\pub const start_symbol = "{f}";
        \\pub const token_count = {d};
        \\pub const token_class_count = {d};
        ...
        \\
    , .{
        std.zig.fmtString(input_path),
        std.zig.fmtString(grammar.postgres_major),
        std.zig.fmtString(grammar.postgres_branch),
        std.zig.fmtString(grammar.postgres_commit),
        std.zig.fmtString(grammar.postgres_commit_date),
        std.zig.fmtString(grammar.postgres_gram_y),
        std.zig.fmtString(grammar.postgres_scan_l),
        std.zig.fmtString(grammar.cockroach_sql_y),
        &source_hash,
        std.zig.fmtString(grammar.start_symbol),
        grammar.tokens.items.len,
        grammar.token_classes.items.len,
    ...
    for (tables.symbols) |symbol| {
        try appendFmt(allocator, &out, "    .{{ .name = \"{f}\", .kind = .{s} }},\n", .{ std.zig.fmtString(symbol.name), @tagName(symbol.kind) });
    }
    ...
    for (grammar.tokens.items) |token| try appendFmt(allocator, &out, "    {f},\n", .{std.zig.fmtId(token)});
    try out.appendSlice(allocator,
        \\};
        \\
    ...
    if (grammar.token_classes.items.len != 0) {
        try out.appendSlice(allocator, "pub const TokenClass = enum {\n");
        for (grammar.token_classes.items, 0..) |token_class, index| {
            var seen = false;
            for (grammar.token_classes.items[0..index]) |previous| {
                if (std.mem.eql(u8, previous.class, token_class.class)) {
                    seen = true;
                    break;
                }
            }
            if (!seen) try appendFmt(allocator, &out, "    {f},\n", .{std.zig.fmtId(token_class.class)});
        }
        try out.appendSlice(allocator,
            \\};
            \\const token_class_map = std.StaticStringMap(TokenClass).initComptime(.{
            \\
        );
        for (grammar.token_classes.items) |token_class| {
            try appendFmt(allocator, &out, "    .{{ \"{f}\", .{f} }},\n", .{ std.zig.fmtString(token_class.token), std.zig.fmtId(token_class.class) });
        }
        try out.appendSlice(allocator,
            \\});
            \\pub fn tokenClassByName(name: []const u8) ?TokenClass {
            \\    return token_class_map.get(name);
            \\}
            \\pub fn tokenClass(token: Token) ?TokenClass {
            \\    return tokenClassByName(@tagName(token));
            \\}
            \\
        );
    }
    ...
    for (grammar.rules.items) |rule| try appendFmt(allocator, &out, "    {f},\n", .{std.zig.fmtId(rule.name)});
    try out.appendSlice(allocator,
        \\};
        \\
    ...
    const terminal_count = grammar.tokens.items.len + 1;
    for (grammar.rules.items, 0..) |rule, rule_idx| {
        try appendFmt(allocator, &out, "        {d} => .{f},\n", .{ ruleSymbolId(terminal_count, rule_idx), std.zig.fmtId(rule.name) });
    }
    try out.appendSlice(allocator,
        \\        else => null,
        \\    };
...
test "generateZigMetadata escapes untrusted labels metadata and Zig identifiers" {
    var arena_impl = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_impl.deinit();
    const arena = arena_impl.allocator();
    const source =
        \\%expect 0
        \\%reference postgres_major 19"dev
        \\%reference postgres_commit abc
        \\%token_class_reference abc
        \\%token_class reserved while
        \\%start switch
        \\%token while
        \\switch:
        \\    while
        \\  ;
    ;
    const generated = try generateZigMetadata(arena, "odd\"path\\fixture\n.y", source);
    try std.testing.expect(std.mem.indexOf(u8, generated, "// Source: odd\\\"path\\\\fixture\\n.y") != null);
    try std.testing.expect(std.mem.indexOf(u8, generated, ".major = \"19\\\"dev\"") != null);
...
test "validateGrammar requires token class metadata to track its pinned revision" {
    var arena_impl = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena_impl.deinit();
    const arena = arena_impl.allocator();
    const source =
        \\%reference postgres_commit newer
        \\%token_class_reference older
        \\%token_class reserved SELECT
        \\%start stmt
        \\%token SELECT
        \\stmt:
        \\    SELECT
    ...
    ;
    const grammar = try parseGrammar(arena, source);
    try std.testing.expectError(error.TokenClassReferenceMismatch, validateGrammar(grammar));
}
