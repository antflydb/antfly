...
- `zig build lib-image-conformance` — checked-in JPEG 2000 conformance plus the external ISO matrix without fetching. The external matrix self-skips when `/tmp/openjpeg-data` is absent. Current baseline when fixtures are present: `pass=4, fail=1, skip=18` of 23 fixtures.
- `../scripts/fetch-conformance-fixtures.sh --suite image` — one-time populate `/tmp/openjpeg-data` via shallow clone of `openjpeg-data` at a pinned commit.
