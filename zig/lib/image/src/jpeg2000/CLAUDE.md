...
Decoder handles `XRsiz/YRsiz != 1` for U8 and U16 via origin-aware per-component geometry (`tile.componentDimensionsAt`) and reference-grid bilinear upsampling. Multi-tile decodes stitch component-grid samples before upsampling, preventing tile-edge clamping seams. U8 processes one compact sample plane at a time during final interleave to bound peak memory; non-8-bit streams retain native precision until after interpolation. Encoder remains 1:1 only.
