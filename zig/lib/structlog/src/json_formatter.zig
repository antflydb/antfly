...
/// Formats log entries as NDJSON (one JSON object per line).
///
/// Output format:
///   {"ts":"2026-04-09T12:00:00Z","level":"info","scope":"metadata","msg":"table created","table":"users"}
pub fn format(
    ...
    try writeJsonEscaped(writer, scope);
...
test "json_formatter: basic output" {
    ...
    // Should start with {"ts":" and end with "}\n"
    try std.testing.expect(std.mem.startsWith(u8, output, "{\"ts\":\""));
    try std.testing.expect(std.mem.endsWith(u8, output, "}\n"));
    // Should contain all expected fields
    try std.testing.expect(std.mem.indexOf(u8, output, "\"level\":\"info\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, output, "\"scope\":\"metadata\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, output, "\"msg\":\"table created\"") != null);
