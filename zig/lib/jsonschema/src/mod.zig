...
const DependentRequired = struct {
    name: []const u8,
    required_fields: [][]const u8 = &.{},
...
const SchemaContext = struct {
    document_root: json.ObjectMap,
    scope_schema: json.ObjectMap,

    fn child(self: SchemaContext, object: json.ObjectMap) SchemaContext {
        return .{
...
const RuntimeValidationContext = struct {
    ...
    fn rootRefGuard(self: *RuntimeValidationContext, value: *const json.Value) !?RootRefGuard {
...
fn parseSchemaUpdateRequest(alloc: std.mem.Allocator, body: []const u8) ![]u8 {
...
fn parseSchema(alloc: std.mem.Allocator, schema_json: []const u8) !TableSchema {
    ...
    var parsed = try json.parseFromSlice(json.Value, alloc, schema_json, .{});
    defer parsed.deinit();
    try validateSchemaValue(parsed.value);
...
pub fn validateJsonSchemaJson(
    ...
    var schema_parsed = try json.parseFromSlice(json.Value, alloc, schema_json, .{});
    defer schema_parsed.deinit();
    var value_parsed = try json.parseFromSlice(json.Value, alloc, value_json, .{});
...
fn documentTtlTimestampNs(
    ...
    var parsed = try json.parseFromSlice(json.Value, alloc, value_json, .{});
    defer parsed.deinit();
...
fn validateWritesAgainstSchema(
    alloc: std.mem.Allocator,
    schema: TableSchema,
    writes: anytype,
) !void {
...
fn validateDocumentJson(
    ...
    while (it.next()) |entry| {
        ...
        if (fieldMatchesDynamicTemplates(schema.dynamic_templates, field_name)) continue;
        if (document_schema) |resolved_document_schema| {
...
fn stringifyJsonValue(alloc: std.mem.Allocator, value: json.Value) ![]u8 {
    return try std.fmt.allocPrint(alloc, "{f}", .{json.fmt(value, .{})});
}

fn validateSchemaValue(value: json.Value) !void {
...
fn validateDocumentSchemas(value: json.Value) !void {
...
fn validateDocumentSchemaDefinition(value: json.Value) anyerror!void {
    const object = switch (value) {
        .object => |object| object,
        else => return error.InvalidSchemaUpdateRequest,
    };
...
fn validateDocumentSchemaDefinitionWithContext(context: SchemaContext, value: json.Value) anyerror!void {
    const object = switch (value) {
        .object => |object| object,
        else => return error.InvalidSchemaUpdateRequest,
    };
...
fn validateDocumentSchemaKeywords(context: SchemaContext, object: json.ObjectMap) anyerror!void {
...
fn validatePropertySchemaDefinition(value: json.Value) anyerror!void {
    const object = switch (value) {
        .object => |object| object,
        else => return error.InvalidSchemaUpdateRequest,
    };
...
fn validatePropertySchemaDefinitionWithContext(context: SchemaContext, value: json.Value) anyerror!void {
    const object = switch (value) {
        .object => |object| object,
        else => return error.InvalidSchemaUpdateRequest,
    };
...
fn validatePropertySchemaKeywords(context: SchemaContext, object: json.ObjectMap) anyerror!void {
    if (object.get("type")) |schema_type| {
...
fn validateEnumDefinition(value: json.Value) !void {
    if (value != .array) return error.InvalidSchemaUpdateRequest;
}

fn validatePositiveNumber(value: json.Value) !void {
...
fn parseSchemaRefPath(value: json.Value) ![]const u8 {
    return switch (value) {
...
fn hasRefSiblings(object: json.ObjectMap) bool {
    var it = object.iterator();
    while (it.next()) |entry| {
...
fn resolveSchemaObject(context: SchemaContext, object: json.ObjectMap) !json.ObjectMap {
    if (object.get("$ref")) |ref_value| {
        const ref_path = try parseSchemaRefPath(ref_value);
...
fn resolveSchemaRef(context: SchemaContext, ref_path: []const u8) !json.ObjectMap {
...
fn resolveSchemaRefWithin(root_schema: json.ObjectMap, ref_path: []const u8) !json.ObjectMap {
    var current: json.Value = .{ .object = root_schema };
...
fn resolveSchemaRefToken(object: json.ObjectMap, token: []const u8) !json.Value {
...
fn validateTypeSpecDefinition(value: json.Value, require_object_only: bool) !ParsedTypeSpec {
    return switch (value) {
...
fn validateTypeArrayDefinition(value: json.Array, require_object_only: bool) !ParsedTypeSpec {
...
fn validateDependentRequiredDefinition(value: json.Value) !void {
    const object = switch (value) {
        .object => |object| object,
        else => return error.InvalidSchemaUpdateRequest,
    };
...
fn validateDefinitionsDefinition(context: SchemaContext, value: json.Value) anyerror!void {
    const object = switch (value) {
        .object => |object| object,
        else => return error.InvalidSchemaUpdateRequest,
    };
...
fn validateDependentSchemasDefinition(context: SchemaContext, value: json.Value) anyerror!void {
    const object = switch (value) {
        .object => |object| object,
        else => return error.InvalidSchemaUpdateRequest,
    };
...
fn validateDependenciesDefinition(context: SchemaContext, value: json.Value) anyerror!void {
    const object = switch (value) {
        .object => |object| object,
        else => return error.InvalidSchemaUpdateRequest,
    };
...
fn validatePatternPropertiesDefinition(context: SchemaContext, value: json.Value) anyerror!void {
    const object = switch (value) {
        .object => |object| object,
        else => return error.InvalidSchemaUpdateRequest,
    };
...
fn validatePrefixItemsDefinition(context: SchemaContext, value: json.Value) anyerror!void {
    if (value != .array) return error.InvalidSchemaUpdateRequest;
    for (value.array.items) |item| try validatePropertySchemaDefinitionWithContext(context, item);
}

fn validateAnyOfDefinition(context: SchemaContext, value: json.Value) anyerror!void {
    if (value != .array) return error.InvalidSchemaUpdateRequest;
    for (value.array.items) |variant| try validatePropertySchemaDefinitionWithContext(context, variant);
}

fn validateOneOfDefinition(context: SchemaContext, value: json.Value) anyerror!void {
    if (value != .array) return error.InvalidSchemaUpdateRequest;
    for (value.array.items) |variant| try validatePropertySchemaDefinitionWithContext(context, variant);
}

fn validateAllOfDefinition(context: SchemaContext, value: json.Value) anyerror!void {
    if (value != .array) return error.InvalidSchemaUpdateRequest;
    for (value.array.items) |variant| try validatePropertySchemaDefinitionWithContext(context, variant);
}

fn validateAdditionalItemsDefinition(context: SchemaContext, object: json.ObjectMap) anyerror!void {
...
fn validateDynamicTemplates(value: json.Value) !void {
    switch (value) {
        .object => |object| {
            var it = object.iterator();
...
fn validateDynamicTemplate(value: json.Value) !void {
    const object = switch (value) {
        .object => |object| object,
        else => return error.InvalidSchemaUpdateRequest,
    };
...
fn validateNonNegativeInteger(value: json.Value) !void {
...
fn parseTableSchemaValue(alloc: std.mem.Allocator, value: json.Value) !TableSchema {
...
fn parseDocumentSchemas(alloc: std.mem.Allocator, value: json.Value) ![]DocumentSchema {
...
fn parseDocumentProperties(alloc: std.mem.Allocator, context: SchemaContext, object: json.ObjectMap) anyerror![]DocumentProperty {
...
fn parseAnonymousProperty(alloc: std.mem.Allocator, context: SchemaContext, unresolved_object: json.ObjectMap) anyerror!*DocumentProperty {
...
fn parseAnonymousPropertyKeywords(alloc: std.mem.Allocator, context: SchemaContext, object: json.ObjectMap) anyerror!*DocumentProperty {
...
fn parseDependentRequired(alloc: std.mem.Allocator, object: json.ObjectMap) ![]DependentRequired {
...
fn parseDependentSchemas(alloc: std.mem.Allocator, context: SchemaContext, object: json.ObjectMap) ![]DependentSchema {
...
fn parseLegacyDependentRequired(alloc: std.mem.Allocator, object: json.ObjectMap) ![]DependentRequired {
    const dependencies = try alloc.alloc(DependentRequired, object.count());
    var initialized: usize = 0;
    errdefer {
        for (dependencies[0..initialized]) |*dependency| dependency.deinit(alloc);
...
fn parseLegacyDependentSchemas(alloc: std.mem.Allocator, context: SchemaContext, object: json.ObjectMap) ![]DependentSchema {
    const dependencies = try alloc.alloc(DependentSchema, object.count());
    var initialized: usize = 0;
    errdefer {
        for (dependencies[0..initialized]) |*dependency| dependency.deinit(alloc);
...
fn parseTypeSpec(alloc: std.mem.Allocator, value: json.Value, require_object_only: bool) !ParsedTypeSpec {
...
fn parseNullableFlag(object: json.ObjectMap) bool {
...
fn parsePatternProperties(alloc: std.mem.Allocator, context: SchemaContext, object: json.ObjectMap) ![]PatternProperty {
...
fn parseRequiredFields(alloc: std.mem.Allocator, required: json.Array) ![][]const u8 {
...
fn parseEnumValues(alloc: std.mem.Allocator, values: json.Array) ![][]const u8 {
...
fn parsePropertyVariants(alloc: std.mem.Allocator, context: SchemaContext, variants: json.Array) ![]DocumentProperty {
...
fn parseDynamicTemplates(alloc: std.mem.Allocator, value: json.Value) ![]DynamicTemplate {
    return switch (value) {
        .object => |object| blk: {
...
fn parseDynamicTemplate(alloc: std.mem.Allocator, default_name: []const u8, value: json.Value) !DynamicTemplate {
    const object = value.object;
    const field_type = if (object.get("mapping")) |mapping|
        if (mapping == .object)
...
fn fieldMatchesDynamicTemplates(dynamic_templates: []const DynamicTemplate, field_name: []const u8) bool {
    for (dynamic_templates) |dynamic_template| {
        if (dynamic_template.match_pattern) |match_pattern| {
            if (globMatch(match_pattern, field_name)) return true;
        }
        if (dynamic_template.path_match) |path_match| {
            if (globMatch(path_match, field_name)) return true;
        }
    }
    return false;
}
...
fn isIntegralJsonNumber(value: json.Value, numeric_value: f64) bool {
    return switch (value) {
        .integer => true,
...
fn validateTtlFieldValue(value: json.Value) !void {
    if (value == .null) return;
    _ = try parseTtlTimestampNs(value);
}

fn validateRequiredFieldsPresent(object: json.ObjectMap, required_fields: []const []const u8) !void {
    for (required_fields) |field_name| {
        if (!object.contains(field_name)) return error.InvalidBatchRequest;
    }
}

fn validateObjectCardinality(object: json.ObjectMap, min_properties: ?u64, max_properties: ?u64) !void {
...
fn validatePropertyNames(
    ...
    while (it.next()) |entry| {
        if (shouldIgnoreSchemaValidationField(entry.key_ptr.*)) continue;
        const key_value: json.Value = .{ .string = entry.key_ptr.* };
...
fn validateDependentRequired(object: json.ObjectMap, dependent_required: []const DependentRequired) !void {
...
fn validateDependentSchemas(
    ...
    for (dependent_schemas) |dependency| {
        if (!object.contains(dependency.name)) continue;
        const object_value: json.Value = .{ .object = object };
...
fn jsonValueEqual(left: json.Value, right: json.Value) bool {
...
fn parseJsonNumber(value: json.Value) !f64 {
    return switch (value) {
...
fn parseTtlTimestampNs(value: json.Value) !u64 {
    return switch (value) {
        .integer => |integer| blk: {
