...
pub const Manifest = struct {
    ...
    pub fn encodeAlloc(self: Manifest, alloc: Allocator) ![]u8 {
        ...
        const total_len = std.math.add(usize, header_size + footer_size, entries_len) catch return error.VectorBlockManifestTooLarge;
        ...
        writeU32(out[60..64], 0);
        writeU32(out[64..68], std.hash.Crc32.hash(out[0..64]));
        var pos = header_size;
...
pub fn decodeAlloc(alloc: Allocator, bytes: []const u8) !Decoded {
    ...
    if (readU16(bytes[8..10]) != version or readU16(bytes[10..12]) != 0) return error.UnsupportedVectorBlockManifestVersion;
    ...
    const expected_len = std.math.add(usize, header_size + footer_size, entries_len) catch return error.InvalidVectorBlockManifest;
    ...
    return .{ .manifest = manifest, .owned_segments = segments };
}
