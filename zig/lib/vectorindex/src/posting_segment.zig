...
pub const Catalog = struct {
    ...
    pub fn collectDeltas(self: Catalog, alloc: Allocator, posting_id: PostingId) ![]DeltaValue {
        ...
        for (self.segments) |segment| {
            if (!segmentMayContainPostingDeltaAfterGeneration(segment.meta, posting_id, null)) continue;
    ...
    pub fn collectDeltasAfterGeneration(self: Catalog, alloc: Allocator, posting_id: PostingId, base_generation: u64) ![]DeltaValue {
        ...
        errdefer deltas.deinit(alloc);
        for (self.segments) |segment| {
            if (!segmentMayContainPostingDeltaAfterGeneration(segment.meta, posting_id, base_generation)) continue;
            var reader = try Reader.init(segment.data);
    ...
    pub fn appendDeltaRecords(
        ...
        for (self.segments) |segment| {
            if (!segmentMayContainPostingDeltaAfterGeneration(segment.meta, posting_id, min_generation)) continue;
...
pub const LazyDirectorySnapshot = struct {
    ...
    pub fn loadBaseDataBatchAlloc(self: LazyDirectorySnapshot, alloc: Allocator, posting_ids: []const PostingId) ![]?[]u8 {
        ...
        var point_reads_stack: [base_data_batch_point_read_stack_capacity]BatchPointValueRead = undefined;
        const use_point_reads_stack = posting_ids.len <= point_reads_stack.len;
        var point_reads_storage: []BatchPointValueRead = if (use_point_reads_stack)
            point_reads_stack[0..posting_ids.len]
        else
            try alloc.alloc(BatchPointValueRead, posting_ids.len);
        defer if (!use_point_reads_stack) alloc.free(point_reads_storage);
    ...
    fn appendDeltaTailAfterGenerationIntoScratchWithStatsFromSegment(
        ...
        for (self.manifest.segments[start_index..]) |entry| {
            ...
            if (!segmentMayContainPostingDeltaAfterGeneration(entry.meta, posting_id, generation)) continue;
    ...
    fn loadDeltaTailFilteredWithStatsAlloc(self: LazyDirectorySnapshot, alloc: Allocator, posting_id: PostingId, min_generation: ?u64) !DeltaTailWithStats {
        ...
        for (self.manifest.segments) |entry| {
            if (!segmentMayContainPostingDeltaAfterGeneration(entry.meta, posting_id, min_generation)) continue;
    ...
    fn readDeltaRecordsAlloc(self: LazyDirectorySnapshot, alloc: Allocator, entry: OwnedManifestEntry, posting_id: PostingId, min_generation: ?u64) ![]posting.PostingDeltaRecord {
        if (!segmentMayContainPostingDeltaAfterGeneration(entry.meta, posting_id, min_generation)) return try alloc.alloc(posting.PostingDeltaRecord, 0);
        if (entry.meta.byte_len > self.options.max_segment_bytes) return error.PostingSegmentTooLarge;
    ...
    fn readDeltaRecordsWithStatsAlloc(
        ...
        if (!segmentMayContainPostingDeltaAfterGeneration(entry.meta, posting_id, min_generation)) return;
        if (entry.meta.byte_len > self.options.max_segment_bytes) return error.PostingSegmentTooLarge;
    ...
    fn readDeltaRecordsIntoScratchWithStatsAlloc(
        ...
        if (!segmentMayContainPostingDeltaAfterGeneration(entry.meta, posting_id, base_generation)) return;
        if (entry.meta.byte_len > self.options.max_segment_bytes) return error.PostingSegmentTooLarge;
...
fn readSegmentPointIndexEntryAlloc(alloc: Allocator, io: std.Io, dir: std.Io.Dir, entry: ManifestEntry, posting_id: PostingId, kind: EntryKind) !?IndexEntry {
    ...
    return try pointIndexEntryFromIndexData(index.data, entry.meta.entry_count, posting_id, kind);
}
...
pub fn readSegmentDeltaRecordsAlloc(alloc: Allocator, io: std.Io, dir: std.Io.Dir, entry: ManifestEntry, posting_id: PostingId, min_generation: ?u64) ![]posting.PostingDeltaRecord {
    ...
    while (index < range.past_index) : (index += 1) {
        ...
        if (min_generation) |generation| {
            ...
            try appendDeltaValueAfterGenerationAlloc(alloc, value, generation, &records);
        } else {
            try appendDeltaValueAllRecordsAlloc(alloc, value, &records);
        }
    }
...
pub fn readSegmentDeltaRecordsWithStatsAlloc(
    ...
    if (all_records_after_generation) {
        try appendDeltaValueRangeAllRecordsWithStatsAlloc(alloc, index_data.data, range, records, stats);
        return;
    }
    ...
    while (index < range.past_index) : (index += 1) {
        ...
        try appendDeltaValueAfterGenerationWithStatsAlloc(alloc, value, base_generation, records, stats);
    }
}
...
pub fn readSegmentDeltaRecordsIntoScratchWithStatsAlloc(
    ...
    if (all_records_after_generation) {
        try appendDeltaValueRangeAllRecordsIntoScratchWithStatsAlloc(alloc, index_data.data, range, scratch, stats);
        return;
    }
    ...
    while (index < range.past_index) : (index += 1) {
        ...
        try appendDeltaValueAfterGenerationIntoScratchWithStatsAlloc(alloc, value, base_generation, scratch, stats);
    }
}
...
const SegmentIndexBuffer = struct {
    data: []u8,
    owned: bool,

    fn deinit(self: SegmentIndexBuffer, alloc: Allocator) void {
        if (self.owned) alloc.free(self.data);
    }
};
...
pub const DirectoryBatchWriter = struct {
    ...
    fn appendPendingDeltaEntriesWithStats(
        ...
        for (self.writer.entries.items) |entry| {
            ...
            try appendDeltaValueAfterGenerationWithStatsAlloc(alloc, entry.value, base_generation, records, stats);
        }
    }
    ...
    fn appendPendingDeltaEntriesIntoScratchWithStats(
        ...
        for (self.writer.entries.items) |entry| {
            ...
            try appendDeltaValueAfterGenerationIntoScratchWithStatsAlloc(alloc, entry.value, base_generation, scratch, stats);
        }
    }
...
fn readSegmentIndexWithScratchAlloc(
    alloc: Allocator,
    io: std.Io,
    dir: std.Io.Dir,
    entry: ManifestEntry,
    ...
    const index_bytes = try segmentIndexByteLen(entry);
    if (index_bytes <= scratch.len) {
        const index_data = scratch[0..index_bytes];
    ...
    return .{
        .data = try readSegmentIndexAlloc(alloc, io, dir, entry),
        .owned = true,
    };
}
...
fn segmentMayContainPostingDeltaAfterGeneration(meta: SegmentMeta, posting_id: PostingId, min_generation: ?u64) bool {
    return meta.mayContainPointKind(posting_id, .delta) and segmentMayContainDeltaAfterGeneration(meta, min_generation);
}
...
fn readDeltaValueRangeAllRecordsOwnedAlloc(
    ...
    const record_count = try deltaValueRangeRecordCount(index_data, range);
...
fn deltaValueRangeRecordCount(index_data: []const u8, range: DeltaValueRange) !usize {
    var record_count: usize = 0;
    var index = range.first_index;
    while (index < range.past_index) : (index += 1) {
        const found = try deltaIndexEntryFromRange(index_data, range, index);
...
fn appendDeltaIteratorAfterGenerationAlloc(
    alloc: Allocator,
...
fn appendDeltaValueRangeAllRecordsWithStatsAlloc(
    alloc: Allocator,
    index_data: []const u8,
    range: DeltaValueRange,
    records: *std.ArrayListUnmanaged(posting.PostingDeltaRecord),
    ...
    while (index < range.past_index) : (index += 1) {
        ...
        const value = try deltaValueFromEntryRange(range, found);
        var iterator = try posting.PostingFormat.DeltaTailIterator.init(value);
        const value_record_count = iterator.recordCount();
        stats.records += value_record_count;
        stats.encoded_value_bytes += value.len;
        stats.records_after_generation += value_record_count;
        while (try iterator.next()) |record| {
            if (record.op == .tombstone) stats.tombstones_after_generation += 1;
            stats.max_sequence_after_generation = @max(stats.max_sequence_after_generation, record.sequence);
            records.appendAssumeCapacity(record);
...
fn appendDeltaValueAfterGenerationWithStatsAlloc(
    alloc: Allocator,
    value: []const u8,
    generation: u64,
    records: *std.ArrayListUnmanaged(posting.PostingDeltaRecord),
...
fn appendDeltaValueRangeAllRecordsIntoScratchWithStatsAlloc(
    alloc: Allocator,
    index_data: []const u8,
    range: DeltaValueRange,
    scratch: anytype,
...
fn appendDeltaValueAfterGenerationIntoScratchWithStatsAlloc(
    alloc: Allocator,
    value: []const u8,
    generation: u64,
    scratch: anytype,
...
pub fn testSegmentPointValueRangeReadsVerifyIndexAndValue() !void {
    ...
    try std.testing.expectError(error.BadPostingSegmentChecksum, readSegmentDeltaTailStatsAlloc(alloc, std.testing.io, tmp.dir, entry, 7, 0));
}
...
pub fn testSegmentDeltaReadersSkipOutOfRangeCorruption() !void {
    ...
    const entry = ManifestEntry{ .meta = committed.entry.meta, .path = committed.entry.path };
    const missing_posting_id: PostingId = 99;
    const records = try readSegmentDeltaRecordsAlloc(alloc, std.testing.io, tmp.dir, entry, missing_posting_id, null);
    defer alloc.free(records);
    try std.testing.expectEqual(@as(usize, 0), records.len);
    ...
    defer records_with_stats.deinit(alloc);
    var stats = posting.PostingDeltaTailStats{};
    try readSegmentDeltaRecordsWithStatsAlloc(alloc, std.testing.io, tmp.dir, entry, missing_posting_id, null, &records_with_stats, &stats);
    try std.testing.expectEqual(@as(usize, 0), records_with_stats.items.len);
    try std.testing.expectEqual(@as(usize, 0), stats.records);
    ...
    defer scratch.deinit(alloc);
    var scratch_stats = posting.PostingDeltaTailStats{};
    try readSegmentDeltaRecordsIntoScratchWithStatsAlloc(alloc, std.testing.io, tmp.dir, entry, missing_posting_id, 0, &scratch, &scratch_stats);
    try std.testing.expectEqual(@as(usize, 0), scratch.deltaRecords().len);
    try std.testing.expectEqual(@as(usize, 0), scratch_stats.records);

    const tail_stats = try readSegmentDeltaTailStatsAlloc(alloc, std.testing.io, tmp.dir, entry, missing_posting_id, 0);
    try std.testing.expectEqual(@as(usize, 0), tail_stats.records);
}
...
test "posting segment filtered delta scratch grows by live records only" {
    ...
    const encoded = try posting.PostingFormat.encodeDeltaTail(alloc, records[0..]);
    defer alloc.free(encoded);
    var scratch = posting.PostingStore.FoldScratch{};
    defer scratch.deinit(alloc);
    var stats = posting.PostingDeltaTailStats{};
...
test "posting lazy latest member scan skips older delta values after best" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();

    const older_sequence = (@as(u64, 9) << 32) | 1;
    const newer_sequence = (@as(u64, 10) << 32) | 1;
    const older_delta = try posting.PostingFormat.encodeDeltaTail(alloc, &.{
        .{ .sequence = older_sequence, .op = .insert, .vector_id = 42 },
    });
    defer alloc.free(older_delta);
    const newer_delta = try posting.PostingFormat.encodeDeltaTail(alloc, &.{
        .{ .sequence = newer_sequence, .op = .tombstone, .vector_id = 42 },
    });
    defer alloc.free(newer_delta);

    var writer = Writer.init(alloc);
    defer writer.deinit();
    try writer.appendDelta(7, older_sequence, older_delta);
    try writer.appendDelta(7, newer_sequence, newer_delta);
    var committed = try commitWriterToDirectoryAlloc(alloc, std.testing.io, tmp.dir, &writer, .{});
    defer committed.deinit(alloc);
    ...
    defer alloc.free(original);
    const reader = try Reader.init(original);
    var delta_iter = reader.deltas(7);
    const older_location = (try delta_iter.next()).?.value;
    const older_offset = @intFromPtr(older_location.ptr) - @intFromPtr(original.ptr);
    var corrupt = try alloc.dupe(u8, original);
    defer alloc.free(corrupt);
    corrupt[older_offset] ^= 0xff;
    try tmp.dir.writeFile(std.testing.io, .{
        .sub_path = committed.entry.path,
        .data = corrupt,
    });
    ...
    const latest = (try lazy.snapshot().latestDeltaRecordAfterGenerationForMember(alloc, 7, 42, 0)).?;
    try std.testing.expectEqual(newer_sequence, latest.sequence);
    try std.testing.expectEqual(posting.PostingDeltaOp.tombstone, latest.op);
    try std.testing.expectError(error.BadPostingSegmentChecksum, lazy.snapshot().loadDeltaTail(alloc, 7));
}
...
test "posting segment delta readers skip out-of-range segment corruption" {
    try testSegmentDeltaReadersSkipOutOfRangeCorruption();
}
...
test "posting segment sorted materialization uses direct record replay for small tails" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    ...
    const records = [_]posting.PostingDeltaRecord{
        .{ .sequence = (@as(u64, 2) << 32) | 1, .op = .insert, .vector_id = 15 },
        .{ .sequence = (@as(u64, 2) << 32) | 2, .op = .tombstone, .vector_id = 20 },
        .{ .sequence = (@as(u64, 2) << 32) | 3, .op = .insert, .vector_id = 20 },
        .{ .sequence = (@as(u64, 2) << 32) | 4, .op = .insert, .vector_id = 40 },
    };
    const delta = try posting.PostingFormat.encodeDeltaTail(alloc, &records);
    defer alloc.free(delta);
    var delta_writer = Writer.init(alloc);
    ...
    try std.testing.expectEqualSlices(posting.VectorId, &.{ 10, 15, 20, 30, 40 }, scratch.member_ids[0..member_count]);
