...
/// A simple spinlock mutex for Zig 0.16+ compatibility.
/// In Zig 0.16, std.Io.Mutex requires an Io context parameter, so we provide
/// a self-contained alternative.
...
/// A simple spinlock-based read-write lock for Zig 0.16+ compatibility.
/// In Zig 0.16, std.Io.RwLock requires an Io context parameter, so we provide
/// a self-contained alternative based on atomics.
pub const SpinRwLock = struct {
    /// State encoding: 0 = unlocked, -1 = write-locked, >0 = reader count
