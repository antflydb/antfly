...
pub const TypeGenerator = struct {
    ...
    fn emitPresenceAwareObjectHelpers(self: *TypeGenerator) !void {
        ...
        try self.w.line("if (openapi_fields.len != struct_info.field_names.len) @compileError(\"OpenAPI object field descriptors must match the generated struct\");", .{});
        try self.w.line("if (.object_begin != try source.next()) return error.UnexpectedToken;", .{});
        try self.w.blank();
        try self.w.line("var result: T = undefined;", .{});
        try self.w.line("var fields_seen = @as([struct_info.field_names.len]bool, @splat(false));", .{});
        ...
        try self.w.line("}};", .{});
        try self.w.blank();
        try self.w.line("inline for (struct_info.field_names, struct_info.field_types, struct_info.field_attrs, openapi_fields, 0..) |field_name_zig, Field, field_attrs, openapi_field, i| {{", .{});
        self.w.indent();
        try self.w.line("if (field_attrs.@\"comptime\") @compileError(\"comptime fields are not supported: \" ++ @typeName(T) ++ \".\" ++ field_name_zig);", .{});
        try self.w.line("if (comptime !std.mem.eql(u8, field_name_zig, openapi_field[1])) @compileError(\"OpenAPI object field descriptor order does not match the generated struct\");", .{});
        ...
        self.w.indent();
        try self.w.line("_ = try std.json.innerParse(Field, allocator, source, options);", .{});
        ...
        self.w.dedent();
        try self.w.line("}}", .{});
        try self.w.line("@field(result, field_name_zig) = try std.json.innerParse(Field, allocator, source, options);", .{});
        ...
        try self.w.line("@field(result, field_name_zig) = try std.json.innerParseFromValue(Field, allocator, entry.value_ptr.*, options);", .{});
        try self.w.line("fields_seen[i] = true;", .{});
        try self.w.line("break;", .{});
        self.w.dedent();
        try self.w.line("}}", .{});
        ...
        try self.w.line("fn openApiFillDefaultStructValues(comptime T: type, comptime openapi_fields: anytype, result: *T, fields_seen: *[@typeInfo(T).@\"struct\".field_names.len]bool) !void {{", .{});
        self.w.indent();
        try self.w.line("@setEvalBranchQuota(100_000);", .{});
        try self.w.line("const struct_info = @typeInfo(T).@\"struct\";", .{});
        try self.w.line("inline for (struct_info.field_names, struct_info.field_types, struct_info.field_attrs, openapi_fields, 0..) |field_name_zig, Field, field_attrs, openapi_field, i| {{", .{});
        ...
        try self.w.line("if (!fields_seen[i]) {{", .{});
        self.w.indent();
        try self.w.line("if (field_attrs.defaultValue(Field)) |default| @field(result, field_name_zig) = default else return error.MissingField;", .{});
        self.w.dedent();
        try self.w.line("}}", .{});
        self.w.dedent();
        try self.w.line("}}", .{});
