...
pub const Reader = struct {
    ...
    pub fn init(alloc: Allocator, bytes: []const u8) !Reader {
        return try initWithDecodeLimits(alloc, bytes, .{});
    }
    ...
    pub fn readPageObject(self: *Reader, page_num: usize) !syntax.Object {
        ...
        if (page_num > pages.len) return error.PageOutOfRange;
    ...
    fn ensurePageIndex(self: *Reader) !void {
        ...
        const pages_obj = root.get("Pages") orelse return error.MissingPages;
    ...
    fn appendPageTreeLeaves(self: *const Reader, out: *std.ArrayList(syntax.Object), node_obj: *const syntax.Object, depth: usize, guard: *TraversalGuard) anyerror!void {
        ...
        var resolved_type = try self.resolveValue(node.get("Type") orelse return error.InvalidPageTree);
    ...
    /// store an indirect reference whose target is an array of stream
    /// references, rather than putting the array directly in the page dict.
    fn collectContentStreamsAlloc(self: *const Reader, contents: *const syntax.Object) ![]syntax.Object {
        var out = std.ArrayList(syntax.Object).empty;
        errdefer {
    ...
    fn collectImagesFromResourcesAlloc(self: *const Reader, resources: *const syntax.Object) ![]PageImage {
        ...
        errdefer {
            for (out.items) |*image| image.deinit(self.alloc);
            out.deinit(self.alloc);
        }
    ...
    fn decodeImageToRgbaGuardedAlloc(
        ...
        if (rgba_len > available_working_set) return error.PdfDecodeWorkingSetTooLarge;
        ...
        if (has_jbig2) {
            ...
            try ensureDecodeWorkingSet(local_decode_limits.max_working_set_bytes, &.{ expected_bitmap_bytes, rgba_len });
            ...
            try ensureDecodeWorkingSet(local_decode_limits.max_working_set_bytes, &.{ input_live_bytes, expected_bitmap_bytes, rgba_len });
            ...
            try ensureDecodeWorkingSet(local_decode_limits.max_working_set_bytes, &.{ input_live_bytes, decoded.pixels.len, rgba_len });
            ...
            if (resolved_color_space.asName()) |color_space| {
                ...
                if (transparency_plan.needsNativeSamples()) {
                    source_samples = try unpackPackedJbig2SamplesAlloc(self.alloc, decoded.pixels, width, height);
                    source_view = .{
                ...
                try ensureDecodeWorkingSet(local_decode_limits.max_working_set_bytes, &.{ input_live_bytes, decoded.pixels.len, rgba_len, pixel_count });
                source_samples = try unpackPackedJbig2SamplesAlloc(self.alloc, decoded.pixels, width, height);
            ...
            if (resolved_color_space.asName() != null) {
                if (source_samples) |samples|
                    try decodeDeviceColorSpaceToRgba(rgba, pixel_count, samples, "DeviceGray", obj.get("Decode"))
                else
        ...
        if (!image_mask and has_jpx) {
            ...
            if (header.bits_per_component > 8 and transparency_plan.needsNativeSamples()) {
                // Color-key comparisons and Matte reversal require exact source
                ...
                try ensureDecodeWorkingSet(local_decode_limits.max_working_set_bytes, &.{ raw.len, sample_bytes, normalized_len, rgba_len });
                ...
                const transparency_live_bytes = try decodeWorkingSetTotal(
                    self.decode_limits.max_working_set_bytes,
                    ancestor_live_bytes,
                    &.{ raw.len, sample_bytes, normalized_len, rgba_len },
                );
                var prepared_matte = try self.prepareMatteSoftMaskAlloc(&transparency_plan, source_view, obj, width, height, context, transparency_live_bytes, mask_depth);
                defer if (prepared_matte) |*prepared| prepared.deinit(self.alloc);
                ...
                const rgba = try self.jpeg2000SamplesToRgbaAlloc(normalized, width, height, decoded_u16.components, decoded_u16.jp2_color, obj, jpx_alpha_mode);
                errdefer self.alloc.free(rgba);
                try self.applyImageTransparencyPlanAlloc(rgba, width, height, &transparency_plan, source_view, context, transparency_live_bytes, mask_depth, if (prepared_matte) |*prepared| prepared else null);
                return .{ .rgba = rgba, .width = width, .height = height };
            }
            ...
            try ensureDecodeWorkingSet(local_decode_limits.max_working_set_bytes, &.{ raw.len, decoded_u8.pixels.len, rgba_len });
            const transparency_live_bytes = try decodeWorkingSetTotal(
                self.decode_limits.max_working_set_bytes,
                ancestor_live_bytes,
                &.{ raw.len, decoded_u8.pixels.len, rgba_len },
            );
            var source_view: ?ImageSourceSamples = null;
            if (transparency_plan.needsNativeSamples()) {
            ...
            var prepared_matte = if (source_view) |view|
                ...
                null;
            defer if (prepared_matte) |*prepared| prepared.deinit(self.alloc);
            const rgba = try self.jpeg2000SamplesToRgbaAlloc(decoded_u8.pixels, width, height, decoded_u8.components, decoded_u8.jp2_color, obj, jpx_alpha_mode);
            errdefer self.alloc.free(rgba);
            try self.applyImageTransparencyPlanAlloc(rgba, width, height, &transparency_plan, source_view, context, transparency_live_bytes, mask_depth, if (prepared_matte) |*prepared| prepared else null);
            return .{ .rgba = rgba, .width = width, .height = height };
        }
        if (!image_mask and streamHasFilter(obj.get("Filter"), "DCTDecode")) {
            if (transparency_plan.nativeMatte()) |matte| {
                // The JPEG API intentionally returns display RGBA. Native
                // Matte reversal is therefore safe only where those channels
                // are the parent sample domain; reject other color spaces
                // before paying for a full entropy decode.
                if (matte.component_count != 1 and matte.component_count != 3) return error.UnsupportedPdfRendering;
                const color_space = (obj.get("ColorSpace") orelse return error.UnsupportedPdfRendering).asName() orelse
                    return error.UnsupportedPdfRendering;
                if (!std.mem.eql(u8, color_space, "DeviceGray") and
                    !std.mem.eql(u8, color_space, "G") and
                    !std.mem.eql(u8, color_space, "DeviceRGB") and
                    !std.mem.eql(u8, color_space, "RGB")) return error.UnsupportedPdfRendering;
            }
            ...
            try ensureDecodeWorkingSet(local_decode_limits.max_working_set_bytes, &.{ encoded.len, jpeg_decoded.rgba.len });
            var source_view: ?ImageSourceSamples = null;
            var gray_samples: ?[]u8 = null;
            defer if (gray_samples) |samples| self.alloc.free(samples);
            if (transparency_plan.needsNativeSamples()) {
                const color_space_obj = obj.get("ColorSpace") orelse return error.UnsupportedPdfRendering;
                ...
                const component_count: usize = if (std.mem.eql(u8, name, "DeviceGray") or std.mem.eql(u8, name, "G"))
                    1
                ...
                if (component_count == 1) {
                    gray_samples = try self.alloc.alloc(u8, pixel_count);
                    for (gray_samples.?, 0..) |*sample, pixel| sample.* = jpeg_decoded.rgba[pixel * 4];
                    source_view = .{
                        .data = .{ .u8 = gray_samples.? },
                        .pixel_count = pixel_count,
                        .component_count = 1,
                        .stride = 1,
            ...
            const gray_live_bytes = if (gray_samples) |samples| samples.len else 0;
            const transparency_live_bytes = try decodeWorkingSetTotal(
                self.decode_limits.max_working_set_bytes,
                ancestor_live_bytes,
                &.{ encoded.len, jpeg_decoded.rgba.len, gray_live_bytes },
            );
            var prepared_matte = if (source_view) |view|
                try self.prepareMatteSoftMaskAlloc(&transparency_plan, view, obj, width, height, context, transparency_live_bytes, mask_depth)
            else
                null;
            defer if (prepared_matte) |*prepared| prepared.deinit(self.alloc);
            if (prepared_matte != null) if (gray_samples) |samples| {
                for (samples, 0..) |sample, pixel| {
                    const offset = pixel * 4;
                    jpeg_decoded.rgba[offset + 0] = sample;
                    jpeg_decoded.rgba[offset + 1] = sample;
                    jpeg_decoded.rgba[offset + 2] = sample;
                }
            };
        ...
        var resolved_color_space = try self.resolveValue(color_space_obj);
        defer resolved_color_space.deinit(self.alloc);
        var unpacked: ?[]u8 = null;
        defer if (unpacked) |bytes| self.alloc.free(bytes);
        const samples = if (bits_i == 8)
            decoded
        else blk: {
            // Packed low-bit images are only unambiguous here for one-component
            // gray spaces (the common soft-mask representation).
    ...
    fn jpeg2000SourceLayout(
        ...
        const component_count = if (image_obj.get("ColorSpace")) |color_space_obj| blk: {
            ...
            const inferred = try jpeg2000InferredDeviceColorSpace(jp2_color, components);
            break :blk if (std.mem.eql(u8, inferred, "DeviceGray")) @as(usize, 1) else if (std.mem.eql(u8, inferred, "DeviceRGB")) @as(usize, 3) else @as(usize, 4);
        };
        var offsets: [4]u8 = .{ 0, 1, 2, 3 };
        if (jp2_color.alphaLayout(components)) |layout| {
            if (component_count != 3) return error.UnsupportedPdfRendering;
            offsets = .{ layout.red, layout.green, layout.blue, layout.alpha };
    ...
    fn jpeg2000SamplesToRgbaAlloc(
        ...
        if (image_obj.get("ColorSpace")) |color_space_obj| {
            ...
            if (jp2_color.alphaLayout(components)) |layout| {
                ...
                try decodeJpeg2000RgbaChannels(rgba, pixel_count, pixels, layout, decode_obj, alpha_mode);
                return rgba;
            }
            if (alpha_mode != .ignore or try self.colorSpaceInputComponentCount(&resolved_color_space) != components)
                return error.UnsupportedPdfRendering;

            if (resolved_color_space.asName()) |color_space| {
                try decodeDeviceColorSpaceToRgba(rgba, pixel_count, pixels, color_space, decode_obj);
            } else if (resolved_color_space == .array) {
                if (try self.tryDecodeIccBasedImageToRgba(rgba, pixel_count, pixels, resolved_color_space.array, decode_obj)) {
                    // handled
                } else if (try self.tryDecodeCalibratedImageToRgba(rgba, pixel_count, pixels, resolved_color_space.array, decode_obj)) {
                    // handled
                } else if (try self.tryDecodeSpotColorSpaceToRgba(rgba, pixel_count, pixels, resolved_color_space.array, decode_obj)) {
                    // handled
                } else {
                    try self.decodeIndexedImageToRgba(rgba, pixel_count, pixels, resolved_color_space.array, decode_obj);
                }
            } else {
                return error.UnsupportedPdfRendering;
            }
        ...
        // PDF permits ColorSpace to be omitted only when the JP2 container
        // supplies enough color metadata. Do not infer four channels as RGBA:
        // enumerated JP2 CMYK and explicit opacity are distinct layouts.
        if (jp2_color.alphaLayout(components)) |layout| {
            try decodeJpeg2000RgbaChannels(rgba, pixel_count, pixels, layout, decode_obj, alpha_mode);
        } else {
            if (alpha_mode != .ignore) return error.UnsupportedPdfRendering;
            const color_space = try jpeg2000InferredDeviceColorSpace(jp2_color, components);
            try decodeDeviceColorSpaceToRgba(rgba, pixel_count, pixels, color_space, decode_obj);
        }
        return rgba;
    }
    ...
    fn decodeJpeg2000RgbaChannels(
        ...
        while (i < pixel_count) : (i += 1) {
            const src = i * 4;
            const dst = i * 4;
            const alpha = if (alpha_mode == .ignore) @as(u8, 0xff) else decoded[src + layout.alpha];
    ...
    fn preflightMaskDimensions(self: *const Reader, width: u32, height: u32) !void {
        const pixels = std.math.mul(usize, width, height) catch return error.RenderedPageTooLarge;
    ...
    fn parentNativeMatteAlloc(self: *const Reader, parent: *const syntax.Object, matte: []const syntax.Object) !NativeMatte {
        const color_space_obj = parent.get("ColorSpace") orelse return error.UnsupportedPdfRendering;
        var color_space = try self.resolveValue(color_space_obj);
        defer color_space.deinit(self.alloc);
    ...
    fn prepareMatteSoftMaskAlloc(
        ...
        if (mask.width != mask_plan.metadata.width or mask.height != mask_plan.metadata.height) return error.InvalidPdfImageMask;
        ...
        switch (plan.*) {
            ...
            .soft_mask => |mask_plan| {
                if (mask_plan.metadata.matte != null) {
                    ...
                    if (prepared.image.width != mask_plan.metadata.width or prepared.image.height != mask_plan.metadata.height)
                        return error.InvalidPdfImageMask;
        ...
        while (true) {
            ...
            switch (section) {
                ...
                .bfrange => {
                    ...
                    switch (dst) {
                        .string => {
                            const dst_bytes = try self.alloc.dupe(u8, dst.string);
...
fn parseXrefEntry(index: usize, line: []const u8) !?XrefEntry {
    ...
    const generation = std.fmt.parseInt(u32, gen_txt, 10) catch |err| switch (err) {
        error.Overflow => return null,
        error.InvalidCharacter => return error.MalformedXrefTable,
    };
    // Real-world incremental PDFs occasionally contain an unused, nominally
    // in-use sentinel entry with generation 65536. A generation cannot exceed
    // the PDF limit of 65535; ignore that one corrupt entry rather than making
    // every valid page in the document unreachable.
    if (generation > std.math.maxInt(u16)) return null;

    const object_offset = std.fmt.parseInt(usize, off_txt, 10) catch |err| switch (err) {
        error.Overflow => return null,
        error.InvalidCharacter => return error.MalformedXrefTable,
    };
...
fn unpackPackedGraySamplesAlloc(alloc: Allocator, encoded: []const u8, width: u32, height: u32, bits: u3) ![]u8 {
    if (bits != 1 and bits != 2 and bits != 4) return error.UnsupportedPdfRendering;
    const width_usize: usize = width;
    const height_usize: usize = height;
    const row_bits = std.math.mul(usize, width_usize, bits) catch return error.UnsupportedPdfRendering;
    const row_bytes = (row_bits + 7) / 8;
    const required = std.math.mul(usize, row_bytes, height_usize) catch return error.UnsupportedPdfRendering;
    if (encoded.len < required) return error.UnsupportedPdfRendering;
    const pixels = std.math.mul(usize, width_usize, height_usize) catch return error.UnsupportedPdfRendering;
    const out = try alloc.alloc(u8, pixels);
    errdefer alloc.free(out);
    const max_sample: u8 = (@as(u8, 1) << bits) - 1;
    for (0..height_usize) |y| {
        const row = encoded[y * row_bytes ..][0..row_bytes];
        for (0..width_usize) |x| {
            const bit_offset = x * bits;
            const shift: u3 = @intCast(@as(usize, 8) - @as(usize, bits) - (bit_offset % 8));
            const sample = (row[bit_offset / 8] >> shift) & max_sample;
            out[y * width_usize + x] = @intCast((@as(u16, sample) * 255) / max_sample);
        }
    }
    return out;
}
...
fn decodeStreamFiltersOwnedAlloc(
    ...
    if (raw.len > decode_limits.max_working_set_bytes) {
        alloc.free(raw);
        return error.PdfDecodeWorkingSetTooLarge;
    }

    var budget = DecodeBudgetAllocator.init(alloc, raw.len, decode_limits.max_working_set_bytes);
    const decode_alloc = budget.allocator();
    return decodeStreamFiltersOwnedBudgetAlloc(
        decode_alloc,
        raw,
        filter_obj,
        decode_parms_obj,
...
fn decodeStreamFiltersOwnedBudgetAlloc(
    alloc: Allocator,
    raw: []u8,
    filter_obj: ?*const syntax.Object,
    decode_parms_obj: ?*const syntax.Object,
...
fn decodeStreamFiltersBeforeOwnedAlloc(
    ...
    var budget = DecodeBudgetAllocator.init(alloc, raw.len, decode_limits.max_working_set_bytes);
    const decode_alloc = budget.allocator();
    return decodeStreamFiltersBeforeOwnedBudgetAlloc(
        decode_alloc,
        raw,
        filter,
        decode_parms_obj,
...
fn decodeStreamFiltersBeforeOwnedBudgetAlloc(
    alloc: Allocator,
    raw: []u8,
    filter: *const syntax.Object,
    decode_parms_obj: ?*const syntax.Object,
...
fn parseType1LocalSubrsLexAlloc(alloc: Allocator, bytes: []const u8, len_iv: i64) ![][]u8 {
    ...
    while (true) {
        ...
        switch (tok) {
            .eof => break,
            .name => |name| if (std.mem.eql(u8, name, "Subrs")) {
                in_subrs = true;
            },
            .keyword => |kw| {
...
fn parseType1GlyphsLexAlloc(
    ...
    while (true) {
        ...
        switch (tok) {
            .eof => break,
            .name => |name| {
                if (std.mem.eql(u8, name, "CharStrings")) {
                    in_charstrings = true;
                } else if (in_charstrings) {
                    if (pending_name) |owned| alloc.free(owned);
                    pending_name = try alloc.dupe(u8, name);
                }
            },
...
fn decodeToUnicodeDestAlloc(alloc: Allocator, bytes: []const u8) ![]u8 {
    ...
    var i: usize = 0;
    while (i + 1 < bytes.len) {
        const first = (@as(u16, bytes[i]) << 8) | bytes[i + 1];
        i += 2;

        const cp: u21 = if (first >= 0xD800 and first <= 0xDBFF) blk: {
            if (i + 1 >= bytes.len) break :blk 0xFFFD;
            const second = (@as(u16, bytes[i]) << 8) | bytes[i + 1];
            if (second < 0xDC00 or second > 0xDFFF) break :blk 0xFFFD;
            i += 2;
            const high: u21 = first - 0xD800;
            const low: u21 = second - 0xDC00;
            break :blk 0x10000 + (high << 10) + low;
...
fn incrementBigEndianBytes(bytes: []u8) bool {
    var i = bytes.len;
    while (i > 0) {
        i -= 1;
        bytes[i] +%= 1;
...
fn clearContentOperandsRetainingNames(
    alloc: Allocator,
    operands: *std.ArrayList(syntax.Object),
    retained_names: *std.ArrayList(syntax.Object),
) !void {
    for (operands.items) |*obj| {
        if (obj.* == .name) {
            try retained_names.append(alloc, obj.*);
            obj.* = .null;
        }
        obj.deinit(alloc);
    }
...
fn applyTextRunOperator(
    ...
    if (isShapePaintOperator(op) or std.mem.eql(u8, op, "sh")) paint_order.* += 1;
    if (std.mem.eql(u8, op, "Do") and
        (operands.len == 0 or operands[operands.len - 1] != .name or findPageForm(forms, operands[operands.len - 1].name) == null))
    {
        paint_order.* += 1;
    }
    ...
    if (std.mem.eql(u8, op, "Td") or std.mem.eql(u8, op, "TD")) {
        if (operands.len >= 2) {
            ...
            // Td updates the text line matrix, not the text matrix after the
            // preceding glyph displacement.
            state.x = state.line_x + tx * state.text_a + ty * state.text_c;
            state.y = state.line_y + tx * state.text_b + ty * state.text_d;
...
fn applyShapeOperator(
    ...
    if (isTextShowOperator(op) or std.mem.eql(u8, op, "sh")) paint_order.* += 1;
    if (std.mem.eql(u8, op, "Do") and
        (operands.len == 0 or operands[operands.len - 1] != .name or findPageForm(forms, operands[operands.len - 1].name) == null))
    {
        paint_order.* += 1;
...
fn applyShadingOperator(
    ...
    if (isTextShowOperator(op) or isShapePaintOperator(op)) paint_order.* += 1;
    if (std.mem.eql(u8, op, "Do") and
        (operands.len == 0 or operands[operands.len - 1] != .name or findPageForm(forms, operands[operands.len - 1].name) == null))
    {
        paint_order.* += 1;
...
fn measureSfntAdvanceAlloc(
    ...
    while (iter.nextCodepoint()) |cp| {
        const glyph_index = font.cmapGlyphIndex(cp) catch {
            ...
            // Text extraction already decoded the character through the PDF
            // encoding, so missing optional SFNT metrics must not abort page
            // rendering and the OCR fallback.
            return estimateDecodedAdvance(decoded, state);
        } orelse {
            advance += estimateCodepointAdvance(cp, state);
            prev_glyph = null;
...
fn isShapePaintOperator(op: []const u8) bool {
    return std.mem.eql(u8, op, "f") or std.mem.eql(u8, op, "F") or
        std.mem.eql(u8, op, "f*") or std.mem.eql(u8, op, "S") or
...
fn transformType3Point(point: [2]f64, run: TextRun, type3: Type3Font, cursor_x: f64, cursor_y: f64) [2]f64 {
    const glyph_x = point[0] * type3.font_matrix[0] + point[1] * type3.font_matrix[2] + type3.font_matrix[4];
    const glyph_y = point[0] * type3.font_matrix[1] + point[1] * type3.font_matrix[3] + type3.font_matrix[5];
    const tx = glyph_x * run.font_size * run.horizontal_scale + cursor_x;
    const ty = glyph_y * run.font_size + cursor_y;
    return .{
...
fn jpeg2000InferredDeviceColorSpace(metadata: image_lib.jpeg2000.Jp2ColorMetadata, components: u8) ![]const u8 {
    if (metadata.enumerated_color_space) |color_space| return switch (color_space) {
        12 => if (components == 4) "DeviceCMYK" else error.UnsupportedPdfRendering,
        16, 18 => if (components == 3) "DeviceRGB" else error.UnsupportedPdfRendering,
        17 => if (components == 1) "DeviceGray" else error.UnsupportedPdfRendering,
        else => error.UnsupportedPdfRendering,
    };
    if (metadata.has_icc_profile) return switch (components) {
        1 => "DeviceGray",
        3 => "DeviceRGB",
        4 => "DeviceCMYK",
        else => error.UnsupportedPdfRendering,
...
fn applyColorKeyMaskFromSamples(rgba: []u8, samples: ImageSourceSamples, mask: ColorKeyMask) !void {
    ...
    switch (samples.data) {
        ...
        .u16 => |words| if (words.len < required_samples or samples.bits_per_component <= 8) return error.UnsupportedPdfRendering,
    }
...
fn applyResampledMatteToSamples(
    ...
    switch (samples.data) {
        .u8 => |bytes| if (bytes.len < required_samples or samples.bits_per_component == 0 or samples.bits_per_component > 8)
            return error.UnsupportedPdfRendering,
        .u16 => |words| if (words.len < required_samples or samples.bits_per_component <= 8 or samples.bits_per_component > 16)
            return error.UnsupportedPdfRendering,
    }
    for (0..samples.component_count) |component| {
        if (samples.component_offsets[component] >= samples.stride) return error.UnsupportedPdfRendering;
...
test "stream filter chains enforce cumulative live allocation budget" {
    const alloc = std.testing.allocator;
    const run_length = &.{ 2, 'A', 'B', 'C', 254, 'Z', 128 };
    var filter = syntax.Object{ .name = @constCast("RunLengthDecode") };
...
test "reader ignores an invalid xref sentinel and repairs a stale zero page count" {
    ...
    const objects = [_][]const u8{
        "1 0 obj\n<< /Type /Catalog /Pages 2 0 R >>\nendobj\n",
        "2 0 obj\n<< /Type /Pages /Count 0 /Kids [3 0 R] >>\nendobj\n",
        "3 0 obj\n<< /Type /Page /Parent 2 0 R /Contents 4 0 R >>\nendobj\n",
        try std.fmt.allocPrint(alloc, "4 0 obj\n<< /Length {d} >>\nstream\n{s}endstream\nendobj\n", .{ content.len, content }),
    };
    defer alloc.free(objects[3]);

    var pdf_bytes = std.ArrayList(u8).empty;
    defer pdf_bytes.deinit(alloc);
    try pdf_bytes.appendSlice(alloc, "%PDF-1.7\n");

    var offsets: [objects.len]usize = undefined;
    for (objects, 0..) |obj_src, i| {
        offsets[i] = pdf_bytes.items.len;
        try pdf_bytes.appendSlice(alloc, obj_src);
    }

    const xref_offset = pdf_bytes.items.len;
    try pdf_bytes.appendSlice(alloc, "xref\n0 6\n0000000000 65535 f \n");
    for (offsets) |off| {
        const line = try std.fmt.allocPrint(alloc, "{d:0>10} 00000 n \n", .{off});
        defer alloc.free(line);
        try pdf_bytes.appendSlice(alloc, line);
    }
    try pdf_bytes.appendSlice(alloc, "0000000000 65536 n \n");
    try pdf_bytes.appendSlice(alloc, "trailer\n<< /Size 6 /Root 1 0 R >>\n");
    const trailer = try std.fmt.allocPrint(alloc, "startxref\n{d}\n%%EOF\n", .{xref_offset});
    defer alloc.free(trailer);
    try pdf_bytes.appendSlice(alloc, trailer);

    var reader = try Reader.init(alloc, pdf_bytes.items);
    defer reader.deinit();
    try std.testing.expectEqual(@as(usize, 1), try reader.pageCount());

    const text = try reader.extractPlainTextAlloc();
    defer alloc.free(text);
    try std.testing.expectEqualStrings("Recovered document text\n", text);
}
...
test "reader decodes and increments UTF-16BE ToUnicode bfrange strings" {
    const alloc = std.testing.allocator;
    const content = "BT\n/F1 12 Tf\n<0001000200030004> Tj\nET\n";
    const cmap =
        "/CIDInit /ProcSet findresource begin\n" ++
    ...
    const text = try reader.extractPlainTextAlloc();
    defer alloc.free(text);
    try std.testing.expectEqualStrings("fifj\u{1f600}\u{fffd}\n", text);
}

test "xref trailer is deinitialized once when recursive Prev parsing fails" {
    const alloc = std.testing.allocator;
    const invalid_previous =
        "xref\n" ++
        "0 0\n" ++
        "trailer\n" ++
        "null\n";
    const current =
        ...
        "trailer\n" ++
        "<< /Size 1 /Prev 0 >>\n";
    const bytes = invalid_previous ++ current;
...
test "xref parser ignores an out-of-range generation sentinel" {
    try std.testing.expectEqual(
        @as(?XrefEntry, null),
        try parseXrefEntry(1, "0000000000 65536 n"),
    );
    try std.testing.expectError(
...
test "reader keeps Type3 encoding difference names alive while building glyphs" {
    // The production server uses the C allocator. It eagerly reuses the freed
    // Encoding dictionary allocation that exposed this borrowed-name lifetime
    // bug, unlike the testing allocator's quarantine behavior.
...
test "reader decodes indexed image with indirect stream lookup" {
    ...
    const objects = [_][]const u8{
        ...
        try std.fmt.allocPrint(
            alloc,
            "5 0 obj\n<< /Type /XObject /Subtype /Image /Width 1 /Height 1 /ColorSpace [/Indexed /DeviceRGB 1 6 0 R] /BitsPerComponent 8 /Length {d} >>\nstream\n{s}\nendstream\nendobj\n",
            .{ image_data.len, image_data },
        ),
        try std.fmt.allocPrint(alloc, "6 0 obj\n<< /Length {d} >>\nstream\n{s}\nendstream\nendobj\n", .{ lookup.len, lookup }),
    };
...
test "JPX embedded alpha obeys SMaskInData modes" {
    const pixels = [_]u8{ 64, 32, 16, 128 };
    const layout = image_lib.jpeg2000.Jp2ColorMetadata.AlphaLayout{
        .red = 0,
        .green = 1,
        .blue = 2,
        .alpha = 3,
    ...
    try Reader.decodeJpeg2000RgbaChannels(&rgba, 1, &pixels, layout, null, .ignore);
    try std.testing.expectEqualSlices(u8, &.{ 64, 32, 16, 255 }, &rgba);

    try Reader.decodeJpeg2000RgbaChannels(&rgba, 1, &pixels, layout, null, .straight);
    try std.testing.expectEqualSlices(u8, &.{ 64, 32, 16, 128 }, &rgba);

    try Reader.decodeJpeg2000RgbaChannels(&rgba, 1, &pixels, layout, null, .premultiplied);
    try std.testing.expectEqualSlices(u8, &.{ 128, 64, 32, 128 }, &rgba);
}

test "JP2 metadata distinguishes CMYK from embedded alpha" {
    try std.testing.expectEqualStrings("DeviceCMYK", try jpeg2000InferredDeviceColorSpace(.{
        .enumerated_color_space = 12,
    }, 4));
    try std.testing.expectError(error.UnsupportedPdfRendering, jpeg2000InferredDeviceColorSpace(.{}, 4));
}
...
test "reader extracts vector text pattern runs" {
    ...
    try std.testing.expectEqual(@as(usize, 1), runs.len);
    try std.testing.expectEqual(@FieldType(PatternRun, "kind").fill, runs[0].kind);
    try std.testing.expectEqual(@FieldType(PatternRun, "mode").tiling, runs[0].mode);
...
test "reader emits shading runs for shading patterns" {
    ...
    try std.testing.expectEqual(@as(usize, 1), runs.len);
    try std.testing.expect(runs[0].mode == .shading);
    try std.testing.expectApproxEqAbs(@as(f64, 5), runs[0].shading.?.x0, 0.0001);
    try std.testing.expectApproxEqAbs(@as(f64, 15), runs[0].shading.?.x1, 0.0001);
}
