...
// PostgreSQL keyword categories are derived from kwlist.h at the pinned
// postgres_commit above. The duplicated reference intentionally makes a
// PostgreSQL revision bump fail generation until this metadata is re-audited.
%token_class_reference 4cc02b80774ecdc4cf2a2d5df09c07df36d68ca5
%token_class column_name BETWEEN COALESCE DATE EXISTS EXTRACT GREATEST INTERVAL JSON LEAST NULLIF OVERLAY POSITION ROW SUBSTRING TIMESTAMP TIMESTAMPTZ TRIM VALUES
%token_class type_function_name AUTHORIZATION BINARY COLLATION CONCURRENTLY CROSS FREEZE FULL ILIKE INNER IS ISNULL JOIN LEFT LIKE NATURAL NOTNULL OUTER OVERLAPS RIGHT VERBOSE
%token_class reserved ALL ANALYZE AND ANY ARRAY AS ASC ASYMMETRIC CASE CAST CHECK COLLATE COLUMN CONSTRAINT CREATE CURRENT_DATE CURRENT_TIMESTAMP DEFAULT DEFERRABLE DESC DISTINCT DO ELSE END EXCEPT FALSE FETCH FOR FOREIGN FROM GRANT GROUP HAVING IN INTERSECT INTO LATERAL LIMIT NOT NULL OFFSET ON ONLY OR ORDER PLACING PRIMARY RETURNING SELECT SOME SYMMETRIC TABLE THEN TO TRUE UNION UNIQUE USER USING WHEN WHERE WINDOW WITH
...
%token ACCESS ADD AGGREGATE ALL ALTER ANALYZE AND ANY ARRAY AS ASC ASYMMETRIC AUTHORIZATION BACKUP BEGIN BETWEEN BY CASCADE CALL CASE CAST CHECK CHECKPOINT CLOSE CLUSTER COLLATE COLLATION COMMIT COMMENT CONCURRENTLY CONFLICT CONSTRAINT CONTINUE CURRENT CURRENT_DATE CURRENT_TIMESTAMP
...
%token NEXT NOTHING OFFSET ONLY OPERATOR OVERRIDING OVERLAPS PERIOD PORTION PREPARED PRIVILEGES PROCEDURE QUERY RANGE REASSIGN RECURSIVE REFRESH REINDEX RELEASE RENAME REPLACE RESET RESTART RESTORE RESTRICT RETURN RETURNING REVOKE RIGHT ROLLBACK ROLE ROUTINE ROW ROWS RULE SAVEPOINT SCHEMA SECURITY SELECT SERVER SET SHARE SHOW SKIP SOME SUBSCRIPTION SYSTEM TABLE TEMP TEMPORARY TIMESTAMP TIMESTAMPTZ TO TRUNCATE
...
unsupported_statement:
  ...
  | CREATE CONSTRAINT TRIGGER trigger_diagnostic_tail_opt
  ...
  | BACKUP DATABASE qualified_name TO STRING
  | RESTORE DATABASE qualified_name FROM STRING
...
index_element:
    expression index_element_opclass_opt index_element_collation_opt index_element_temporal_opt order_modifier_opt nulls_order_opt
  ;
...
index_element_collation_opt:
    /* empty */
  | COLLATE identifier_name
  ;
