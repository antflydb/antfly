    ...
    pub fn from(arena: Allocator, val: anytype) Allocator.Error!Value {
        ...
        if (info == .@"struct") {
            ...
            inline for (info.@"struct".field_names) |field_name| {
                const fv = @field(val, field_name);
                try m.put(arena, field_name, try from(arena, fv));
            }
            return .{ .map = m };
        }
...
fn makeCtx(arena: Allocator, entries: anytype) !Value {
    var m: ValueMap = .{};
    inline for (@typeInfo(@TypeOf(entries)).@"struct".field_names) |field_name| {
        const val = @field(entries, field_name);
        try m.put(arena, field_name, val);
    }
    return .{ .map = m };
}
