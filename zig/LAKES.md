...
| Sidecar builders over real lake rows | Binding/declaration layer exists, and `serverless/build/lake_sidecar_text.zig` now provides the first real builder over `RowSource`: it validates every batch against a pinned text sidecar binding, consumes bytes/JSON text columns from external Parquet/Iceberg/Lance-compatible `ColumnBatch` values, encodes the existing Antfly text segment format with external row-ref document ids, returns an owned declared sidecar artifact, and fails closed on stale source snapshots. Vector, sparse, graph, algebraic publication wiring, artifact-store writes, and rebuild orchestration are still missing for lake sidecars. | Full-text, dense vector, sparse vector, graph, and algebraic sidecars build from external row refs, publish snapshot-keyed artifacts through the serverless artifact store, and produce candidate row refs that hydrate projected lake columns. |
...
5. Complete sidecar builders over real external row refs: the first full-text
   builder now consumes pinned `RowSource` batches and returns a declared text
   sidecar artifact with external row-ref doc ids; remaining work is to wire it
   through artifact-store publication/rebuild workflows and add vector, sparse,
