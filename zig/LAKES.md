...
| Freshness and consistency enforcement | Snapshot binding rules are designed; scaffold validation exists for declared artifacts and batches. `serverless/segment/sidecar_manifest.zig` now validates a declared sidecar manifest against a pinned lake base source, requiring the sidecar source kind, snapshot id, and schema fingerprint to match the base source before it can be treated as fresh. `serverless/query/lake_explain.zig` can now accept declared sidecars in the explain request, validates those declarations against the pinned base source, and rejects stale sidecars or declarations whose artifact refs are not present in the plan artifact set. Runtime scan/index selection still needs the production policy for ignore-vs-rebuild behavior and candidate hydration enforcement. | Query execution, not only explain planning, pins one source snapshot, rejects or ignores stale sidecars by policy, and never mixes candidate row refs from one snapshot with data files from another. |
