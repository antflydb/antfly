...
6. Finish production freshness and cache policy: route public queries through
   ...
   execution beyond the current helper contract, wire protected cache policy
   into runtime defaults and persistent cache storage, and add checksum behavior
   beyond object-version/length checks for object-store range reads.
