...
row refs. The first concrete bridge for this now exists for text sidecars:
`serverless/query/lake_sidecar_candidates.zig` searches an existing text segment
payload, treats text hit document ids as encoded external row-ref keys, decodes
them through the source-binding inverse codec, and validates them against the
declared sidecar snapshot before handing them to lake hydration. It can now also
load declared text sidecar payloads through the serverless artifact-store
interface, select explicit text sidecar names, and fail closed when an implicit
text-sidecar request would be ambiguous. For broad
...
6. Finish production freshness and cache policy: the text sidecar path now has
   a concrete candidate producer that searches text segment payloads, can load
   declared text sidecars through the serverless artifact-store interface,
   converts hit doc IDs through the validated sidecar row-ref decoder, and fails
   closed when implicit sidecar selection would be ambiguous. Remaining work is
   to invoke that producer from public text query/explain operators with a real
   artifact-store handle, add equivalent vector/sparse/graph candidate
   producers, feed generated candidate sets into the sidecar context already
   carried by public row-query/aggregate routing and public explain, choose
