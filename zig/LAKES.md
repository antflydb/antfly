...
5. Complete sidecar builders over real external row refs: full-text, dense
   ...
   operations for those sidecar families, and the first executor can run those
   operations against a supplied pinned `RowSourceProvider` and artifact store;
   remaining work is to derive desired operations from catalog/index metadata,
   bind them to resolved snapshot inventories, and run them from
