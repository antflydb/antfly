...
6. Finish production freshness and cache policy: route public queries through
   ...
   from concrete text/vector/sparse/graph query operators, pass those candidate
   sets into public explain, broaden exact sidecar candidate execution beyond
   the current helper contract, choose deployment-specific values for the
   public serving-cache budget, add persistent cache storage, and extend
   checksum behavior to persisted cache metadata and object-store checksum
   provenance for object-store range reads.
