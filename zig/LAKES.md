...
5. Complete sidecar builders over real external row refs: full-text, dense
   ...
   operations for those sidecar families, the first executor can run those
   operations against a supplied pinned `RowSourceProvider` and artifact store,
   and executor results can reconcile rebuild/reuse/drop outcomes into a
   validated next sidecar manifest. Remaining work is to derive desired
   operations from catalog/index metadata, bind them to resolved snapshot
   inventories, and invoke the executor plus manifest reconciler from
   reconcile/operator workflows.
