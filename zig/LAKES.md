...
- Parquet files, row groups, delete files, and table metadata map to
  ...
  persist that descriptor through the normal manifest store; the remaining
  production step is for the live external-table publication workflow to call
  the attach path when publishing a table head.
...
2. Attach external snapshot metadata to published manifests. The scaffold now
   ...
   generations, and the serverless build layer has a helper that applies an
   external-source plan to an owned manifest before it is written.
...
1. Finish production public catalog routing: harden the installed
   ...
   path with production S3/GCS credential behavior, invoke the external-source
   manifest applicator from live table publication, and add broader
   fixture-backed route coverage for cloud URI shapes.
