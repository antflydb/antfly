...
- Parquet files, row groups, delete files, and table metadata map to
  ...
  bindings still need the object-store discovery path to produce that resolved
  plan with a durable snapshot id before publication.
...
2. Attach external snapshot metadata to published manifests. The scaffold now
   ...
   publication plan to the manifest before publish. The remaining step for
   `.current` external bindings is object-store discovery: the publisher must
   first discover, encode, and pin the current snapshot inventory, then pass
   that resolved plan into the live builder.
...
1. Finish production public catalog routing: harden the installed
   ...
   path with production S3/GCS credential behavior, harden credentialed
   publication for cloud source shapes, and add broader fixture-backed route
   coverage for cloud URI shapes.
...
5. Complete sidecar builders over real external row refs: full-text, dense
   ...
   `count`/`sum`/`min`/`max` materializations for a pinned source snapshot, the
...
6. Finish production freshness and cache policy: route public queries through
   the serverless automatic sidecar-aware scan helper, produce candidate sets
   from concrete text/vector/sparse/graph query operators, broaden exact
   sidecar candidate execution beyond the current helper contract, choose
   deployment-specific values for the public serving-cache budget, add
   persistent cache storage, and add checksum
   behavior beyond object-version/length checks for object-store range reads.
