...
| Sidecar builders over real lake rows | Binding/declaration layer exists, and `serverless/build/lake_sidecar_text.zig` now provides the first real builder and publisher over `RowSource`: it validates every batch against a pinned text sidecar binding, consumes bytes/JSON text columns from external Parquet/Iceberg/Lance-compatible `ColumnBatch` values, encodes the existing Antfly text segment format with external row-ref document ids, writes the payload through the serverless artifact store, returns an owned declared sidecar artifact with store-backed artifact id/checksum/length, and fails closed on stale source snapshots. Vector, sparse, graph, algebraic publication, and rebuild orchestration are still missing for lake sidecars. | Full-text, dense vector, sparse vector, graph, and algebraic sidecars build from external row refs, publish snapshot-keyed artifacts through the serverless artifact store, and produce candidate row refs that hydrate projected lake columns. |
...
5. Complete sidecar builders over real external row refs: the first full-text
   path now consumes pinned `RowSource` batches and publishes a declared text
   sidecar artifact with external row-ref doc ids through the serverless
   artifact store; remaining work is to wire it into rebuild/operator workflows
   and add vector, sparse, graph, and algebraic builders that consume pinned
   Parquet/Iceberg batches and publish snapshot-keyed artifacts.
