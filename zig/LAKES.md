...
4. Finish Iceberg delete and evolution correctness: metadata JSON refresh,
   ...
   pruning, and structured delete-file planning exist for the first path. The
   remaining work is to read active position-delete Parquet files with projected `file_path`/`pos` columns and feed them into the
   tested scan-result row-ref adapter, pass those refs through the common `lake_rows`
   delete filter, implement active equality-delete matching, and complete
