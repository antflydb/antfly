...
declared sidecar snapshot before handing them to lake hydration. For broad
...
6. Finish production freshness and cache policy: the text sidecar path now has
   a concrete candidate producer that searches text segment payloads and
   converts hit doc IDs through the validated sidecar row-ref decoder. Remaining
   work is to invoke that producer from public text query/explain operators,
   add equivalent vector/sparse/graph candidate producers, feed generated
