...
1. Finish production public catalog routing: keep the installed
   external-lake routing source on the public `rows:query`/`rows:aggregate`
   path, then add service-level credential reference resolution, scoped object
   store opening, source snapshot pinning, durable manifest/state recording,
   and successful fixture-backed route coverage.
