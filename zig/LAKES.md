...
6. Finish production freshness and cache policy: the text, sparse, dense
   ...
   case-insensitive exact, and simple trailing-prefix text-pattern filters over
   row-query predicates and aggregate source predicates when the field maps to
   exactly one declared text sidecar, then feed generated candidate sets into
   the pinned scanner/explain context. Remaining work is to broaden automatic producer
