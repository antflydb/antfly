...
- Parquet files, row groups, delete files, and table metadata map to
  ...
  attached artifact set plus lake base-source descriptor; the remaining
  production step is to persist that descriptor through the live
  manifest/catalog wire format.
...
2. Attach external snapshot metadata to published manifests. The scaffold now
   ...
   descriptor; the next step is making that descriptor durable alongside the
   published generation in the manifest codec/catalog head path.
...
1. Finish production public catalog routing: harden the installed
   ...
   path with production S3/GCS credential behavior, persist attached external
   base-source descriptors through live table publication, and add broader
