...
Algebraic materialized group-by and global expression-fold execution now
follow the same freshness rule: when a caller supplies the required source
kind/id/snapshot/schema contract, `serverless/query/lake_rows.zig` only serves
a fold whose embedded source matches that contract; stale or cross-source folds
fall back to a RowSource scan.
