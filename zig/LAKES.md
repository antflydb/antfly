...
4. Implement Iceberg metadata and manifest expansion: metadata JSON refresh,
   ...
   pruning, structured delete-file planning, position-delete row-ref mapping, row-level position/equality delete application, and schema evolution checks.
...
6. Finish production freshness and cache policy: the text, sparse, dense
   ...
   artifact-store-backed text candidate producer for safe positive single-term
   text-pattern filters over row-query predicates and aggregate source
   predicates when the field maps to exactly one declared text sidecar, then
   feed generated candidate sets into the pinned
   scanner/explain context. Remaining work is to broaden automatic producer
