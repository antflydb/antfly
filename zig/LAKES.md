...
| --- | --- | --- |
| Real Parquet scanner | Not implemented. Current external row sources are batch adapters used to prove the `RowSource` contract. Parquet footer preflight parsing now validates trailing `PAR1` trailers and plans exact footer metadata ranges in `serverless/query/lake_parquet_footer.zig`; compact-Thrift footer metadata parsing in `serverless/query/lake_parquet_metadata.zig` extracts row groups and column chunks, and can enrich raw inventories so projected scan planning can produce column-chunk reads. `serverless/query/lake_parquet_page.zig` parses data page headers, scans uncompressed PLAIN i64/byte-array column chunks across pages, and has a first flat optional i64 DataPageV2 byte-definition-level path that produces `NullBitmap` values. `serverless/query/lake_parquet_rowgroup.zig` assembles required and optional PLAIN i64 row groups into `ColumnBatch` values with stable external row refs, plans projected row-group source inputs from footer-enriched inventories, and exposes both preloaded and object-range-backed row-group `RowSource` adapters that can feed `lake_rows` scans. Compression, general RLE/bit-packed definition and repetition levels, dictionary decoding, broader nullable column types, automatic footer-to-source discovery, and public `rows:query` routing are still missing. | Filesystem-backed Parquet fixture can run projection, scalar predicate, row-group pruning, and stable external row refs through `rows:query`. |
| Object-store range I/O | Range planning, footer-tail reads, version-aware cache keys, cache lanes, coalescing rules, URI-to-object-ref parsing, and inventory-to-column-chunk read planning exist in `serverless/query/lake_range_io.zig`. Deterministic raw Parquet prefix snapshot planning exists in `serverless/external_source/object_snapshot.zig`. `serverless/query/lake_parquet_rowgroup.zig` can now read planned column-chunk byte ranges through an `ObjectRangeReader` seam for the first required PLAIN i64 path; production adapters should wrap Antfly's object-storage client there. Footer reads, coalesced physical-read dispatch, and cache-keyed range reuse are not yet wired into the scanner. | Scanner reads footers from object tails and coalesces projected column chunk ranges through Antfly's object-storage abstraction with cache keys including object version/ETag and byte range. |
