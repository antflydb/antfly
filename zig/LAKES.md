...
5. Complete sidecar builders over real external row refs: full-text and sparse
   paths now consume pinned `RowSource` batches and publish declared sidecar
   artifacts with external row-ref doc ids through the serverless artifact
   store; remaining work is to wire them into rebuild/operator workflows and add
   dense vector, graph, and algebraic builders that consume pinned
