...
- Parquet files, row groups, delete files, and table metadata map to
  ...
  metadata into published manifests; dynamic `.current` bindings still need the
  inventory publication path so they can be resolved to a durable snapshot id
  before publication.
...
2. Attach external snapshot metadata to published manifests. The scaffold now
   ...
   manifest builders. The remaining attach step is for `.current` external
   bindings and inventory-artifact publication, where the publisher must first
   discover and pin the current snapshot.
...
1. Finish production public catalog routing: harden the installed
   ...
   path with production S3/GCS credential behavior, resolve `.current`
   external bindings into pinned inventory artifacts during live publication,
   and add broader fixture-backed route coverage for cloud URI shapes.
