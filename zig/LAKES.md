...
| --- | --- | --- |
| Real Parquet scanner | Partially implemented for the first flat column paths. Parquet footer preflight parsing now validates trailing `PAR1` trailers and plans exact footer metadata ranges in `serverless/query/lake_parquet_footer.zig`; compact-Thrift footer metadata parsing in `serverless/query/lake_parquet_metadata.zig` extracts row groups and column chunks, and can enrich raw inventories so projected scan planning can produce column-chunk reads. `serverless/query/lake_parquet_page.zig` parses data page and dictionary page headers, scans uncompressed PLAIN i64/byte-array column chunks across pages, decodes uncompressed required dictionary-encoded i64 pages through Parquet's RLE/bit-packed hybrid index stream, and has a first flat optional i64 DataPageV2 hybrid definition-level path that produces `NullBitmap` values. `serverless/query/lake_parquet_rowgroup.zig` assembles required PLAIN i64, optional PLAIN i64, and required dictionary i64 row groups into `ColumnBatch` values with stable external row refs; its supported-i64 row-group path now dispatches PLAIN vs dictionary decoding from footer-enriched column-chunk encoding metadata and feeds both preloaded and object-range-backed `RowSource` adapters into `lake_rows` scans. Compression, repetition levels/nested data, nullable dictionary columns, broader nullable column types, automatic footer-to-source discovery, and public `rows:query` routing are still missing. | Filesystem-backed Parquet fixture can run projection, scalar predicate, row-group pruning, dictionary/plain mixed column chunks, and stable external row refs through `rows:query`. |
| Object-store range I/O | Range planning, footer-tail reads, version-aware cache keys, cache lanes, coalescing rules, URI-to-object-ref parsing, and inventory-to-column-chunk read planning exist in `serverless/query/lake_range_io.zig`. Deterministic raw Parquet prefix snapshot planning exists in `serverless/external_source/object_snapshot.zig`. `serverless/query/lake_parquet_rowgroup.zig` can now read planned column-chunk byte ranges through an `ObjectRangeReader` seam and decode supported uncompressed i64 encodings from inventory metadata; production adapters should wrap Antfly's object-storage client there. Footer reads, coalesced physical-read dispatch, and cache-keyed range reuse are not yet wired into the scanner. | Scanner reads footers from object tails and coalesces projected column chunk ranges through Antfly's object-storage abstraction with cache keys including object version/ETag and byte range. |
| Catalog/schema lake binding | External table binding contract exists in `serverless/external_source/catalog_binding.zig` with URI, format, credential ref, snapshot mode, schema fingerprint, read-only MVP policy, source-kind mapping, and manifest base-source conversion. It is not yet persisted in relational table schema/catalog metadata. | Table schema accepts and persists that external base-source binding, then query planning resolves `.current` bindings to pinned snapshot ids before execution. |
| Relational `rows:query` integration | Scaffold query helpers exist; public row-plan execution is not yet routed through external lake sources. Projected scan planning in `serverless/query/lake_scan_plan.zig` now validates external table bindings against pinned inventories and turns projected columns into logical and coalesced physical object range reads. | `rows:query` over an external Parquet table supports projection, scalar filters, limit, simple ordering, and JSON-row adaptation while typed execution matures. |
