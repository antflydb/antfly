...
| --- | --- | --- |
| Real Parquet scanner | Not implemented. Current external row sources are batch adapters used to prove the `RowSource` contract. Parquet footer preflight parsing now validates trailing `PAR1` trailers and plans exact footer metadata ranges in `serverless/query/lake_parquet_footer.zig`; Thrift metadata decoding and column/page decoding are still missing. | Filesystem-backed Parquet fixture can run projection, scalar predicate, row-group pruning, and stable external row refs through `rows:query`. |
