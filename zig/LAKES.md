...
5. Complete sidecar builders over real external row refs: full-text, dense
   ...
   desired full-text/vector/sparse/graph sidecars for a pinned source snapshot,
   the first executor can run those operations against a supplied pinned
   `RowSourceProvider` and artifact store, and executor results can reconcile
   rebuild/reuse/drop outcomes into a validated next sidecar manifest.
   Remaining work is to bind that derivation to resolved snapshot inventories,
   add algebraic catalog-derived build specs, and invoke the executor plus
   manifest reconciler from reconcile/operator workflows.
6. Finish production freshness and cache policy: route public queries through
   the serverless sidecar-aware scan helper, broaden exact sidecar candidate
   execution beyond the current helper contract, instantiate the named serving
   cache policy from runtime/public lake routing with deployment-specific
   budgets, add persistent cache storage, and add checksum behavior beyond
   object-version/length checks for object-store range reads.
