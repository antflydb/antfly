...
| Cache isolation | Cache accounting classes exist, and the Parquet object-range scanner now has a cache-keyed `ObjectRangeCache` seam for footer, metadata, and coalesced column-chunk bytes. `ObjectRangeCache` now records per-lane hit/miss/stored/rejected byte counts from `RangeRead.cacheLane()` and supports optional per-lane byte admission limits, so metadata/footer ranges and broad-scan scratch bytes can be accounted and capped independently before the persistent cache layer exists. Runtime integration with persistent cache storage, protected serving-sidecar admission, and real eviction policy is not complete. | Metadata/footer, decoded data/page, broad-scan scratch, and serving-critical sidecar caches have separate admission/eviction policy. |
