...
| --- | --- | --- |
| Real Parquet scanner | Not implemented. Current external row sources are batch adapters used to prove the `RowSource` contract. Parquet footer preflight parsing now validates trailing `PAR1` trailers and plans exact footer metadata ranges in `serverless/query/lake_parquet_footer.zig`; compact-Thrift footer metadata parsing in `serverless/query/lake_parquet_metadata.zig` extracts row groups and column chunks, and can enrich raw inventories so projected scan planning can produce column-chunk reads. `serverless/query/lake_parquet_page.zig` parses data page headers and scans uncompressed PLAIN i64/byte-array column chunks across pages. `serverless/query/lake_parquet_rowgroup.zig` assembles required PLAIN i64 row groups into `ColumnBatch` values with stable external row refs and exposes a preloaded row-group `RowSource` that can feed `lake_rows` scans. Compression, definition/repetition levels, dictionary decoding, nullable columns, object-store backed routing, and public `rows:query` routing are still missing. | Filesystem-backed Parquet fixture can run projection, scalar predicate, row-group pruning, and stable external row refs through `rows:query`. |
| Object-store range I/O | Range planning, footer-tail reads, version-aware cache keys, cache lanes, coalescing rules, and inventory-to-column-chunk read planning exist in `serverless/query/lake_range_io.zig`. Deterministic raw Parquet prefix snapshot planning exists in `serverless/external_source/object_snapshot.zig`. The real Parquet scanner still needs to call those pieces through Antfly's object-storage abstraction. | Scanner reads footers from object tails and coalesces projected column chunk ranges through Antfly's object-storage abstraction with cache keys including object version/ETag and byte range. |
