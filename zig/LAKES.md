...
| --- | --- | --- |
| Real Parquet scanner | Not implemented. Current external row sources are batch adapters used to prove the `RowSource` contract. Parquet footer preflight parsing now validates trailing `PAR1` trailers and plans exact footer metadata ranges in `serverless/query/lake_parquet_footer.zig`; compact-Thrift footer metadata parsing in `serverless/query/lake_parquet_metadata.zig` extracts row groups and column chunks, and can enrich raw inventories so projected scan planning can produce column-chunk reads. `serverless/query/lake_parquet_page.zig` parses data page headers and scans uncompressed PLAIN i64/byte-array column chunks across pages. `serverless/query/lake_parquet_rowgroup.zig` assembles required PLAIN i64 row groups into `ColumnBatch` values with stable external row refs, plans projected row-group source inputs from footer-enriched inventories, and exposes both preloaded and object-range-backed row-group `RowSource` adapters that can feed `lake_rows` scans. Compression, definition/repetition levels, dictionary decoding, nullable columns, automatic footer-to-source discovery, and public `rows:query` routing are still missing. | Filesystem-backed Parquet fixture can run projection, scalar predicate, row-group pruning, and stable external row refs through `rows:query`. |
