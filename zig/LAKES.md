...
| Relational aggregate integration | Algebraic artifact scaffolding exists, and `api/relational_rows.zig` now has a narrow scan-first bridge that lowers supported `rows:aggregate` requests to lake scans and folds projected lake rows back into the normal public `RelationalRowsAggregateResult` for `count`, `sum`, `min`, `max`, and `avg`. The bridge now supports both global aggregates and field-only `group_by` over scanned scalar columns: group keys are included in the lake projection, folded without row JSON materialization, and emitted through the normal aggregate response shape. Field-only scalar aggregate `filter` predicates, scalar `filter_in`/`not_in` membership predicates, and keyword/text `filter_text_patterns` are also evaluated over typed lake cells after scan, so filtered and unfiltered aggregates can share the same projected lake batch without full row materialization. Simple aggregate expressions now work for the supported aggregate ops as a bridge path: scan planning collects referenced row fields into the lake projection, evaluates the existing typed rows expression AST against each projected row, and folds non-null/numeric expression results into the normal aggregate output. `api/table_reads.zig` dispatches bound external-base-source aggregate plans through the same lake scan hook, so the public typed aggregate envelope can return the normal aggregate response shape for the supported aggregate subset, and the pinned scanner helper reuses the same inventory validation/object-range scanner path. The configured public route test now proves grouped filtered, membership-filtered, and expression aggregates over a real external Parquet object through the resolver-backed routing path. Array/JSON/expression aggregate filters, HAVING/order/limit over aggregate outputs, full typed column-batch execution for expressions, full production catalog/object-store source resolution, and automatic selection of algebraic materializations are not yet production. | Add richer JSON/expression filter families and typed column-batch expression execution, then promote repeated aggregate shapes into algebraic materializations when available. |
