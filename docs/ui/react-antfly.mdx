...
The Retrieval Agent is Antfly's advanced Q&A system. It provides query classification, reasoning, retrieval, generation, confidence scoring, and follow-up questions through `/agents/retrieval`.

### AnswerResults

The `<AnswerResults>` component is the legacy React component name for rendering Retrieval Agent responses with optional classification, reasoning, and follow-up questions.
...
Stream Retrieval Agent responses with state management. The hook name is kept for component API compatibility.
    ...
    startStream({
      ...
      request: {
        ...
        steps: {
          ...
          classification: { enabled: true, with_reasoning: true },
