...
On Apple silicon the Gemma 4 audio encoder runs on Metal end to end: the conformer blocks (clamped projections, chunked local attention with relative position bias, gated depthwise convolution) execute as device kernels inside one command buffer, the projector's tensors are served from the session's resident copies, and the projector file is parsed once per model rather than per request. A 2.5-second clip encodes in well under a tenth of a second on an M-series laptop and the audio prompt tokens then flow through the same framed prefill as text. Set `TERMITE_SERVER_GENERATE_TIMING=1` to log the `multimodal_prepare`, `prefill` and `decode` split for each request.
