...
Models come from Hugging Face by their full `owner/name` reference or a documented friendly alias such as `bge-m3`. Unknown bare names are rejected. The `hf:` prefix is accepted. Append `@revision` to request a branch, tag, or commit; omit it to use `main`. Each pull resolves one immutable snapshot for all files. See [Model Downloads](/docs/guides/model-downloads) for format selection and revision examples.
