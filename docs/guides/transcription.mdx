...
Recordings arrive as `http://`, `https://`, `s3://`, or `data:` URLs. The runtime decodes WAV, FLAC, and MP3 natively. It also accepts AAC/M4A (iPhone and macOS voice memos), Opus and Vorbis in Ogg or WebM/Matroska (browser recorders, Zoom, OBS), AIFF, CAF, and AU containers, but the pure-Zig AAC and Opus decoders are still narrow lanes and can produce a poor transcript on real-world encodes; convert those to FLAC or MP3 until the decoders are qualified. A recording may be up to 128 MiB by default (about one hour of 128 kbps MP3), and length is not otherwise limited: audio over 30 seconds is transcribed in windows cut at pauses, with each window conditioned on the previous one.
