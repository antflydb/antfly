...
- `pending_reasons` gives typed aggregate blockers or failure reasons.
...
- Search should land on the precise passage but return the document it came from: union document and chunk vectors, then group by hierarchy.
- Two enrichments write different JSON shapes into one searchable text index: use source-local `field` overrides.
- Relationships come from more than one extractor: order the graph `sources` so the extractor you trust more wins on shared edges.
- Serving before the backfill finishes is acceptable: watch `readiness.queryable`; treating results as complete needs `readiness.complete`.
...
- [Search Images by What's in Them](/docs/guides/multimodal): a single-source embeddings index over remote media
