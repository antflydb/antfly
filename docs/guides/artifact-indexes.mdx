...
coverage. `pending_reasons` and captured/applied revisions are aggregate
diagnostics for the complete configured source set. Artifact-backed indexes also
return `readiness.sources` in configuration order. Each entry identifies the
artifact stream and reports its captured target revision, published revision,
state, and blockers, so operators can distinguish (for example) document-vector
lag from chunk-vector lag without inspecting internal replay state. These
watermarks are operational diagnostics; continue to use `queryable` and
`complete` as the serving and convergence authorities.
...
The released v0.2 embeddings fields remain supported for their documented
compatibility window. `source_artifact_name` is descriptive when paired with
`embedding_name`; the matching enrichment is authoritative. Internal graph
manifest and edge materialization fields are engine-owned, not alternate public
APIs. No graph status aliases were released, and the unreleased Go v0.2 SDK
surface does not create an additional compatibility obligation.
