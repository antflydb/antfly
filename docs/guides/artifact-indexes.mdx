...
bidirectional traversal first probes each shard's reverse index with one
compact key batch, then expands only the shards that report matching edges.
This preserves exact cross-range paths without synchronous cross-shard dual
writes or cluster-wide graph materialization work.
