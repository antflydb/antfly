...
You'll need a vision-capable model: Ollama with LLaVA locally, or Vertex AI / Gemini credentials.
...
- **`{{remoteMedia url="..."}}`** - Downloads media and prepares it as model inference input
- **`{{remotePDF url="..."}}`** - **Deprecated.** Downloads and extracts text from born-digital PDFs
...
Template helpers run while a template is rendered. They do not persist PDF page units or make OCR output independently reprocessable. For durable PDF ingestion, configure a `document_extraction` asset producer on the URL field:
...
`document_extraction` downloads the URL server-side and persists one unit per PDF page. Pages with zero native text are rendered at 150 DPI and sent to the local `antflydb/Florence-2-base` reader by default. Install it explicitly with `antfly inference pull antflydb/Florence-2-base`; Antfly does not download it automatically. If it is absent, the page is stored with `extraction_status: "failed_ocr"`, `ocr_used: false`, and a `ModelNotFound` warning. Install the model and explicitly reprocess the artifact to retry. Set `ocr_fallback` to `false` or `ocr.enabled` to `false` to disable this fallback.

The built-in parser targets unencrypted, born-digital PDFs. For a broader file-ingestion workflow, including formats the built-in parser does not support, use the [docsaf example](https://github.com/antflydb/antfly/tree/main/examples/docsaf).
...
Because image fields are commonly optional, `partial` treats missing images as
intentional skips while still waiting for every product to settle. Before
running semantic image searches, wait for the index explicitly:
...
### Legacy example: PDF Document Search with `remotePDF`

This compatibility example uses the deprecated `remotePDF` helper. Prefer `document_extraction` for durable ingestion.
...
2. **Template Design**:
   ...
   - For PDFs, use `document_extraction`; `{{remotePDF}}` is deprecated compatibility behavior
...
- `{{remotePDF url=this}}` - **Deprecated.** Fetches and extracts content from PDFs
...
### Legacy example: Search by PDF Content

The deprecated `remotePDF` helper remains available for compatibility in query templates:
...
Then prompt it with the outcome — e.g. "Build image search over my photos with a CLIP embeddings index" — and use this page to judge the result.
