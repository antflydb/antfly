...
| `.afb` | A portable Antfly backup archive | Archival retention, object-storage uploads, repeatable migrations, restoring into a normal instance |

Never treat `.aflite` as the backup format. A normal Antfly can restore directly from one, but that is a convenience for promotion, not an archive. The `.aflite` open path accepts the documented v1 native format and fails explicitly on unknown versions or invalid headers; pre-release directory and LSM-container experiments are not read. To keep one of those, export it with the matching development build and restore the resulting `.afb`.
...
`backup` writes a portable `.afb`. `restore` creates a new Lite database from one. `snapshot` copies a stable live checkpoint of the `.aflite` file itself, which is the right tool for local copies, test fixtures, and handing a database to another Lite application.

`import` loads into a database that already exists:
...
`antfly restore --input app.aflite --table docs --url http://localhost:8080` does the same thing through the normal restore command. Either way, omitting `--location` stages the backup under `~/.antfly/lite/backups`; pass it when the target needs a shared `file://`, `s3://`, or `gs://` location. For a repeatable release or disaster-recovery workflow, make the `.afb` explicit first:
...
**A normal instance into Lite.** Back up the table, then restore or import the `.afb`:
...
- **One writer, one machine.** Lite pins one shard and one replica. If two processes need to write the same data, run a normal instance.
- **Bring your own vectors, or defer them.** With no inference runtime, Lite still indexes and queries the embeddings you supply. Configure a remote provider or ship the full build when the application itself must embed.
- **Ship the file, back up the archive.** Snapshot `.aflite` to hand a database to another Lite app. Write an `.afb` for anything that has to outlive the storage engine.
- **Promote when you outgrow it.** The path to a normal instance is one `promote`; nothing about the data model changes.
