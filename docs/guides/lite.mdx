...
`antfly restore --input app.aflite --table docs --url http://127.0.0.1:8080` does the same thing through the normal restore command. Either way, omitting `--location` stages the backup under `~/.antfly/lite/backups`; pass it when the target needs a shared `file://`, `s3://`, or `gs://` location. For a repeatable release or disaster-recovery workflow, make the `.afb` explicit first:
...
- **Ship the file, back up the bundle.** Snapshot `.aflite` to hand a database to another Lite app. Write a full `.afb` for anything that has to outlive the storage engine: archival retention, object-storage transfer, restore into a normal instance, cross-version migration.
- **Portable for Lite, native for speed.** Choose the portable representation whenever Lite or another engine will read the bundle. Choose native only for fast restore into a compatible normal Antfly runtime.
