...
By the end of this guide, your React app has a search page and an answer box on the same Antfly table. Type `billing` and results appear before you finish the word, with a category facet you can click to narrow. Type "how do refunds work?" into the ask box and an answer streams in, cites the documents it used, and offers a follow-up. You need Antfly running with an indexed table and Ollama with a small model pulled. The examples use a table called `docs` with an embeddings index named `body_embeddings`; the [Quickstart](/docs/guides/quickstart) shows how to create one.

```bash
curl -s http://localhost:8080/db/v1/tables/docs | head -c 300
...
Type into the search box and watch the results and facet counts move. Then ask a full question in the ask box and press Ask. If search works and the ask box does nothing, it is a `searchBoxId` typo. If both are silent, open the network tab: an empty multiquery means no widget is contributing a query.

## Why the Answer Box Gets Its Own Input

Running both off one box is tempting: type once, see results and an answer. The library will not do it, and the restriction is the good part.
...
It also matches how people read. Someone scanning results wants the list to move with them. Someone asking a question has composed a full sentence and expects a pause. Two inputs make the pause deliberate.
