...
closure.
Edge direction is relative to `from` and defaults to `out`. Use `direction: "in"`
to traverse a stored relationship in reverse, or `direction: "both"` to treat one
stored relationship as undirected. A physical self-loop still contributes only
one relationship binding under `both`.
