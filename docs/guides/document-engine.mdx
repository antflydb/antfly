...
Traversal outputs can seed another traversal with
`$graph_results.<query-name>`. MATCH produces rows with multiple aliases, so a
downstream selector must also name the returned `binding`:
...
answer. Path-producing results are not currently valid node selectors.
Table-qualified identities use `{"key":"…","table":"…"}`; omitting `table`,
or explicitly naming the queried table, has the same identity semantics.
...
`term_range` wrappers so their types remain unambiguous in every SDK.
  ...
  "graph_queries": {
    "authored_replies": {
      ...
      "match": {
        "anchor": "author",
        "nodes": {
          "author": {"filter": {"term": "person", "field": "kind"}},
          "post": {},
          "reply": {
            "filter": {
              "numeric_range": {"field": "score", "min": 0.8}
            }
          }
        },
        "edges": [
...
`match.anchor` names the alias enumerated from the query table; aliases reached
through cross-table edges retain their table-qualified identities. Anchor
...
node. Because exact anchors are cursor-scanned in `_id` order, fields used by the
anchor or authorization filters must have native index coverage. Antfly returns
...
MATCH and compatibility-pattern expansion work is budgeted across the whole
request, not once per named operation. Traversal and path operations retain
their explicit per-operation depth and result bounds.
...
adapter for the v0.2 shape. New clients should generate only `graph_queries`.
