...
Identifier-heavy fields such as SKUs, file paths, hostnames, and code symbols often need "contains" matching rather than word matching. Add `substring` to the shorthand (`"x-antfly-types": ["text", "substring"]`) or use `"type": "substring"` in a dynamic template to get a `fieldName._substring` companion that indexes every suffix of each token and of each adjacent token pair. A `match` or `prefix` query on that companion then finds `RAG3-WEAVER` for `g3we` without scanning the term dictionary, and it matches across separators, so `rag3_weaver` and `rag3weaver` are found too. The companion costs several times the size of its text, so opt in only where infix matching earns it.
...
- `query_modes` lists modes such as `full_text`, `exact`, `range`, `geo`, `autocomplete`, and `substring`.
