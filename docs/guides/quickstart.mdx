...
<Callout type="info">
Standalone mode starts metadata, data nodes, and Antfly inference in a single process — ideal for local development. You can also access the [web dashboard](/docs/guides/dashboard) at `http://127.0.0.1:8080` to manage tables, run queries, and monitor your cluster. The numeric loopback address matches standalone's secure default bind on systems where `localhost` resolves to IPv6 first.
</Callout>
...
Now pull the text embedding model used for semantic search. [Antfly
inference](/docs/guides/inference) runs models locally without an external API:
...
To continue once 10% of the source documents have durable, query-visible
embedding outcomes, run:
...
Continue as soon as this command succeeds. The examples below search a meaningful,
safely published slice of the corpus while the remaining documents continue
indexing, so result counts and rankings can improve as coverage grows. When an
application requires a fixed, fully covered corpus, use `--until complete` to
wait for the stronger `ready` state instead.
...
it times out before the index reaches 10% source coverage. Use `publication_policy:
...
<Callout type="info">
For chunked indexes, `INDEXED` and `SEARCHABLE` describe
physical chunks or vector records, not source documents, so they can exceed
10,000. Embeddings status keeps these dimensions separate: `source_coverage`
reports durable source outcomes, `searchable_vectors` reports query-visible
physical entries, and `activity` reports incarnation-scoped chunk and embedding
work. Activity explains current speed but never changes readiness.
Its phase is reported by the worker that owns preparation, embedding,
publication, or retry work; incomplete coverage alone never implies
`embedding`. If the metadata leader has not received a fresh owner heartbeat,
`activity` is `null` and the CLI reports activity as unavailable. A transient
status-sampling miss retains the last matching owner sample for up to 90 seconds,
so normal lifecycle contention does not make progress flicker.
Status reads never inspect or wait for the resident writer. If a newly accepted
target has not yet been observed by its owner, the last published incarnation
and searchable counts remain available and `queryable_partial`; only
`complete` gains a `target_observation` blocker until the next publication.
`queryable_partial` means the active generation is safe to query but incomplete;
`ready` means coverage is complete.
</Callout>

## Search

<Callout type="info">
You can run these queries using the CLI (shown below) or the [web dashboard](/docs/guides/dashboard) at `http://127.0.0.1:8080`.
</Callout>
...
Adding an index to a table with existing data triggers background embedding generation. The `partial` coverage policy treats articles without a `thumbnail_url` as intentional skips while still requiring every article to reach a durable outcome. Wait until the image index has durable, query-visible embeddings for 10% of the source corpus before continuing:
...
reached the guide's source-coverage threshold. `queryable_partial` is safe to
use while coverage grows, but an empty published generation cannot produce
results. If `index wait --until source-covered=10%` times out, capture the full
...
1. **[Explore the Dashboard](/docs/guides/dashboard)** — Manage tables, run queries, and monitor your cluster at `http://127.0.0.1:8080`
