...
dataset. The CLI reads JSONL and batches documents automatically. Applications
...
This guide waits for complete source coverage so every later example sees the
same dataset. Omitting `--queryable` retains that full-completion behavior:
...
Model download and backfill time depends on hardware and network speed. The
wait command reports bounded periodic progress and enforces the timeout; do not
continue if it times out. Use `publication_policy: atomic` when a replacement
generation must remain unavailable until fully built and validated. See
...
```bash
antfly inference list
rg 'standalone inference paths' antfly.log
```
...
Do not continue to semantic, image, or RAG steps with an incomplete index.
Capture the full status and recent watchdog messages:

```bash
antfly index list --table wikipedia --output json | jq '.'
rg 'watchdog|derived worker|NotFound|EnrichmentWaitTimeout' antfly.log | tail -n 100
```
