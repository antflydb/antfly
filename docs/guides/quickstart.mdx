...
```bash
antfly standalone 2>&1 | tee "antfly.log"
```
...
article has `title`, `body`, and `url`; 2,477 of the 10,000 articles also have
...
requires a fixed, fully covered corpus, omit `--queryable` to wait for the
...
not. If `index wait --queryable` times out, capture the full status and recent
watchdog messages:
...
against your running instance — and each [guide](/docs/guides) ends with a
prompt you can hand it directly.
