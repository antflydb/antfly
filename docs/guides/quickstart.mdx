...
Pull a multimodal embedding model for text and image search. [Antfly inference](/docs/guides/inference) is Antfly's built-in ML inference engine — it runs quantized models locally for fast inference, no external APIs needed.
...
curl -X POST http://localhost:8080/db/v1/tables/wikipedia/batch \
  -H "Content-Type: application/json" \
  -d '{
    "inserts": {
      "Albert Einstein": {
...
Embedding generation runs in the background. Wait for the semantic index to be
fully queryable before running semantic, hybrid, or RAG examples:
...
On a laptop, the initial local-model download and 10,000-document backfill can
take several minutes; the wait command reports bounded periodic progress and
...
if err != nil { return err }
createdIndex, err := client.CreateIndex(context.Background(), "wikipedia", "thumbnail", *request)
if err != nil { return err }
```
  </TabsContent>
...
resp, err := client.RetrievalAgentWithResponse(
	...
	antfly.RetrievalAgentJSONRequestBody{
		...
		Generator: &antfly.Generator{
			Provider: strPtr("antfly"),
			Model:    strPtr("ggml-org/gemma-4-E4B-it-GGUF"),
			MaxTokens: 128,
		},
