...
In the status output, articles can produce multiple chunks, so `SEARCHABLE` may exceed the document count, and `avg_embeddings` measures chunk embeddings per second, not documents.
