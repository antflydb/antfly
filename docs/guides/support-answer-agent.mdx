...
Retrieve more than you need and let the pruner cut. A `limit` of 20 with a strict pruner beats a `limit` of 3 with no pruner, because the reranker sees more candidates and the model sees only the survivors. Start strict, watch what gets pruned on real questions, and loosen only when good articles are being cut.
