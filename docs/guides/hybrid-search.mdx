...
Add `highlight` to any query that carries `full_text_search` and every hit that
returns stored source also carries `_highlights`: a map from field name to
...
is one re-analysis of each highlighted field per returned hit.
