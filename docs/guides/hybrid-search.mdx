...
This guide assumes Antfly is running with the Wikipedia table from the [Quickstart](/docs/guides/quickstart). One query, both retrieval methods:
...
`--full-text-search` finds documents that contain the words; `--semantic-search` finds documents that mean the same thing. Antfly runs both and fuses the ranked lists.
...
Then prompt it with the outcome — e.g. "Add hybrid search with RRF and a reranker to my products table" — and use this page to judge the result.
