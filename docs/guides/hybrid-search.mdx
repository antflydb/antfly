...
`hits.total` continues to report the underlying retrieval match count.

<Callout type="tip">
Set `candidate_count` higher than `offset + limit` (often 50-100) to improve recall while keeping remote scoring cost bounded. Use the normal query `limit` for the final response size. A larger candidate set can improve recall but increases latency.
</Callout>
