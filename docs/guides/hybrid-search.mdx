...
The `field` parameter tells the reranker which document field to score against the query. `candidate_count` controls the expensive scoring window, while the query's `limit` controls the number returned. The `antfly` provider uses the inference service's configured default reranker when `model` is omitted; set `model` explicitly to select an installed model such as `mixedbread-ai/mxbai-rerank-base-v1`.

The executable reranker providers are `antfly`, `cohere`, and `vertex`.
Antfly retrieves the bounded window from each shard, retains the globally best
`candidate_count` hits, and makes one batched reranker call at the coordinator.
`offset` and `limit` are applied after reranking. The legacy reranker `top_n`
field remains as a deprecated limit override and is validated against
`candidate_count`. Reranking changes the returned order and page, while
`hits.total` continues to report the underlying retrieval match count. The
effective candidate window is capped at 1,000, including when it is derived
from `offset + limit`. Vertex has a provider-specific ceiling of 200. Oversized
requests return `422 reranker_candidate_limit_exceeded` with the selected
provider and its exact maximum before shard fan-out or provider work begins.

<Callout type="tip">
Set `candidate_count` to at least `offset + limit` (often 50-100, up to 1,000; Vertex supports up to 200) so the requested page fits inside the scoring window. Use the normal query `limit` for the final response size. A larger candidate set can improve recall but increases latency and external-provider cost.
</Callout>
