...
The `field` parameter tells the reranker which document field to score against the query. `candidate_count` controls the expensive scoring window, while the query's `limit` controls the number returned. The built-in `antfly` reranker uses a cross-encoder model bundled in the binary. For larger or custom models, use `"provider": "antfly"` with a model like `cross-encoder/ms-marco-MiniLM-L-6-v2`.
...
<Callout type="tip">
Set `candidate_count` higher than `offset + limit` (often 50-100, up to 1,000; Vertex supports up to 200) to improve recall while keeping remote scoring cost bounded. Use the normal query `limit` for the final response size. A larger candidate set can improve recall but increases latency and external-provider cost.
</Callout>
