...
### 5. Show Why a Hit Matched

Add `highlight` to any query that carries `full_text_search` (or named
full-text queries) and every hit carries `_highlights`: a map from field name to
fragments of the stored value, each with byte-offset spans marking the text the
query matched. Highlights come from the unprojected document, so a `fields`
projection that leaves the highlighted field out of `_source` does not hide it.

```json
{
  "full_text_search": {"match": "g3we", "field": "sku._substring"},
  "highlight": {"fields": ["sku", "title"], "fragment_size": 120, "max_fragments": 2},
  "limit": 10
}
```
...
```json
{
  "_id": "sku:41",
  "_score": 3.1,
  "_source": {"sku": "RAG3-WEAVER", "title": "Rag3 Weaver kit"},
  "_highlights": {
    "sku": [{"text": "RAG3-WEAVER", "offset": 0, "spans": [{"start": 2, "end": 7}]}]
  }
}
```

Spans are located by re-analyzing the stored value with the field's analyzer, so
a query for `handbooks` highlights `Handbook`. Companion fields resolve to their
root, so a `sku._substring` query highlights `sku`, and substring matches mark
the exact contained bytes even when they cross a separator. Omit `fields` to
highlight every field the query references. Highlights are computed on the
node that owns the documents, so they add no coordinator round trips; the cost
is one re-analysis of each highlighted field per returned hit, plus one stored
document load per hit when `_source` was projected or omitted.
