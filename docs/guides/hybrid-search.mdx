...
For search-as-you-type clients using Antfly's standalone HTTP/1 listener, send
`X-Antfly-Cancel-On-Disconnect: true` on query requests. When the client closes
its request direction before receiving a response, Antfly treats that query as
abandoned and cancels queued work and downstream inference. Without the header,
an orderly TCP half-close still allows the client to read its response. A model
run already executing on a GPU stops at its next safe batch boundary; the
header does not interrupt a driver call in progress. HTTP/2 stream resets and
abortive HTTP/1 disconnects already signal cancellation without this header.
