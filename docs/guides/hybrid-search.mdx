...
from `offset + limit`; oversized requests are rejected before shard fan-out or
provider work begins.

<Callout type="tip">
Set `candidate_count` higher than `offset + limit` (often 50-100, up to 1,000) to improve recall while keeping remote scoring cost bounded. Use the normal query `limit` for the final response size. A larger candidate set can improve recall but increases latency and external-provider cost.
</Callout>
