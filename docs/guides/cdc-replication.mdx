...
- PostgreSQL **14+** (for pgoutput protocol v2). PostgreSQL 15+ is required
  only when creating a publication with `publication_filter`.
...
| `key_template` | No | `"_id"`, then `"id"` fallback | How to derive the Antfly document key. Plain column name or `{{col}}` template. |
...
| `publication_filter` | No | — | PostgreSQL 15+ row filter, applied only when Antfly creates the publication. Changing it does not alter an existing publication. |
| `on_update` | No | passthrough | Transform ops for INSERT/UPDATE events |
| `on_delete` | No | auto-derived | Transform ops for DELETE events |

### Key Templates

When omitted, Antfly uses the `_id` column if present and falls back to `id`.
Set the field explicitly when the source uses another primary key.
...
Never put credentials directly in the DSN. Use a protected Antfly secret-store file:
