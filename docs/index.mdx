...
- **Durable Document Extraction**: URL-backed artifacts persist PDF pages and chunks; pages without native text use local OCR when the configured model is installed
- **Multimodal Remote Processing**: Template-time download and inference inputs for images and other media, with SSRF prevention and content validation

### Developer Experience
- **Client Libraries**: Auto-generated SDKs for Go, TypeScript, and Python from OpenAPI spec
- **Antfly Lite**: Embedded local-first Antfly with a single `.aflite` database file and portable AFB2 backup bundles
...
Ready to dive in? Check out our [Quickstart Guide](/docs/guides/quickstart) to get AntflyDB running in minutes, use [Antfly Lite](/docs/guides/lite) for embedded local-first applications, or explore the [Architecture](/docs/architecture) to understand how it all works under the hood.

For the query model itself, see the [Document Engine guide](/docs/guides/document-engine) for how mappings, full-text search, vector search, filters, graph search, sorting, and cursor pagination compose.
